...
PartnerCard.DEFAULT_TALK = "Cuối cùng cũng tìm được nơi dừng chân, nơi đây yên tĩnh, ta rất thích." 				-- 默认泡泡
...
PartnerCard.tbDevilMsg =  								-- 心魔操作描述
{
	[PartnerCard.DEVIL_STATE_HOT] = {
		[PartnerCard.DEVIL_CURE_RIGHT] = "[3FF200]%s[-][87CEFA] dội nước lạnh vào [-][3FF200]%s[-][87CEFA], [-][3FF200]%s[-][87CEFA] đã bình tĩnh hơn[-]";
		[PartnerCard.DEVIL_CURE_WRONG] = "[3FF200]%s[-][87CEFA] dội nước lạnh vào [-][3FF200]%s[-][87CEFA], [-][3FF200]%s[-][87CEFA] vô cùng tức giận[-]";
	};
	[PartnerCard.DEVIL_STATE_LOST] = {
		[PartnerCard.DEVIL_CURE_RIGHT] = "[3FF200]%s[-][87CEFA] đã truyền khí cho [-][3FF200]%s[-][87CEFA], [-][3FF200]%s[-][87CEFA] đã bình tĩnh hơn[-]";
		[PartnerCard.DEVIL_CURE_WRONG] = "[3FF200]%s[-][87CEFA] đã truyền khí cho [-][3FF200]%s[-][87CEFA], [-][3FF200]%s[-][87CEFA] chân khí truyền ngược, đau khổ vô cùng[-]";
	};
	[PartnerCard.DEVIL_STATE_FEAR] = {
		[PartnerCard.DEVIL_CURE_RIGHT] = "[3FF200]%s[-][87CEFA] đã xoa dịu [-][3FF200]%s[-][87CEFA] vài cái, [-][3FF200]%s[-][87CEFA] chợt thức tỉnh[-]";
		[PartnerCard.DEVIL_CURE_WRONG] = "[3FF200]%s[-][87CEFA] đã xoa dịu [-][3FF200]%s[-][87CEFA], [-][3FF200]%s[-][87CEFA] cảm thấy khó hiểu[-]";
	};
}
...
function PartnerCard:CheckAcceptTask(pPlayer, nCardId)
	...
	if tbTaskId then
		if nFinishTaskIdx < #tbTaskId then
			return false, "Hãy hoàn thành nhiệm vụ hiện tại"
		end
	end
	if nUpdateDay == nNowDay then
		return false, "Hôm nay đã nhận nhiệm vụ"
	end
...
function PartnerCard:CheckQuestion(nQuestionId, nAnswerIdx, tbAnswer)
	...
	if not tbQuestionInfo then
		return false, "Câu hỏi chưa biết"
	end
	if not tbQuestionInfo.tbAnswer or not tbQuestionInfo.tbAnswer[nAnswerIdx] then
		return false, "Đáp án chưa biết"
	end
	if #tbAnswer ~= 4 then
		return false, "Số đáp án chưa biết"
	end
...
function PartnerCard:CheckBubble(szMsg)
	if version_kor then
		...
		if nKorLen > self.nKorBubbleMax or nKorLen < self.nKorBubbleMin then
			return false, string.format("Độ dài nội dung từ %d-%d ký tự", self.nVNBubbleMin, self.nVNBubbleMax)
		end
	...
	elseif version_th then
		...
		if nNameLen > self.nTHBubbleMax or nNameLen < self.nTHBubbleMin then
			return false, string.format("Độ dài nội dung từ %d-%d ký tự", self.nTHBubbleMin, self.nTHBubbleMax)
		end
	else
		local nNameLen = Lib:Utf8Len(szMsg);
		if nNameLen > self.nBubbleMax or nNameLen < self.nBubbleMin then
			return false, string.format("Độ dài nội dung từ %d-%d ký tự", self.nBubbleMin, self.nBubbleMax)
		end
		if Lib:HasNonChineseChars(szMsg) then
	        return false, "Nội dung chỉ được dùng ký tự tiếng Việt, hãy sửa và thử lại!"
	    end
	end
	if ReplaceLimitWords(szMsg) then
		return false, "Nội dung có từ ngữ không hợp lệ, hãy sửa và thử lại!"
	end
	return true
end
...
function PartnerCard:CheckLiveHouse(pPlayer, nCardId)
	if not House:IsInOwnHouse(pPlayer) then
		pPlayer.CenterMsg("Trong gia viên của mình mới được thao tác", true)
		return 
	end
	if not PartnerCard:IsHaveCard(pPlayer, nCardId) then
		return false, "Hãy nhận Môn Khách trước"
	end
	if not House:CheckOpen(pPlayer) then
		return false, "Hãy xây gia viên trước" 
	end
	if self:IsCardLiveHouse(pPlayer, nCardId) then
		return false, "Môn Khách đã vào gia viên"
	end
	...
	if  Lib:CountTB(tbCardData) >= nMaxLiveHouse then
		return false, "Môn Khách quá nhiều, không thể vào"
	end
	return true
end
...
function PartnerCard:CheckLeaveHouse(pPlayer, nCardId)
	...
	if not self:IsCardLiveHouse(pPlayer, nCardId) then
		return false, "Môn Khách chưa vào gia viên"
	end
	local nActState = PartnerCard:GetCardActState(pPlayer, nCardId)
	if not nActState then
		return false, "Trạng thái chưa biết"
	end
	if PartnerCard:IsActing(pPlayer.dwID, nCardId) then
		return false, "Đang phái Môn Khách, không thể rời gia viên"
	end
	if nActState ~= PartnerCard.CARD_ACT_STATE_NONE then
		return false, "Môn Khách có thưởng chờ nhận, hãy đến giao diện Sai Phái nhận thưởng"
	end
	return true
end
...
function PartnerCard:CheckHouseOperation(pPlayer)
	if not House:IsInOwnHouse(pPlayer) then
		return false, "Chỉ được thao tác trong gia viên của mình"
	end
	return true
end
...
function PartnerCard:CheckAddStateCommon(pPlayer, nCardId, nActType, bNotCheckState, bNotCheckCost, bNotCheckOpenType, bNotCheckTimes)
	if not PartnerCard:IsOpen() then
		return false, "Chưa mở tính năng Môn Khách"
	end
	local tbCardInfo = self:GetCardInfo(nCardId)
	if not tbCardInfo then
		return false, "Không có Môn Khách"
	end
	if not PartnerCard:IsHaveCard(pPlayer, nCardId) then
		return false, "Hãy chiêu mộ Môn Khách trước"
	end
	...
	if not tbActInfo then
		return false, "Thao tác chưa biết"
	end
	...
	if not tbCard then
		return false, "Môn Khách đã vào mới được thao tác"
	end
	if PartnerCard:IsActing(pPlayer.dwID, nCardId) then
		return false, "Đang phái Môn Khách"
	end
	if not bNotCheckState and tbCard.nActState ~=  PartnerCard.CARD_ACT_STATE_NONE then
		return false, "Hãy nhận thưởng sai phái"
	end
		...
		for _, tbInfo in pairs(tbCost) do
			...
			if nType == Player.award_type_money then
				if pPlayer.GetMoney(tbInfo[1]) < tbInfo[2] then
					return false, string.format("%s không đủ ", Shop:GetMoneyName(tbInfo[1]), tbInfo[2]);
				end
			elseif nType == Player.award_type_item then
				...
				if nCount < tbInfo[3] then
					...
					return false, string.format("%s không đủ ", szItemName, tbInfo[3]), tbInfo[2];
				end
			end
		end
	end
...
function PartnerCard:CheckStartTripFuben(pPlayer, nCardId)
	...
	if not nActSubTime or nNowTime < nActSubTime or nActSubTime < nActTime then
		return false, "Chưa đến thời gian kích hoạt"
	end
	if tbCardData[nCardId].bTrigger then
		return false, "Đã giúp đỡ xong"
	end
