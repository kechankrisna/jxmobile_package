...
function PartnerCard:CanUnlockCardPos(pPlayer, nCardPos)
	...
	if not self:IsCardPosOpen(pPlayer, nCardPos) then
		return false, "Vị trí Môn Khách chưa mở"
	end
	if PartnerCard:IsCardPosUnlock(pPlayer, nCardPos) then
		return false, "Đã mở vị trí Môn Khách"
	end
	local tbCost = self:GetCardPosUnlockConsume(nCardPos)
	if tbCost then
		for _, tbInfo in pairs(tbCost) do
			...
			elseif nType == Player.award_type_item then
				...
				if nCount < tbInfo[3] then
					...
					return false, string.format("%s không đủ ", szItemName, tbInfo[3]), nil, szItemName, tbInfo[3], nCount
				end
			end
		end
	end
...
function PartnerCard:CanDismissCard(pPlayer, nCardId)
	...
	if not PartnerCard:IsHaveCard(pPlayer, nCardId) then
		return false, string.format("Hãy chiêu mộ đồng hành %s trước", tbCardInfo.szName)
	end
	if PartnerCard:IsCardUpPos(pPlayer, nCardId) then
		return false, string.format("Môn Khách %s đã ra trận, không thể sa thải", tbCardInfo.szName)
	end
	if PartnerCard:IsCardLiveHouse(pPlayer, nCardId) then
		return false, string.format("Môn Khách %s đã vào Gia Viên, không thể sa thải", tbCardInfo.szName)
	end
...
function PartnerCard:CanAddCard(pPlayer, nCardId)
	...
	if not bCan then
		return false, "Môn Khách không thể chiêu mộ bằng cách này"
	end
...
function PartnerCard:CanSendGift(pPlayer, nCardId, nItemId, nCount)
	...
	if nCount < 1 then
		return false, "Số lượng bị lỗi"
	end
	local tbGiftInfo = self.tbCardGift[nItemId]
	if not tbGiftInfo then
		return false, "Đạo cụ không thể tặng"
	end
	...
	if nLevel >= self.nMaxCardLevel then
		return false, "Đã đạt cấp tối đa, không thể tặng"
	end
	if tbGiftInfo.tbBelongCard then
		if not tbGiftInfo.tbBelongCard[nCardId] then
			return false, "Đạo cụ không thể tặng cho Môn Khách này"
		end
	end
...
function PartnerCard:GetCardFightPower(pPlayer, nCardId)
	local nFightPower = 0
	...
	local nLevel = self:GetCardSaveInfo(pPlayer, nCardId, LEVEL_IDX_STEP)
