...
JingMai.szJingMaiLevelTimeDes = "Chu Thiên đang xoay chuyển, %s sau có thể kích hoạt" 					-- 经脉升级中倒计时文本描述
...
function JingMai:CheckJingMaiLevelUp(pPlayer, nJingMaiId, bNotCheckCost, bNotCheckRunning, bNotCheckCoin)
	...
	if not tbJingMaiSetting then
		return false, "Không có dữ liệu kinh mạch liên quan??"
	end
	...
	if not bNotCheckRunning then
		...
		if bRet then
			...
			return false, string.format("%s đang xoay chuyển, không thể thao tác", szRunningName or "Chu Thiên khác")
		end
	end
	...
	if not tbNextLevelAttrib then
		...
		return false, string.format("Mở cần top %s-%d Huyệt vị đạt tầng %s", szJingMaiName or "", JingMai.nJingMaiPreNum, nLevel or 0)
	end
	local nRequestLevelTime = tbJingMaiLevelData.nRequestLevelTime or 0
	if nRequestLevelTime ~= 0 then
		return false, "Đang xin tăng cấp"
	end
		...
		for _, tbInfo in pairs(tbCost) do
			...
			if nType == Player.award_type_money then
				...
				if bCheckMoney and pPlayer.GetMoney(tbInfo[1]) < tbInfo[2] then
					return false, string.format("%s không đủ %s", Shop:GetMoneyName(tbInfo[1]), tbInfo[2]);
				end
			end
			...
			if nType == Player.award_type_item then
				...
				if nCount < tbInfo[3] then
					...
					if (pPlayer.nFaction >= 18) then
						szItemName = Item:GetNewItemShowInfo(tbInfo[2], pPlayer.nFaction, pPlayer.nSex);
					end
					
					
					return false, string.format("%s không đủ %s", szItemName, tbInfo[3]);
				end
			end
		end
	end
...
function JingMai:CheckJingMaiOpen(nJingMaiId)
	...
	if not tbJingMai then
		return false, "Kinh mạch không tồn tại";
	end

	local nTimeNow = GetTime();
	if self.tbOpenInfo then
		if nJingMaiId >= self.tbOpenInfo.nNextOpenJingMaiId and nTimeNow < self.tbOpenInfo.nOpenTime then
			return false, string.format("Sau %s sẽ mở kinh mạch này", Lib:TimeDesc2(self.tbOpenInfo.nOpenTime - nTimeNow));
		end
	end

	local nOpenTime = TimeFrame:CalcTimeFrameOpenTime(tbJingMai.szOpenTimeFrame);
	if nTimeNow < nOpenTime then
		return false, string.format("Sau %s sẽ mở kinh mạch này", Lib:TimeDesc2(nOpenTime - nTimeNow));
	end
...
function JingMai:CheckOpen(pPlayer)
	if TimeFrame:GetTimeFrameState(self.szOpenTimeFrame) ~= 1 then
		return false, "Chức năng chưa mở";
	end

	if pPlayer.nLevel < self.nOpenLevel then
		return false, string.format("Đạt Lv%s mới được thao tác!", self.nOpenLevel);
	end

	if Task:GetTaskFlag(pPlayer, self.nOpenTaskId) ~= 1 then
		return false, "Chưa hoàn thành nhiệm vụ trước";
	end
...
function JingMai:CheckXueWeiLevelup(pPlayer, nXueWeiId, bNotCheckCoin)
	...
	if not tbXueWei then
		return false, "Huyệt vị không tồn tại";
	end
	...
	if self:GetLevelupLastTimes(pPlayer) <= 0 then
		return false, string.format("Đại hiệp xung huyệt thành công [FFFE0D]%s[-] lần, nội lực đã hết không thể tiếp tục!", self.nMaxLevelupTimes);
	end
	...
	for _, tbInfo in pairs(tbXueWei.tbRequireXueWei) do
		...
		if nRLevel < nRequire_Level then
			return false, string.format("Huyệt vị trước không đủ %s", nRequire_Level);
		end
	end
	...
	if not nLevel then
		return false, "Số liệu bất thường";
	end

	if nLevel >= tbXueWei.nMaxLevel or not tbXueWei.tbRequireLevel[nLevel + 1] then
		return false, "Đã đạt cấp cao nhất";
	end

	if pPlayer.nLevel < tbXueWei.tbRequireLevel[nLevel + 1] then
		return false, string.format("Cần đạt Lv%s!", tbXueWei.tbRequireLevel[nLevel + 1]);
	end
...
function JingMai:XueWeiLevelup(pPlayer, nXueWeiId, nXueWeiLevel)
	...
	if not bSuccess then
		pPlayer.CenterMsg(string.format("Xung huyệt [FFFE0D]%s[-] thất bại!", JingMai.tbXueWeiSetting[nXueWeiId].szName));
	...
	if not bRet then
		pPlayer.CenterMsg("Đan Điền rối loạn không thể xung huyệt!");
	...
	if nLevel == 0 then
		pPlayer.CenterMsg(string.format("Đả thông huyệt vị %s thành công!", JingMai.tbXueWeiSetting[nXueWeiId].szName));
	else
		pPlayer.SendBlackBoardMsg(string.format("Xung huyệt [FFFE0D]%s[-] thành công, cảnh giới tăng [FFFE0D]%s[-] tầng!", JingMai.tbXueWeiSetting[nXueWeiId].szName, nLevel + 1));
	end
...
function JingMai:OnGetProtentialItemByPartner(pPlayer, nUseItemProtentialValue)
	...
	if nSaveInfo == 0 then
		pPlayer.SendBlackBoardMsg("Trạng thái hiện tại của Đan Điền là [FFFE0D]Trống[-]!", true);
	end
...
function JingMai:OnUsePartnerProtentialItem(pPlayer, nCount, bNotNotify)
	...
	if nSaveInfo > 0 then
		...
		if not bNotNotify then
			pPlayer.CenterMsg(string.format("Đồng Hành đã dùng [FFFE0D]%s[-] Tư Chất Đơn, độ mệt mỏi của Đan Điền giảm [FFFE0D]%s[-]!", nCount, nCount), true);
			if nLast <= 0 then
				pPlayer.SendBlackBoardMsg("Trạng thái hiện tại của Đan Điền là [FFFE0D]Đầy[-]!", true);
			end
		end
	end
	...
	pPlayer.CenterMsg(string.format("Lần này Đồng Hành đã dùng [FFFE0D]%s[-] Tư Chất Đơn, nhận được [FFFE0D]%s[-] chân khí!", nCount, nZhenQiCount), true);
...
function JingMai:CheckCanResetXueWei(pPlayer, nXueWeiId)
	if not self.bOpenResetXueWei then
		return false, "Chưa mở tính năng Thiết lập huyệt vị"
	end
	local nLevel = self:GetXueWeiLevel(pPlayer, nXueWeiId);
	if not nLevel or nLevel <= 0 then
		return false, "Không cần tái lập";
	end
	...
	for _, nBRXueWeiId in pairs(tbBeenRequired) do
		...
		if nBRLevel and nBRLevel > 0 then
			return false, string.format("Cần thiết lập huyệt vị [FFFE0D]%s[-]", self.tbXueWeiSetting[nBRXueWeiId].szName);
		end
	end
...
function JingMai:ResetXueWei(pPlayer, nXueWeiId)
	...
	pPlayer.SendBlackBoardMsg(string.format("Thiết lập huyệt vị [FFFE0D]%s[-] thành công, nhận [FFFE0D]%s[-] Chân Khí và hoàn trả [FFFE0D]%s[-] số lần xung huyệt!", self.tbXueWeiSetting[nXueWeiId].szName, nZhenQiCount, nLevel), true);
...
function JingMai:CheckCanResetJingMai(pPlayer, nJingMaiId)
	if not self.bOpenResetJingMai then
		return false, "Chưa mở tính năng Tái lập"
	end
	local tbJingMai = self.tbJingMaiSetting[nJingMaiId];
	if not tbJingMai then
		return false, "Kinh mạch không tồn tại";
...
function JingMai:ResetJingMai(pPlayer, nJingMaiId)
	...
	pPlayer.SendAward(tbAward, nil, true, Env.LogWay_XueWeiResetAward);
	pPlayer.SendBlackBoardMsg(string.format("Thiết lập kinh mạch [FFFE0D]%s[-] thành công, nhận [FFFE0D]%s[-] Chân Khí và hoàn trả [FFFE0D]%s[-] số lần xung huyệt!", self.tbJingMaiSetting[nJingMaiId].szName, nZhenQiCount, nTotalLevel), true);
...
function JingMai:OnFinishTask(nTaskId)
	...
	me.SendBlackBoardMsg("Các hạ đã lĩnh ngộ cách đả thông kinh mạch, vào giao diện [FFFE0D]Đồng Hành-Kinh mạch[-] để xem!", true);
	...
	if nZhenQi > 0 then
		local tbMail = {
			To = me.dwID;
			Title = "Chân Khí vận hành";
			Text = "      Tư chất rất tốt, mới đó đã lĩnh ngộ được cách vận hành chân khí trong [FFFE0D]Kinh mạch[-], trước đó Đồng Hành của ngươi khi dùng Tư Chất Đơn đã ngưng kết rất nhiều [FFFE0D]Chân Khí[-] trong Đan Điền, lão phu sẽ giúp ngươi nhận số chân khí đó để tùy ý sử dụng.";
			From = "Huyền Thiên Đạo Nhân";
