...
StoneMgr.COST_MONEY_COUNT = 100;--剥离消耗100李氏
...
function StoneMgr:CanInset(nEquipId, nStoneTemplateId, pPlayer, nInsetPos)
	...
	if not tbData then
		...
		return false, "Không tồn tại Đá Hồn này";
	end
	...
	if nInsetPos > nHoleCount then
		return false, string.format("Trang bị chỉ có %d lỗ", nHoleCount)
	end

	 --装备的 最高魂石等级
	if tbBaseInfo.nLevel > nInsetLevel then
		return false,   string.format("Trang bị chỉ được khảm Đá Hồn cao nhất Lv%d", nInsetLevel)
	end

	local szEquipPart = Item.EQUIPTYPE_EN_NAME[pEquip.nItemType];
	if tbData[szEquipPart] ~= 1 then
		return false, "Vị trí trang bị không phù hợp";
	end

	if self:IsCrystal(nStoneTemplateId) then
		return false, "Thủy Tinh không thể khảm";
	end
	...
	if nOrgTemplate and nOrgTemplate ~= 0 then
		...
		if tbBaseInfo.nLevel < tbBaseInfoOrg.nLevel then
			...
			return false, "Không thể thay đá hồn cấp cao thành đá hồn cấp thấp" 
		end
	end
...
function StoneMgr:CanUnLoadStone( pPlayer )
	...
	if nCdTime > 0 then
		return false, string.format("Tách Đá Hồn lần kế cần thêm %s, vui lòng chờ", Lib:TimeDesc2(nCdTime))  --xx天xx小时xx分xx秒
	end
	if pPlayer.GetMoney(self.COST_MONEY_TYPE) < self.COST_MONEY_COUNT then
		...
		return false, string.format("Ngân Sức không đủ %s, không thể tách", self.COST_MONEY_COUNT)
	end
	if pPlayer.GetVipLevel() < self.REMOVE_INSET_VIP then
		return false, string.format("Đạt đến Kiếm Hiệp %d có thể tách", self.REMOVE_INSET_VIP)
	end
	return true
end