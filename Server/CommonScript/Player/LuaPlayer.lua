...
function _LuaPlayer.CheckNeedArrangeBag()
	...
	if nCurCount >= (GameSetting.MAX_COUNT_IN_BAG + Item:GetExtBagCount(self)) then
		return true, "Số lượng đạo cụ trong hành trang quá nhiều, hãy sắp xếp lại!";
	end

	return nJueYaoCount >= GameSetting.MAX_COUNT_JUEYAO, "Yếu Quyết Trận Pháp quá nhiều, hãy sắp xếp lại!";
end
...
function _LuaPlayer.GetFreeBagCount()
	...
	if nFree > 0 and nZhenFaFree < nFree then
		return nZhenFaFree, "Yếu Quyết Trận Pháp quá nhiều, hãy sắp xếp lại!";
	end
	return nFree, "Hành trang đã đầy";
end
