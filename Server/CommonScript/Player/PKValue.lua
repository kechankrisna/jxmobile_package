...
function PKValue:GetAddValue(pKiller, pKilled)
	if TeacherStudent:IsMyTeacher(pKiller, pKilled.dwID) then
		return 2, "Đã đả thương sư phụ, điểm PK tăng 2", "Đại hiệp đã bị đồ đệ làm trọng thương!"
	end
	return 1
end
...
function PKValue:AddValue(pKiller, pKilled)
	...
	pKiller.Msg(string.format("Điểm PK tăng, điểm PK hiện là %s", nCurValue));
	if szKillerMsg then
...
function PKValue:ReduceValue(pPlayer)
	...
	pPlayer.Msg(string.format("Điểm PK giảm, điểm PK hiện là %s", nCurValue));
