...
function QuickBuy:NoticeToCharge()
	me.CenterMsg("Nguyên Bảo không đủ! Hãy nạp thêm");
...
function QuickBuy:NotifyQuickBuy(pPlayer, szBuyType, tbParams)
    ...
    local szMsg = string.format("%s (Tốn %d %s) \nHôm nay còn được mua %d lần", szTip, nMoney, szMoneyName, tbSetting.GetLeftTimes(pPlayer) );
