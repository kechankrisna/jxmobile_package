...
function RecordStone:CanSetCurRecordStone(pPlayer, nStoneId, nInsetPos)
    ...
    if not pEquip then
        return false, "Không có trang bị vũ khí"
    end
    local nHoleNum = self:GetEquipCanInsetNum(pEquip.dwTemplateId)
    if nHoleNum <= 0 then
        return false, "Chỉ có thể khắc lên vũ khí Hoàng Kim và Hiếm từ bậc 6 trở lên"
    end

    local tbStoneInfo = self:GetStoneInfo(nStoneId)
    if not tbStoneInfo then
        return false, "Đá Khắc Chữ vô hiệu"
    end
    ...
    if nInsetPos > nHoleNum then
        return false, string.format("Tối đa dùng %d Đá Khắc Chữ", nHoleNum)
    end

    for i,v in ipairs(tbStoneList) do
        if v == nStoneId then
            return false, "Đã có Đá Khắc Chữ này"
        end
    end

    local nCost = self:GetInsetCost(nStoneId)
    if pPlayer.GetMoney("Coin") < nCost then
    	return false, "Tiêu hao Bạc không đủ"
    end
...
if not MODULE_GAMESERVER then
	function RecordStone:OnClientSuccess()
		me.CenterMsg("Sử dụng Đá Khắc Chữ thành công")
	...
	function RecordStone:DoRequestRecord(nTemplateId, nInsetPos)
        ...
        if #tbStoneList == RecordStone.MAX_RECORD_STONE_NUM then
        	szMsg = string.format("Dùng [FFFE0D]%d Bạc[-]  đổi chữ đã khắc", RecordStone:GetInsetCost(nTemplateId))
        else
        	szMsg = string.format("Dùng [FFFE0D]%d Bạc[-]  để khắc?", RecordStone:GetInsetCost(nTemplateId));
        end
