...
function tbBase:StartFight()
	...
	local fnNofiy = function (pPlayer)
		pPlayer.CenterMsg("Chiến đấu bắt đầu! Mời mọi người đến tiền tuyến")
...
function tbBase:OnKillPlayer(pPlayerNpc, pDeader, nComboLevel, nTitleLevel)
	if not self.nFirstBloodPlayerId then
		...
		self:BattleFieldTips(string.format("%s đã diệt %s, lập được chiến công đầu", pPlayerNpc.szName, pDeader.szName))
	end
