...
function ActivityQuestion:CheckSubmitTask()
    ...
    local szMsg = "Chưa trả lời hết câu hỏi"
    if not bReceived then
        szMsg = "Chưa nhận nhiệm vụ Vấn Đáp"
...
function ActivityQuestion:GetCurQuestionDesc()
    if self:IsAnswerNumMax() then
        return "Vấn Đáp", "Tìm [FFFE0D]Công Tôn Tích Hoa[-] trả nhiệm vụ"
    end
    ...
    local szDesc = string.format("Tìm [FFFE0D]%s[-] tìm hiểu giang hồ (%d/%d)", szName, math.min(self.ANSWER_NUM_MAX, nHadAnswer + 1), self.ANSWER_NUM_MAX)
    return "[Ngày] Vấn Đáp", szDesc
end
...
function ActivityQuestion:OnAcceptSuccess()
    self:OnTrack()
    me.CenterMsg("Bắt đầu Vấn Đáp hôm nay")
    Task:OnTaskUpdate(ActivityQuestion.TASK_ID)
    UiNotify.OnNotify(UiNotify.emNOTIFY_ACTIVITY_QUESTION_UPDATE)
end
--------------------client end--------------------