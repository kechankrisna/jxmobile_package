...
function Partner:CheckCanUseSkillBook(pPlayer, nPartnerId, nItemId)
	...
	if not tbSkillSetting then
		return false, "Đạo cụ bất thường";
	end

	if tbSkillSetting.nSeries > 0 and nSeries ~= tbSkillSetting.nSeries then
		return false, string.format("%s không phù hợp ngũ hành sách này!", szName);
	end
	...
	for i = 1, self.MAX_PARTNER_SKILL_COUNT do
		...
		if nCurSkillId > 0 then
			...
			if not tbSSetting then
				return false, "Cấu hình bất thường";
			end

			if tbSSetting.nType == tbSkillSetting.nType then
				if tbSSetting.nLevel >= tbSkillSetting.nLevel then
					return false, tbSSetting.nLevel == tbSkillSetting.nLevel and "Đã có kỹ năng tương tự" or "Đã có kỹ năng bậc cao cùng loại";
				else
	...
	if not bCanUse then
		return false, "Sách Kỹ Năng không thể dùng";
	end
...
function Partner:CheckCanGradeLevelup(pPlayer, nPartnerId)
	...
	if nGradeLevel >= #self.tbGradeLevelProtentialLimit then
		return false, "Đồng hành đã đột phá đến cực hạn";
	end
	...
	if nPLevel < tbNextInfo[1] then
		return false, "Hiện không cần đột phá";
	end
...
function Partner:CheckCanUseProtentialItem(pPlayer, nPartnerId, nProtentialType)
	...
	if not szProtentialType then
		return false, "Hãy chọn tư chất cần tăng";
	end

	local pPartner = pPlayer.GetPartnerObj(nPartnerId);
	if not pPartner then
		return false, "Đồng Hành không tồn tại";
	end
	...
	if not nType then
		return false, "Loại tư chất vô hiệu";
	end
	...
	if nPValue >= nLimitProtential then
		if bMax then
			return false, string.format("Tiềm năng %s đã đạt giới hạn", self.tbProtentialName[nProtentialType]);
		else
			return false, string.format("%s đã đạt giới hạn hiện tại, đột phá mới được tăng", self.tbProtentialName[nProtentialType]);
		end
	end

	local nCount, tbItem = pPlayer.GetItemCountInAllPos(self.nPartnerProtentialItem);
	if nCount <= 0 or not tbItem[1] then
		return false, "Tư Chất Đơn không đủ";
	end
...
function Partner:CheckReinitPartner(pPlayer, nPartnerId)
	local pPartner = pPlayer.GetPartnerObj(nPartnerId);
	if not pPartner then
		return false, "Không có đồng hành này!";
	end

	local tbPosInfo = pPlayer.GetPartnerPosInfo();
	for i = 1, 4 do
		if tbPosInfo[i] == nPartnerId then
			return false, "Đồng hành đã ra trận không thể tẩy tủy";
		end
	end
	...
	if not nCount or nCount < nDiscountCount then
		return false, "Tẩy Tủy Đơn không đủ";
	end
	...
	if not nProtentialItemCount then
		return false, "Đồng hành này không thể tẩy tủy";
	end
...
function Partner:GetGrowthTypeByTemplateId(nPartnerTemplateId)
	local tbPartnerInfo = self.tbAllPartnerInfo[nPartnerTemplateId] or {};
	return tbPartnerInfo.szGrowthType == "" and "Tinh Võ" or tbPartnerInfo.szGrowthType;
end
...
function Partner:CheckCanDecomposePartner(pPlayer, tbPartnerList)
	...
	for nPartnerId in pairs(tbPartnerList) do
		local pPartner = pPlayer.GetPartnerObj(nPartnerId);
		if not pPartner then
			return false, "Chưa phát hiện đồng hành cần bỏ!";
		end
		...
		if not nP then
			return false, string.format("%s không thể bỏ", pPartner.szName);
		end
...
{
	{"Sinh lực",		Partner.GetValueByBaseValue, "nLifeMax", "lifemax_v", "lifemax_p"},
	{"Tấn công",		Partner.GetBaseAttack},
	{"Thể chất",		"nVitality", "vitality_v"},
	{"Nhanh nhẹn",		"nDexterity", "dexterity_v"},
	{"Sức mạnh",		"nStrength", "strength_v"},
	{"Linh hoạt",		"nEnergy", "energy_v"},
	{"Tăng thể chất",	"Protential", "Vitality"},
	{"Tăng nhanh nhẹn",	"Protential", "Dexterity"},
	{"Tăng sức mạnh",	"Protential", "Strength"},
	{"Tăng linh hoạt",	"Protential", "Energy"},
	{"Hồi phục sinh lực",	"recover_life_v"},
	{"Kháng Kim",	Partner.GetResis, "metal"},		-- metal_resist_v all_series_resist_v
	{"Kháng Mộc",	Partner.GetResis, "wood"},		-- wood_resist_v
	{"Kháng Thủy",	Partner.GetResis, "water"},		-- water_resist_v
	{"Kháng Hỏa",	Partner.GetResis, "fire"},		-- fire_resist_v
	{"Kháng Thổ",	Partner.GetResis, "earth"},		-- earth_resist_v
	{"Chính xác",		Partner.GetValueByBaseValue, "nAttackRate", "attackrate_v", "attackrate_p"},
	{"Né tránh",		Partner.GetValueByBaseValue, "nDefense", "defense_v", "defense_p"},
	{"Bỏ qua né tránh",	"ignore_defense_v"},
	{"Tỉ lệ bạo kích",	Partner.GetDeadlyStrike},
	{"Sát thương bạo kích",	Partner.GetDeadlyStrikeDamage},
	{"Tỉ lệ kháng bạo kích",	"weaken_deadlystrike_v"},
	{"Miễn sát thương bạo kích",	Partner.GetBaseValueWhithP,	"weaken_deadlystrike_damage_p"},
	{"Tỉ lệ bị thương",	Partner.GetValueWithAllValue, "state_hurt_attackrate", "add_seriesstate_rate_v"},
	{"Tỉ lệ choáng",	Partner.GetValueWithAllValue, "state_stun_attackrate", "add_seriesstate_rate_v"},
	{"Tỉ lệ làm chậm",	Partner.GetValueWithAllValue, "state_slowall_attackrate", "add_seriesstate_rate_v"},
	{"Tỉ lệ tàn phế",	Partner.GetValueWithAllValue, "state_zhican_attackrate", "add_seriesstate_rate_v"},
	{"Tỉ lệ tê liệt",	Partner.GetValueWithAllValue, "state_palsy_attackrate", "add_seriesstate_rate_v"},
	{"Kháng tỉ lệ bị thương",	Partner.GetValueWithAllValue, "state_hurt_resistrate", "resist_allseriesstate_rate_v"},
	{"Kháng tỉ lệ choáng",	Partner.GetValueWithAllValue, "state_stun_resistrate", "resist_allseriesstate_rate_v"},
	{"Kháng tỉ lệ làm chậm",	Partner.GetValueWithAllValue, "state_slowall_resistrate", "resist_allseriesstate_rate_v"},
	{"Kháng tỉ lệ tàn phế",	Partner.GetValueWithAllValue, "state_zhican_resistrate", "resist_allseriesstate_rate_v"},
	{"Kháng tỉ lệ tê liệt",	Partner.GetValueWithAllValue, "state_palsy_resistrate", "resist_allseriesstate_rate_v"},
	{"Kháng thời gian bị thương",	Partner.GetValueWithAllValue, "state_hurt_resisttime", "resist_allseriesstate_time_v"},
	{"Kháng thời gian choáng",	Partner.GetValueWithAllValue, "state_stun_resisttime", "resist_allseriesstate_time_v"},
	{"Kháng thời gian làm chậm",	Partner.GetValueWithAllValue, "state_slowall_resisttime", "resist_allseriesstate_time_v"},
	{"Kháng thời gian tàn phế",	Partner.GetValueWithAllValue, "state_zhican_resisttime", "resist_allseriesstate_time_v"},
	{"Kháng thời gian tê liệt",	Partner.GetValueWithAllValue, "state_palsy_resisttime", "resist_allseriesstate_time_v"},
}

Partner.tbJingMaiExtAttribDef =
{
	{"Bỏ qua Kháng Kim", Partner.GetIgnoreResis, "ignore_metal_resist_v", "ignore_all_resist_v"},
	{"Bỏ qua Kháng Mộc", Partner.GetIgnoreResis, "ignore_wood_resist_v", "ignore_all_resist_v"},
	{"Bỏ qua Kháng Thủy", Partner.GetIgnoreResis, "ignore_water_resist_v", "ignore_all_resist_v"},
	{"Bỏ qua Kháng Hỏa", Partner.GetIgnoreResis, "ignore_fire_resist_v", "ignore_all_resist_v"},
	{"Bỏ qua Kháng Thổ", Partner.GetIgnoreResis, "ignore_earth_resist_v", "ignore_all_resist_v"},
	{"Ngăn sát thương nhân vật", Partner.GetAbsBaseValueWhithP, "playerdmg_npc_p"},
	{"Sát thương quái", Partner.GetBaseValueWhithP, "damage4npc_p"},
	{"Sát thương lên người chơi", Partner.GetBaseValueWhithP, "damage4player_p"},
	{"Sinh lực cơ bản", Partner.GetBaseValueWhithP, "lifemax_p"},
};
...
--	Lib:CallBack({PartnerCard.OnCreatePartnerNpc, PartnerCard, pNpc.nId, pPartner.nTemplateId, nSkillLevel});
	...
	if pPartner.GetAwareness() == 1 then
		...
		if tbAwareness and tbAwareness.nAwarenessSkillId and tbAwareness.nAwarenessSkillId > 0 then
			...
			if not string.find(pNpc.szName, "Thức tỉnh") then
				pNpc.SetName(string.format("%s-Thức Tỉnh", pNpc.szName));
			end
		end
	end
end
