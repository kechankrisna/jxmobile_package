...
function Partner:CheckCanAwareness(pPlayer, nPartnerId, tbUsePartnerInfo)
	...
	if not tbNeedInfo then
		return false, "Đồng Hành không thể thức tỉnh";
	end
	...
	if Lib:CountTB(tbUsePartnerInfo) ~= 5 then
		return false, "Nguyên liệu thức tỉnh không đủ";
	end
	...
	for i = 1, 5 do
		...
		for j = 1, 4 do
			if tbPosInfo[j] == nPId then
				return false, "Đồng Hành đã ra trận không thể dùng để thức tỉnh";
			end
		end

		if pUPObj.nTemplateId ~= tbNeedInfo["nCost" .. i] then
			return false, "Đồng Hành không phù hợp";
		end
		...
		if tbNeedInfo.bNeedWeapon and pUPObj.nWeaponState ~= 1 then
			...
			if nWeaponItemId then
				...
				if nCount <= 0 then
					return false, "Vũ khí bổn mạng Đồng Hành không đủ";
				end
		...
		if not nProtentialItemCount then
			return false, "Đồng Hành này không thể dùng để thức tỉnh";
		end
