...
function ChatDecorate:CanApply(pPlayer,nPartsID)
	...
	if nCurTime < GetTime() then
		return false,XT(string.format("Hãy nhận chủ đề %s trước hoặc chủ đề đã hết hạn",self:GetTitleByThemeId(nThemeID,pPlayer.nSex) or ""))
	end
