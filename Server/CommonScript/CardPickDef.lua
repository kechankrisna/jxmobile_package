...
nCoinCost = 200000;  --银两单抽价格
nCoinTenCost = 1800000;  --银两十连抽价格
nGoldCost = 2000;  --元宝单抽价格
nGoldTenCost = 4000;  --元宝十连抽价格
...
szKinMsg = "「%s」nhận được %s thông qua Rút 10 Lần Nguyên Bảo";
szWorldMsg = "「%s」nhận được %s thông qua Rút 10 Lần Nguyên Bảo";
};
...
for szItemType, tbId in pairs(CardPicker.Def.tbTenGoldPCForbidReplace) do
	CardPicker.Def.tbTenGoldPCForbidReplaceRef[szItemType] = {}
	for _, nId in ipairs(tbId) do
		CardPicker.Def.tbTenGoldPCForbidReplaceRef[szItemType][nId] = true
	end
end
...
function CardPicker:CheckPickCutAct()
	...
	if CardPicker:IsPickCutActOpen() then
		...
		if nWeekDay == nOpenWeekDay then
			...
			local szNewInfoMsg = "    Các vị đại hiệp, cho đến %s, dùng Nguyên Bảo chiêu mộ Đồng Hành 10 lần đầu sẽ được giảm giá 40%%, hãy mau đến chiêu mộ nào!";
			local szTimeDesc = Lib:TimeDesc9(self.nCardPickCutActValidTime);
			szNewInfoMsg = string.format(szNewInfoMsg, szTimeDesc);
			local tbSetting = { szTitle = "Ưu Đãi Chiêu Mộ Đồng Hành"};
...
function CardPicker:LoadCardPickSetting()
	...
	local tbCardPickerSetting = LoadTabFile("Setting/CardPicker/CardPicker.tab",
		"ssddddddddddddddddddddddds", nil, {"PickerType", "ItemType", "ItemId",
	...
	for nProbIdx = 0, #CardPicker.tbCardPickProbTimeFrameMap do
		...
		tbProbItems.tbItems["TenGold"] = LoadProbSetting("Prob" .. nProbIdx, "TenGold");
		tbProbItems.tbHighVipItems["TenGold"] = LoadProbSetting("ProbHighVip" .. nProbIdx, "TenGold");
	end
	...
	if MODULE_GAMESERVER then
		self.tbPickerTips = {}
		for _, tbLineData in ipairs(tbCardPickerSetting) do
			self.tbPickerTips[tbLineData.PickerType] = self.tbPickerTips[tbLineData.PickerType] or {}
			self.tbPickerTips[tbLineData.PickerType][tbLineData.ItemType] = self.tbPickerTips[tbLineData.PickerType][tbLineData.ItemType] or {}
			assert(not self.tbPickerTips[tbLineData.PickerType][tbLineData.ItemType][tbLineData.ItemId], string.format("[Error] Repeat Picker Item:%s, %s, %d", tbLineData.PickerType, tbLineData.ItemType, tbLineData.ItemId))
			self.tbPickerTips[tbLineData.PickerType][tbLineData.ItemType][tbLineData.ItemId] = {}
			self.tbPickerTips[tbLineData.PickerType][tbLineData.ItemType][tbLineData.ItemId].szName = tbLineData.Name
			self.tbPickerTips[tbLineData.PickerType][tbLineData.ItemType][tbLineData.ItemId].nKinTips = tbLineData.nKinTips
			self.tbPickerTips[tbLineData.PickerType][tbLineData.ItemType][tbLineData.ItemId].nWorldTips = tbLineData.nWorldTips
		end
	end
end
...
function CardPicker:GetTipInfo(szPickerType, szItemType, nItemId)
	return self.tbPickerTips[szPickerType] and self.tbPickerTips[szPickerType][szItemType] and self.tbPickerTips[szPickerType][szItemType][nItemId]
end
...
function CardPicker:IsPartnerCardChooseHit(nGoldTenCount)
	local nRate = CardPicker.Def.tbPartnerCardChooseRate[nGoldTenCount] or -1;
	return MathRandom() <= nRate;
end