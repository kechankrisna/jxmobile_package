...
ChuangGong.szRequestGoSafeTip = "Hiện không thể truyền công, về khu an toàn xin được truyền công?"
ChuangGong.szBtnGoSafeTip = "Hiện không thể truyền công, về khu an toàn tiến hành truyền công?"
...
function ChuangGong:CheckSelfChuanGong(pPlayer)
    ...
    if not tbKinData then
        return false, "Cần vào bang hội trước"
    end

    if GetTimeFrameState(self.szOpenFrame) ~= 1 then
        return false, "Hiện chưa mở Truyền Công"
    end
    ...
    if not Env:CheckSystemSwitch(pPlayer, Env.SW_ChuangGong) then
        return false, "Trạng thái hiện tại không thể tu luyện"
    end
...
function ChuangGong:CheckLevelLimi(nSendLevel, nGetLevel,nSendVip,nGetVip)
	if nGetLevel < self.nGetMinLevel then
		return false, self.nGetMinLevel .. " cấp sau mới được nhận truyền công"
	end
	...
	if nSendLevel < self.nSendMinLevel then
		return false, string.format("Người truyền công cần Lv%d", self.nSendMinLevel)
	end
    ...
    if nSendLevel - nGetLevel < nMinusMinLevel then
        return false, string.format("Hai bên chênh lệch cấp chưa đủ %d, không thể truyền công", nMinusMinLevel)
    end
