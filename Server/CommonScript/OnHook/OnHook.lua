...
function OnHook:CheckCommond(pPlayer)
	...
	if not self:IsOpen(pPlayer) then
		return false,"Ủy thác rời mạng chưa mở";
	end

	local nExpTime = self:ExpTime(pPlayer);
	if nExpTime < 60 then													-- 离线时间按分钟换算经验,所以不能小于60秒
		return false,"Không có EXP rời mạng để nhận"
	end
...
function OnHook:PayTips(pPlayer,nPayType)
	...
	if nExpTime > nBaiJuWanTime then
		...
		if nHaveNum > 0 then
			...
			szTip = szTip ..string.format("Đã có [FFFE0D]%d[-] %s",nHaveNum,szItemName);
		end
		...
		if nLackNum > 0 then																	-- 需要用元宝的情况
			...
			local nNeedPay = nLackNum * nPerBaiJuWanPrice;
			 ...
			 local szNeedBuy = string.format("Cần mua thêm [FFFE0D]%d[-] ",nLackNum)
			 if nHaveNum <= 0 then
				...
				if (pPlayer.nFaction >= 18) then
			 	...
			 	szNeedBuy = szNeedBuy .. szItemName
			 end
			 local szNeedPay = string.format(", tốn [FFFE0D]%d[-] %s",nNeedPay,szMoneyEmotion);
...
function OnHook:CheckStartOnlineHook(pPlayer)
	if not self:IsOpen(pPlayer) then
		return false,"Ủy thác online chưa mở";
	end

	if not Map:IsCityMap(pPlayer.nMapTemplateId) then
		return false,"Ở thành thị, Tân Thủ Thôn, Gia Viên và Tửu Quán mới được ủy thác"
	end

	if OnHook:OnHookTime(pPlayer) < 60 and not self:IsCrossDay(pPlayer,GetTime(),self:LoginTime(pPlayer)) then
		return false,"Thời gian ủy thác hôm nay đã hết"
	end

	if self:OnLineOnHookTime(pPlayer) > 0 then
		return false,"Đã mở treo máy online"
	end
...
function OnHook:CheckRequestOnlineOH(pPlayer)
	if pPlayer.nRequestStartOnlineOHTime and GetTime() < pPlayer.nRequestStartOnlineOHTime then
		...
		return false,string.format("Thao tác quá nhanh, %d giây sau hãy tiếp tục",nSecond)
	end
