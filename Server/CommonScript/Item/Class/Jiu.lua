...
tbJiuItem.tbJiuName		= {XT("Nữ Nhi Hồng")};				-- 每一种酒的Name
...
function tbJiuItem:OnUse(it)
	...
	if not nType or not nDuraTime or  nDuraTime == 0 or nType == 0 then
		me.CenterMsg("Tham số đạo cụ lỗi")
		return 0 
	end
	if me.GetNpc().GetSkillState(self.nJiuSkillId)  then
		me.CenterMsg("Đại hiệp tửu lượng còn kém, xin chờ lát sau hãy uống!")
		return 0
	end
...
--  技能Id, 等级, 状态类型(类型为0取的是表中时间,1取的是后面自定义时间), 时间, 死亡后是否消失, 覆盖原技能效果.
	...
	me.CenterMsg("Đã uống 1 bình Nữ Nhi Hồng")
	if me.dwTeamID ~= 0 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format("「%s」uống một bình %s", me.szName, it.szName), me.dwTeamID)
	else
		me.Msg(string.format("Đã uống 1 bình %s", it.szName))
	end
