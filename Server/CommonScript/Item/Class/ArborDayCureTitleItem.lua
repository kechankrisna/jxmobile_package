local szActingTip = "Hoạt động chưa kết thúc"
...
function tbItem:OnClientUse(pItem)
    ...
    if not tbTitle then
        me.CenterMsg("Điểm hoạt động không tồn tại")
        return 1
    end
...
function tbItem:OnRequestUse(pPlayer, nTitleId, nItemID)
    ...
    if not tbTitle then
        pPlayer.CenterMsg("Điểm hoạt động không tồn tại")
        return
    end

    if not tbTitle[nTitleId] then
        pPlayer.CenterMsg("Không thể chọn danh hiệu này")
        return
    end
    ...
    if Item:Consume(pItem, 1) < 1 then
        pPlayer.CenterMsg("Đạo cụ tiêu hao thất bại, hãy thử lại")
        return
    end
