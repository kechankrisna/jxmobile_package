...
function tbItem:OnUse(pItem)
	...
	if nItemCount > 0 then
		...
		local szMsg = string.format("Đồng ý tạo mới ngẫu nhiên [FFFE0D]%s[-]?", pBrocadeBoxItem.szName)
		me.MsgBox(szMsg, {{"Đồng ý", function ()
					...
					if not pBrocadeBoxItem or not pRerandItem then
						me.CenterMsg("Đạo cụ không tồn tại")
						return
					end
		...
		me.CenterMsg("Hành trang của các hạ không có hộp gấm")
	end
