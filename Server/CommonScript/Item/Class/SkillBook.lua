...
function tbBook:CheckUseEquip(pPlayer, pEquip, nEquipPos)
    ...
    if not nOpenLevel then
        return false, "Đã tối đa!";
    end

    if nOpenLevel > pPlayer.nLevel then
        return false, string.format("Tăng đến Lv%s có thể trang bị nhiều bí kíp hơn", nOpenLevel);
    end
    ...
    if tbBookInfo.LimitFaction > 0 and tbBookInfo.LimitFaction ~= pPlayer.nFaction then
        return false, string.format("Phái không hợp!");
    end

    local bRet = self:HaveSkillBook(pPlayer, pEquip.dwTemplateId);
    if bRet then
        return false, "Cùng loại bí kíp tối đa chỉ được trang bị 1";
    end
...
function tbBook:CheckBookLevelUp(pPlayer, nItemID)
    ...
    if not tbBookNextLevel then
        return false, "Đã đã đạt cấp tối đa!";
    end
    ...
    if tbBookLevelInfo.CostExp > nBookExp then
        ...
        return false, "Tu vi không đủ, không thể tăng cấp" .. szMsg;
    end
...
function tbBook:AutCostXiuWeiBook(pPlayer)
    ...
    for _, nXiuWeiID in ipairs(self.tbAllXiuWeiBook) do
        ...
        if pItem then
           ...
           return true, ", tự động tốn 1 quyển"..szName;
        end
    end
...
function tbBook:DoBookLevelUp(pPlayer, nItemID)
    ...
    pPlayer.CenterMsg("Bí kíp tăng 1 cấp!");
...
function tbBook:BookSkillLevelUp(pPlayer, pEquip, tbBookLevelInfo)
    ...
    pPlayer.CenterMsg("Lĩnh ngộ thành công, kỹ năng bí kíp tăng 1 cấp!");
...
function tbBook:CheckRecycleSkillBook(pPlayer, nItemID)
    ...
    if nBookLevel <= 1 then
        return false, string.format("Không thể hủy bí kíp Lv%s", nBookLevel);
    end

    if tbBookInfo.LimitRecycle == 1 then
        return false, "Không thể hủy bí kíp";
    end
...
function tbBook:RecycleSkillBook(pPlayer, nItemID)
    ...
    pPlayer.CenterMsg(string.format("Hủy bí kíp được %s Tu Vi", nCostExp));
...
function tbBook:CheckBookUpgrade(pPlayer, nItemID)
    ...
    if tbBookInfo.UpgradeItem <= 0 then
        return false, "Bí kíp không thể tăng bậc!";
    end

    if tbBookInfo.UpgradeID <= 0 then
        return false, "Chưa mở tăng bậc!";
    end
    ...
    if GetTimeFrameState(tbBookUpgrade.TimeFrame) ~= 1 then
        return false, string.format("%s nữa để tăng bậc", Lib:TimeDesc2(CalcTimeFrameOpenTime(tbBookUpgrade.TimeFrame) - GetTime()));
    end

    if pPlayer.nLevel < tbBookUpgrade.PlayerLevel then
        return false, string.format("Chưa đạt cấp %s", tbBookUpgrade.PlayerLevel);
    end

    local nBookLevel = pEquip.GetIntValue(self.nSaveBookLevel);
    if nBookLevel < tbBookUpgrade.BookLevel then
        return false, string.format("Bí kíp chưa đạt cấp %s", tbBookUpgrade.BookLevel);
    end

    local nBookSkillLevel = pEquip.GetIntValue(self.nSaveSkillLevel);
    if nBookSkillLevel < tbBookUpgrade.BookSkillLevel then
        return false, string.format("Kỹ năng Bí kíp chưa đạt cấp %s", tbBookUpgrade.BookSkillLevel);
    end
    ...
    for _, tbCost in pairs(tbBookUpgrade.tbAllCostIteam) do
        ...
        if nCount < tbCost.nItemCount then
			...
			if (pPlayer.nFaction >= 18) then
				szName = Item:GetNewItemShowInfo(tbCost.nItemTID, pPlayer.nFaction, pPlayer.nSex);
			end
			
            return false, string.format("%s không đủ %s", szName, tbCost.nItemCount);
        end
    end
...
function tbBook:DoBookUpgrade(pPlayer, nItemID)
    ...
    if nTotalXiuExp > 0 then
        ...
        pPlayer.CenterMsg(string.format("Bí kíp sau khi tăng bậc, nhận được %s Tu Vi", nTotalXiuExp), true);
    end

    local nBookLevel = pEquip.GetIntValue(self.nSaveBookLevel);
    local nSkillLevel = pEquip.GetIntValue(self.nSaveSkillLevel);
    pPlayer.CenterMsg("Bí kíp tăng bậc thành công!", true);
...
function tbBook:CheckBookTuPo(pPlayer, nItemID)
    ...
    if not tbTuPoInfo then
        return false, "Bí kíp không thể đột phá";
    end

    local nBookLevel = pEquip.GetIntValue(self.nSaveBookLevel);
    if nBookLevel < tbTuPoInfo.BookLevel then
        return false, string.format("Bí kíp chưa đạt cấp %s", tbTuPoInfo.BookLevel);
    end

    local nBookSkillLevel = pEquip.GetIntValue(self.nSaveSkillLevel);
    if nBookSkillLevel < tbTuPoInfo.BookSkillLevel then
        return false, string.format("Kỹ năng Bí kíp chưa đạt cấp %s", tbTuPoInfo.BookSkillLevel);
    end
    ...
    if nBookExp < tbTuPoInfo.CostExp then
        return false, string.format("Bí kíp không đủ %s Tu Vi", tbTuPoInfo.CostExp);
    end
...
function tbBook:DoBookTuPo(pPlayer, nItemID)
    ...
    pEquip.ReInit();
    pPlayer.CenterMsg("Bí kíp đột phá thành công!", true);
