...
tbItem.szDBZBlackMsg = "Bị Đoạt Bảo Tặc mai phục...";
tbItem.szBossBlackMsg = "Không cẩn thận đào phải sào huyệt Thủ Lĩnh Dã Ngoại...";
...
function tbItem:CheckCanWaBao(pPlayer, nItemId)
	...
	if not pItem or pItem.szClass ~= "CangBaoTu" then
		return false, "Tàng Bảo Đồ đâu rồi!";
	end
...
function tbItem:OnEndProgress(nItemId)
	...
	if not bRet then 						-- 没有触发事件
		...
		if nRet ~= 1 then
			me.CenterMsg("Đào kho báu thất bại rồi!");
			return;
		end
	...
	if (bRet and self.TYPE_NORMAL == nItemType) or nCount <= 1 then
		...
		me.CallClientScript("Ui:OpenQuickUseItem", nItemDwId, "Dùng");
		if self.TYPE_NORMAL == nItemType then
...
function tbItem:OnUse(it)
	...
	if nItemType == self.TYPE_SENIOR then 
		if me.nLevel < nSeniorUseLevel then
			me.CenterMsg(string.format("Đại hiệp cần rèn luyện thêm, cần đạt Lv%d",nSeniorUseLevel))
			return
		end
	...
	if nItemtype == self.TYPE_SENIOR then 								-- 读条前判断
		...
		if not bAllReady then
			me.CenterMsg(szTips);
			me.CallClientScript("Ui:OpenQuickUseItem", it.dwId, "Dùng");
			return
		end
	end
	self:UpdateHeadState(me, true, self.tbSetting[nItemtype].nWaBaoTime);
	GeneralProcess:StartProcessExt(me, self.tbSetting[nItemtype].nWaBaoTime * Env.GAME_FPS, true, 0, 0, "Đang đào kho báu", {self.OnEndProgress, self, it.dwId}, {self.OnBreakProgress, self, it.dwId});
	return 0;
end
...
function tbItem:CheckTeamMember(it,bCheckCommon)
	...
	if not me.dwTeamID or me.dwTeamID == 0 then
		return false,string.format("Cần %s người trở lên đến đào kho báu",self.nNeedTeamate);
	end

	local tbMember = TeamMgr:GetMembers(me.dwTeamID)
	if not next(tbMember) then
		return false,"Có đồng đội mới có thể đào kho báu";
	end
	...
	if nMapMemberCount < Lib:CountTB(tbMember) then
		return false,"Đợi tất cả thành viên tập trung đủ rồi đào"
	end
