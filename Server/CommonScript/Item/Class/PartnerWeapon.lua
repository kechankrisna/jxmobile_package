...
function tbItem:OnUse(pItem)
	...
	if not nPartnerId then
		...
		me.CenterMsg(string.format("Hãy xuất trận Đồng Hành【%s】", szPName));
		return;
	end

	if tbPartner.nWeaponState ~= 0 then
		me.CenterMsg(string.format("【%s】đã có vũ khí này", tbPartner.szName));
		return;
	end
	...
	me.MsgBox(string.format("Trang bị [FFFE0D]%s[-] Vũ Khí Bổn Mạng [FFFE0D]%s[-]?\n[FFFE0D](Sau khi trang bị không được gỡ, sa thải sẽ hoàn trả)[-]", tbPartner.szName, szItemName),
		{
			{"Đồng ý", self.UseWeapon, self, me.dwID, nPartnerId, pItem.dwId},
			{"Hủy"}
		});
	return;
end
