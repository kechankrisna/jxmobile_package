...
function tbSeriesStone:FormatTips(tbRandomAttribGroup, tbActiveDark, nEquipLevel, nEquipPos, nSeries)
	...
	table.insert(tbAttribs, {string.format("Vị trí: %s", Item.EQUIPPOS_NAME[nEquipPos]) , 1 });
	table.insert(tbAttribs, {"Thuộc tính cơ bản"});

	for i,v in ipairs(tbRandomAttribGroup[1]) do
		table.insert(tbAttribs, v)
	end
	table.insert(tbAttribs, {string.format("Kích Hoạt Ngũ Hành") });	
