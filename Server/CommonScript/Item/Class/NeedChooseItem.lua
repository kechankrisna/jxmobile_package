...
function tbItem:OnSelectItem(pItem, tbChoose)
    ...
    for nTemplateId, nCNum in pairs(tbChoose) do
        ...
        if not tbItemChoose then
            me.CenterMsg("Danh sách chọn có đạo cụ chưa mở");
            return;
        end
        ...
        if nCNum > nNum then
            me.CenterMsg("Vượt quá số lượng được chọn")
            return
        end
    ...
    if nCurAllNeedCount > 0 then
        ...
        if me.GetItemCountInAllPos(dwTemplateId) < nCurAllNeedCount then
            me.CenterMsg(string.format("Đạo cụ không đủ %s", nCurAllNeedCount));
            return
        end
    ...
    if nChoosedNum ~= nCanChooseNum then
        me.CenterMsg(string.format("Hãy chọn lại %d đạo cụ", nCanChooseNum))
        return
    end
...
function tbItem:OpenWindow(nUiX, nUiY, nTID, nID)
    ...
    if not tb4Choose then
        me.CenterMsg("Không có đạo cụ có thể chọn")
        return
    end
