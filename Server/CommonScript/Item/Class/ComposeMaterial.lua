...
function tbItem:GetIntroBottom(nTemplateId)
	if Compose.EntityCompose:IsNeedConsume(nTemplateId) then
		...
		return string.format("Ghép tiêu hao %s %d", szMoneyEmotion, nConsumeCount);
	end
end
...
function tbItem:GetUseSetting(nTemplateId, nItemId)
	...
	if not Compose.EntityCompose:CheckIsCanCompose(me, nTemplateId) then--and tbItemBase.szClass == "horse_equip" then
		...
		if Shop:CanSellWare(me, nItemId, 1) then
			return {szFirstName = "Bán", fnFirst = "SellItem", szSecondName = "Xem trước", fnSecond = fnPreview};
		end
		return {szFirstName = "Xem trước", fnFirst = fnPreview};
	else
		if Shop:CanSellWare(me, nItemId, 1) then
			return {szFirstName = "Bán", fnFirst = "SellItem", szSecondName = "Ghép", fnSecond = "ComposeEntityItem"};
		end
		return {szFirstName = "Ghép", fnFirst = "ComposeEntityItem"};
	end
end
...
function EquipMeterial:GetIntrol(nTemplateId, nItemId)
	...
	if nSumPrice then
		local _, szMoneyEmotion = Shop:GetMoneyName(szMoneyType)
		szSellIntro = string.format("\n\n\n[73cbd5]Bán nhận: %s%d[-]", szMoneyEmotion, nSumPrice)
	end
