...
function tbItem:OnUse(it)
	...
	if nPlayInterval < 3 then
		me.CenterMsg(string.format("[FFFE0D]%s giây[-] sau mới được dùng", 3 - nPlayInterval > 0 and 3 - nPlayInterval or 0))
		return
	end
...
function tbItem:IsWhiteMap(pPlayer)
    ...
    return false, "Chỉ được dùng tại [FFFE0D]Lễ Đường, Vong Ưu Đảo, Thành Chính, Lãnh Địa Bang Hội, Gia Viên[-]"
end
...
if not MODULE_GAMESERVER then
	function tbItem:OnUseEnd(nCount, nItemId)
		...
		if nCount <= 1 then
			...
			if pItem then
				self:OpenQuickUseItem(pItem.dwId, "Dùng")
			else
				Ui:CloseWindow("QuickUseItem")
			end
		else
			self:OpenQuickUseItem(nItemId, "Dùng")
		end
	end
