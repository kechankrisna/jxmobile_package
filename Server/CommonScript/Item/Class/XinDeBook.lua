...
function tbXinDe:CheckUseXinDe(pPlayer, pItem)
    ...
    if pPlayer.nLevel < tbItemInfo.MinPlayerLevel then
        return false, string.format("Chưa đạt Lv%s, không thể dùng Sách Tâm Đắc", tbItemInfo.MinPlayerLevel);
    end

    if pPlayer.nLevel > tbItemInfo.PlayerLevel then
        return false, string.format("Cao hơn Lv%s, không thể dùng Sách Tâm Đắc này", tbItemInfo.PlayerLevel);
    end

    local nPlayerNextExp = pPlayer.GetNextLevelExp();
    local nPlayerExp = pPlayer.GetExp();
    if nPlayerExp >= nPlayerNextExp then
        return false, string.format("EXP đã đạt tối đa, không thể dùng Sách Tâm Đắc");
    end

    local nCount = DegreeCtrl:GetDegree(pPlayer, "XinDeBook");
    if nCount <= 0 then
        return false, "Số lần dùng hôm nay không đủ";
    end
...
function tbXinDe:GetTip(pItem)
    ...
    local szMsg = string.format("Cấp Sách Tâm Đắc: %s\nCòn được dùng: %s lần\nNhận EXP: %s", tbItemInfo.PlayerLevel, nCount, nGetExp);
    return szMsg;
end
...
function tbXinDe:GetUseSetting(nItemTemplateId, nItemId)
    ...
    return {szFirstName = "Tặng", fnFirst = fnSong, szSecondName = "Dùng", fnSecond = "UseItem"};
end
...
function tbEmpty:GetTip(pItem)
    ...
    local szMsg = string.format("Cấp: %s\nTu luyện: %s%%", tbLevelInfo.PlayerLevel, math.floor(nSaveExp / nMaxXiuLianExp * 100));
    return szMsg;
end
