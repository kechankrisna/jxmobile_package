...
function tbEquip:GetTipOneAttrib(nDetailType, nItemType, nEquipLevel, tbAttrib, nOrgEquipLevel)
	...
	if szExt then
		local nChangeQuality = Item.GoldEquip.DetailTypeColorQuality[nDetailType]
...
function tbEquip:GetLightAttribMsg(pPlayer)
    ...
    if Lib:CountTB(tbAttibMsg) > 0 and nEndTime > 0 then
    	...
    	local szTime = string.format("Hạn: %s", Lib:TimeDesc2(nRetTime));
...
function tbEquip:FormatTips(nTemplateId, tbBaseAttrib, tbRandomAttrib, tbInsetAttrib, tbSuitAttrib, tbLightAttirb, tbSuitInfo, tbTrainAttrib, tbRecordStoneInfo, tbSeriesAttribInfo, nPlayerLevel)
	...
	for i, v in ipairs(tbBaseAttrib) do
		...
		if type(szText) == "table" then
			...
			if tbV[1] and not Lib:IsEmptyStr(tbV[1][3]) then
				tbV[1][3] = string.format("(Cường hóa %s)", szText[3])
			end
	...
	if next(tbRandomAttrib) then
		...
		table.insert(tbAttribs, {string.format("Thuộc tính kèm (%d/%d)", #tbRandomAttrib, nFullCount) });
	...
	if tbRecordStoneInfo and next(tbRecordStoneInfo) then
		table.insert(tbAttribs, {string.format("Khắc (%d/3)", #tbRecordStoneInfo )});
	...
	if tbTrainAttrib and next(tbTrainAttrib) then
		table.insert(tbAttribs, {"Thuộc tính tinh luyện"});
	...
	if tbSuitAttrib and next(tbSuitAttrib) and tbSuitInfo  then
		table.insert(tbAttribs, {string.format("Thuộc tính bộ (%d/%d)", tbSuitInfo[2] ,tbSuitInfo[1])});
	...
	if next(tbInsetAttrib) or nHoleCount > 0 then
		...
		table.insert(tbAttribs, {string.format("Khảm Đá Hồn (%d/%d)", #tbInsetAttrib, nHoleCount)});
		...
		for i = #tbInsetAttrib + 1, nHoleCount do
			table.insert(tbAttribs, {"", 1, "none", nil, "", "[C8C8C8](Chưa khảm)[-] ", szFrameColor });
		end
	end
	if tbSeriesAttribInfo then
		local nSeries = tbSeriesAttribInfo.nSeries
		table.insert(tbAttribs, {"Thuộc tính cơ bản Đá Ngũ Hành"});
		...
		table.insert(tbAttribs, {string.format("Kích Hoạt Ngũ Hành (%d/%d)", nActiveNum,  #tbSeriesAttribInfo.tbAttribs[2]) });

		table.insert(tbAttribs, {string.format("Ngũ Hành: [%s]%s[-]", Npc.SeriesColor[nSeries], Npc.Series[nSeries]), 1 });
	...
	if tbLightAttirb and next(tbLightAttirb) then
		table.insert(tbAttribs, {"Thuộc tính Phụ Ma"});
