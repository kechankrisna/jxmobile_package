...
function tbItem:CheckUseEquip(pPlayer, pEquip, nEquipPos)
	...
	if not JueXue:CheckFaction(pEquip.dwTemplateId, pPlayer.nFaction) then
		return false, "Không thể trang bị tuyệt học của môn phái này"
	end

	if not JueXue:CheckSoleTag(pPlayer, pEquip.dwTemplateId) then
		return false, "Đã trang bị Tuyệt Học cùng loại"
	end
...
function tbItem:OnClientUse(pEquip)
	...
	if not nPos then
		me.CenterMsg("Ô Tuyệt Học đã đầy")
	else
...
function tbItem:GetAttribs(nItemTID, tbValue)
	local tbAttribs = {}
	table.insert(tbAttribs, {"Thuộc tính Tuyệt Học"})
	...
	table.insert(tbAttribs, {"Hiệu quả kỹ năng"})
	...
	local szLevelDesc         = string.format("Cấp: %d/%d", nSkillLevel, nSkillMaxLevel)
	...
	if nSkillLevel >= nSkillMaxLevel then
		...
		if nAreaId then
			...
			if nSkillLevel < nXL then
				...
				return string.format("Cấp kế cần Tu Vi Tuyệt Học đạt tầng %d", nNeedXW)
			else
				local nNeedXL = (nSkillLevel + 1) * tbDef.nXiuLian4SkilllLv
				return string.format("Cấp tối đa kỹ năng kế cần Tuyệt Học luyện đến Lv%d", nNeedXL)
			end
		end
	end
	return ""
...
function tbItem:GetCustomDescInTips(nItemId)
	...
	if pEquip and pEquip.nPos ~= Item.emITEMPOS_BAG then
		local nAreaId = JueXue:GetJuexueAreaId(pEquip.nPos)
		if nAreaId then
			return string.format("Tu vi Tuyệt Học: Tầng %d", JueXue:GetAreaXiuWei(me, nAreaId))
		end
	end
end
