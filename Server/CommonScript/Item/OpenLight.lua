...
function OpenLight:CheckOpenLight(pPlayer, nItemID)
    ...
    if pItem.szClass ~= "OpenLightStone" then
        return false, "Hãy dùng Phụ Ma Thạch";
    end
    ...
    if not pEquip then
        return false, "Hãy trang bị vũ khí rồi tiến hành phụ ma"
    end    

    if tbLightInfo.WeaponRealLevel > pEquip.nRealLevel then
        return false, string.format("Trang bị %s trở lên mới phụ ma", self.tbRealLevlMsg[tbLightInfo.WeaponRealLevel] or "Trang bị hiếm Lv40");
    end
    ...
    if tbCurLight and tbCurLight.LightType == tbLightInfo.LightType and 
       ...
       return false, "Không thể đổi thuộc tính cao"
    end    
...
function OpenLight:CheckUseLightType(pPlayer, nItemID)
    ...
    local szMsg = string.format("Dùng [64db00]%s[-] thay thế [64db00]%s[-]", pItem.szName, tbCurItemInfo.szName);
    if tbRetInfo.nRetCount > 0 then
        local szName = KItem.GetItemShowInfo(tbRetInfo.nStoneID);
        szMsg = szMsg..string.format(", nhận được [64db00]%s %s (Phong)[-]", tbRetInfo.nRetCount, szName);
    end
...
function OpenLight:OpenLightItem(pPlayer, nItemID)
    ...
    pPlayer.CenterMsg("Phụ ma vũ khí thành công!");
...
if MODULE_GAMESERVER and tbCurLightInfo then
    local szMailInfo = string.format("    【%s】đã hết hạn.", KItem.GetItemShowInfo(tbCurLightInfo.LightStoneID) or "-");
    local tbMail =
    {
        To = me.dwID,
        Text = szMailInfo;
