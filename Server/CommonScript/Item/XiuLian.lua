...
--- 策划填写-----------------------------
tbDef.nAddExpPercent = 1000; --击杀野怪的经验倍率
...
tbDef.nXiuLianExpTiLi = 300; --每30分钟经验上限（体力基准经验）
...
function XiuLian:CheckOpenXiuLian(pPlayer, nOpenTime)
    if tbDef.nXiuLianLevel > pPlayer.nLevel then
        return false, string.format("Chưa đạt cấp %s", tbDef.nXiuLianLevel);
    end

    local nResidueTime = self:GetXiuLianResidueTime(pPlayer);
    if nOpenTime > nResidueTime then
        return false, "Thời gian tu luyện còn lại không đủ";
    end

    local nStateTime = self:GetCurXiuLianTime(pPlayer)
    nStateTime = nStateTime + nOpenTime;
    if nStateTime >= tbDef.nMaxOpenTime then
        return false, string.format("Tối đa mở %s giờ tu luyện", math.floor(tbDef.nMaxOpenTime / 60 / 60));
    end

    local bRet = Map:IsFieldFightMap(pPlayer.nMapTemplateId);
    if not bRet then
        return false, "Hãy đến khu vực dã ngoại rồi mở tu luyện";
    end    
...
function XiuLian:OpenXiuLianTime(pPlayer)
    ...
    pPlayer.CenterMsg("Đã mở tu luyện 30 phút");
...
function XiuLian:OnPlayerLevelUp(pPlayer, nLevel)
    ...
    Mail:SendSystemMail({
        To = pPlayer.dwID,
        Title = "Tu Luyện Châu",
        Text = [==[    Ngọc thần kỳ có thể tăng tu luyện đánh quái dã ngoại 10 lần, được gọi là 【Tu Luyện Châu】. Tu Luyện Châu 4:00 mỗi ngày tự động hồi phục 1 giờ tu luyện, tối đa chỉ tích lũy 7 giờ tu luyện, trạng thái tu luyện chỉ hiệu quả khi diệt quái ở khu vực dã ngoại.]==],
        From = "Nạp Lan Chân",
