...
function tbChangeColor:DoChangeColor(pPlayer, nItemId, nTargetId, bConfirm)
	...
	for nId, _ in pairs(tbInfo.tbItemList) do
		...
		for _, pCurItem in pairs(tbItemList) do
			if pCurItem.dwTemplateId == nTargetId then
				pPlayer.CenterMsg("Ngoại trang đã có màu này.");
				return;
			end
			local szName = KItem.GetItemShowInfo(nId, pPlayer.nFaction, pPlayer.nSex)
			
			if (pPlayer.nFaction >= 18) then
				szName = Item:GetNewItemShowInfo(nId, pPlayer.nFaction, pPlayer.nSex);
			end
	...
	if not pItem then
		pPlayer.CenterMsg("Ngoại trang không tồn tại")
		return;
	end
	if not self:CanChangeColor(pItem.dwTemplateId) then
		pPlayer.CenterMsg("Trang bị không thể nhuộm màu!")
		return
	end

	if not tbInfo.tbItemList[pItem.dwTemplateId] then
		pPlayer.CenterMsg("Ngoại trang không thể nhuộm màu này");
		return;
	end

	if not self:CanColorItemShow(pPlayer, nTargetId) then
		pPlayer.CenterMsg("Không có Ngoại Trang Màu này");
		return;
	end
	...
	if pPlayer.ConsumeItemInBag(nConsumeItem, nConsumeCount, Env.LogWay_ChangeColor, nil, nTargetId) < nConsumeCount then
		...
		pPlayer.CenterMsg(string.format("%s không đủ, không thể nhuộm màu", tbBaseProp.szName));
		return;
	end
	...
	pPlayer.CenterMsg("Nhuộm thành công!");
end
...
function tbChangeColor:AddShowColor(pPlayer, nTargetId)
	...
	if not tbLimitInfo then
		pPlayer.CenterMsg("Ngoại Trang Màu tương ứng không tồn tại");
		return false;
	end
	...
	if (pPlayer.nFaction >= 18) then
		szOrgItemName = Item:GetNewItemShowInfo(tbLimitInfo.OrgColorItem, pPlayer.nFaction, pPlayer.nSex);
	end
	
	
	local pOrgColorItem = unpack(pPlayer.FindItemInPlayer(tbLimitInfo.OrgColorItem) or {});
	if not pOrgColorItem then
		pPlayer.CenterMsg(string.format("Chưa có ngoại trang ban đầu [ffff00]%s[-], thu thập ngoại trang này rồi thử lại", szOrgItemName))
		return false;
	end

	if self:ItemHasShowColor(pOrgColorItem, tbLimitInfo.Position) then
		pPlayer.CenterMsg("Đại hiệp đã học cách nhuộm này, không thể học lại");
		return false;
	end
	...
	if (pPlayer.nFaction >=  18) then
		szItemName = Item:GetNewItemShowInfo(nTargetId, pPlayer.nFaction, pPlayer.nSex);
	end
	
	pPlayer.CenterMsg(string.format("Đại hiệp đã học cách nhuộm [ffff00]%s[-], nhuộm xong nhớ dùng ngoại trang này!", szItemName));
	return true;
end
