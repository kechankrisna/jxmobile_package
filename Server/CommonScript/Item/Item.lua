...
--	end
	...
	if pItem.nUseLevel > me.nLevel then
		return 0, string.format("Sau Lv%d dùng", pItem.nUseLevel);
	end
	-- TODO cd
...
function Item:UseEquip(nId, bSlient, nEquipPos)
	...
	if nResult == 1 then
		...
		if nEquipPos == Item.EQUIPPOS_HORSE then
			...
			me.CenterMsg(string.format("%s xuất trận thành công", szEquipName));
		end
...
function Item:UnuseEquip(nPos)
	...
	if not pEquip then
		me.CenterMsg("Vị trí hiện tại không có trang bị")
		return
	end
	local tbEquipClass;
	if not Lib:IsEmptyStr(pEquip.szClass) then
...
function Item:NotifyTimeOut(dwTemplateId, nCount)
	...
	local szMsg = string.format("Vật phẩm %s đã quá hạn!", szItemName)
	me.CenterMsg(szMsg);
	me.Msg(szMsg)
...
function Item:GetItemTimeOut(pItem)
	if pItem and pItem.GetIntValue(-9996) > 0 then
		return os.date("%Y-%m-%d %H:%M:%S", pItem.GetIntValue(-9996))
	end
end
...
--===========================================================================
Item.tbEquipShowType = Item.tbEquipShowType or {};
Item.tbEquipShow = Item.tbEquipShow or {};
function Item:LoadNewEquipShowType()
	self.tbEquipShowType = {};
	
	local tbItemSettingFile = LoadTabFile("Setting/Item/ItemSetting.tab", "dss", nil, {"FileType","FilePath", "Remark"});
	
	if (not tbItemSettingFile) then
		return;
	end
	for _,tbInfo in pairs (tbItemSettingFile) do
		if (tbInfo.FilePath) then
				local tbItemSet = LoadTabFile(tbInfo.FilePath, "ddd", "TemplateId", {"TemplateId","ShowType", "ItemType"});
				if (tbItemSet) then
					for _,tbI in pairs(tbItemSet) do
						self.tbEquipShowType[tbI.TemplateId] = tbI.ShowType;
					end
				end
...
function Item:LoadNewEquipShowSetting()
	self.tbEquipShow = {};
	local tbShowRes = {};
	local tbRes = LoadTabFile("Setting/Item/EquipShow.tab", "dssddd", nil, {"ShowType","Name", "Faction", "Sex", "IconId","BigIconId"});
	if (tbRes) then
		for _, tbInfo in ipairs(tbRes) do
			local nSex = tonumber(tbInfo.Sex);
			...
			for _, szFaction in ipairs( tbFactions ) do
				...
				local nShowType = tonumber(tbInfo.ShowType);
				tbShowRes[nShowType] = tbShowRes[nShowType] or {}
				tbShowRes[nShowType][nFaction] = tbShowRes[nShowType][nFaction] or {};
				if (nSex <= 0) then
					tbShowRes[nShowType][nFaction][1] = {tbInfo.Name,tbInfo.IconId,tbInfo.BigIconId};
					tbShowRes[nShowType][nFaction][2] = {tbInfo.Name,tbInfo.IconId,tbInfo.BigIconId};
				else
					tbShowRes[nShowType][nFaction][nSex] = {tbInfo.Name,tbInfo.IconId,tbInfo.BigIconId};
				end					
			end			
		end
	end
	self.tbEquipShow = tbShowRes;
end

function Item:GetNewEquipShowType(nTemplate)
	...
	return self.tbEquipShowType[nTemplate] or 0;
	
end
function Item:GetNewItemShowInfo(nTemplate, nFaction , nSex)
	local szName,nIcon,nView,nQuality;
	if (nFaction >= 18) then
		
		szName, nIcon, nView, nQuality = KItem.GetItemShowInfo(nTemplate, 15 , nSex);
		
		local nShowType = self:GetNewEquipShowType(nTemplate);
		if (nShowType > 0) then
			local tbItemShowRes = self.tbEquipShow;
			if (tbItemShowRes) then
				tbItemShowRes = self.tbEquipShow[nShowType];
			end
			if (tbItemShowRes) then
				tbItemShowRes = Item.tbEquipShow[nShowType][nFaction];
			end
			if (tbItemShowRes) then
				tbItemShowRes = Item.tbEquipShow[nShowType][nFaction][nSex];
			end
			if (tbItemShowRes) then
				szName = tbItemShowRes[1];
				nIcon = tbItemShowRes[2];
				nView = tbItemShowRes[3];
			end
		end
	else
		szName, nIcon, nView, nQuality = KItem.GetItemShowInfo(nTemplate, nFaction , nSex);
	end
	
	return szName,nIcon,nView,nQuality;  
end
function Item:NewEquipInit()
	self:LoadNewEquipShowSetting();
	self:LoadNewEquipShowType();
end
--===========================================================================
