...
function Kin:GetBuildingUpgradeCost(nBuildingId, nNextLevel)
	...
	assert(nCost, string.format("KinBuilding.tab danh sách điền bị lỗi %d, %d", nBuildingId, nNextLevel));
...
function Kin:RedBagIsModifyValid(nEventId, nAddGold, nCount, nVip)
	if not nEventId or nEventId<=0 then
		if nAddGold>0 then
			return false, "Lì Xì này không thể tăng Nguyên Bảo"
		else
			return true
		end
	end
	...
	if nCount<tbSetting.nCount then
		return false, "Số Lì Xì không thể ít hơn nữa"
	end

	local bGlobal = self:RedBagIsEventGlobal(nEventId)
	if not bGlobal and nCount>100 then
		return false, "Số Lì Xì không thể nhiều hơn nữa"
	end
	...
	if nAddGold>0 then
		...
		if not bValidAddGold then
			return false, "Không ủng hộ tăng mức quá cao"
		end

		if not self:RedBagIsMultiValid(nVip, tbSetting.nBaseGold, nAddGold) then
			return false, "Cấp Kiếm Hiệp chưa đạt"
		end
	end
...
function Kin:RedBagGetContent(tbRedBag)
	...
	elseif nTypeId==Kin.tbRedBagEvents.wsd_rank then
		local szRank = string.format("Top %d", tbSetting.nTarget)
...
function Kin:IsNameValid(szName)
	...
	if string.find(szName, "%s") then
		return false, "Tên bang hội không thể chứa khoảng trống"
	end
	...
	if bInvalidWords then
		return false, "Tên bang hội chứa ký tự nhạy cảm, hãy sửa rồi thử lại"
	end
	...
	if version_tx or version_th or version_kor then
		...
		if nNameLen > Kin.Def.nMaxKinNameLength or nNameLen < Kin.Def.nMinKinNameLength then
			return false, string.format("Tên bang hội cần trong khoảng %d-%d ký tự", Kin.Def.nMinKinNameLength, Kin.Def.nMaxKinNameLength)
		end
	else
		local nNameLen = string.len(szName)
		if nNameLen > Kin.Def.nMaxKinNameLength then
			return false, "Tên quá dài, hãy nhập lại!";
		elseif nNameLen < Kin.Def.nMinKinNameLength then
			return false, "Tên quá ngắn, hãy nhập lại";
		end
	end
	return true
end
...
function Kin:RedBagCheckKind(nKind, szVoicePwd)
	if nKind ~= self.Def.tbRedBagKind.NORMAL and nKind ~= self.Def.tbRedBagKind.VOICE then
		return false, "Loại bị lỗi"
	end
	if nKind == self.Def.tbRedBagKind.VOICE then
		local nMin, nMax = unpack(self.Def.tbVoicePwdLen)
		local nLen = Lib:Utf8Len(szVoicePwd or "")
		if nLen < nMin or nLen > nMax then
			return false, string.format("Lệnh Lì Xì giới hạn trong %d-%d ký tự", nMin, nMax)
		end
		if szVoicePwd and ReplaceLimitWords(szVoicePwd) then
			return false, "Nội dung có ký tự không phù hợp, hãy sửa và thử lại"
		end
		if szVoicePwd and Lib:HasNonChineseChars(szVoicePwd) then
			return false, "Nội dung chỉ được dùng tiếng Việt"
		end
		if not CheckNameAvailable(szVoicePwd) then
			return false, "Nội dung có ký tự đặc biệt"
		end
	end
	return true
end
