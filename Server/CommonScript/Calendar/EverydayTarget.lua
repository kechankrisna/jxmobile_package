...
function EverydayTarget:CheckGainAward(pPlayer, nAwardIdx)
	...
	if nGainInfo == 1 then
		return false, "Nhận thất bại, đã nhận thưởng rồi"
	end

	local nTotalActive = self:GetTotalActiveValue(pPlayer)
	if nTotalActive < nActiveValue then
		return false, "Năng động không đủ, không thể nhận thưởng"
	end
...
function EverydayTarget:SendRandomAward(nAwardIdx)
	...
	local nRet, szMsg, tbAllAward = Item:GetClass("RandomItemByLevel"):GetAwardByLevel(me, nItemID, "Mục Tiêu Ngày")
	me.ItemLogWay = nil;
	if nRet ~= 1 then
