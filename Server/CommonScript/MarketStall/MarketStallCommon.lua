...
MarketStall.nOpenLevel = 110					-- 摆摊功能开启等级
...
function MarketStall:GetStallAwardName(szMainType, nSubType, nFaction, nSex)
	...
	local szName = "Tên đạo cụ"
	nFaction = nFaction or 0;
	if nType == Player.award_type_item then
		szName = KItem.GetItemShowInfo(nSubType, nFaction, nSex)
		
		if (nFaction >= 18) then
			szName = Item:GetNewItemShowInfo(nSubType, nFaction, nSex);
		end
...
function MarketStall:IsMarketOpen(pPlayer)
	if GetTimeFrameState(self.szCreateRoleTimeLimitTimeFrame) == 1 and
		...
		return false, "24 giờ sau khi tạo nhân vật sẽ mở bày bán";
	end

	if pPlayer.nLevel < self.nOpenLevel then
		return false, string.format("Lv%d mở Bày Bán", self.nOpenLevel)
	end
...
function MarketStall:CheckCanSellItem(pPlayer, szMainType, nSubType)
	...
	if not nAwardType then
		return false, "Vật phẩm bày bán chưa biết";
	end
	...
	if nAwardType == Player.award_type_item then
		...
		if not pItem then
			return false, "Đạo cụ vô hiệu, không thể bán";
		end

		if MODULE_GAMESERVER then
			if self.bAllForbidden or (self.tbForbidden and self.tbForbidden[pItem.dwTemplateId]) then
				return false, "Vật phẩm này tạm thời không bán";
			end
		end

		local tbInfo = (self.tbAllStallInfo[szMainType] or {})[pItem.dwTemplateId];
		if not tbInfo then
			return false, "Vật này không thể bày bán";
		end

		if tbInfo.szCloseTimeFrame and tbInfo.szCloseTimeFrame ~= "" and GetTimeFrameState(tbInfo.szCloseTimeFrame) == 1 then
			return false, "Vật phẩm lỗi thời, không thể bày bán";
		end

		if pItem.nPos ~= Item.emITEMPOS_BAG then
			return false, "Đạo cụ trong hành trang mới được bày bán";
		end

		local bCanSell = Item:CheckCanSell(pItem);
		if not bCanSell then
			return false, "Vật phẩm này không thể bán";
		end
...
function MarketStall:CheckCanNewSellItem(pPlayer, szMainType, nSubType, nCount, nPrice, bIgnorCheckCost)
	if self:IsMyStoreFull(pPlayer) then
		return false, "Không thể bày bán thêm vật phẩm"
	end

	if not self.tbAllowCount[nCount] then
		return false, "Số lượng bày bán bị lỗi"
	end
	...
	if not bCanSell then
		return false, string.format("%s sau mới có thể bày bán vật phẩm này", Lib:TimeDesc6(nLastTime));
	end

	if not bIgnorCheckCost and nCurCount < nCount then
		return false, "Không đủ vật phẩm!";
	end

	if not tbPriceInfo or not tbPriceInfo[nPrice] then
		return false, "Vật phẩm không thể bán với giá này!";
	end
	...
	if not bIgnorCheckCost and nHasGold < nCost then
		return false, "Phí không đủ, không thể bày bán";
	end
...
function MarketStall:CheckCanUpdateItemPrice(pPlayer, szMainType, nSubType, nCount, nPrice, bIgnorCheckCost)
	if nCount <= 0 then
		return false, "Vật phẩm này đã bán hết, không thể bày bán lại!";
	end
	...
	if not bCanSell then
		return false, string.format("%s sau mới có thể bày bán lại vật phẩm này", Lib:TimeDesc6(nLastTime));
	end
...
function MarketStall:TipBuyItemFromMarket(pPlayer, nItemId, szTipsInfo)
	...
	if not szTipsInfo then
		...
		szTipsInfo = string.format("[FFFE0D]%s[-] không đủ, đến Bày Bán mua không?", szItemName);
	end
	pPlayer.MsgBox(szTipsInfo,
	{
		{"Đến", function () Ui:OpenWindow("MarketStallPanel", 1, nil, "item", nItemId); end },
		{"Hủy"}
	});
end
...
function MarketStall:TipBuyItemFromShop(pPlayer, nItemId, szTipsInfo)
	...
	if not szTipsInfo then
		...
		szTipsInfo = string.format("[FFFE0D]%s[-] không đủ, đồng ý đến Kỳ Trân Các mua?", szItemName);
	end
	pPlayer.MsgBox(szTipsInfo,
	{
		{"Đến", function () Ui:OpenWindow("CommonShop", "Treasure", "tabAllShop", nItemId) end },
		{"Hủy"}
	});
end
