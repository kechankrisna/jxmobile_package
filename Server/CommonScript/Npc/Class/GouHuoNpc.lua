		...
		if nType ~= tbGouhuoNpc.KIND_TEAM then
			...
			if tbPlayer then
				for _, pPlayer in pairs(tbPlayer) do
					pPlayer.Msg("Lửa Trại đã đốt, nhận EXP liên tục")
				end
			end
		end
end
	...
	if tbTmp.nCanUseJIu ~= 0 then
		...
		if tbTmp.nQuotiety ~= nQuotiety or tbTmp.szJiuName ~= szJiuName then
			...
			if szJiuName and nQuotiety and nJiuMax then
				if nTeamId ~= 0 then
					szLastGuohuoTipContent = string.format("Có %d thành viên đội cùng chia sẻ %s, nhận %d%% EXP Lửa Trại", nJiuMax, szJiuName, nQuotiety)
					...
					szLastGuohuoTipContent = string.format("Đang ở trạng thái uống rượu, nhận %d%% EXP Lửa Trại", nQuotiety)
					if pOwnner.szLastGuohuoTipContent ~= szLastGuohuoTipContent then
						pOwnner.Msg(szLastGuohuoTipContent)
					end
				end
			else
				szLastGuohuoTipContent = "Rượu đã hết, nhận 100% EXP Lửa Trại"
				if pOwnner.szLastGuohuoTipContent ~= szLastGuohuoTipContent then
	...
	if nAnnouce == 1 then
		if nJiuMax == 0 then
			if szEventJiuName then
				szMsg = string.format(XT("Dùng [1AB8E1] %s[-] , "), szEventJiuName);
			else
				szMsg = string.format(XT("Không dùng rượu, "));
			end
			szMsg = string.format(XT("Bội số EXP Lửa Trại %s hiện nhận là [1AB8E1]%s[-] ."), szMsg, nQuotiety)
		else
			if szEventJiuName then
				szMsg = string.format(XT("Đồng đội đang chia sẻ %s và %s, EXP cộng thêm %s％"), szJiuName, szEventJiuName, nQuotiety)
			else
				szMsg = string.format(XT("Đồng đội đang chia sẻ %s, EXP cộng thêm %s％"), szJiuName, nQuotiety)
			end
