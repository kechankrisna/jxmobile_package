...
function tbNpc:OnDialog(szParam)
	...
	if not KinDinnerParty:GatherThingInTask(me, nMatureId) then
		me.CenterMsg("Chưa có nhiệm vụ của vật thu thập này")
		return;
	end
	
	if bMature then
		GeneralProcess:StartProcessUniq(me, 6 * Env.GAME_FPS, him.nId, "Đang thu thập...", self.EndProcess, self, me.dwID, him.nId, nMatureId, nUnMatureId);
	else
		...
		local szTip = string.format("%d phút sau chín",  math.ceil(nSurviveSec / 60));
		me.CenterMsg(szTip);
	end
end
