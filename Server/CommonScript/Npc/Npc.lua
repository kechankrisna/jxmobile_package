...
-- 任何Npc对话，系统都会调用这里
Npc.tbDialogTitle = {
	["CommerceTask"] = "Tại hạ muốn làm nhiệm vụ Thương Hội",
	["SwornFriendsNpc1"] = "Kết Bái",
	["SwornFriendsNpc2"] = "Kết Bái",
}
function Npc:OnDialog(szClassName, szParam)
	...
	local szDefaultText = Lib:IsEmptyStr(him.szDefaultDialogInfo) and "Tìm ta có chuyện gì?" or him.szDefaultDialogInfo
	...
	if ActivityQuestion:IsCurQuestionNpc(me.nMapTemplateId, him.nTemplateId) then
		if bTaskFlag then
			table.insert(tbDialogInfo.OptList, {Text = "Vấn Đáp", Type = "Script", Callback = ActivityQuestion.BeginAnswer, Param = {ActivityQuestion, me.nMapTemplateId, him.nTemplateId}})
		else
	...
	if next(tbDialogInfo.OptList) then
		local szText = self.tbDialogTitle[him.szClass] or "Tại hạ muốn làm việc khác"
...
function Npc:ShowDefaultDialog(szClassName, szParam)
	if  Lib:IsEmptyStr(szClassName) or not self.tbClass[szClassName] then
		...
		if not szText then
			szText = Lib:IsEmptyStr(him.szDefaultDialogInfo) and "Tìm ta có chuyện gì?" or him.szDefaultDialogInfo;
		end
