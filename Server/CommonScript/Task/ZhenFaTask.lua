...
ZhenFaTask.tbSheJiaoTaskQuestion = {
	[ZhenFaTask.RELATIONSHIP_SS_LOVER] = "Đại hiệp có quan hệ tình duyên với ai dưới đây?";
	[ZhenFaTask.RELATIONSHIP_SS_MARRY] = "Đại hiệp có quan hệ phu thê với ai dưới đây?";
	[ZhenFaTask.RELATIONSHIP_SS_SHI_FU] = "Dưới đây ai là sư phụ của đại hiệp?";
	[ZhenFaTask.RELATIONSHIP_SS_TU_DI] = "Dưới đây ai là đệ tử của đại hiệp?";
	[ZhenFaTask.RELATIONSHIP_SS_JIE_BAI] = "Đại hiệp có quan hệ kết bái với ai dưới đây?";
	[ZhenFaTask.RELATIONSHIP_SS_FRIEND] = "Đại hiệp có quan hệ hảo hữu với ai dưới đây?";
}
...
ZhenFaTask.tbBlackBoardMsg = {
	[70044] = {"[FFFE0D]%s[-] vui vẻ nhìn pháo hoa ngập trời, gương mặt hạnh phúc khiến đại hiệp nhìn ngất ngây...", "Đại hiệp vui vẻ nhìn pháo hoa ngập trời, gương mặt hạnh phúc khiến [FFFE0D]%s[-] nhìn ngất ngây..."};
	[70045] = {"[FFFE0D]%s[-] vui vẻ nhìn pháo hoa ngập trời, gương mặt hạnh phúc khiến đại hiệp nhìn ngất ngây...", "Đại hiệp vui vẻ nhìn pháo hoa ngập trời, gương mặt hạnh phúc khiến [FFFE0D]%s[-] nhìn ngất ngây..."};
}
...
function ZhenFaTask:OnFinishTask(nTaskId)
	...
	for _, nPlayerId in pairs(tbMember or {}) do
		...
		if pMember and nPlayerId ~= me.dwID then
			...
			szMsg = string.format("Hoàn thành [FFFE0D]Hỗ Trợ Thí Luyện Pháp Trận[-] lần này! Phần thưởng hỗ trợ hôm nay nhận còn [FFFE0D]%s[-] lần", nLast);
		elseif nPlayerId == me.dwID then
			szMsg = string.format("Hoàn thành thí luyện lần này! Hôm nay còn [FFFE0D]%s[-] lần", nLastCount);
		end
	...
	if nLastCount <= 0 then
		me.SendBlackBoardMsg("Đại hiệp đã hoàn thành thí luyện hôm nay!", true);
	end
...
function ZhenFaTask:DoNextTask(pPlayer, nDealyTime)
	...
	if not bRet and szOtherName then
		pPlayer.MsgBox("Thành viên không ở xung quanh, thí luyện bị gián đoạn, đại hiệp vào phần Lịch hoạt động nhận lại không?", {{"Xác nhận", function ()
...
function ZhenFaTask:CheckCanAcceptTaskCommon(pPlayer)
	...
	if pPlayer.nLevel < self.nMinLevel then
		return false, "Đại hiệp chưa đủ kinh nghiệm";
	end

	local nTaskId = ZhenFaTask:GetZhenFaTask(pPlayer);
	if nTaskId then
		return false, "Đại hiệp đã nhận nhiệm vụ Thí Luyện";
	end

	local nLastCount = DegreeCtrl:GetDegree(pPlayer, "ZhenFaTask");
	if nLastCount <= 0 then
		return false, "Đại hiệp đã hoàn thành thí luyện hôm nay!";
	end
...
function ZhenFaTask:CheckCanAcceptTaskByNpc(pPlayer)
	...
	local teamData = TeamMgr:GetTeamById(pPlayer.dwTeamID);
	if not teamData or teamData.nCaptainID ~= pPlayer.dwID then
		return false, "Chỉ đội trưởng được thao tác";
	end

	local nRelation, szMsg, tbAllRelationSSMember = self:GetTeamRelationship(pPlayer);
	if not nRelation then
		return false, "Phải có đại hiệp có quan hệ hiệp lữ, tình duyên, sư đồ, hảo hữu tổ đội!";
	end
	...
	if tbAllRelationSSMember[ZhenFaTask.RELATIONSHIP_SS_NONE] and #tbAllRelationSSMember[ZhenFaTask.RELATIONSHIP_SS_NONE] > 0 then
		return false, "Có thành viên và đội trưởng không phải quan hệ hiệp lữ, tình duyên, sư đồ, hảo hữu!";
	end
...
function ZhenFaTask:CheckCanAcceptTask(pPlayer)
	...
	for _, nPlayerId in pairs(tbMember) do
		local pOther = KPlayer.GetPlayerObjById(nPlayerId);
		if not pOther then
			return false, "Có thành viên không online, không thể nhận nhiệm vụ";
		end

		if pPlayer.nMapId ~= pOther.nMapId then
			return false, string.format("%s không ở gần, không thể nhận nhiệm vụ", pOther.szName), nil, nil, nil, pOther.szName;
		end
	end
...
function ZhenFaTask:AcceptNewTask(pPlayer)
	...
	if not DegreeCtrl:ReduceDegree(pPlayer, "ZhenFaTask", 1) then
		pPlayer.SendBlackBoardMsg("Đại hiệp đã hoàn thành thí luyện hôm nay!", true);
		return;
	end
...
function ZhenFaTask:OnZhenFaTaskDoNext(pPlayer, nNpcId, nTaskId)
	...
	local tbMember = TeamMgr:GetMembers(pPlayer.dwTeamID);
	if not teamData or teamData.nCaptainID ~= pPlayer.dwID then
		pPlayer.CenterMsg("Chỉ đội trưởng được thao tác");
		return;
	end

	if Lib:CountTB(tbMember) <= 1 then
		pPlayer.CenterMsg("Nhiệm vụ Trận Pháp Thí Luyện cần tổ đội hoàn thành");
		return;
	end
