...
Task.tbAllVideoTask = 
{
	[Task.VIDEO_TYPE_JYFL] = {
		fnOnOpen = function ()
			Pandora:OpenVideo()
			me.SendBlackBoardMsg("Cốt truyện đang chuẩn bị, vui lòng chờ");
		end;
		nTaskId = 9013;
		szVideoTitle = "Dự\nBáo";
	}; 		
	[Task.VIDEO_TYPE_JYFL_2] = {
		fnOnOpen = function ()
			...
			me.MsgBox("Xem nội dung cần thêm lưu lượng thêm, nếu gặp các vấn đề như phát chậm hoặc không thể phát có thể chọn bỏ qua và đến đối thoại với Lâm Thanh Vũ ở Lâm An Thành để trải nghiệm nội dung", {{"Đồng ý", fnAgree}, {"Hủy"}})
...
Task.tbFlowSetting = 
{
	...
	[Task.Flow_Line_3] = 
	{
		...
		szFlowQuestionTitle = "Báo thù";
	};
	[Task.Flow_Line_4] = 
	{
		nStartTaskId = 9035;
		nEndTaskId = 9039;
		szTitle = "Nghiệt Duyên";
		nQuestionTaskId = 9039;
		szFlowQuestionTitle = "Nghiệt Duyên";
	};
}
...
function Task:CommonCheck(pPlayer, nTaskId, nNpcId, bIsCheckAccept)
	...
	local tbTask = self:GetTask(nTaskId);
	if not tbTask then
		return false, "Không có nhiệm vụ này!";
	end
	...
	if not bIsCheckAccept then
		...
		for nItemId, nNeedCount in pairs(tbCollectItem) do
			if pPlayer.GetItemCountInAllPos(nItemId) < nNeedCount then
				return false, "Đạo cụ nhiệm vụ không đủ!";
			end
		end
	end
	...
	if nFlag == 1 and not tbTask.bCanRepeat then
		return false, "Không thể lặp lại nhiệm vụ này!";
	end
...
function Task:CheckCanFinishTask(pPlayer, nTaskId, nNpcId)
	...
	if not tbTargetInfo then
		return false, "Chưa nhận nhiệm vụ này!";
	end
	...
	for szType, value in pairs(tbTask.tbTargetInfo) do
		...
		if not fnCheckTargetFunc then
			return false, "" .. szType .. " Chưa thiết lập mục tiêu!";
		end
		...
		if not bOK or not bRet then
			return false, szMsg or "Chưa biết nguyên nhân, không thể hoàn thành nhiệm vụ!";
		end
	end
...
function Task:CheckCanAcceptTask(pPlayer, nTaskId, nNpcId)
	...
	if tbTargetInfo then
		return false, "Đã nhận nhiệm vụ này!";
	end
	...
	for szFunc, value in pairs(tbTask.tbRequireInfo) do
		...
		if not fnCheckRequireFunc then
			return false, "" .. szFunc .. " Chưa thiết lập điều kiện!";
		end
...
function Task:CheckZoneTaskAddExtPoint(nTaskId)
	if not self.tbZoneTaskRef[nTaskId] then
		return false, "Không phải nhiệm vụ trang cá nhân"
	end
...
function Task:CheckAnswerFlowQuestion(pPlayer, nFlowType, tbAnswer, bNotCheckCost)
	...
	if not nQuestionTaskId then
		return false, "Nhiệm vụ chưa biết"
	end
	if not Task:HaveAcceptTask(pPlayer, nQuestionTaskId) then
		return false, "Hãy nhận nhiệm vụ trước"
	end
	if self:IsFinish(pPlayer, nQuestionTaskId) then
		return false, "Đã hoàn thành nhiệm vụ này"
	end
	...
	if #tbQuestion ~= #tbAnswer then
		return false, "Hãy trả lời tất cả câu hỏi trước"
	end
