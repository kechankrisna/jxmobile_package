...
function _TeamData:Available2Join(pPlayer)
	...
	if pPlayer.nLevel < TeamMgr.OPEN_LEVEL then
		return false, string.format("Cấp %d trở lên mới cho phép tổ đội ", TeamMgr.OPEN_LEVEL);
	end

	if not pPlayer.CanTeamOpt() then
		return false, "Đang ở nơi không thể tổ đội";
	end
	...
	if not captain or not captain.CanTeamOpt() or 
		...
		return false, "Bản đồ không cho phép tổ đội";
	end
...
function _TeamData:AddMember(nMemberID, bIgnoreCheck)
	...
	if not CTeamMgr.TeamAddMember(self.nTeamID, nMemberID) then
		return false, "Thêm đội thất bại";
	end
...
function _TeamData:UpdateCaptain()
	...
	local szTips = string.format("「%s」 trở thành đội trưởng mới", player.szName);
...
function _TeamData:ChangeCaptain(nTargetPlayerID)
	...
	if not targetPlayer or self.nTeamID ~= targetPlayer.dwTeamID then
		return false, "Đối phương hiện không ở trong đội";
	end

	if self.nCaptainID == nTargetPlayerID then
		return false, "Đối phương hiện là đội trưởng";
	end
...
function _TeamData:ChangeTargetActivity(nActivityId)
			...
			if nActivityId then
				...
				if not bRet then
					...
					if bCanHelp then
						...
						member.CenterMsg("Vào trạng thái Trợ Chiến, tham gia hoạt động không tốn số lần tham gia cũng không được nhận thưởng!", true, ChatMgr.SystemMsgType.Team);
					else
	...
	if szActivityName then
		szTips = string.format("Mục tiêu đội đổi thành： %s", szActivityName);
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, szTips, self.nTeamID);
	end
