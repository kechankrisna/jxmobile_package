...
function tbFuben:OnFirstJoin(pPlayer)
	if pPlayer.dwID ~= self.dwOwnerId then --加入owner 的队伍
		...
		local szNotifyMsg = string.format("Hiệp sĩ [FFFE0D]%s[-] gia nhập động thần bí", pPlayer.szName)
		if pOwner then
...
function tbFuben:OnOpenGate()
	...
	if nRand > FubenMgr.nRandFLooor2 then
		szNotify = "Đống lửa đã nhóm, xung quanh nhận được kinh nghiệm"
		...
		Timer:Register(Env.GAME_FPS * 60, function ()
			if Fuben.tbFubenInstance[nMapId] then
				Fuben.tbFubenInstance[nMapId]:BlackMsg("Cái này động quật đã không có cái gì đáng giá thăm dò")
			end
		end)

	else
		szNotify = "Mời đã đóng! Bên trái xuất hiện một đạo cửa vào! Đi xem đến tột cùng!"
...
function tbFuben:GameLost()
	self.bLost = true;
	local fnExcute = function (pPlayer)
		pPlayer.SendBlackBoardMsg("Động thần bí đã biến mất");
...
function tbFuben:OnKillNpc(pNpc, pKiller)
	...
	local szAwardDesc = string.format("Bạn nhận được %d bạc", nRandCoin)
	local fnExcute = function (pPlayer)
