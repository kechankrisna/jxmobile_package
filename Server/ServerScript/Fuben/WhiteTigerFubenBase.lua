...
function tbFuben:OnStartFight()
    ...
    Timer:Register(Env.GAME_FPS * tbFubenMgr.PREPARE4PK, function ()
        ...
        self:SetFubenProgress(-1, "Chờ đợi đầu mục tầng 1 hiện thân")
...
function tbFuben:InitFightRoom()
    ...
    self.tbRoomState    = {}
    local tbFloorName   = {[2] = "2", [3] = "3", [4] = "4"}
    for i = 1, self.nTopRoom do
        local nFloor  = self:GetFloor(i)
        local szTitle = (nFloor <= 2) and "Chờ mở PK" or string.format("Chờ đầu mục tầng %s xuất hiện", tbFloorName[nFloor])
...
function tbFuben:ChangePkMode()
    ...
    local szTitle = "Chờ đợi đầu mục tầng 2 hiện thân"
    for i = 1, 4 do
    ...
    if pPlayer then
        ...
        if pPlayer.dwKinId > 0 then
            ...
            if nCombo >= 20 and nCombo <= 100 and nCombo%10 == 0 then
                local szMsg = string.format("「%s」trong Bạch Hổ Đường, đạt liên trảm %d !", pPlayer.szName, nCombo)
...
function tbFuben:OnOpenCrossDoor()
    ...
    self:BlackMsg("Lối vào tầng thứ 5 đã mở")

    local szTitle = "Đến tầng 5"
    ...
    if me.dwKinId ~= 0 and me.dwKinId == self.nWinKin then
        ...
        Timer:Register(2, function ()
            ...
            if bRet then
                ...
                pPlayer.CenterMsg("Vào phó bản thất bại")
            end
        end)
    end
end
