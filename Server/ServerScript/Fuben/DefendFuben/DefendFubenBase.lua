...
function tbFuben:OnNpcCreateFubenBase(pNpc)
	...
	self.tbNpcHpNotice[pNpc.nId] = Npc:RegisterNpcHpChange(pNpc, function(nOldHp, nNewHp, nMaxHp)
		if GetTime() > self.nMingXiaHitTime + DefendFuben.nMingXiaHitMsgInteval then
			self:Msg("Trương như mộng đang cùng tử sĩ giao thủ, nhanh bảo hộ hắn!")
		end
...
function tbFuben:ActiveNpc(nNpcID,dwID)
	...
	if not self.bStart then
		pPlayer.CenterMsg("Kẻ địch chưa xuất hiện, xin sau một lát",true)
		return
	end

	local pDialogNpc = KNpc.GetById(nNpcID);
	if not pDialogNpc then
		pPlayer.CenterMsg("NPC Không tồn tại",true)
		return
	end

	if self.tbActive[self.nRound] then
		pPlayer.CenterMsg("Ngươi đã chọn một người trợ chiến, lần sau hãy sử dụng nữa !",true)
		return
	end

	if not self.tbDialogNpc[nNpcID] then
		pPlayer.CenterMsg("Ngươi muốn khởi động NPC nào??",true)
		return
	end
	local tbSetting = self.tbSetting
...
function tbFuben:SendReward(pPlayer)
	...
	local szTitle = "Ngũ nhất giang hồ triển thân thủ"
	local szText = string.format("Thiếu hiệp, tại ngày mồng một tháng năm giang hồ triển thân thủ trong hoạt động, ngươi cùng đồng bạn chung thành công chống cự [c8ff00] %d Vòng [-] Địch nhân tiến công, đây là tại hạ nho nhỏ tâm ý, mong rằng không được cự tuyệt!",self.nRound)
	local tbMail = {
		To = pPlayer.dwID;
		Title = szTitle;
		From = "Độc Cô Kiếm";
...
function tbFuben:OnPlayerDeath()
	me.CallClientScript("Ui:OpenWindow", "CommonDeathPopup", "AutoRevive", "Hồi sinh sau %d giây", DefendFuben.REVIVE_TIME);
