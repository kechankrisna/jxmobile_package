...
function WhiteTigerFuben:SetupPreMapCallback()
    ...
    tbReadyMap.OnEnter = function (tbMap, nMapId)
        ...
        me.CallClientScript("Fuben:SetFubenProgress", -1, "Hãy chọn một cổng để vào")
...
function WhiteTigerFuben:OnReconnect(bReconnect)
    ...
    if self.nStartTime then
        ...
        if nEndTime > 0 then
            ...
            szTitle = "Hãy chọn một cổng để vào"
        end
    end
...
function WhiteTigerFuben:NotifyPlayer()
    local szMsg = "Bạch Hổ Đường đã mở, các vị thiếu hiệp có thể thông qua hoạt động vào khiêu chiến!"
...
function WhiteTigerFuben:TryEnterPrepareMap(pPlayer, bMsg)
    if pPlayer.nMapId == self.nPrepareMapId then
        return false, "Đã ở trong khu chuẩn bị"
    end
...
function WhiteTigerFuben:TryBackToPrepareMap(pPlayer)
    ...
    elseif self.nState == self.STATE_FIGHTING then
        pPlayer.CenterMsg("Chiến đấu bắt đầu không thể trở lại khu chuẩn bị")
        return
    end
...
local tbIndex = {"Một", "Hai", "Ba", "Bốn", "Năm", "Sáu", "Bảy", "Tám"}
function WhiteTigerFuben:TryEnterOutSideFuben(pPlayer, nRoomId)
    ...
    pPlayer.CenterMsg(string.format("Đại hiệp đã tiến vào Bạch Hổ đường Cổng %s", tbIndex[nRoomId]), true)
...
function WhiteTigerFuben:CheckCanEnter(pPlayer)
    if not pPlayer then
        return false, "Lỗi chưa biết, mời thử lại"
    end
    ...
    if self.nState ~= self.STATE_PREPARE then
        return false, "Đã hết thời gian báo danh, không thể tham gia"
    end
    ...
    if not Env:CheckSystemSwitch(pPlayer, Env.SW_SwitchMap) then
        return false, "Trạng thái hiện tại không thể vào bản đồ"
    end
    ...
    if not Fuben.tbSafeMap[pPlayer.nMapTemplateId] and Map:GetClassDesc(pPlayer.nMapTemplateId) ~= "fight" and
        ...
        return false, "Bản đồ hiện tại không cho phép vào phó bản!";
    end

    if Map:GetClassDesc(pPlayer.nMapTemplateId) == "fight" and pPlayer.nFightMode ~= 0 then
        return false, "Không phải khu vực an toàn, không thể vào!";
    end
...
function WhiteTigerFuben:OnKillCrossBoss(nKillKinId, tbHostileKin)
    ...
    local szDefeatMsg = string.format("Bang %s anh dũng đánh bại", tbKinData.szName)
    for i, tbInfo in ipairs(tbHostileKin) do
        local szDesc = Sdk:GetServerDesc(tbInfo[1])
        szDefeatMsg = string.format("%s%s %s Bang %s", szDefeatMsg, szDesc, tbInfo[2], i == #tbHostileKin and "" or " và ")
    end
    KPlayer.SendWorldNotify(1, 999, szDefeatMsg, 1, 1)

    local szMsg = string.format("Chúc mừng Bang %s tiêu diệt BOSS Bạch Hổ Đường liên server tầng 5 %s", tbKinData.szName, szBossName)
...
function WhiteTigerFuben:OnBeDefeatInCross(tbKinInfo, nKinId)
    ...
    local szMsg = string.format("Trong Bạch Hổ Đường liên server bổn Bang bị %s Bang %s đánh bại", szDesc, tbKinInfo[2])
