...
function tbFightMap:StopFight()
	...
	local szNotify = string.format("Tranh tài kết thúc, chúc mừng %s thu được thắng lợi", self.tbTeamNames[nWinTeam]);
...
function tbFightMap:OnPlayerTrap(szTrapName)
	if self.nBattleOpen ~= 1 then
		me.CenterMsg("Trận chiến chưa bắt đầu, xin vui lòng chờ trong giây lát.")
		return
	end
