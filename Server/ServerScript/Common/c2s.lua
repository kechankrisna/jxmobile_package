...
function c2s:OnFinishTaskDialog(nTaskId, nState, nNpcId)
	...
	if nState == Task.STATE_CAN_ACCEPT then
		if not Task:CheckCanClientAccetp(nTaskId) and (not me.nCurAcceptTaskId or me.nCurAcceptTaskId ~= nTaskId) then
			me.CenterMsg(string.format("Nhiệm vụ %s không thể thông qua người sử dụng bưng xác nhận", nTaskId));
			return;
		end
...
function c2s:OnTeamRequest(szType, ...)
	if not me.CanTeamOpt(szType) then
		me.CenterMsg("Bản đồ hiện tại không thể tiến hành thao tác này");
		return
	end
...
function c2s:ActiveTitle(nTitleID)
	if me.bTitileLock  then
		me.CenterMsg("Hiện tại không thể thay đổi danh hiệu")
		return
	end
...
function c2s:AcceptFriendRequest(dwAlliedPlayerID)
	...
	if not bRet then
		me.CenterMsg(szMsg or "Thêm hảo hữu thất bại")
	else
		--删除A 通缉B 或者B 通缉A 的数据
		...
		me.CenterMsg(string.format("Thêm「%s」vào hảo hữu!", tbRoleStayInfo.szName))
		me.OnEvent("AddFriend", dwAlliedPlayerID);

		local pAlliedPlayer = KPlayer.GetPlayerObjById(dwAlliedPlayerID)
		if pAlliedPlayer then
			pAlliedPlayer.Msg(string.format("Thêm「%s」vào hảo hữu!", me.szName))
...
function c2s:RequesDelFriend(dwAlliedPlayerID)
	...
	if not bRet then
		me.CenterMsg(szMsg or "Xóa hảo hữu thất bại")
	else
		me.CenterMsg("Xóa hảo hữu thành công")
	end
end
...
function c2s:RequestBlackHim(dwAlliedPlayerID)
	...
	if not bRet then
		me.CenterMsg(szMsg or "Thêm sổ đen thất bại")
	else
		me.CenterMsg("Đã thêm vào sổ đen")
	end
end
...
function c2s:SearchRole(NameOrId, bAddAFriend)
	...
	if NameOrId == me.szName or NameOrId == me.dwID then
		me.CenterMsg("Không thể tìm kiếm bản thân")
		return
	end

	if type(NameOrId) == "string" and not CheckNameAvailableServer(NameOrId) then
		me.CenterMsg("Chứa các ký tự không hợp lệ, vui lòng sửa đổi và thử lại")
		return
	end
	...
	if tbRoleStayInfo then
		...
		me.CenterMsg("Người tìm kiếm không tồn tại");
	end
...
function c2s:RefuseAllRequet()
	...
	if not bRet then
		me.CenterMsg(szMsg or "Một số thất bại")
	else
...
function c2s:RequesDelEnemy(dwAlliedPlayerID)
	...
	if not bRet then
		me.CenterMsg(szMsg or "Xóa kẻ thù thất bại")
	else
		me.CenterMsg("Xóa kẻ thù thành công")
	end
end
...
function c2s:DoChangeFaction(nFaction, nSex)
	...
	if not Faction.tbFactionInfo[nFaction] then
		me.CenterMsg("Hiện tại không có môn phái!!", true);
        return;
    end
...
function c2s:RequestCatchHim(dwWantedId, dwSenderId)
	...
	if not bRet then
		me.CenterMsg(szMsg or "Bắt truy nã")
		return
	end
end
...
function c2s:CallPartnerFunc(szCmd, ...)
    ...
    if pNpc.nShapeShiftNpcTID > 0 then
    	me.CenterMsg("Đang biến thân không thể thao tác", true);
        return;
    end
...
function c2s:OnStrengthenCheckItemExchange( ... )
	Strengthen:OnStrengthenCheckItemExchange(me, ...);
end
...
function c2s:BattleRequestReadyMapTime()
	Battle:RequestReadyMapTime(me)
end
...
function c2s:GetHelperSyncData(nPlayerId)
	...
	if not bRet then
		me.CenterMsg("Đồng bộ tư liệu thất bại!");
...
function c2s:RequestRecharge(szProductId)
	if not Recharge.IS_OPEN then
		me.CenterMsg("Trong quá trình thử nghiệm này, nạp thẻ chưa mở")
		return
	end
...
function c2s:PlayerLeaveMap(szMsg)
	...
	local szMsg = szMsg or "Xác định rời hoạt động?"
	if me.szCanLeaveMapMsg then
		szMsg = me.szCanLeaveMapMsg;
	end

	me.MsgBox(szMsg, {{"Xác nhận", function (nMapId)
...
function c2s:TryCheckFriendAchievement(szAchi )
	FriendShip:TryCheckFriendAchievement(me, szAchi)
end
...
function c2s:TeamBattleTrySynPreTime()
	TeamBattle:TrySynPreTime(me);
end
...
function c2s:OpenTaskVideo(nVideoType, nTaskId)
	Task:OpenTaskVideo(me, nVideoType, nTaskId);
end
...
function c2s:BeichenBossHomeJoin()----by 初心 QQ353837822
	FactionBattle:TrapIn2(me)
end
...
function c2s:CallMarketStallFunc(szFunc, ...)
	...
	if not MarketStall:CheckOpen() then
		me.CenterMsg("Gian hàng đang được bảo trì!");
		return;
	end

	local bRet, szMsg = MarketStall:IsMarketOpen(me)
	if not bRet then
		me.CenterMsg(szMsg or "Lỗi không xác định")
		return;
	end
...
function c2s:TLogHangup(tbInfo)
	me.TLog("SecDeathHangUpFlow",
		tbInfo.szClientTime,
		me.szName,
		me.nLevel,
		me.GetFightPower(),
		me.nFaction,
		me.szIP,
		tbInfo.nDeathMapTemplate,
		tbInfo.szDeathPos,
		tbInfo.nReviveCost,
		tbInfo.nReviveMap,
		tbInfo.szRevivePos,
		tbInfo.nFirstChangeMapTime,
		tbInfo.nToFinalAutoFightTime,
		tbInfo.szMapsId,
		tbInfo.szMapsTime,
		tbInfo.szMapsDistance,
		tbInfo.nMaxStayMap,
		tbInfo.szMaxStayPos,
		tbInfo.szKillerOpenId,
		tbInfo.nKillerId,
		tbInfo.nKillerFaction,
		tbInfo.szKillerName,
		tbInfo.nKillerLevel,
		tbInfo.nKillerFightPower,
		tbInfo.nTotalTime,
		tbInfo.nFollowFight
		);
end
...
function c2s:ImperialTombLeave()
	if me.nFightMode == 1 then
		GeneralProcess:StartProcess(me, 5 * Env.GAME_FPS, "Đang truyền tống...", ImperialTomb.LeaveRequest, ImperialTomb, me)
	else
...
function c2s:InDifferRequestReadyMapTime()
	InDifferBattle:RequestReadyMapTime(me)
end
...
function c2s:TryLeaveQingRenJieMap()
	if not Env:CheckSystemSwitch(me, Env.SW_SwitchMap) then
		me.CenterMsg("Trạng thái hiện tại không thể truyền tống")
		return
	end
...
function c2s:PackupFurniture(nId)
	if not House:IsInOwnHouse(me) then
		me.CenterMsg("Chỉ có thể trang bị gia viên của mình!");
		return;
	end
...
function c2s:HousePeachReq(szType, ...)
	House.tbPeach:ClientReq(szType, ...);
end

function c2s:GoBackFromDrinkHouse()
	if not Env:CheckSystemSwitch(me, Env.SW_SwitchMap) then
		me.CenterMsg("Trạng thái hiện tại không thể truyền tống")
		return
	end
	if me.nMapTemplateId == DrinkHouse.tbDef.NORMAL_MAP then
		local LEAVE_RAND_POS = DrinkHouse.tbDef.LEAVE_RAND_POS
		local nRand = MathRandom(1, #LEAVE_RAND_POS)
		me.SwitchMap(unpack(LEAVE_RAND_POS[nRand]))
	end
end
...
function c2s:RequestOpenPandoraPlayerSpace(dwRoleId)
	...
	if not pPlayer then
		me.CenterMsg("Người chơi không trực tuyến")
		return
	end
...
function c2s:ChuanZhenQiQiaoReq(szType, ...)
	Activity:OnPlayerEvent(me, "Act_ChuanZhenQiQiaoReq", szType, ...)
end

function c2s:QueQiaoXiangHuiReq(szType, ...)
	Activity:OnPlayerEvent(me, "Act_QueQiaoXiangHuiReq", szType, ...)
end

function c2s:JueXueOnClientCall(...)
	JueXue:OnClientCall(me, ...)
end

function c2s:KinEncounterReq(szType, ...)
	KinEncounter:OnClientReq(me, szType, ...)
end

function c2s:PartnerCardOnClientCall(...)
	PartnerCard:OnClientCall(me, ...)
end

function c2s:KeyQuestFubenRequest(...)
	Fuben.KeyQuestFuben:OnClientCall(me, ...)
end

function c2s:SynPartnerCardPickData()
	if not Activity:__IsActInProcessByType("PartnerCardPickAct") then
		return 
	end
	Activity:OnPlayerEvent(me, "Act_SynPartnerCardPickData")
end

function c2s:AlterWeddingPromise(...)
	Item:GetClass("WeddingPromiseItem"):AlterWeddingPromise(me, ...)
end

function c2s:KinDinnerPartyReq(szFunc, ...)
	KinDinnerParty:OnRequest(szFunc, ...)
end

function c2s:WaiYiTryReq(szType, ...)
	WaiYiTry:OnClientReq(me, szType, ...)
end

function c2s:LoverTaskOnClientCall(...)
	LoverTask:OnClientCall(me, ...)
end

function c2s:DrinkHousePlayerRequest( ... )
	DrinkHouse:PlayerRequest(me, ...)
end

function c2s:ToyReq(szType, ...)
	Toy:OnClientReq(me, szType, ...)
end

function c2s:StrongerRecommendRequest()
	Player.Stronger:RecommendRequest(me)
end

function c2s:RequestRefreshAchievementLike(...)
	Achievement:RefreshLikeList(me, ...)
end

function c2s:TryAnswerFlowQuestion(nFlowType, tbAnswer)
	Task:TryAnswerFlowQuestion(me, nFlowType, tbAnswer)
end

function c2s:TryCreateAsyncBattle(nMapTID, nRestrainType)
	Task.__AsyncBattle:TryEnter(me, nMapTID, nRestrainType)
end

function c2s:SendServerMsg(...)
	ChatMgr:SendServerMsg(me, ...)
end

function c2s:SendServerPrivateMsg(...)
	ChatMgr:SendServerPrivateMsg(me, ...)
end

function c2s:OnSyncShangShengDianData(...)
	Activity:OnPlayerEvent(me,"Act_SyncShangShengDian_Data", ...)
end

RegisterZoneC2S("SendServerMsg", WS_EXECUTE);
RegisterZoneC2S("SendServerPrivateMsg", WS_EXECUTE);

function c2s:MaterialCollectCall(szCmd, ...)
	Activity:OnPlayerEvent(me, "Act_MaterialCollectClientCall", szCmd, ...);
end