...
function ScheduleTask:SendTimeFrameMail(szTimeFrame, nLevel, szMail, szAttach, nDelayDay)
	...
	local szDate;
	if nDelayDay then
		szDate =  os.date(XT("%Y năm %m tháng %d ngày"), GetTime() + nDelayDay * 24 * 3600)  ..  os.date(XT(" %H:%M"), nOpenTime);
	else
		szDate	= os.date(XT("%Y năm %m tháng %d ngày %H:%M"), nOpenTime);
	end
...
function ScheduleTask:SendAnnounce(szTimeFrame, nLevel, szMsg)
	local nOpenTime 	= CalcTimeFrameOpenTime(szTimeFrame);
	local szDate		= os.date(XT("%Y năm %m tháng %d ngày %H:%M"), nOpenTime);
