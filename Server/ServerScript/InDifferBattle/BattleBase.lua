...
function tbBattleBase:ChooseFaction(pPlayer, nFaction)
	...
	if not bFind then
		pPlayer.CenterMsg("Môn phái không hợp lệ")
		return
	end
			...
			if nFaction == 0 then
				for i,_nFaction in ipairs(tbChooseFactionSet) do
					if not tbHasSelFaction[_nFaction] then
						...
						ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format("Hệ thống tự động chọn cho 「%s」 phái「%s」", pRole and pRole.szName or "", Faction:GetName(_nFaction)), dwTeamID)
						bAutoChange = true
...
function tbBattleBase:OnDrapAwardListNpcDeath(pNpc, pKiller)
	...
	self:OnWorldNotify(string.format("%s đã bị giết", him.szName))
end
...
function tbBattleBase:DelayRevivePlayer(dwRoleId)
	...
	Dialog:SendBlackBoardMsg(pPlayer, "Dùng 【Đá hồi sinh】 để hồi sinh")
end
...
function tbBattleBase:OnPlayerDeath(pKillerLuna)
	...
	if not bCanRevive  then
		...
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format("「%s」Đã bị giết", me.szName), me.dwTeamID)
		...
		if tbTeamServerInfo.nLivePlayerNum == 0  then
			...
			for dwRoleId,v in pairs(self.tbTeamFactionInfo[me.dwTeamID]) do
				local pPlayer = self:GetPlayerObjById(dwRoleId)
				if pPlayer  then
					Dialog:SendBlackBoardMsg(pPlayer, "Đội đã bị đánh bại, rời khỏi sau 5 giây")
				end
			end		
...
function tbBattleBase:DirectEndGameMatch() --只剩一支队伍时直接结束比赛
	...
	self:SetWinnerTeam(dwWinnerTeam, string.format("Cuối cùng %d người chiến đấu đến cuối cùng dành chiến thắng", nSurviveNum) )
...
function tbBattleBase:SetWinnerTeam(dwWinnerTeam, szWinInfo)
	...
	self:OnWorldNotify(string.format("Xin chúc mừng %s%s", table.concat( tbPlayerNames, "、"), szWinInfo))
...
function tbBattleBase:GetWinnerTeam()
	...
	if #tbSortTeams == 1 then
		return tbSortTeams[1].dwTeamID, string.format("Cuối cùng %d người chiến đấu đến cuối cùng dành chiến thắng", math.floor(tbSortTeams[1].nSort / 10000)) 
	end
	...
	if nSort1 > nSort2 then
		if math.floor(nSort1 / 10000) == math.floor(nSort2 / 10000) then
			return dwSort1TeamID, string.format("Cuối cùng %d người sống sót %d lợi thế của tổng số điểm của đội là người chiến thắng trong tưởng tượng kỳ diệu này!", tbSort1TeamInfo.nLeftPlayerNum, nLastGetScore)
		else
			return dwSort1TeamID, string.format("Cuối cùng %d Lợi thế của sự sống còn của con người là người chiến thắng trong tưởng tượng kỳ diệu này!", tbSort1TeamInfo.nLeftPlayerNum)
		end
	end
	if tbSortTeams[1].nSurvieFightPower > tbSortTeams[2].nSurvieFightPower then
		return dwSort1TeamID, string.format("Cuối cùng %d người sống sót %d tổng số điểm của đội, tổng lực của đội sinh tồn trong tổng sức mạnh của đội đã giành chiến thắng trong tưởng tượng kỳ diệu này!", tbSort1TeamInfo.nLeftPlayerNum, nLastGetScore, tbSort1TeamInfo.nSurvieFightPower)
	end

	local nRand = MathRandom(2)
	return tbSortTeams[nRand].dwTeamID , string.format("Cuối cùng %d người sống sót %d Tổng số điểm của đội, tổng sức mạnh của người chơi còn sống trong tổng sức mạnh của đội đã giành chiến thắng trong tưởng tượng kỳ diệu này!", tbSort1TeamInfo.nLeftPlayerNum, nLastGetScore, tbSort1TeamInfo.nSurvieFightPower)
end
...
function tbBattleBase:SetCloseRoomGateNpc(nRoomIndex)
	for _,nNpcId in ipairs(self.tbGateCloseNpcId[nRoomIndex]) do
		local pNpc = KNpc.GetById(nNpcId)
		if pNpc then
			pNpc.SetName("Lối vào đã sụp đổ")
		end
	end
end
...
function tbBattleBase:UseItem(pPlayer, nItemId)
	if pPlayer.nFightMode == 2 then
		pPlayer.CenterMsg("Hiện không thể thao tác")
		return
	end
...
function tbBattleBase:UseEquip(pPlayer, nId, nEquipPos)
	...
	if pEquip and pEquip.nEquipPos ~= Item.EQUIPPOS_HORSE then
		...
		pPlayer.CenterMsg(string.format("Thành công trang bị %s", szEquipName));	
	end
...
function tbBattleBase:RequestLeave(pPlayer)
	if pPlayer.nFightMode ~= 2 and self.nSchedulePos ~= #self.STATE_TRANS then
		pPlayer.CenterMsg("Bạn không thể rời đi bây giờ")
		return
	end

	local szMsg = "Nhất định phải rời hoạt động?"
	pPlayer.MsgBox(szMsg, {{"Xác Nhận", function (nMapId)
...
function tbBattleBase:SendAward(pPlayer, tbAward, bNotAutoSell)
	for i,v in ipairs(tbAward) do
		...
		if szAwardMsg then
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format("「%s」Thu được %s", pPlayer.szName, szAwardMsg), pPlayer.dwTeamID)
		end
	end
...
function tbBattleBase:OnSendRandDropAwardToTeam(pPlayer, pNpc, szDropList, nRandMin, nRandMax)
	...
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format("「%s」đã mở %s, nhận được %d kho báu chia đều cho các thành viên", pPlayer.szName, pNpc.szName, nCount), pPlayer.dwTeamID)
end
...
function tbBattleBase:OnAddRandPosNpcSet(pPlayer, pNpc, nTemplateId, nLevel, nRandMin, nRandMax)
	...
	self:OnWorldNotify(string.format("Tâm ma huyễn tượng đã được nhóm người bí ẩn mở ra, khu %d xuất hiện rất nhiều bảo  rương", nRoomIndex))
end
