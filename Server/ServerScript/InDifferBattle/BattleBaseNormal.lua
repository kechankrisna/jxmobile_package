	...
	for nRoomIndex=1,nMaxRoomNum do
		...
		for szPosName, tbRowCol in pairs(tbDefine.tbGateDirectionModify) do
			...
			if nTarRoomIndex then
				...
				if pNpc then
					pNpc.SetName(string.format("Chuyển đến khu vực %d", nTarRoomIndex))
...
function tbBattleBase:OnKillPlayerAndGetAward(pKiller, pDeather)
    ...
    ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format("「%s」Đánh bại người thần bí,nhân được %d kho báu chia đều cho đội", pKiller.szName, nRobAwardNum), dwKillerTeamId)
		...
		if pPlayer.nFightMode ~= 2 then
			...
			if self.nSchedulePos == 7 then
				local nBuffId, nLevel, nTime = unpack(tbDefine.tbLastSkillBuff)
		         ...
		         pPlayer.CenterMsg("Cưỡi ngựa bị cấm ở giai đoạn này", true)
			end
		end
	end
	if self.nSchedulePos == 7 then
...
function tbBattleBase:ShopBuy(pPlayer, nNpcId, nTemplateId, nBuyCount)
	if pPlayer.nFightMode == 2 then
		pPlayer.CenterMsg("Ngài đã bỏ mình, không cách nào tiến hành thao tác")
		return
	end
	local pNpc = KNpc.GetById(nNpcId)
	if not pNpc then
		pPlayer.CenterMsg("Thần bí thương nhân đã rời đi")
		return
	end
	...
	if not nLeftItemNum or nLeftItemNum < nBuyCount then
		pPlayer.CenterMsg("Hàng tồn kho không đủ")
		return
	end

	if pPlayer.GetNpc().GetDistance(nNpcId) >= tbDefine.nCanBuyDistance then
		pPlayer.CenterMsg("Bạn ở quá xa thần bí thương nhân")
		return
	end
...
function tbBattleBase:SellItem(pPlayer, nItemId, nCount)
	if pPlayer.nFightMode == 2 then
		pPlayer.CenterMsg("Không thể thực hiện thao tác")
		return
	end
...
function tbBattleBase:BeforeSwithToRoom(pPlayer, nRoomIndex, bNotCheck)
	if not self.tbCanUseRoomIndex[nRoomIndex] then
		pPlayer.CenterMsg(nRoomIndex .. "Lối vào đã sụp đổ")
		return
	end
	...
	if  not bNotCheck and pPlayer.nFightMode == 2 then --TODO 
		...
		if not bCan then
			pPlayer.CenterMsg("Tình trạng hiện tại của bạn không thể dịch chuyển đến khu vực trên")
			return
		end
	end
	return true
	...
	for dwTeamID,v in pairs(self.tbTeamRoomInfo) do
			...
			if tbReadyCloseRoomIndex[nRoomIndex] then
				...
				if pPlayer then
					...
					if pPlayer.nFightMode ~= 2 then
						...
						pPlayer.CenterMsg("Huyễn cảnh dị biến, bị lực lượng thần bí càn quét đến tận đây!", true)
		...
		if pAlivePlayer then  --三个都死在房间等传出去也是可能的
			...
			for _, pPlayer in ipairs(tbDeathPlayers) do 
				...
				Dialog:SendBlackBoardMsg(pPlayer, "Huyễn cảnh dị biến, bị lực lượng thần bí càn quét đến tận đây!");	
			end		
		end
	end
end
