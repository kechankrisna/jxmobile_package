		...
		for nReadyMapId,v in pairs(self.tbReadyMapTypes) do
			...
			for i,pPlayer in ipairs(tbPlayers) do
				if not self.tbRoleZoneIndex[pPlayer.dwID] then
					pPlayer.CenterMsg("Chưa thể vào hoạt động, hãy chờ trận tiếp theo!", true)
				end
			end	
		end
	end
...
function InDifferBattle:OnBattleMapCreate(nMapId)
	...
	for dwTeamID,v in pairs(tbTeamMemrbName) do
		...
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, "Thành viên tâm ma lần này:" .. szAllName, dwTeamID)
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, "Vui lòng chọn phái trong 6 môn phái sau!", dwTeamID)
	end
...
function InDifferBattle:GiveMoneyTo(pPlayer, dwRoleId2, nMoney)
	...
	if pPlayer2.nFightMode == 2 then
		pPlayer.CenterMsg("Đối phương đã bỏ mình, không thể nào tiếp thu được đưa tặng")
		return
	end
	...
	if pPlayer.CostMoney(tbDefine.szMonoeyType, nMoney, Env.LogWay_InDifferBattle) then
		...
		pPlayer2.Msg( string.format("Thu hoạch được 「%s」 tặng #987%d", pPlayer.szName, nMoney))
