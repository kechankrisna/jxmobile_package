...
function InDifferBattle:OnServerOnReadyMapCreate(tbReadyMapTypes)
	...
	local szMsg = string.format("%s Tâm Ma ảo cảnh đã mở ，các hiệp sĩ có thể thông qua 「hoạt động 」 để báo danh tham gia !", szTitle) 
...
function InDifferBattle:SignUp(pPlayer)
	if not self.tbReadyMapList  then
		pPlayer.CenterMsg("Tâm Ma ảo cảnh hoạt động chưa bắt đầu !")
		return
	end
	local tbPlayers = {};
	...
	if 	#tbPlayers == 1 then
		...
		if not bRet then
			if szTopTypePlayerName and szQualifyName then
				ChatMgr:SendTeamAndCenterMsg(pPlayer, string.format("Đội viên 「%s」 có được tư các %s không thể tham gia", szTopTypePlayerName, szQualifyName ))
			else
				ChatMgr:SendTeamAndCenterMsg(pPlayer, szMsg)
			end
			return
	...
	for i,pPlayer in ipairs(tbPlayers) do
		if not pPlayer.SwitchZoneMap(nReadyMapId, unpack(tbDefine.READY_MAP_POS[MathRandom(#tbDefine.READY_MAP_POS)]) ) then
			pPlayer.CenterMsg("Tạm thời không thể vào trường chuẩn b")
		end
	end	
end
...
function InDifferBattle:CheckTeamSignUp(pPlayer, szTrySignUpBattleType)
	...
	if not tbMember then
		return false, "Nhóm không hợp lệ"
	end 

	local teamData = TeamMgr:GetTeamById(pPlayer.dwTeamID);
	if not teamData or teamData.nCaptainID ~= pPlayer.dwID then
		return false, "Chỉ đội trưởng mới có thể đăng ký"
	end

	if #tbMember > tbDefine.nMaxTeamRoleNum then
		return false, string.format("Mời lấy một mình hoặc đội ngũ nhân số không cao hơn %d người đến đây báo danh", tbDefine.nMaxTeamRoleNum);        
	end
	...
	for _, nPlayerId in pairs(tbMember) do
		if nPlayerId ~= pPlayer.dwID then
			...
			if not pPlayer2 then
				return false, "Thành viên nhóm không xác định, không thể đăng ký!";
			end	
...
function InDifferBattle:SetPlayerQualify(pPlayer, szBattleType, nScore)
	...
	if not bNowCanInNext then
		...
		Mail:SendSystemMail({
			To = pPlayer.dwID,
			Title = string.format("%s Tâm ma ảo cảnh", tbNextBattteSetting.szName),
			Text = string.format("    Lần này %s Tâm ma huyễn cảnh cá nhân điểm tích lũy vì: [FFFE0D]%s[-][%s](%s)[-]\n    Chúc mừng ngươi đã thu hoạch được [FFFE0D]%s Tâm ma huyễn cảnh [-] Tư cách dự thi, đem với [FFFE0D]%s[-] Mở ra, đến lúc đó ngươi đem với cùng nhập vây %s Thi đấu hiệp sĩ cộng đồng tranh đấu, chờ mong ngươi tham dự!\n    [00FF00][url=openwnd:Hiểu rõ quy tắc, GeneralHelpPanel, 'InDifferBattleContestHelp'][-]",
		...
		if pPlayer.dwKinId ~= 0 then
			local szNotifyMsg = string.format("Chúc mừng bang phái thành viên 「%s」 Thu được [FFFE0D]%s Tâm ma huyễn cảnh [-] Tư cách dự thi!", pPlayer.szName, tbNextBattteSetting.szName)
...
function InDifferBattle:SendPlayerAwardS(dwRoleId, nResult, nMatchTime, nScore, nKillCount, szBattleType, nDeathState)
	...
	if  pPlayer then
		if nResult == Env.LogRound_SUCCESS then
			...
			local szNotifyMsg = string.format("Chúc mừng 「%s」 trong %s tâm ma ảo cảnh đã giành chiến thắng!", pPlayer.szName, tbBattleSetting.nNeedGrade and tbBattleSetting.szName .. "賽" or "")
			if dwKinId ~= 0 then
	...
	Mail:SendSystemMail({
		To = dwRoleId,
		Title =  tbBattleSetting.szName ..  "Phần thưởng ảo cảnh",
		Text = string.format("Tôn kính hiệp sĩ: \n\n    %s Lần này tâm ma huyễn cảnh cá nhân điểm tích lũy vì: [FFFE0D]%d[-][%s](%s)[-], phụ kiện là ngài tâm ma huyễn cảnh ban thưởng, mời kiểm tra và nhận!\n\n Tiểu đề bày ra: Cá nhân điểm tích lũy càng cao ban thưởng càng phong phú![00FF00][url=openwnd:Quy tắc tính điểm, AttributeDescription, '', false, '%s'][-]",  
				nResult == Env.LogRound_SUCCESS and "Chúc mừng bạn đã chiến thắng Tâm Ma Ảo Cảnh!" or "", nScore, tbGradeSetting.szColor, tbGradeSetting.szName, tbBattleSetting.szScroeDescHelpKey ),
		tbAttach = tbGetAward,
		nLogReazon = nLogWay,
...
function InDifferBattle:AddPlayerHonorBox(pPlayer, nHonor, nLogReazon, nLogReazon2)
    ...
    if next(tbAwardList) then
    	...
    	pPlayer.CenterMsg(string.format("Ngài thu hoạch được %d Điểm tâm ma huyễn cảnh vinh dự", nHonor), true)
    end
end
...
function InDifferBattle:OnGetNotOpenAward(dwRoleId, szBattleType)
 	...
 	Mail:SendSystemMail({
 		To = dwRoleId,
 		Title =  tbBattleTypeSetting.szName .. "Tâm ma đền bù ban thưởng",
