...
function tbBattleBase:ChangePlayerFactionFight(pPlayer, nFaction)
	...
	if nOldFaction == nFaction then
		pPlayer.CenterMsg("Môn phái giống nhau")
		return
	end
	local pNpc = pPlayer.GetNpc()
	...
	pNpcFaction.SetName( string.format("%s Môn phái chi lực", Faction:GetName(nOldFaction)) )
	--队伍里的变化
	...
	if pCurEquip then
		...
		for i,v in ipairs(tbBookType) do
			if v == nOldBookType then
				pPlayer.CenterMsg("Đã có ngang cấp bí kíp")
				return
			end
		end
		pCurEquip.Delete(0);
...
function tbBattleBase:GatherItem(pPlayer, nNpcTemplateId)
	...
	if not pPlayer.bNotifyUseItemTip then
		...
		pPlayer.CenterMsg("Nhấn hai biểu tượng đạo cụ xuống để sử dụng trực tiếp!", true)
	end
	return true;
end
...
function tbBattleBase:TrowItem(pPlayer, nItemId)
	...
	if not nFromNpcId then
		pPlayer.CenterMsg("Không thể vứt bỏ đạo cụ")
		return
	end
	pItem.Delete(0)
...
function tbBattleBase:SelectPlayerDeathDrop(pPlayer, nIndex, nNpcId)
	...
	if not pNpc then
		pPlayer.CenterMsg("Bảo rương đã biến mất")
		return
	end

	local tbDropNpcs = pNpc.tbDropNpcs
	if not tbDropNpcs then
		pPlayer.CenterMsg("Vô hiệu npc")
		return
	end

	local tbSelInfo = tbDropNpcs[nIndex]
	if not tbSelInfo then
		pPlayer.CenterMsg("Nên rơi xuống vật đã biến mất")
	...
	if bRet then
		if not nFaction then
			...
			pPlayer.CenterMsg(string.format("Thu được 「%s」！", szNpcName), true)
		end
