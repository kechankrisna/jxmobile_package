	...
	for nId, tbItem in pairs(tbAuctionData.tbItems) do
		if not tbItem.bSold and tbItem.nBidderId then
			if bInterrupt then
				...
				local tbMail = {
					To = tbItem.nBidderId;
					Title = "Đấu giá đấu giá thất bại";
					Text = string.format("[FFFE0D]%s[-] Đấu giá thất bại, phí đấu giá [FFFE0D]%d%s[-] gửi lại cho bạn.", tbItemInfo.szName, tbPriceInfo.nCurPrice, szMoneyName);
					From = "Đấu giá hệ thống";
...
function Auction:PersonalAuctionFinish(tbItem)
	...
	if bSold then
		local tbMail = {
			To = tbItem.nOwnerId;
			Title = "Thu nhập đấu giá cá nhân";
			Text = string.format("Chúc mừng [FFFE0D]%s[-]với [FFFE0D]%d Nguyên bảo[-], hãy kiểm tra và nhận", tbItemInfo.szName, tbItem.nCurPrice);
			From = "Đấu giá";
		...
		local tbMail = {
			To = tbItem.nOwnerId;
			Title = "Đấu giá cá nhân";
			Text = string.format("[FFFE0D]%s[-]Không ai đấu giá đã trả về",tbItemInfo.szName);
			From = "Đấu giá";
...
function Auction:FinishAuction(nKinId, szType, bInterrupt)
		...
		if not tbItem.bSold then
			if bInterrupt then
				if tbItem.nBidderId then
					...
					local tbMail = {
						To = tbItem.nBidderId;
						Title = "Đấu giá không thành công";
						Text = string.format("[FFFE0D]%s[-]Giá thầu không thành công và chi phí đấu giá [FFFE0D]%d%s[-]Đã được trả lại cho bạn.", tbItemInfo.szName, tbPriceInfo.nCurPrice, szMoneyName);
						From = "Hệ thống đấu giá";
						tbAttach = {{tbPriceInfo.szMoneyType, tbPriceInfo.nCurPrice}};
						nLogReazon = Env.LogWay_BidFail;
						tbParams = {LogReason2 = Env.LogWay_AuctionClosed};
					};
	...
	if nGoldEach > 0 then
		...
		szGoldGain = string.format("%d Nguyên bảo", nGoldEach);
	end

	if nSilverEach > 0 then
		table.insert(tbAttach, {"SilverBoard", nSilverEach});
		szSilverGain = string.format("%s%d Ngân Sức", nGoldEach > 0 and "、" or "", nSilverEach);
	end
	local szMailText = string.format("    Chúc mừng bạn 【%s】 thu được [FFFE0D]%s%s[-].Vui lòng kiểm tra đính kèm!", Kin.AuctionName[szType] or "", szGoldGain, szSilverGain);

	local tbMail = {
		To = nil;
		Title = "Lợi tức bang hội";
		Text = szMailText;
		From = "Đấu giá";
		tbAttach = tbAttach;
	...
	if nJoinMember > 0 then
		local szMsg = string.format("Chúc mừng tham gia 【%s】 Hoạt động thành viên thu được %s%s lợi tức đấu giá!", Kin.AuctionName[szType], szGoldGain, szSilverGain);
...
function Kin:AddAuction(nKinId, szType, tbPlayerIds, tbItems)
	...
	if not next(tbItems) then
		local szMsg = string.format("Lần này [FFFE0D]%s[-] Trong hoạt động cũng không có thu hoạch, các vị huynh đệ không cần thiết nhụt chí, không ngừng cố gắng!", Kin.AuctionName[szType] or "");
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, nKinId);
...
function Auction:Send10MinutsNotify(tbItems, szAuctionType, nKinId)
	local szInfo = "Vật phẩm đấu giá lần này:";
	...
	local tbNotifyMsg = {
		"Sẽ bắt đầu đấu giá sau 10 phút",
		"Sẽ bắt đầu đấu giá sau 5 phút",
		"Đã bắt đầu đấu giá",
	};
	local nCount = 1;
	local fnNotify = function ()
		...
		if szAuctionType == "Global" then
			local szMsg = string.format("Vòng đâu giá toàn Server %s，%s", tbNotifyMsg[nCount], szInfo);
			KPlayer.SendWorldNotify(Kin.Def.nLevelLimite, 1000, szMsg, ChatMgr.SystemMsgType.System, 1);
		elseif szAuctionType == "Dealer" then
			local szMsg = string.format("【 Hành Cước Thương Nhân】 Mang theo trân phẩm hiện thân，%s。", tbNotifyMsg[nCount]);
			KPlayer.SendWorldNotify(Kin.Def.nLevelLimite, 1000, szMsg, ChatMgr.SystemMsgType.System, 1);
		else
			local szMsg = string.format("【%s】Bang hội%s，%s", Kin.AuctionName[szAuctionType] or "", tbNotifyMsg[nCount], szInfo);
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, nKinId);
		end
...
function Auction:OnDeal(nKinId, szType, nId, bIsBidOver)
	...
	if bidder and bIsBidOver then
		local szInfoMsg = string.format("Chúc mừng bạn đã đấu giá được %d cái %s", tbItemData.nCount, tbItemInfo.szName);
		...
		local szChatTip = string.format("【%s】× %d Bị 「%s」 dùng %d%s mua với giá chót", tbItemInfo.szName, tbItemData.nCount, bidder.szName, tbPriceInfo.nCurPrice, szMoneyName);
		...
		local tbMail = {
			...
			Title = "Đấu giá";
			Text = string.format(" Chúc mừng tại 【%s】đấu giá bang hội tốn [FFFE0D]%s%s[-] giành lấy  [FFFE0D]%s x %d[-]",
...
function Auction:CheckItemAvailable(pPlayer, szType, nId, nPrice, bBidOver)
	...
	if not tbAuctionData then
		return false, "Không tìm thấy tông tin vật phẩm";
	end

	if not tbAuctionData.bOpen then
		return false, "Đấu giá kết thúc";
	end

	local nNow = GetTime();
	if nNow < tbAuctionData.nStartTime then
		return false, "Cuộc đấu giá chưa bắt đầu, xin vui lòng chờ trong giây lát.";
	end

	local nVipLimit = Auction:GetBidBidVipLimitByType(szType);
	if pPlayer.GetVipLevel() < nVipLimit then
		return false, string.format("Đạt kiếm hiệp %d có thể đấu giá", nVipLimit);
	end

	local tbItemData = tbAuctionData.tbItems[nId];
	if not tbItemData or tbItemData.bSold then
		return false, "Vật phẩm đã bị người khác đấu giá";
	end

	if tbItemData.nOwnerId == pPlayer.dwID then
		return false, "Không thể dấu giá vật phẩm của mình";
	end
	...
	if tbItemData.nTimeOut < nNow then
		return false, "Vật phẩm đấu giá đã kết thúc";
	end
	...
	if nPrice ~= tbPriceInfo.nNextPrice and not bBidOver then
		...
		return false, "Giá đấu đã thay đổi";
	end

	if bBidOver and nPrice ~= tbPriceInfo.nMaxPrice then
		return false, "Giá chót có vấn đề";
	end
...
function Auction.AfterBidPaid(nPlayerId, bSuccess, szBillNo, bBidOver, szType, nId, nPrice)
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
	if not pPlayer then
		return false, "Đang trong đấu giá bạn đã offline";
	end

	if not bSuccess then
		return false, "Thanh toán không thành công, vui lòng thử lại sau";
	end
	...
	local tbAuctionData = Auction:GetAuctionData(pPlayer.dwKinId, szType);
	if not tbAuctionData then
		return false, "Mục đấu giá không tồn tại";
	end
	...
	if formerBidder and tbItemData.nBidderId ~= pPlayer.dwID then
		...
		local tbItemInfo = KItem.GetItemBaseProp(tbItemData.nItemId);
		formerBidder.CenterMsg(string.format("[FFFE0D]%s[-]Đấu giá thất bại,[FFFE0D]%d%s[-] đã hoàn trả", tbItemInfo.szName, tbPriceInfo.nCurPrice, szMoneyName));
		local tbMsgData = {
			szType = "Auction";
	...
	elseif tbItemData.nBidderId and tbItemData.nBidderId ~= pPlayer.dwID then
		...
		local tbMail = {
			To = tbItemData.nBidderId;
			Title = "Đấu giá không thành công";
			Text = string.format("[FFFE0D]%s[-]Đấu giá thất bại,[FFFE0D]%d%s[-] đã hoàn trả", tbItemInfo.szName, tbPriceInfo.nCurPrice, szMoneyName);
			From = "Đấu giá hệ thống";
			tbAttach = {{tbPriceInfo.szMoneyType, tbPriceInfo.nCurPrice}};
			nLogReazon = Env.LogWay_BidFail;
...
function Auction:BidOver(szType, nId, nPrice)
	...
	local szMoneyName = Shop:GetMoneyName(tbPriceInfo.szMoneyType);
	if nMyMoney < nRealCost then
		return false, string.format("%s không đủ！", szMoneyName);
	end
...
function Auction:OnSyncAuctions(szType, nVersion, player)
	...
	if player.dwKinId == 0 and not Auction:IsPermanent(szType) then
		return false, "Lỗi dữ liệu yêu cầu đấu giá";
	end
...
function Auction:AskDealListInfo(szType)
	if me.dwKinId == 0 and not Auction:IsPermanent(szType) then
		return false, "Yêu cầu lịch sử đấu giá lỗi";
	end
...
function Auction:DeletePersonalAuctionItem(nFrame, nItemId, nCount)
	...
	if not tbPersonAuctionData then
		...
		return false, "Không thể tìm thấy vật phẩm đấu giá";
	end
	...
	if bSuccess then
		...
		me.CenterMsg("Không thể tìm thấy vật phẩm đấu giá");
	end
	...
	for nId, tbItem in pairs(tbAuctionData.tbItems or {}) do
		if tbItem.nBidderId and not tbItem.bSold then
			...
			local tbMail = {
				To = tbItem.nBidderId;
				Title = "Đấu giá không thành công";
				Text = string.format("[FFFE0D]%s[-]đấu giá thất bại [FFFE0D]%d%s[-]đã được trả lại cho bạn.", tbItemInfo.szName, tbPriceInfo.nCurPrice, szMoneyName);
				From = "Hệ thống đấu giá";
				tbAttach = {{tbPriceInfo.szMoneyType, tbPriceInfo.nCurPrice}};
				nLogReazon = Env.LogWay_BidFail;
				tbParams = {LogReason2 = Env.LogWay_AuctionClosed};
