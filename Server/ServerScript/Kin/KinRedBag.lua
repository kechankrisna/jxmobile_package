...
function Kin:_RedBagCanSend(tbRedBag, nOwnerId)
	if tbRedBag.tbOwner.nId~=nOwnerId then
		return false, "Lì Xì không là của ngươi"
	end

	if tbRedBag.nSendTime>0 then
		return false, "Lì Xì đã bị mở"
	end
	return true
end
...
function Kin:_RedBagSendByPlayer(pPlayer, szId, nAddGold, nCount)
	...
	if not tbKinData then
		return false, "Bạn đã không có Bang hội"
	end
	...
	if not tbRedBag then
		return false, "Lì Xì không tồn tại"
	end
...
function Kin:_RedBagSend(szId, nAddGold, nCount)
	...
	if not tbRedBag then
		me.CenterMsg("Lì Xì không tồn tại")
		return
	end
	...
	if nAddGold>0 then
		if me.GetMoney("Gold")<nAddGold then
			me.CenterMsg("Nguyên Bảo không đủ ，gửi đi Lì Xì thất bại")
			return
		end
		
		me.CostGold(nAddGold, Env.LogWay_KinRedBag, nil, function(nPlayerId, bSuccess)
			if not bSuccess then
				return false, "Nguyên Bảo không đủ ，gửi đi Lì Xì thất bại"
			end

			local pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
			if not pPlayer then
				return false, "Trong quá trình lì xì, bạn đang ngoại tuyến."
			end
...
function Kin:_RedBagDoSendChatLinks(tbRedBag, nKinId, bOnline, bGlobal)
	...
	if bOnline then
		...
		local szMsg = string.format("#971%s, chia sẻ 1 Lì Xì %s <Xem Lì Xì>", szContent, szMulti)
		if bGlobal then
			szMsg = string.format("#940%s, chia sẻ thế giới 1 Lì Xì %s <Xem Lì Xì>", szContent, szMulti)
		end
		...
		local szSysMsg = string.format("Tổng Quản Bang Hội: #971「%s」có Lì Xì quá hạn lão phu thay mặt phát đi <Xem Lì Xì>", pOwner.szName)
		if bGlobal then
			szSysMsg = string.format("#940「%s」có lì xì thế giới quá hạn hệ thống thay mặt tự động phát đi <Xem Lì Xì>", pOwner.szName)
		end
...
function Kin:_RedBagCanGrab(tbRedBag, nPlayerId)
	...
	if self:RedBagIsPlayerGrabEnough(pPlayer) then
		return false, "Đại Hiệp hôm nay nhận số lì xì cá nhân đã đạt mức cao nhất"
	end

	local nSendTime = tbRedBag.nSendTime
	if nSendTime<=0 then
		return false, "Phát Lì Xì"
	end

	if GetTime()-nSendTime>Kin.Def.nRedbagExpireTime then
		return false, "Lì Xì quá hạn"
	end

	if #tbRedBag.tbReceivers>=tbRedBag.nMaxReceiver then
		return false, "Lì Xì đã bị cướp"
	end
	...
	for _,tbReceiver in ipairs(tbRedBag.tbReceivers) do
		...
		if tbPack.nId==nPlayerId then
			return false, "Đã nhận lì xì này"
		end
	end
...
function Kin:_RedBagCheckDead(tbRedBag, nKinId, bGlobal)
	if tbRedBag.nMaxReceiver<=#tbRedBag.tbReceivers then
		...
		if pLuckyMan and pOwner then
			local szMsg = string.format("「%s」có %d Lì Xì, tổng cộng %d#999 %s bị đoạt mất.「%s」may mắn đoạt được %d#999",
...
function Kin:RedBagUpdateReq(pPlayer, szId, nVersion, nGlobalVersion)
	...
	if szId=="" then
		...
		if not tbRedBag then
			pPlayer.CenterMsg("Lì Xì không tồn tại")
			return
		end
