...
function Kin:_OpenSrvGetRankDesc(nRank)
	if nRank==1 then
		return "[ffb400]【Đệ Nhất Bang Hội】[-]"
	elseif nRank<=10 then
		return "[ff68a1]【Thập Đại Bang Hội】[-]"
	elseif nRank<=20 then
		return "[9f5fff]【Bang Hội Tinh Anh】[-]"
	end
end
				...
				for _,nId in ipairs(tbIds) do
					...
					if tbInfo then
						...
						if nNow-nJoinTime>=24*3600*nRewardMinJoinDay then
							...
							local szText = string.format("    Chúc mừng bang hội của bạn đứng thứ [FFFE0D]%d[-]", i)
							local szRankDesc = self:_OpenSrvGetRankDesc(i)
							if szRankDesc then
								szText = string.format("%s, được đánh giá là %s。", szText, szRankDesc)
							end
							Mail:SendSystemMail({
								To = nId,
							    Title = "Đua Top Bang Hội",
							    Text = szText,
							    From = "Hệ Thống",
							    tbAttach = tbAttach,
							})
...
function Kin:_OpenSrvDontQuitMail()
	...
	for nKinId in pairs(self.KinData) do
		Mail:SendKinMail({
			KinId = nKinId,
			Text = string.format("    Chư vị hiệp sĩ thân mến, đua top bang hội sẽ kết thúc: [FFFE0D]%s（%d ngày）[-]!\n    [FFFE0D]Chú ý: Thoát bang hội sẽ không được nhận thưởng[-]", szEndDate, nDaysLeft),
