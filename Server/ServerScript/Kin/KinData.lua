...
function _KinData:TryPushMail()
	...
	for nId in pairs(tbValidIds) do
		...
		if pStayInfo then
			...
			Mail:SendSystemMail({
				To = nId,
				Title = "Bang hội bách ứng",
				From = "Tổng Quản Bang Hội",
				Text = string.format("    Vì thuận tiện bang phái quản lý đối bang phái lớn nhỏ công việc thống nhất chỉ huy hành động, hiện đặc biệt đối bang phái bưu kiện gia tăng điện thoại hệ thống nhắc nhở công năng, bang phái bưu kiện không gần như chỉ ở trong trò chơi gửi đi cho mỗi cái bang phái thành viên, đồng thời cũng đem thông qua điện thoại hệ thống nhắc nhở trực tiếp gửi đi đến đã khai thông kiếm hiệp tình duyên game điện thoại hệ thống nhắc nhở cho phép quyền bang phái thành viên điện thoại, bang phái đại sự giây phút tức đạt, điểm kích [FFFE0D][url=openinnerurl:< Xem xét giáo trình >,%s][-], nhưng xem xét điện thoại hệ thống nhắc nhở mở ra phương pháp.", szUrl),
			})
...
function _KinData:Available2Join(bIsNewer)
	...
	return false, "Thành viên bang hội đã đủ"
end
...
function _KinData:DeleteMember(nMemberId)
	if nMemberId==self.nLeaderId then
		...
		Mail:SendKinMail({
			KinId = self.nKinId,
			Text = "  Vì Thủ Lĩnh rời khỏi Bang Hội, trong vòng 3 ngày bình chọn【THủ Lĩnh】mới, nếu thành viên nào có tích lũy cống hiến cao nhất sẽ trở thành Thủ Lĩnh mới.",
			From = "Tổng Quản Bang Hội",
		})
...
function _KinData:ChangePublicDeclare(szPublicDeclare)
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, me.szName .. "Đổi mới Bang Hội thông báo!", self.nKinId);
...
function _KinData:AddPrestige(nValue, nLogReason)
	...
	if nValue > 0 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Uy danh Bang Hội trong giang hồ tăng %d", nValue), self.nKinId);
	end
end
...
function _KinData:SetLeaderId(nLeaderId, bSendMail)
	...
	if bSendMail and nOldLeaderId~=nLeaderId then
		...
		if tbMember then
			...
			Mail:SendKinMail({
				KinId = self.nKinId,
				Text = string.format("    Bang phái chư vị huynh đệ! Đại hiệp「%s」 Đang bang phái bên trong tính gộp lại cống hiến đã đạt %d, viễn siêu người khác, trở thành tân nhiệm bang phái 【Thủ Lĩnh】 Chính là thực chí danh quy, trong tộc đều huynh đệ, mong rằng các vị huynh đệ ngày sau có thể hết sức giúp đỡ, chúng ta cùng nhau phấn chiến, đồng tâm hiệp lực, viết bang phái huy hoàng!", szName, nContribution),
				From = "Tổng Quản Bang Hội",
			})
		end
	end
	...
	if bSendMail then
		Mail:SendSystemMail({
	        To = nLeaderId,
	        Title = "Bổ nhiệm bang hội",
	        Text = "Ngài được bổ nhiệm làm 【Thủ Lĩnh】\n Được phép nhận đuổi đường chủ, nhận đuổi thành viên",
	        From = "Tổng Quản Bang Hội",
	        tbAttach = {},
	    })
	end
...
function _KinData:AppointLeader(nCandidateLeaderId)
	...
	if nNow-self.nAppointLeaderTime<=24*3600 then
		...
		return false, string.format("Cần %d giờ sau mới có thể thao tác bổ nhiệm lại Thủ Lĩnh", nHours)
	end
	...
	if pLeader and pCandidate then
		local szMsg = string.format("「%s」 được 「%s」 bổ nhiệm làm 【Thũ lĩnh Tạm thời】 sau 72 giờ sẽ trở thành 【Bang Chủ】 chính thức", pLeader.szName, pCandidate.szName)
...
function _KinData:OnLeaderLogin()
	if self.nCandidateLeaderId>0 then
		...
		Mail:SendSystemMail({
	 		To = self.nLeaderId,
	        Title = "Thũ lĩnh nhậm chức",
	        Text = string.format("    「%s」được bổ nhiệm làm 【Thũ lĩnh tạm thời】, sau %s chính thức trở thành 【Thũ lĩnh】, trong thời gian có thể hủy bỏ bổ nhiệm.", tbCandidate:GetName(), szTimeStr),
	        From = "Hệ Thống",
	        tbAttach = {},
		})
	elseif self.nChangeLeaderTime>0 then
		...
		Mail:SendKinMail({
			KinId = self.nKinId,
			Text = "    Vì Bang chủ đương nhiệm tái xuất giang hồ, hủy bỏ bình chọn Thũ lĩnh",
			From = "Tổng Quản Bang Hội",
		})
	end
end
...
function _KinData:CheckLeaderOff()
	...
	if nLeaderOffSeconds and nLeaderOffSeconds>=7*24*3600 then
		...
		Mail:SendKinMail({
			KinId = self.nKinId,
			Text = "    Bởi vì đương nhiệm Thủ Lĩnh ẩn lui giang hồ ( Offline quá 7 ngày ), tân nhiệm 【Thủ Lĩnh】 sẽ có 3 ngày bình chọn, tích lũy cống hiến cao nhất trong tất cả thành viên bang hội sẽ là Thủ Lĩnh mới.",
			From = "Tổng Quản Bang Hội",
		})
	end
end
...
function _KinData:_ChangeLeader()
	...
	if self.nCandidateLeaderId==nLeaderId then
		...
		if pLeader and pCandidate then
			Mail:SendKinMail({
				KinId = self.nKinId,
				Text = string.format("    「%s」được「%s」 bổ nhiệm làm 【Thủ Lĩnh】 mới", pLeader.szName, pCandidate.szName),
				From = "Tổng Quản Bang Hội",
			})
			bSendMail = false
		end
...
function _KinData:CheckAddBattleApplysLimit(nItemId, nNum)
	...
	if (self.tbAddBattleSupplyLimit[nItemId] or 0) + nNum > nLimitNum then
		...
		return false, string.format("Mua nhiều nhất mỗi ngày %d cái %s", nLimitNum, tbItembase.szName)
	end
	return true
end
--todo 战争坊增加物资
...
function _KinData:ChangeName(szKinName, bNotNotify)
	...
	if not bNotNotify then
		Mail:SendKinMail({
			KinId = self.nKinId,
			Title = "Đổi tên Bang hội",
			Text = string.format("Bang hội đổi tên thành 【%s】", szKinName),
			From = "Tổng Quản Bang Hội",
		})

		local szWorldNotifyMsg = string.format("hư vị hiệp sĩ, giang hồ dậy sóng giao tranh quyết liệt giữa các nhân sĩ, bang hội【%s】hiện đã đổi tên Bang Hội thành【%s】, nay thông báo cho quý đồng đạo nắm rõ.", szOldName, szKinName)
...
function _KinMember:JoinKin(kinData)
	...
	if nMasterId ~= self.nMemberId then
		local szInfo = string.format("[FFFE0D]「%s」[-]trở thành thành viên bang hội!#49", tbRoleStayInfo.szName);
	...
	if nCareer==Kin.Def.Career_New and nMasterId~=self.nMemberId then
		Mail:SendSystemMail({
			...
			Title = "Thư Bang Hội",
			From = "Tổng Quản Bang Hội",
			Text = "  Hoan nghênh ngươi gia nhập bang hội! Vì ngươi cấp thấp hơn so với quy định, nên chỉ làm thành viên Kiến Tập! Kiến Tập ở bang hội vẫn được hưởng đặc biệt quyền, nhưng vẫn bị hạn chế[FFFE0D]【Truyền Công】với các thành viên khác[-], mong ngươi sớm thể hiện thực lực của mình sau này nhậm chức để hưởng lương bổng nhiều hơn!",
		})
	end
...
function _KinMember:SendSetCareerMail(nCareer)
	...
	local szMailContent = string.format("Chúc mừng ngươi nhận chức 【%s】\n", Kin.Def.Career_Name[nCareer]);
	...
	if tbAuthority and next(tbAuthority) then
		szMailContent = szMailContent .. "có đặc quyền: ";
		local szDot = "";
	...
	if nCareer==Kin.Def.Career_Mascot then
		szMailContent = string.format("%s có đặc quyền: Chat Voice ở kênh Bang Hội thời gian 30 giây, nhận được tiền tố trò chuyện Bảo Bối", szMailContent)
	end

	local tbMail = {
		To = self.nMemberId;
		Title = "Nhậm chức Bang Hội";
		From = "Tổng Quản Bang Hội";
		Text = szMailContent;
	};
