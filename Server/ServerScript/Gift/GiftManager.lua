...
function GiftManager:CheckGiftTimes(pPlayer,nAcceptId,nCount,nGiftType,nItemId)
	...
	if not nAcceptId then
		return false,"Chọn hảo hữu muốn tặng";
	end

	if not nCount or nCount < 1 then
		return false,"Chọn số lượng";
	end

	if not Gift.AllGift[nGiftType] then
		return false,"Chọn vật phẩm muốn tặng"
	end
	...
	if not nTimes then
		...
		return false,"Tư liệu dị thường, xin liên lạc phục vụ khách hàng"
	end
	...
	if nRemain < 1 or nCount > nRemain then
		return false,"Số lượng còn lại không đủ";
	end
...
function GiftManager:CheckItemSend(pPlayer,nCount,nGiftType,nItemId)
	if not nCount or nCount < 1 then
		return false,"Vui lòng chọn số lượng";
	end
	...
	if not nTimes then
		...
		return false,"Dữ liệu bất thường, vui lòng liên hệ với dịch vụ khách hàng"
	end
...
function GiftManager:CheckItemAccept(pPlayer,nCount,nGiftType,nItemId)
	...
	if nRemain < 1 or nCount > nRemain then
		return false,"Số lần nhận quà đối phương không còn", nRemain;
	end
