...
function Server:OnStartup()
	...
	Lib:CallBack({ScriptData.OnStartUp, ScriptData});
	Lib:CallBack({DrinkHouse.OnStartUp, DrinkHouse});

	Lib:CallBack({Player.Stronger.OnServerStart, Player.Stronger});
	local tbRandomItemByVersion = Item:GetClass("RandomItemByVersion");
	Lib:CallBack({tbRandomItemByVersion.OnServerStart, tbRandomItemByVersion});
	Lib:CallBack({LoverTask.OnServerStart, LoverTask});
	local nServerId = GetServerIdentity()
	if math.mod(nServerId, 2) == 1 then
		--选取部分服测试
		Log("SetPackFlushFrame", nServerId)
		SetPackFlushFrame(0);
	end
end
...
function Server:LoadZoneGroup()
	self.tbZoneGroup = LoadTabFile("Setting/ZoneGroup.tab", "dddddddddddddddd", "ServerId", {"ServerId", "Normal", "Battle", "Baihu", "TeamBattle", "Indiffer",
					 "DaXueZhang","BiWuZhaoQin", "WLDH", "BLBoss", "ZoneBossMaster", "FactionBattle", "QYHCross", "CrossDomain", "KinEncounter", "KeyQuestFuben"});
