...
function CollectionSystem:OnUseShenZhouKa()
    if not self:IsInProcess(self.JINXIUSHANHE) then
        me.CenterMsg("Không có trong thời gian hoạt động")
        return
    end
...
function CollectionSystem:ActivatePosition(nCollectionId, nCardTemplateId)
    ...
    local szMsg = string.format("Bạn đã thu thập thành công[%s]%s[-](Thu thập Lăng tuyệt phong)", TxtColor or "848484", szCardName )
    me.Msg(szMsg)
...
function CollectionSystem:SendRankAward_1()
    ...
    local tbNewInfoRank = {}
    local tbMail = {Title = "Thu thập Lăng Tuyệt Phong", From = "", nLogReazon = Env.LogWay_CollectionSystem}
    local szMailText = "Thiếu hiệp, hoạt động Thu Thập Lăng Tuyệt Phong tháng trước đã kết thúc. Ngươi xếp vị trí thứ %d, phần thưởng được đính kèm xin kiểm tra và nhận. Tháng này, hoạt động thu thập mới đã bắt đầu. Hãy tiếp tục cố gắng nhé!"
...
function CollectionSystem:SendRankAward_2()
    ...
    local tbRankPlayer = RankBoard:GetRankBoardWithLength("CardCollection_" .. nCollectionId, 99999, 1)
    local tbNewInfoRank = {}
    local tbMail = {Title = "Cẩm Tú Sơn Hà", From = "", nLogReazon = Env.LogWay_CollectionSystem}
    local szMailText = "Đại hiệp, hoạt động Thu Thập Cẩm Tú Sơn Hà đã kết thúc. Ngươi xếp vị trí thứ %d, phần thưởng được đính kèm xin kiểm tra và nhận.!"
    for nRank, tbRankInfo in ipairs(tbRankPlayer or {}) do
        local tbAward = self:GetRankAward(nCollectionId, nRank)
        if tbAward and next(tbAward) then
