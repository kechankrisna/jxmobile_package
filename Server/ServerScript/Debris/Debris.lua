...
function Debris:AddRandomDerisToPlayer(pPlayer, nItemId)
	local nKind = Debris.tbItemIndex[nItemId]
 	if not nKind then
 		return false, "Phi pháp thao tác 2"
	end
	local tbKindInfo = Debris.tbSettingLevel[nKind]
...
function Debris:AddDerisToPlayer(pPlayer, nItemId, nIndex)
	if nIndex <= 0 then
		return false, "Phi pháp thao tác 1"
	end
	local nKind = Debris.tbItemIndex[nItemId]
 	if not nKind then
 		return false, "Phi pháp thao tác 2"
	...
	if nIndex > tbKindInfo.nNum then
		return false, "Phi pháp thao tác 3"
	end

	local nVal = tbEquipDebris:GetValue(pPlayer, nItemId, nIndex)
	if nVal == 1 then
		return false, "Đã có được nên mảnh vỡ"
	end
...
function Debris:RemoveDerisFromPlayer(dwRoleId, nItemId, nIndex)
	...
	if pPlayer then
		...
		if not pAnsyPlayer then
			...
			return false, "Tư liệu đã qua kỳ"
		end
	...
	if nVal ~= 1 then
		...
		return false, "Không nên đạo cụ mảnh vỡ"
	end
...
function Debris:AvoidRob(dwRoleId, nAvoidTime)
	...
	if nAvoidTime <= 0 then
		...
		return false, "Phi pháp thao tác 5"
	end
	local tbAvoidRob = self.tbAllAvoidRob
	local nTimeNow = GetTime();
	local pPlayer = KPlayer.GetPlayerObjById(dwRoleId)
	...
	if nDuraTime >= 24* 3600 then
		return false, "Mở ra thất bại, trước mắt miễn thời gian chiến tranh ở giữa vượt qua 24 Giờ"
	end
...
function Debris:BuyDebrisAvoidRobTime(pPlayer, nIndex)
	...
	if not tbBuyInfo then
		pPlayer.CenterMsg("Phi pháp thao tác 1")
		return
	end
	...
	if tbBuyInfo[1] == "item" then
		...
		if nCost < tbBuyInfo[4] then
			pPlayer.CenterMsg("Phi pháp thao tác 2")
			return
		end
	elseif tbBuyInfo[1] == "Gold" then --其他货币
		...
		if pPlayer.GetMoney(tbBuyInfo[1] ) < nCost then
			pPlayer.CenterMsg("Phi pháp thao tác 3")
			return
		end
	...
	pPlayer.CenterMsg("Mua miễn thời gian chiến tranh ở giữa thành công")
	pPlayer.CallClientScript("Debris:RefreshMainPanel");
end
...
function Debris:RobHim(pPlayer, dwRobId, nItemId, nIndex, nCount)
	...
	if nDegree <= 0 then --购买的话客户端自己判断就好了
		pPlayer.CenterMsg("Ngài cướp đoạt số lần không đủ")
		return
	end

	if tbEquipDebris:GetValue(pPlayer, nItemId) == 0 then
		pPlayer.CenterMsg("Nhất định phải có một khối tương quan mảnh vỡ mới có thể cướp đoạt") --必须要有一块相关的碎片才能抢
		pPlayer.CallClientScript("Debris:RefreshMainPanel")
		return
	end
	...
	local nVal = tbEquipDebris:GetValue(pPlayer, nItemId, nIndex)
	if nVal == 1 then
		pPlayer.Msg("Đã có được mảnh vỡ đoạt không được")
		return
	end
	...
	if not pRoleStay then
		pPlayer.CenterMsg("Ngài số liệu quá thời hạn")
		fnCearList();
		return
	end
	...
	if tbAvoidRob[dwRobId] then
		if tbAvoidRob[dwRobId] > nTimeNow then
			pPlayer.CenterMsg("Đối phương ở vào miễn chiến kỳ")
			fnCearList();
			return
		else
	...
	if pPlayer.dwKinId ~= 0 and pPlayer.dwKinId == pRoleStay.dwKinId then
		pPlayer.CenterMsg("Đối phương cùng ngươi là cùng bang phái thành viên")
		fnCearList()
		return
	end

	if FriendShip:IsFriend(dwMyId, dwRobId) then
		pPlayer.CenterMsg("Đối phương cùng ngươi là bạn tốt quan hệ")
		fnCearList()
		return
	end
