...
function CodeAward:DoTakeAward(pPlayer, szCode)
	return false, "Công năng quan bế";
end
...
local tbRespondTips = {
	[RSP_CODE_OK]            = "Thành công nhận lấy gói quà mã, mời đến hộp thư ở trong xem xét";
	[RSP_CODE_USED]          = "Nên gói quà đã bị sử dụng";
	[RSP_CODE_CHANNEL_WRONG] = "Nên gói quà mã không thể tại bản con đường sử dụng";
	[RSP_CODE_SERVER_WRONG]  = "Nên gói quà mã không thể tại bản phục sử dụng";
	[RSP_CODE_NOT_EXIST]     = "Nên gói quà mã không tồn tại";
	[RSP_CODE_DB_WRONG]      = "Nghiệm chứng gói quà mã thời điểm, server phát sinh một sai lầm";
}
...
function CodeAward:VerifyRespond(nRoleId, szGiftCode, nRespondCode)
	...
	local tbMail = {
		To = nRoleId;
		Title = "Gói quà mã ban thưởng";
		Text = tbRewardSetting.MailTxt;
		From = "Gói quà mã cấp cho hệ thống";
...
function CodeAward:SendXGCodeGift(nPlayerId, szGiftType)
	...
	if pPlayer then
		pPlayer.CenterMsg("Thành công nhận lấy gói quà mã, mời đến hộp thư ở trong xem xét");
	end
