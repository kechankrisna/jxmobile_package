...
function HeroChallenge:DoChallengeMaster(pPlayer)
    ...
    if pPlayer.dwTeamID > 0 then
        pPlayer.MsgBox("Đang trong tổ đội không thể khiêu chiến, thoát đội?",
            {
                {"Thoát", TeamMgr.QuiteTeam, TeamMgr, pPlayer.dwTeamID, pPlayer.dwID},
                {"Hủy"},
            })
        return;
    end
    ...
    if not nChallengeID then
        pPlayer.CenterMsg("Tái đấu với đối thủ");
...
function HeroChallenge:DoChallengeWin(pPlayer, nFloor, nStartTime)
    ...
    if nFloor == 10 and pPlayer.dwKinId ~= 0 then
        ...
        local szShowMsg = string.format("Thành viên bang %s, võ công cái thế, mới đó đã thông qua tầng 10 Khiêu chiến anh hùng!", pPlayer.szName);
