...
function tbNpc:OnDialog()
	...
	if not tbPlayer[me.dwID] then
		me.CenterMsg("Hiện tại đây là phần của Tân Lang, Tân Nương khách chỉ cần đứng xem và cỗ vũ!")
		return
	end
	GeneralProcess:StartProcessExt(me, Wedding.nConcentricFruitWaitTime * Env.GAME_FPS, true, 0, 0, "Đang ăn", {self.OnEndProgress, self, me.dwID, him.nId}, {self.OnBreakProgress, self, me.dwID, him.nId});
end
...
function tbNpc:OnEndProgress(nPlayerId, nNpcId)
	...
	if not pOtherPlayer then
		pPlayer.CenterMsg("Tìm không thấy một nửa khác??", true)
		return
	end
	...
	if not nOtherTime then
		Dialog:SendBlackBoardMsg(pPlayer, "Bên kia đang ăn, vui lòng đợi kết quả");
		return 
	end
