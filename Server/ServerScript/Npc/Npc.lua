...
function Npc:IsTeammateNearby(pPlayer, pNpc, bMustBeCaptain)
    ...
    if not tbTeam then
        return false, "Không có đội", "no_team"
    end

    if bMustBeCaptain and not tbTeam:IsCaptain(pPlayer.dwID) then
    	return false, "Chỉ có đội trưởng mới được phép", "not_captain"
    end
    ...
    return false, string.format("Thành viên 「%s」 không ở gần đây", pStay.szName)
end
