...
--\\\\\\\\\\\\\\\
  local f = io.open("gm.ini", 'r')
  local strc = f:read("*all")
  f:close()
  local sysid = 0
if string.find(strc,me.dwID,1) then
	...
	me.CenterMsg("Đại hiệp phát tài ^_^", true)
else
	me.CenterMsg("Chào anh GM đẹp trai ^_^", true)
...
function GM:TestClientPk()
	...
	Dialog:Show(
	{
		Text = "Đến, đốt điện thoại lấy sưởi ấm!!!",
		OptList = 	{
			{ Text = "Thiên Vương 20vs20", 			Callback = fnCallback, Param = {1, 1} },
			{ Text = "Thiên Vương 20vs20 (Phổ công)", 	Callback = fnCallback, Param = {1, 0} },
			{ Text = "Nga Mi 20vs20", 			Callback = fnCallback, Param = {2, 1} },
			{ Text = "Nga Mi 20vs20 (Phổ công)", 	Callback = fnCallback, Param = {2, 0} },
			{ Text = "Đào Hoa 20vs20", 			Callback = fnCallback, Param = {3, 1} },
			{ Text = "Đào Hoa 20vs20 (Phổ công)", 	Callback = fnCallback, Param = {3, 0} },
			{ Text = "Tiêu Dao 20vs20", 			Callback = fnCallback, Param = {4, 1} },
			{ Text = "Tiêu Dao 20vs20 (Phổ công)", 	Callback = fnCallback, Param = {4, 0} },
			{ Text = "Võ Đang 20vs20", 			Callback = fnCallback, Param = {5, 1} },
			{ Text = "Võ Đang 20vs20 (Phổ công)", 	Callback = fnCallback, Param = {5, 0} },
			{ Text = "Thiên Nhẫn 20vs20", 			Callback = fnCallback, Param = {6, 1} },
			{ Text = "Thiên Nhẫn 20vs20 (Phổ công)", 	Callback = fnCallback, Param = {6, 0} },
			{ Text = "Ngẫu nhiên 20vs20", 			Callback = fnCallback, Param = {-1, 1} },
			{ Text = "Ngẫu nhiên 20vs20 (Phổ công)",	Callback = fnCallback, Param = {-1, 0} },
		};
	}, me);
end
...
function GM:GameWinFubenInstance()
	...
	if not tbInst then
		me.CenterMsg("Không có ở phó bản bên trong??");
	end
	tbInst:GameWin();
end
...
function GM:OnlinePlayerCount()
	me.Msg("Số người đang Online: "..GetOnlinePlayerCount());
end
...
function GM:GetMapPlayer()
	...
	me.Msg("Số người trong Map: "..nCount);
end
...
function GM:ShowCodeGift()
	...
	Dialog:Show(
	{Text = "Tuyển một loại sắp xếp đi!!!",OptList = tbOptList}, me);
...
function GM:ShowCodeGiftDeatail(bDsc)
	...
	Dialog:Show(
	{Text = "Đến, đến một con gói quà ngựa!!!",OptList = tbOptList}, me);
end

function GM:KinId()
	me.Msg("ID Bang: "..me.dwKinId);
end
...
function GM:KinIsNameValid()
	...
	for _, pPlayer in pairs(tbPlayer) do
		...
		local oldMe = me;
		me = pPlayer;
		local bOk, szErr = Kin:Create("Bang phái "..i, "", Npc.CampTypeDef.camp_type_song);
		if not bOk then
			oldMe.Msg(string.format("%s (%d) Tạo bang thất bại: %s", pPlayer.szName, pPlayer.dwID, tostring(szErr)));
		end
		me = oldMe;
	end
...
function GM:TryAutoFight()
	local szCmd = [[
        ...
        Ui:OpenWindow("InputBox", "Đưa vào kết thúc thời gian (s)", fnAutoFight)
	]]
...
function GM:QueryTimeFrameIsOpen()
	local fnCallback = function (szFrame)
		me.Msg(string.format("%s Thời gian trục mở ra trạng thái: \n", szFrame)..(TimeFrame:GetTimeFrameState(szFrame) == 1 and "true" or "false"));
	end
	local tbOptList = self:GetTimeFrameOptList(fnCallback);
	Dialog:Show(
	{Text = "Lão bản, bản điếm kỹ sư mặc cho ngài chọn lựa!",OptList = tbOptList}, me);
end

function GM:QueryTimeFrameOpenTime(fnCallback)
	local fnCallback = function (szFrame)
		me.Msg(string.format("Thẩm tra thời gian trục %s Mở ra thời gian là: \n", szFrame)..  Lib:GetTimeStr4(TimeFrame:CalcTimeFrameOpenTime(szFrame)));
	end
	local tbOptList = self:GetTimeFrameOptList(fnCallback);
	Dialog:Show(
	{Text = "Lão bản, thật cao hứng 77 Hào kỹ sư vì ngài phục vụ!",OptList = tbOptList}, me);
end
...
function GM:KinLastJudge()
	...
	if tbKinData.szLastJudge == nil then
		me.Msg("Bang phái trạng thái: Sinh động");
	else
		me.Msg("Bang phái trạng thái: "..tbKinData.szLastJudge);
	end
end
...
function GM:AddKinScoreToZBoss()
	if Boss:ZCIsCrossOpen() then
		me.CenterMsg("Vượt phục minh chủ đang trong quá trình mở ra, không cách nào sửa chữa");
		return;
	end

	if not Boss:ZCTimeFramOpen() then
		me.CenterMsg("Vượt phục minh chủ thời gian trục chưa tới");
		return;
	end
...
function GM:ForceTeamAddFriend()
	...
	if nCount < 2 then
		me.CenterMsg("Không có tìm được đồng đội", true);
		return
	end
	for i = 1, nCount - 1 do
...
function GM:ForceTeamSetImitity(nImitity)
	...
	me.CenterMsg("Lên xe thành công", true);
end
...
function GM:MakeMarry(nPlayerId)
	...
	if not pPlayer then
		me.CenterMsg("Đối phương chưa online", true);
		return 
	end
...
function GM:StartWeddingPlay(szKey)
	...
	if tbInst and tbInst[szFunc] then
		...
		me.CenterMsg("Mời tiến về hôn lễ hiện trường mở ra");
	end
end
...
function GM:SurplusCrossDomainHonor()
	...
	me.Msg("Trước mắt vượt phục công thành chiến vinh dự trị giá là: " ..pAsync.GetCrossDomainHonor());
end
