...
--根据旧道具的摆摊限制情况重新设置新道具禁止摆摊数据
function MarketStall:ResetItemSellLimit(nOldItemTemplateId, nNewItemTemplateId, pPlayer)
	if not self.bSellLimitOpen then
		return;
	end
	local tbOldLimitInfo = (self.tbSellLimitInfo["item"] or {})[nOldItemTemplateId or 0];
	local tbNewimitInfo = (self.tbSellLimitInfo["item"] or {})[nNewItemTemplateId or 0];

	if not tbOldLimitInfo or not tbNewimitInfo then
		return
	end

	local tbSellLimitInfo = self:GetSellLimitInfo(pPlayer);
	local nLimitStartTime = GetTime();
	if tbSellLimitInfo[tbOldLimitInfo.nType] and tbSellLimitInfo[tbOldLimitInfo.nType][tbOldLimitInfo.nLevel]
		and tbSellLimitInfo[tbOldLimitInfo.nType][tbOldLimitInfo.nLevel][tbOldLimitInfo.nSubType] then

			nLimitStartTime = tbSellLimitInfo[tbOldLimitInfo.nType][tbOldLimitInfo.nLevel][tbOldLimitInfo.nSubType]
	end

	self:OnBuyItem_SellLimit(tbNewimitInfo.nType, tbNewimitInfo.nSubType, tbNewimitInfo.nLevel, nLimitStartTime, pPlayer);
end