...
function MarketStall:DoCheckSendWarningMail()
	...
	Mail:SendGlobalSystemMail({
		...
		Text = "Thiếu hiệp thân mến:\n Chúng tôi luôn kiểm soát các hành vi giao dịch bất thường trong trò chơi nhằm tránh tình trạng đầu cơ trục lợi. Hãy là người chơi chân chính để có thể duy trì game lâu dài.",
		From = "Võ Lâm Truyền Kỳ Mobile",
...
function MarketStall:CostGold_NewSellItem(nPlayerId, bSuccess, szMainType, nSubType, nCount, nPrice)
	...
	if not bCanNewSellItem then
		return false, "Thanh toán quá thời gian";
	end
	...
	if nAwardType == Player.award_type_item then
		...
		if nConsumeCount ~= nCount then
			...
			return false, "Khấu trừ vật phẩm thất bại";
		end
	elseif nAwardType == Player.award_type_money then
		...
		return false, "Không biết nguyên nhân";
	end
	...
	if nStallId <= 0 then
		...
		return false, "Gian hàng bất thường";
	end
...
function MarketStall:CostGold_UpdateMyItemPrice(nPlayerId, bSuccess, nStallId, nCurPrice)
	...
	if not tbStallInfo or pPlayer.dwID ~= tbStallInfo.nPlayerId then
		...
		return false, "Hết thời gian thanh toán";
	end
	...
	if tbStallInfo.nCount > 0 then
		...
		if pPlayer then
			...
			local tbMail = {
				...
				Title = "Trả lại hàng";
				From = "Hệ Thống";
				Text = "Các mặt hàng được trả lại từ kệ, xin vui lòng kiểm tra trong tập tin đính kèm!";
				tbAttach = tbAllAward;
...
function MarketStall:UpdateAllStall(pPlayer, bCostMoney, nLastUpdateTime)
	...
	if pPlayer.GetVipLevel()<self.nManualRefreshVipMin then
		pPlayer.CenterMsg(string.format("Cần đạt kiếm hiệp %d mới có thể làm mới", MarketStall.nManualRefreshVipMin))
		return
	end
	...
	if self.tbUpdateTimes[pPlayer.dwID] >= 100 then
		pPlayer.CenterMsg("Hôm nay đã dùng hết số lần làm mới hàng hóa");
		return;
	end
	...
	if not bRet then
		pPlayer.CenterMsg("Thanh toán không thành công, vui lòng thử lại sau");
		return;
	end
end
...
function MarketStall:BuyItem(pPlayer, szMainType, nSubType, nCount, nPrice)
	...
	local nType = self:GetItemType(szMainType, nSubType);
	if not nType then
		pPlayer.CenterMsg("Các mặt hàng được mua không tồn tại!");
		return;
	end
...
function MarketStall:GetCanBuyStallInfo(pPlayer, szMainType, nSubType, nPrice)
	local nType = self:GetItemType(szMainType, nSubType);
	if not nType then
		return false, "Mặt hàng mua không tồn tại";
	end
...
function MarketStall:CostGold_BuyItemOld(nPlayerId, bSuccess, szBillNo, szMainType, nSubType, nCount, nPrice)
	...
	for nSellerId, nSellCount in pairs(tbSellerInfo) do
		...
		local tbMail = {
			...
			Title = "Bày bán";
			From = "Hệ Thống";
			Text = string.format("Bán thành công: %s\n Đơn giá:%s Nguyên Bảo\nSố lượng：%s\nTổng cộng：%s Nguyên Bảo", szAwardName, nPrice, nSellCount, nPrice * nSellCount);
	...
	if nCurCount < nCount then
		if nCurCount > 0 then
			pPlayer.CenterMsg(string.format("Hàng có sẵn không đủ, thành công mua %s cái!", nCurCount), true);
		else
			pPlayer.CenterMsg("Hàng có sẵn không đủ, Nguyên bảo trả về!", true);
		end
