...
function FactionBattle:OnLocal8th(nPlayerId)
	...
	if not self.tbMonthPlayer[nPlayerId] then
		...
		local tbMail = {
			To = nPlayerId;
			Title = "Thi đấu môn phái Tháng",
			From = XT("Hệ thống"),
			nLogReazon = Env.LogWay_FactionBattle,
			Text = string.format("Chúc mừng bạn đã nhận được vé tham gia thi đấu môn phái Tháng\nTham gia lúc [FFFE0D]%s[-]", Lib:GetTimeStr3(FactionBattle:GetNextMonthlyBattleTime())),
		};
...
function FactionBattle:OnMonthly8th(nPlayerId)
	if not self.tbSeasonPlayer[nPlayerId] then
		...
		local tbMail = {
			To = nPlayerId;
			Title = "Thi đấu mốn phái Quý",
			From = XT("Hệ thống"),
			nLogReazon = Env.LogWay_FactionBattle,
			Text = string.format("Chúc mừng bạn đã nhận được vé tham gia thi đấu môn phái Quý\nTham gia lúc [FFFE0D]%s[-]", Lib:GetTimeStr3(FactionBattle:GetNextSeasonBattleTime())),
		};
...
function FactionBattle:OnCrossEliminationWinner(nPlayerId, nWinCount)
	...
	FactionBattle:SendEliminationWinnerNotify(pStayInfo, nWinCount, "Liên SV", self:GetCrossTypeName());
end
...
function FactionBattle:OnCrossAwardChampionStart(nPlayerId, nZoneServerId, szName,  nFaction, nLevel, nPortrait, nHonorLevel, nFightPower, szKinName)
	...
	KPlayer.SendWorldNotify(0, 1000,
			string.format(XT("Chúc mừng 「%s」nhận được Thi đấu môn phái %s-%s Tân Nhân Vương"),
	...
	for _,tbInfo in pairs(tb16thPlayer) do
		if tbInfo.dwOrgPlayerId and tbInfo.dwOrgPlayerId > 0 and nServerId == tbInfo.nZoneServerId then
			...
			if pStayInfo then
				...
				tbMail.Text = string.format("Chúc mừng bạn tại Thi đấu môn phái %s nhận được %d, hãy nhận thưởng!", self:GetCrossTypeName(),szDesc);
				Mail:SendSystemMail(tbMail)
...
function FactionBattle:OnCrossAwardWithout16th(tbSyncSortList, nTotal)
	local tbMail = {
		Title = string.format("Liên SV %s", FactionBattle.FINAL_AWARD_MAIL_TITLE),
		From = XT("Hệ thống"),
		nLogReazon = Env.LogWay_FactionBattle,
		tbAttach = {},
	};
	...
	for nRank,tbPlayerInfo in pairs(tbSyncSortList) do
		if tbPlayerInfo.dwOrgPlayerId and tbPlayerInfo.dwOrgPlayerId > 0 and tbPlayerInfo.nZoneServerId == nServerId then
			...
			if pStayInfo then
				...
				local tbFixAward = tbAwadrList[nAwardIdx]
				FactionBattle:FillMailAttach(tbMail.To, tbMail.tbAttach, tbFixAward);
				tbMail.Text = string.format("Chúc mừng trong thi đấu môn phái %s nhận được %s, hãy nhận thưởng !", self:GetCrossTypeName(), "Thưởng tham dự");
				Mail:SendSystemMail(tbMail)
...
function FactionBattle:OnCrossBoxAward(tbBoxAwardRecord)
	local tbMail = {
		Title = "Thi đấu môn phái Liên SV",
		From = XT("Hệ thống"),
		Text = string.format("Trong Thi đấu môn phái %s lần này bạn xuất sắc nhận được", self:GetCrossTypeName()),
		nLogReazon = Env.LogWay_FactionBattle,
		tbAttach = {},
	};
