...
function tbBaseFaction:OnMapEnter()
	...
	if self.nState == FactionBattle.SIGN_UP then
		if self.nPlayerCount >= FactionBattle.MAX_ATTEND_PLAYER then
			szMsg = string.format(XT("Số người báo danh đã đạt %d người, đạt mức cao nhất, hãy quay lại báo danh sau"), FactionBattle.MAX_ATTEND_PLAYER);
		elseif me.nLevel < FactionBattle.MIN_LEVEL then
			szMsg = XT("Không đủ ")..FactionBattle.MIN_LEVEL..XT(" cấp, không thể tiến nhập hoạt động.")
	...
	elseif self.nState == FactionBattle.FREE_PK or self.nState == FactionBattle.FREE_PK_REST then
		...
		if self.nPlayerCount < FactionBattle.MAX_ATTEND_PLAYER and 
			...
			szMsg = XT("Ngươi tự động báo danh, hãy đợi chút!");
		end
	elseif self.nState > FactionBattle.FREE_PK_REST then
		if not self.tbPlayer[me.dwID] then
			szMsg = XT("Thi Đấu Môn Phái đã bắt đầu diễn ra, không thể báo danh");
		end
	else
		return 0;
	end
	...
	if MODULE_ZONESERVER then
		me.CenterMsg(string.format("Ngươi đã vào Thi đấu môn phái Liên SV %s", FactionBattle:GetCrossTypeName()))
	end
end
...
function tbBaseFaction:_StartFreedomPK(nOpenCount)
		...
		for _,tbInfo in pairs(tbResult) do
			if tbInfo.pPlayer then
				...
				if not MODULE_ZONESERVER then
					...
					self:Send16thAward(nPlayerId, 0, XT("Thi Đấu Môn Phái lần này số người quá ít, không thể mở hoạt động, có phần quà đặc biệt được khuyến khích tặng cho các Đại Hiệp!"))
				...
				tbInfo.pPlayer.Msg(XT("Thi Đấu Môn Phái lần này số người quá ít, không thể mở hoạt động, có phần quà đặc biệt 8 Bảo Rương được khuyến khích tặng cho các Đại Hiệp!"))
			end
		end
		if MODULE_ZONESERVER then
	...
	elseif nTotal <= 16 and nOpenCount <= 1 then
		...
		self:BoardCenterMsgToMapPlayer(XT("Số người tham dự không đủ, trực tiếp lọt vào vòng 16!"));
	...
	self:BoardMsgToMapPlayer(string.format(XT("Mở vòng phân cấp giai đoạn %s!"), nOpenCount));
...
function tbBaseFaction:StartFreedomPK()
	...
	if nRet == 0 then
		self:BoardMsgToMapPlayer(string.format(XT("Môn phái số người tham gia không đủ %d người, không thể nào mở"), FactionBattle.MIN_ATTEND_PLAYER))
	end
	return nRet;
end
...
function tbBaseFaction:EndFreedomPKDelay()
	...
	for i, tbInfo in ipairs(self.tbSort) do
		...
		if nResultCount < 16 then
			...
			if pPlayer then
				pPlayer.CallClientScript("Ui:OpenWindow", "FactionBattleRankEffect", XT("Vòng 16"));
			end
	...
	self:BoardMsgToMapPlayer(XT("Top 16 đã xuất hiện, hãy xem bảng thi đấu"));
end
...
function tbBaseFaction:StartElimination()
	...
	for i = 1, math.ceil(#self.tbEliminationPlayer / 2) do
		...
		elseif pPlayer1 and pPlayer1.nMapId == self.nMapId then
			...
			pPlayer1.CenterMsg(XT("Đối thủ thiếu trận, ngươi trực tiếp được xét thắng cuộc!"))
		elseif pPlayer2 and pPlayer2.nMapId == self.nMapId then
			...
			pPlayer2.CenterMsg(XT("Đối thủ thiếu trận, ngươi trực tiếp được xét thắng cuộc!"))
		else
	...
	self:BoardMsgToMapPlayer(string.format(XT("Thi Đấu Môn Phái %s bắt đầu!"), FactionBattle.STATE_TRANS[self.nStateJour][2]));
end
...
function tbBaseFaction:CloseElimination()
	...
	self:BoardMsgToMapPlayer(string.format(XT("%s tuyển thủ sinh sản ，tường chuyện nhưng kiểm tra đối trận biểu"), self:GetNextStateName()));
end
...
function tbBaseFaction:AwardChampionStart(nWinnerId)
	...
	if pPlayer  then
		pPlayer.CallClientScript("Ui:OpenWindow", "FactionBattleRankEffect", XT("Quán Quân"));
		...
		if not MODULE_ZONESERVER then
			...
			KPlayer.SendWorldNotify(0, 1000,
					string.format(XT("[FFFF00]「%s」[-]đoạt[FF0066]「%s」[-]Tân Nhân Vương Thi Đấu Môn Phái", pPlayer.szName, Faction:GetName(self.nFaction))),
...
function tbBaseFaction:TestMail()
	...
	for nAwardIdx=20,100, 20 do
			...
			tbMail.Text = string.format(FactionBattle.FINAL_AWARD_MAIL_CONTENT, "Thưởng tham dự"--[[, nGetHonor, nCurHonor, nBoxCount, nLeftHonor]]);
...
function tbBaseFaction:EndChampionAward()
	self:BoardMsgToMapPlayer(XT("Thi Đấu Môn Phái lần này đã kết thúc"))
	self:SysMsgToMapPlayer(XT("Thi Đấu Môn Phái lần này đã kết thúc"))
