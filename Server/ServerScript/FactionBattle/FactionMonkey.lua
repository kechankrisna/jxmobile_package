...
function FactionMonkey:ArrangeResult(tbMonkey)
	...
	for nFaction,tbPlayers in pairs(tbMonkey) do
		if tbPlayers[1] and next(tbPlayers[1]) then
			...
			tbPlayerInfo.szName = pPlayerStay.szName or XT("Vô")
			...
			if tbTitleId then
				...
				if pStayInfo then
					...
					if nTitleId then
						...
						local szTitle = FactionBattle.tbMailSetting[nFaction].szTitle or "Bình chọn Môn Phái"
						local szMonkey = pStayInfo.nSex == Player.SEX_MALE and "Đại Sư Huynh" or "Đại Sư Tỷ"
						local szFactionName = Faction:GetName(nFaction) or ""
						local szText = FactionBattle.tbMailSetting[nFaction].szText and string.format(FactionBattle.tbMailSetting[nFaction].szText, (szFactionName ..szMonkey)) or "Thưởng Bình chọn môn phái"
						local tbMail = {
							To = nMonkeyId;
							Title = szTitle;
							From = "Hệ thống";
...
function FactionMonkey:OnSynData(pPlayer)
	...
	for index,tbPlayerInfo in pairs(tbData.tbMonkey) do
		...
		tbData.tbMonkey[index].szName = pPlayerStay.szName or XT("Vô")
...
function FactionMonkey:VoteMonkey(pPlayer,dwMonkeyID)
	...
	if not self:IsMonkeyStarting() or GetTime() > (tbMonkeyData.nStartTime + FactionBattle.MONKEY_VOTE_TIME * 60 * 60) then
		pPlayer.CenterMsg("Phiếu bình chọn đã quá hạn, lần sau hãy tham dự!")
	...
	if not tbMonkey then
		pPlayer.CenterMsg("Không tìm được người bình chọn")
	...
	pPlayer.CenterMsg("Bỏ phiếu thành công");
