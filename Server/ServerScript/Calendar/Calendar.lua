...
function Calendar:CheckData(pPlayer)
	...
	if #tbAllAward > 0 then
		local tbMail = {
            To = pPlayer.dwID,
            Title = "Vinh dự Võ lâm",
            From = "Độc Cô Kiếm",
            Text = "      Kính gửi đại hiệp,chúc mừng sự xuất sắc của bạn trong tháng này và trở thành một nhân vật nổi tiếng trong Võ lâm. Hãy tiếp tục rèn luyện chăm chỉ để vươn lên tầm vao mới nhé！",
            tbAttach = tbAllAward,
...
function Calendar:SendNotifyMsg(pPlayer, szKey, nDivision, _, nParam2)
    ...
    local szNotifyMsg    = string.format("Chúc mừng「%s」tham gia %s %s, vinh dự võ lâm tháng này tăng lên %s.", pPlayer.szName, szActName, szRank, self.tbDivisionName[nDivision])
