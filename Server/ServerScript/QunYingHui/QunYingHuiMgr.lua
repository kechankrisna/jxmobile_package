...
function QunYingHui:CheckPlayeGame(pPlayer, bCheckMap)
    if not self.bStartGame then
        return false, "Quần Anh Hội chưa mở";
    end
    ...
    if nDegreeCount <= 0 then
        return false, "Số lần thi đấu không đủ";
    end

    local bRet = Map:CheckEnterOtherMap(pPlayer);
    if not bRet and bCheckMap then
        return false, "Khu vực không an toàn không thể tham gia";
    end

    if pPlayer.nLevel < tbDef.nMinLevel then
        return false, "Không đủ cấp"..tbDef.nMinLevel;
    end    
...
function QunYingHui:StartGame()
    ...
    KPlayer.SendWorldNotify(1, 999, "Quần Anh Hội bắt đầu báo danh!", 1, 1);
...
function QunYingHui:UpdatePlayerUiShow(pPlayer)
    if self.nTotalMatchCount == 0 then
        ...
        tbShowInfo.szShow = string.format("Đang đợi lượt ghép thứ %s", tbDef.nTotalMatchCount - self.nTotalMatchCount + 1);
...
function QunYingHui:EndGame()
    ...
    KPlayer.SendWorldNotify(1, 999, "Quần Anh Hội đã kết thúc!", 1, 1);
