KinEscort.szKinEscortOpenMessage 		= "Hoạt động Vận Tiêu Bang Hội mở, Các thành viên Bang Hội tập chung tại Phế Tích Dạ Lang áp tiêu";    --家族运镖活动开启系统通告
KinEscort.szKinEscortCloseMessage 		= "Hoạt động Vận Tiêu Bang Hội kết thúc";    --家族运镖活动关闭系统通告

KinEscort.szNpcTips = "Một đống lớn hàng hóa muốn chuyên chở ra ngoài, thật sự là kiện đau đầu sự tình a......";               --劫镖NPC提示语
KinEscort.szTitle   = "Bang %s hộ tống";                                              --镖车称号

KinEscort.tbKinChannelMsg = {                                                     --家族频道公告
	["szStart"] = "Bang Hội Tiêu Xa đã di chuyển! Tiêu xa lần này là %s, thành viên Bang Hội mau đến hộ tống %s",        --镖车启动时，%s:颜色，%s坐标链接
	["szComplete"] = "Nhiệm Vụ Vận Tiêu Bang Hội hoàn thành!",                                        --镖车完成时
	["szUnderAttack"] = "Bang Hội Tiêu Xa đang bị tấn công, hãy mau đến cứu viện! %s",                 --镖车收到攻击时，%s坐标链接
	["szCrossHPStage"] = "Bang Hội Tiêu Xa lượng máu thấp %d%%, rơi xuống vật phẩm!",                      --镖车打到百分比时，%d：血量百分比
	["szUpgrade"] = "%s dùng %d điểm Kiến Thiết Bang Hội nâng Bang Hội Tiêu Xa thành công! Bang Hội Tiêu Xa hiện tại là %s",      --镖车升阶时，%s：玩家名字，%s：品质，即颜色
	["szDeath"] = "Bang Hội Tiêu Xa bị %s đánh bại, Vận Tiêu Bang Hội lần này thất bại!",                         --%s:killer
}
...
KinEscort.tbNpcColor = {"Trắng","Lục","Lam","Tím","Cam"};            --对应镖车升级到品质1~5的NpcId所需建设资金
...
KinEscort.szDropRewardWrodMsg = "Xe Tiêu bị cướp, rớt xuống toàn bộ vật phẩm";                       --劫镖过程掉落物品世界通告
...
function KinEscort:CheckAccept(tbKin)
	if not self:IsLevelEnough(me) then
		return false, string.format("%d cấp mới có thể tham gia", self.nMinAttendLevel)
	end
	...
	if tbKin:GetLastKinEscortDate() == Lib:GetLocalDay() then
		return false, "Nhận Tiêu thất bại, Bang Hội của ngươi hôm nay đã nhận tiêu rồi";
	end

	if self.tbKinEscortStatus[tbKin.nKinId].nCarriageNpcId ~= -1 then
		return false, "Nhận Tiêu thất bại, Bang Hội Tiêu Xa đã tồn tại";
	end

	if self:GetAcceptCostFound() > tbKin:GetFound() then
		return false, "Điểm Kiến Thiết Bang Hội không đủ";
	end
...
function KinEscort:GiveReward(pCarriageNpc, nRechIndex, tbArrivalReward, bArrive)
	...
	for nId in pairs(pCarriageNpc.tbJoinPlayer) do
		if kinMembers[nId] then
			...
			Mail:SendSystemMail({
				To       = nId,
				Title    = "Thưởng Vận Tiêu Bang Hội",
				Text     = bArrive and "Vận Tiêu Bang Hội hoàn thành, đây là phần thưởng khích lệ" or "Vận Tiêu Bang Hội thất bại, hãy cố lên",
...
function KinEscort:CheckUpgrade(nPlayerId, nCarriageNpcId, nFoundCost)
	...
	if nFoundCost > tbKin:GetFound() then
		pPlayer.CenterMsg("Điểm Kiến Thiết Bang Hội không đủ")
		return false
	end

	local tbKinState = self:GetKinEscortData(pCarriageNpc.dwKinId);
	if tbKinState.nState~=self.States.prepare then
		pPlayer.CenterMsg("Tiêu Xa không ở trạng chờ xuất phát, nâng cấp thất bại!!")
		return false
	end
...
function KinEscort:OnUpgrade(nPlayerId, nKinId, bConfirm)
	...
	if not bConfirm  then
		pPlayer.MsgBox(string.format("Tiêu %d điểm Kiến Thiết Bang Hội để tiến hành nâng Tiêu Xa?", nFoundCost),
					   {{"Xác nhận", self.OnUpgrade, self, nPlayerId, nKinId, true}, {"Hủy"}});
		return ;
	end
