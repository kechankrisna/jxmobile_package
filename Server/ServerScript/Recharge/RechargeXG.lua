...
function Recharge:DoRequest(pPlayer, szProductId)
	...
	if nLastRechargeRequestTime + self.RequestCDTime > nNow then
		pPlayer.CenterMsg(string.format("Vui lòng đợi %s trước khi thử lại", Lib:TimeDesc(nLastRechargeRequestTime + self.RequestCDTime - nNow)))
		return
	end
	...
	if nTradeTimeOut then
		pPlayer.CenterMsg("Hiện đang ở trạng thái giá trị được lưu trữ, vui lòng đợi")
		return
	end

	if not IsGoServerConnected() then
		pPlayer.CenterMsg("Hệ thống giá trị được lưu trữ là bất thường, vui lòng liên hệ với GM")
		return
	end
	...
	local szUrl = string.format("http://192.168.200.1/api/checkonline.aspx?OnlineCount=%s&ServerId=%d", GetOnlinePlayerCount(), nServerId)
	pPlayer.CallClientScript("Sdk:DoHttpRequest", szUrl)
	
	local md5str = KLib.GetStringMd5(string.format("%s",Sdk.JackKey))
	...
	local order = string.format("%d%.2d%.2d%.2d%.2d%.2d", tbTime.year, tbTime.month, tbTime.day, tbTime.hour, tbTime.min,tbTime.sec)
	local tbParam = {
		type = "verify-order";
		serverId = nServerId;
		roleId = dwRoleId;
		roleName = pPlayer.szName;
		roleLevel = pPlayer.nLevel;
		roleVipLevel = pPlayer.GetVipLevel();
		currencyName = tbBuyInfo.szMoneyType;
		productId = szProductId;
		productDesc = tbBuyInfo.szDesc;
		productUnitPrice = tbBuyInfo.nMoney * 2000;
		totalAmount = tbBuyInfo.nMoney * 100;
		sign = md5str;
		tradeNo = szTradeNo;
		paidTime = timenow;
	...
	local fnUse = function ()
		Sdk:JxPayNapTheRequest(pPlayer,tbParamEncode)
	end
	pPlayer.MsgBox(string.format("Dùng [FFFE0D]%s[-] mua [FFFE0D]%s[-]?", tbBuyInfo.nMoney * 100, tbBuyInfo.szDesc),
	{
		{"Xác định", fnUse},
		{"Hủy bỏ"},
	});
