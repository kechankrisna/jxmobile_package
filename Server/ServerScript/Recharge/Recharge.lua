	...
	if nTotalExt > 0 then
		...
		if  pPlayer.GetUserValue(nGroup2, nSaveKey2) ~= nToday then
			...
			if nWeekExt then
				...
				pPlayer.CenterMsg(string.format("Nguyên bảo đại lễ hoạt động ngoài định mức nhận lấy %s%s%s", MonyszName, szMoneyEmotion, nGiveGold) , true)
			end
			if nActExt then
				local nGiveGold = math.floor(tbAward[1][2] * nActExt / 100)
				pPlayer.AddMoney("Gold", nGiveGold , Env.LogWay_ActivyRechargeCard)
				pPlayer.CenterMsg(string.format("%s Cuồng hoan hoạt động ngoài định mức nhận lấy %s%s%s", self.szCardActName, MonyszName, szMoneyEmotion, nGiveGold) , true)
			end
...
function Recharge:BuyVipAward(pPlayer, nBuyLevel)
	...
	if nCurLevel < nBuyLevel then
		pPlayer.CenterMsg("【 Kiếm hiệp V】 Đặc quyền đẳng cấp không đủ")
		return
	end
	...
	if nBuydeBit == 1 then
		pPlayer.CenterMsg("Ngài đã mua qua nên gói quà")
		return
	end
	...
	pPlayer.CostGold(tbInfo.nRealPrice, Env.LogWay_BuyVipAward, nil, function (nPlayerId, bSucceed)
		...
		local nBuydeBit = KLib.GetBit(nBuyedVal, nBuyLevel + 1)
		if nBuydeBit == 1 then
			return false, "Số liệu quá thời gian"
		end
...
function Recharge:TakeGrowInvestAward(pPlayer, nIndex, nGroupIndex)
	if not self.IS_OPEN_INVEST then
		pPlayer.CenterMsg("Rất xin lỗi một vốn bốn lời công năng tạm thời đóng lại")
		return
	end
	...
	if nTakeeBit ~= 0 then
		pPlayer.CenterMsg("Ngài đã nhận lấy qua")
		return
	end
	...
	if tbInfo.nDay then
		...
		if nShowTakeDay ~= nIndex then
			pPlayer.CenterMsg("Sai lầm nhận lấy số trời")
			return
		end
		...
		if pPlayer.GetUserValue(self.SAVE_GROUP, nSaveKey) == nDay then
			pPlayer.CenterMsg("Ngài hôm nay đã nhận lấy qua")
			return
		end
...
function Recharge:CheckVipLevelChange(pPlayer, nRechareGold, nRechareCard)
	...
	if Server:IsInterConnectServer() and pPlayer.nPlatId == 0 then
		...
		if nGetGold > 0 then
			Mail:SendSystemMail({
				To = pPlayer.dwID;
				Title = "Trữ giá trị chiết khấu sai biệt đền bù";
				Text = string.format("Vì bảo đảm [FFFE0D] Liên hệ phục [-] Bên trong song bình đài trữ giá trị chiết khấu nhất trí, sẽ lấy Android bình đài làm cơ chuẩn đối trữ giá trị chiết khấu sai biệt tiến hành đền bù, riêng thiếu hiệp lần này trữ giá trị đền bù chênh lệch giá trị [FFFE0D]%d Nguyên bảo [-], mời thiếu hiệp tại phụ kiện nhận lấy. Cám ơn ngài ủng hộ cùng lý giải!", nGetGold);
		...
		if not bRet then --直接按价格返回对应元宝数 能买失败的也就一个了
			...
			if pPlayer.dwKinId ~= 0 then
				...
				if tbItemBase then
 	               ...
 	               ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Bang phái thành viên「%s」Xuất thủ xa xỉ, mua [%s]<%s>[-]!", pPlayer.szName, TxtColor, tbItemBase.szName), pPlayer.dwKinId, 
...
function Recharge:OnBuyBackGiftCallBack(pPlayer, nEndTime, nGroupIndex)
	...
	pPlayer.CenterMsg("Còn thừa số lần không đủ, đã chuyển đổi thành phổ thông trữ giá trị", true)
	self:AddTotalCardRecharge(pPlayer, tbBuyInfo.nMoney*nBuyCount)
