...
function tbAct:GetSendList(pPlayer)
	...
	if pPlayer.nLastRequestSendListTime and pPlayer.nLastRequestSendListTime + 60 >= nNow then
		pPlayer.CenterMsg("Xin sau lại nếm thử đổi mới")
		return
	end
...
function tbAct:CheckAddToSendList(dwRoleId, pPlayerMe)
	...
	if not pPlayer then
		return false, "Người chơi không tuyến bên trên không thể đưa tặng"
	end
	local tbItems = pPlayer.FindItemInBag(tbItem.nActStartSendItem) 
	if #tbItems ~= 1 then
		return false, "Người chơi không có hoạt động đạo cụ"
	end
	...
	if tbData.nGetSendCount >= tbAct.MAX_GETSEND_COUNT then
		return false, "Đã đạt hôm nay bị đưa tặng hạn mức cao nhất"
	end
...
function tbAct:CheckAddToRobList(dwRoleId, pPlayerMe, bGetCanRobItems)
	...
	if not tbCurInfo or not next(tbCurInfo) then
		return false, "Nên người chơi đã mất mảnh vỡ"
	end
	local pPlayer = KPlayer.GetPlayerObjById(dwRoleId)
	if pPlayer then
		return false, "Người chơi tuyến bên trên không thể đoạt"
	end
	...
	if nRobedcount and  nRobedcount >= self.MAX_ROBED_COUNT then
		return false, "Nên người chơi đã đạt hôm nay bị cướp đoạt hạn mức cao nhất"
	end
	...
	if dwMyKinId ~= 0 then
		...
		if dwKinId == dwMyKinId then
			return false, "Cùng bang phái không thể cướp đoạt"
		end
	end
...
function tbAct:SendHim(pPlayer, dwSendRoleId, nDerbisId)
	...
	if not FriendShip:IsFriend(dwMyRoleId, dwSendRoleId) then
		local dwKinId = pPlayer.dwKinId
		if dwKinId == 0 then
			pPlayer.CenterMsg("Không phải hảo hữu không giống bang phái ở giữa không thể đưa tặng")	
			return
	...
	if not tbDebrisData or not tbDebrisData[nDerbisId] or tbDebrisData[nDerbisId] < 1 then
		pPlayer.CenterMsg("Ngài không có đối ứng mảnh vỡ")
		return
	end
	...
	if nLastSendTime + self.SEND_CD > nNow then
		pPlayer.CenterMsg("Đưa tặng làm lạnh bên trong")
		return
	end
	...
	if tbMyData.nSendCount >= self.MAX_SEND_COUNT then
		pPlayer.CenterMsg("Hôm nay đã mất đưa tặng số lần")
		return
	end
	...
	pPlayer.CenterMsg(string.format("Đưa tặng thành công! Hôm nay đã đưa tặng %d/%d", tbMyData.nSendCount, self.MAX_SEND_COUNT))
end

function tbAct:Robhim(pPlayer, dwRobRoleId)
	local dwMyRoleId = pPlayer.dwID
	if FriendShip:IsFriend(dwMyRoleId, dwRobRoleId) then
		pPlayer.CenterMsg("Hảo hữu ở giữa không thể cướp đoạt")
		return 
	end
	--自己抢夺次数和抢夺cd
	...
	if tbData.nLastRobTime then
		if tbData.nLastRobTime + self.ROB_CD > nNow then
			pPlayer.CenterMsg("Cướp đoạt làm lạnh bên trong")
			return
		end
	...
	if tbData.nRobCount then
		if tbData.nRobCount >= self.MAX_ROB_COUNT then
			pPlayer.CenterMsg("Hôm nay đã mất cướp đoạt số lần")
			return
		end
	end
	...
	self:RequestMyInfo(pPlayer)
	pPlayer.CenterMsg(string.format("Cướp đoạt thành công! Hôm nay đã cướp đoạt %d/%d", tbData.nRobCount, self.MAX_ROB_COUNT))
end
...
function tbAct:CombieClueItem(pPlayer)
	local tbItems = pPlayer.FindItemInBag(tbItem.nActStartSendItem) 
	if #tbItems ~= 1 then
		pPlayer.CenterMsg("Không hoạt động đạo cụ")
		return
	end
	local pItem = tbItems[1];
	...
	if not tbItem:CanCombieDebris(tbItemData) then
		pPlayer.CenterMsg("Tàn quyển không đủ")
		return
	end
	...
	local szNotifyMsg = string.format("Trải qua một đoạn thời gian không ngừng tìm kiếm,「%s」Cuối cùng tập hợp đủ tất cả phân quyển cũng thành công hợp thành [ff8f06]【Thần Châu đại địa bảo quyển】[-]! #49", pPlayer.szName)
...
function tbAct:CombieClueDerbis(pPlayer, nTemplateId)
	...
	if not nTarItemId then
		pPlayer.CenterMsg("Vô hiệu đạo cụ id")
		return
	end
	...
	pPlayer.CenterMsg("Hợp thành thành công")
