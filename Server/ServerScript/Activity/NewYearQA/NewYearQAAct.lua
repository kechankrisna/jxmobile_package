...
function tbAct:OnTrigger(szTrigger)
	...
	if szTrigger == "Init" then
		...
		local tbMail = {Title = self.szMailTitle, From = "Hệ thống", LevelLimit = self.nRequireLv, nRecyleTime = nEndTime - GetTime(), nLogReazon = Env.LogWay_NewYearQAAct, Text = self.szMailContent}
...
function tbAct:OnLevelUp(pPlayer, nNewLevel)
	if nNewLevel == self.nRequireLv then
		local _, nEndTime = self:GetOpenTimeInfo()
		local tbMail = {Title = self.szMailTitle, From = "Hệ thống", To = pPlayer.dwID, nRecyleTime = nEndTime - GetTime(), nLogReazon = Env.LogWay_NewYearQAAct, Text = self.szMailContent}
		tbMail.tbAttach = {{"Item", self.nActEnterItem, 1, nEndTime}}
...
function tbAct:TrySetQuestion(pPlayer, nRound, tbQuestion, bCostGold)
	if bCostGold and pPlayer.GetMoney("Gold") < self.nSetQuestionGold then
		pPlayer.CenterMsg(string.format("Nguyên bảo không đủ %s", self.nSetQuestionGold))
		return
	end
...
function tbAct:FreeRefreshQuestion(pPlayer)
	...
	if tbData.tbAnswerInfo.nDayRefreshTimes and tbData.tbAnswerInfo.nDayRefreshTimes >= self.nDayRefreshTimes then
		pPlayer.CenterMsg(string.format("Mỗi ngày chỉ có thể đổi mới %d Lần", self.nDayRefreshTimes))
		return true
	end
	if tbData.tbAnswerInfo.nDataDay ~= Lib:GetLocalDay(GetTime() - self.nNewDayTime) then
		...
		pPlayer.CallClientScript("Activity.NewYearQAAct:BeginAnswer", tbData.tbAnswerInfo)
		pPlayer.CenterMsg("Mỗi ngày miễn phí đề mục đã đổi mới")
...
function tbAct:TryRefreshQuestion(pPlayer)
	...
	if pPlayer.GetMoney("Gold") < self.nRefreshQuestionGold then
		pPlayer.CenterMsg(string.format("Nguyên bảo không đủ %s", self.nRefreshQuestionGold))
		return
	end
	...
	local fnCostCallback = function (nPlayerId, bSuccess)
		...
		if self:FreeRefreshQuestion(pPlayer) then
			return false, "Đề mục đã tự hành đổi mới"
		end

		self:CheckAnswerData(pPlayer.dwID, nil, true)
		local tbData = self:GetPlayerData(pPlayer.dwID)
		pPlayer.CallClientScript("Activity.NewYearQAAct:BeginAnswer", tbData.tbAnswerInfo)
		pPlayer.CenterMsg("Đổi mới thành công")
		return true
	end
...
function tbAct:GetPlayerName(nPlayerId)
	local szPlayerName = "Người thần bí"
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
	if pPlayer then
...
function tbAct:TryAnswer(pPlayer, nQIdx, nAnswer)
	...
	pPlayer.CenterMsg(bRight and "Trả lời chính xác" or "Trả lời sai lầm")
		...
		if bRight then
			...
			if bRet then
				...
				if self.bWolrdNotify then
					...
					KPlayer.SendWorldNotify(1, 999, string.format("[eebb01]%s[-] Đáp đúng [eebb01]%s[-] Một đầu cao cấp vấn đề, quan hệ của hai người thật đúng là thân mật vô gian đâu!", szPlayerName, szFriendName), 1, 1)
				else
					local pFriend = KPlayer.GetPlayerObjById(nFriendId)
					if pFriend then
						pFriend.CenterMsg(string.format("[eebb01]%s[-] Đáp đúng ngươi cao cấp vấn đề, hai ngươi quan hệ thật sự là thân mật vô gian", szPlayerName))
					end
				end
				local tbMail =
				{
					...
					Text = string.format("[eebb01]%s[-] Đáp đúng ngài đi hỏi đề, đại hiệp thành công thu hoạch được 1000 Cống hiến", szPlayerName),
...
function tbAct:OnClientCall(pPlayer, szFunc, ...)
	...
	if pPlayer.nLevel < self.nRequireLv then
		pPlayer.CenterMsg("Cấp bậc chưa đủ, không cách nào tham gia nên hoạt động")
		return
	end
