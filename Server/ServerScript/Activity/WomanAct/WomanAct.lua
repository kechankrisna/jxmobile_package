...
function tbAct:OnTrySendLabel(pPlayer, tbParam)
	...
	if not pAcceptPlayer then
		pPlayer.CenterMsg(string.format("Cần [FFFE0D] Song phương đồng đều tuyến bên trên [-] Mới có thể tăng thêm %s A", self.szActDes), true)
		return
	end
...
function tbAct:CheckData(pPlayer, pAcceptPlayer, nType, szLabel)
	...
	if nType == self.FreeLabel then
		if tbFreeSender[pPlayer.dwID] then
			return false, string.format("Ngài đã đối mục tiêu hiệp sĩ tăng thêm qua nên %s, mời lựa chọn cái khác %s", self.szActDes, self.szActDes)
		end

		if not self.bTSMode and nHimSex ~= Gift.Sex.Girl then
			return false, string.format("Sản xuất hàng loạt ấn tượng ký chỉ có thể là nữ tính hảo hữu tăng thêm %s", self.szActDes)
		end

		if not bExistLabel and nLabelCount >= self.nMaxLabel then
			return false, string.format("Đối phương %s Đã đủ, mời mặt khác chọn lựa một cái hiệp sĩ đi", self.szActDes)
		end
	elseif nType == self.PayLabel then
		...
		if tbPayLabelPlayer[szLabel] and tbPayLabelPlayer[szLabel][pPlayer.dwID] then
			return false, string.format("Không thể nhiều lần tăng thêm cùng一%s", self.szActDes)
		end
	end
...
function tbAct:TrySendLabel(nPlayerId, bSuccess, szBillNo, nAcceptId, nType, szLabel)
	...
	local pAcceptPlayer =  KPlayer.GetPlayerObjById(nAcceptId);
	if not pAcceptPlayer then
		return false, "Khấu trừ phí thủ tục nửa đường, đối phương rơi dây!";
	end
	...
	if not nMySex or not nHimSex then
		pPlayer.CenterMsg("Không biết giới tính", true)
		return false, "Không biết giới tính"
	end
	...
	if nType == self.FreeLabel then
		...
		if nConsume < self.nNeedConsumeImpressionLabel then
			szMsg = string.format("Khấu trừ đạo cụ %s Thất bại!",KItem.GetItemShowInfo(self.nImpressionLabelItemID, pPlayer.nFaction, pPlayer.nSex) or "-")
			pPlayer.CenterMsg(szMsg, true)
			return false, szMsg
		end
	end
	...
	local szMsg = string.format("「%s」Đối「%s」Tăng thêm mới %s「%s」", pPlayer.szName or "-", pAcceptPlayer.szName or "-", self.szActDes, szLabel)
	...
	pPlayer.CenterMsg(string.format("Đã thành công đối「%s」Tăng thêm %s", pAcceptPlayer.szName or "-", self.szActDes), true)
	pAcceptPlayer.CenterMsg(string.format("「%s」Đối ngươi tăng thêm mới %s「%s」", pPlayer.szName or "-", self.szActDes, szLabel), true)
		...
		if next(tbLabelInfo) then
			...
			for szLabel, tbInfo in pairs(tbPayLabelPlayerTemp) do
				...
				for nPlayerId, nTime in pairs(tbInfo) do
					...
					tbTemp.szName = pPlayerStay.szName or "Không biết"
					tbTemp.nTime = nTime
