...
function tbAct:OnTrigger(szTrigger)
    if szTrigger == "Start" then
        ...
        Activity:RegisterNpcDialog(self, 99,  {Text = "Ngày mồng một tháng năm giang hồ động linh cơ", Callback = self.OnNpcDialog, Param = {self}})
        Activity:RegisterNpcDialog(self, "WeekendQuestionNpc",  {Text = "Ngày mồng một tháng năm giang hồ động linh cơ", Callback = self.OnQuestionDialog, Param = {self}})
...
function tbAct:CheckTeamMember(pP)
    ...
    if pTarget.dwTeamID == 0 then
        return false, "Mời tạo thành hai người đội ngũ lại đến"
    end

    local tbTeamMember = TeamMgr:GetMembers(pTarget.dwTeamID)
    if #tbTeamMember ~= 2 then
        return false, "Đội ngũ nhân số nhất định phải vì hai người"
    end
    ...
    for nIdx, nPlayerId in ipairs(tbTeamMember) do
        ...
        if not pPlayer then
            return false, "Không tìm được người chơi"
        end

        if pPlayer.nLevel < self.JOIN_LEVEL then
            return false, string.format("「%s」Cấp bậc chưa đủ %d Cấp", pPlayer.szName, self.JOIN_LEVEL)
        end
...
function tbAct:CheckMemberData(pPlayer)
    ...
    if pPlayer.GetUserValue(self.DATA_GROUP, self.ANSWERD) >= self.MAX_COUNT then
        return false, "Hôm nay đề mục đã đáp xong"
    end
    if pPlayer.GetUserValue(self.DATA_GROUP, self.AWARD_FLAG) ~= 0 then
        return false, "Đã hoàn thành hôm nay bài thi"
    end
    return true
end
...
function tbAct:CheckDistance()
    ...
    if nMapId1 ~= nMapId2 then
        return false, string.format("Đồng đội %s Không tại bản địa đồ",szName)
    end

    local fDists = Lib:GetDistsSquare(nX1, nY1, nX2, nY2)
    if fDists > (self.MIN_DISTANCE * self.MIN_DISTANCE) then
        return false, string.format("Đồng đội %s Không tại phụ cận",szName)
    end
...
function tbAct:GetTips(pP,bNoTip)
    ...
    local szTip = "Hạ cái ra đề mục người manh mối: "
    ...
    if nScore == 2 and tbInfo then
        ...
        local szLocaltion = string.format("%s, nằm ở <%s(%d,%d)>",szNpcName, tbMapSetting.MapName, tbInfo.nX*Map.nShowPosScale, tbInfo.nY*Map.nShowPosScale);
...
function tbAct:TryBeginQuestion()
    ...
    if me.nSex == Gift.Sex.Boy then
        if tbBoyData.bAnswered or (tbBoyData.nBeginTime > 0 and (GetTime() - tbBoyData.nBeginTime) >= self.TIME_OUT) then
            me.CenterMsg("Ngươi đã hoàn thành cái này vòng bài thi")
            return
        end
        ...
        if not tbBoyData.bAnswered then
            if tbBoyData.nBeginTime > 0 and GetTime() - tbBoyData.nBeginTime >= self.TIME_OUT then
                ...
                me.CenterMsg("Xin chờ đợi đồng đội hoàn thành bài thi")
                return
            end
        end
        if tbGirlData.bAnswered then
            me.CenterMsg("Đã hoàn thành trả lời, xin bắt đầu vòng tiếp theo bài thi")
            return
        end
...
function tbAct:TryAnswer(pPlayer,bRight)
    ...
    if pPlayer.nSex == Gift.Sex.Boy then
        if tbBoyData.bAnswered then
            pPlayer.CenterMsg("Ngươi đã hoàn thành cái này vòng bài thi")
            return
        end
        if tbBoyData.nBeginTime == 0 then
            pPlayer.CenterMsg("Mời bắt đầu trước bài thi")
            return
        end
        ...
        if bRight then
            ChatMgr:SendTeamOrSysMsg(pPlayer, string.format("%s Đáp đúng đề mục, thật sự là cơ trí hơn người!",pPlayer.szName))
        end
        ...
        for _, nPlayerId in ipairs(tbData.tbPlayerId) do
            if nPlayerId ~= pPlayer.dwID then
                ...
                pGirl.CenterMsg("Đồng đội đã trả lời xong tất, đến phiên ngươi tiến hành bài thi")
                bRet,szMsg = self:CheckDistance()
                if not bRet then
        ...
        if not tbBoyData.bAnswered then
            if tbBoyData.nBeginTime > 0 and GetTime() - tbBoyData.nBeginTime >= self.TIME_OUT then
                ...
                pPlayer.CenterMsg("Xin chờ đợi đồng đội hoàn thành bài thi")
                return
            end
        end
        if tbGirlData.bAnswered then
            pPlayer.CenterMsg("Đã hoàn thành trả lời, xin bắt đầu vòng tiếp theo bài thi")
            return
        end
...
function tbAct:BeginNextQuestion()
    ...
    for i = 1, 2 do
        ...
        if nAnswered >= self.MAX_COUNT then
            pPlayer.CenterMsg("Bản nhật bài thi đã hoàn thành")
        end
    end
