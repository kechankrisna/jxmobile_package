...
tbAct.tbTrigger = { 
                    ...
                    LotteryBig = {},
                    End = {},
                    SendWorldNotify = { {"WorldMsg", "Vui vẻ tháng bảy, may mắn rút thưởng hoạt động cường lực đột kích, sinh động độ đạt [FFFE0D]60[-] Sau này đi tìm [FFFE0D] Tương Dương Nạp Lan thật [-] Nhận lấy vé xổ số tham gia hoạt động đi!", 1} },
                  }
...
tbAct.tbTypeData = 
{
	[tbAct.TYPE.BIG] = {
		...
		szNewInfomationTitle = "Nguyên Tiêu lớn rút thưởng kết quả";                                        -- 最新消息标题
        ...
        szAvMailText = "Chúc mừng ngươi tham gia「Tháng bảy lớn rút thưởng」Hoạt động, thu hoạch được [FFFE0D] May mắn thưởng [-], mời kiểm tra và nhận!";                   -- 纪念奖邮件内容
        szRankMailText = "Chúc mừng ngươi tham gia「Tháng bảy lớn rút thưởng」Hoạt động, thu hoạch được [FFFE0D]%s[-], phụ kiện vì ban thưởng, mời kiểm tra và nhận!";       -- 排名奖励邮件内容,%s为奖励排名
        szMailTitle = "Tháng bảy lớn rút thưởng";                                                                                   -- 邮件标题
	...
	[tbAct.TYPE.DAY] = {
		...
		szNewInfomationTitle = "Tháng bảy rút thưởng kết quả";
        --szWorldNotifyHit = "今日“迎国庆幸运抽奖”活动结果已产生，请前往“最新消息”相关页面查看中奖名单。";              
        nDefaultShowIdx = 3;
        szAvMailText = "Chúc mừng ngươi tham gia「Tháng bảy rút thưởng」Hoạt động, thu hoạch được [FFFE0D] May mắn thưởng [-], mời kiểm tra và nhận! Không nên quên tại [FFFE0D]7 Nguyệt 31 Nhật [-] Còn có tháng bảy thưởng lớn chờ ngươi, ban thưởng càng phong phú, phải nhớ được đến a!";
        szRankMailText = "Chúc mừng ngươi tham gia「Tháng bảy rút thưởng」Hoạt động, thu hoạch được [FFFE0D]%s[-], phụ kiện vì ban thưởng, mời kiểm tra và nhận! Không nên quên tại [FFFE0D]7 Nguyệt 31 Nhật [-] Còn có tháng bảy thưởng lớn chờ ngươi, ban thưởng càng phong phú, phải nhớ được đến a!";
        szMailTitle = "Tháng bảy rút thưởng";
...
function tbAct:OnUseNewYearJiangQuan(pPlayer)
    ...
    if nHave < 1 then
        pPlayer.CenterMsg("Không có vé xổ số",true)
        return
    end
    local nStartTime = Activity:GetActBeginTime(self.szKeyName)
    ...
    if not szLotteryDate or nPassDay > nLastExeDay then
        pPlayer.CenterMsg("Đã không có rút thưởng hoạt động",true)
        return
    end
    ...
    if nNewYearUseDay ~= nLastExeDay then
        ...
        local szTip = "Mở thưởng thời gian " .. string.format("[FFFE0D]" ..szLotteryDate .."[-]") ..", đừng quên [FFFE0D]7 Nguyệt 31 Nhật [-] Còn có tháng bảy thưởng lớn chờ ngươi a!"
       ...
       pPlayer.CenterMsg("Đã có rút thưởng tư cách", true)
    end
end
...
function tbAct:GetChouJiang()
    ...
    me.CenterMsg(string.format("Nhận lấy %s Thành công",pItem.szName))
...
function tbAct:SendWorldTip(dwID, nRank, nType)
    ...
    if szName then
        local szType = (nType and nType == tbAct.TYPE.DAY) and "Tháng bảy rút thưởng" or "Tháng bảy lớn rút thưởng"
        local szRank = ChouJiang:GetRankDes(nRank)
        local szMsg = string.format("Chúc mừng người chơi %s Tại %s Trong hoạt động thu được %s", szName, szType, szRank)
...
function tbAct:SendKinTip(dwID, nRank, nType)
    ...
    if szName and nKinId and nKinId ~= 0 then
        local szType = (nType and nType == tbAct.TYPE.DAY) and "Tháng bảy rút thưởng" or "Tháng bảy lớn rút thưởng"
        local szRank = ChouJiang:GetRankDes(nRank)
        local szMsg = string.format("Chúc mừng bang phái thành viên %s Tại %s Trong hoạt động thu được %s", szName, szType, szRank)
...
function tbAct:Lottery(nType)
    ...
    local szTitle = tbTypeData.szMailTitle or "Rút thưởng"
...
-- 》》   开始抽排名奖励
    for nRank, tbInfo in ipairs(tbAwardData) do
        ...
        local szRank = ChouJiang:GetRankDes(nRank)
        local szRankText = string.format(tbTypeData.szRankMailText or "Chúc mừng ngươi tham gia rút thưởng hoạt động, thu hoạch được [FFFE0D]%s[-], phụ kiện vì ban thưởng mời kiểm tra và nhận!", szRank)
...
function tbAct:SendAVAward(tbHouXuan, tbTypeData, nLogWay, szTitle)
    ...
    if next(tbHouXuan.tbPlayer) and next(tbAVAward) then
        ...
        local szAVText = tbTypeData.szAvMailText or "Rút thưởng hoạt động may mắn thưởng"
        -- 先随大号
