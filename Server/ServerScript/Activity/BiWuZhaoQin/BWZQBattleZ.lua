...
function BWZQBattle:PlayerAvatar()
	local function fnAvatar(self,pPlayer)
	    ...
	    if not Player:ChangePlayer2Avatar(pPlayer, pPlayer.nFaction, tbAvatarInfo.nLevel, tbAvatarInfo.szEquipKey, tbAvatarInfo.szInsetKey, tbAvatarInfo.nStrengthLevel) then
	        ...
	        pPlayer.CenterMsg("Chuyển đổi không khác biệt nhân vật thất bại",true)
	        pPlayer.ZoneLogout();
	    end
	end
...
function BWZQBattle:ClcResult()
	...
	local function fnResult(self,pPlayer)
		...
		if pPlayer.dwOrgPlayerId == nWinPlayerId then
			local szMsg = "Chúc mừng ngài thắng được tranh tài, tấn cấp hạ giai đoạn!"
			pPlayer.Msg(szMsg)
			Dialog:SendBlackBoardMsg(pPlayer, szMsg)
		elseif pPlayer.dwOrgPlayerId == nLostPlayerId then
			local szMsg = "Ngài thua, không ngừng cố gắng!"
			pPlayer.Msg(szMsg)
			Dialog:SendBlackBoardMsg(pPlayer, szMsg)
		end
	end
