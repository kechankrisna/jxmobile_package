...
function BiWuZhaoQin:EndTip(tbPreMapLogic)
    local fnMsg = function (self,pPlayer)
        local szMsg = "Bởi vì không nhân sâm càng so với thi đấu hoặc người dự thi mất đi tư cách, tranh tài kết thúc!"
        pPlayer.Msg(szMsg)
...
function BiWuZhaoQin:CompleteTip(tbPreMapLogic,nWinerId)
    ...
    if pWiner then
        szMsg = "Chúc mừng ngài thu được tranh tài quán quân!"
        pWiner.Msg(szMsg)
        Dialog:SendBlackBoardMsg(pWiner, szMsg)
    end
    local tbPlayerInfo = tbPreMapLogic.tbJoinPlayerInfo[nWinerId] or {}
    local szName = tbPlayerInfo.szName or "Không biết"
    szMsg = string.format("Chúc mừng hiệp sĩ [FFFE0D]%s[-] Thu được bổn tràng luận võ chọn rể tranh tài quán quân!",szName)
    local fnMsg = function (self,pPlayer)
        pPlayer.Msg(szMsg)
        Dialog:SendBlackBoardMsg(pPlayer, szMsg)
    end
...
function BiWuZhaoQin:OnUpdateNormalKinName(nPlayerID)
    ...
    if pPlayer.dwOrgKinId ~= 0 and not Lib:IsEmptyStr(szTitle) then
        local szKinName = string.match(szTitle, "^［.* Phục］(.+)$");
        if szKinName then
            szKinName = string.format("%s%s", "［Bang］", szKinName);
        else
            szKinName = szTitle;
        end
