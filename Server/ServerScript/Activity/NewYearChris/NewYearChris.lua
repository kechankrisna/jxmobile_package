...
tbAct.szMailText = "Ngài tại [FFFE0D] Cầu nguyện bảng xếp hạng [-] Trung vị liệt thứ [FFFE0D]%d[-] Tên, phụ kiện vì ban thưởng, mời kiểm tra và nhận!"
tbAct.tbRankAward = {
    {1, {"Item", 7352, 1}},
    {10, {"Item", 7353, 1}},
    {30, {"Item", 7354, 1}},
...
function tbAct:SendUnidentWishItem(pPlayer, nCount)
	...
	Dialog:SendBlackBoardMsg(pPlayer, "Chúc mừng ngươi thu được chưa giám định cầu nguyện phù!")
	local nTeamId = pPlayer.dwTeamID
	if nTeamId and nTeamId>0 then
		local szMsg = string.format("Đội ngũ thành viên %s Thu được chưa giám định cầu nguyện phù!", pPlayer.szName)
...
function tbAct:IdentifyWishItem(pPlayer, nItemId)
	...
	if nCur>=self:GetWishGiftMaxIdentifyCount() then
		pPlayer.CenterMsg("Ngài hôm nay đã không thể giám định!")
		return
	end
...
function tbAct:LockWishItem(pPlayer, nItemId)
	...
	pPlayer.CenterMsg("Phong ấn thành công")
end

function tbAct:CanWish(pPlayer)
	if not self:CheckLevel(pPlayer) then
		return false, string.format("Đẳng cấp chưa đạt tới %d Cấp", self.JOIN_LEVEL)
	end

	local tbItems = pPlayer.FindItemInBag("WishItem")
	if not tbItems or not next(tbItems) then
		return false, "Không có cầu nguyện phù"
	end
...
function tbAct:OnDlgNewYearChrisNpc(pPlayer, pNpc)
	...
	local tbOptList = {
		{Text = "Cầu nguyện", Callback = fnOpenWishPanel, Param = {pPlayer.dwID}},
		{Text = "Bảng xếp hạng", Callback = fnOpenRankboard, Param = {pPlayer.dwID}},
		{Text = "Hiểu rõ tường tình", Callback = fnDetail, Param = {pPlayer.dwID}}
	}

    Dialog:Show({
        Text    = "Cầu ước nguyện đi!",
        OptList = tbOptList,
    }, pPlayer, pNpc)
end
...
function tbAct:MakeWish(pPlayer)
	...
	pPlayer.MsgBox(string.format("Ngươi trước mắt có được các loại cầu nguyện phù tổng cộng %d Trương, đem toàn bộ dùng để cầu nguyện?", nCount), {
		{"Xác định", function(nPlayerId)
...
function tbAct:DoMakeWish(nPlayerId)
	...
	for _, pItem in ipairs(tbItems) do
		...
		if pPlayer.ConsumeItem(pItem, nCount, Env.LogWay_NewYearChris)~=nCount then
			...
			pPlayer.Msg("Tiêu hao cầu nguyện phù thất bại", 1)
			return
		end
	...
	if nTotalWishCount>0 then
		...
		pPlayer.CenterMsg(string.format("Tiêu hao %d Trương hi hữu cầu nguyện phù, thu hoạch được %d Lần cầu phúc số lần", nTotalAdvanceWishItem, nTotalWishCount), true)
	end
	Dialog:SendBlackBoardMsg(pPlayer, string.format("Đại hiệp thành công cầu nguyện, lần này cầu nguyện thu hoạch được %d Điểm tích lũy, cách mình mộng tưởng lại tới gần một bước!", nScore))
...
function tbAct:SendRankBoardRewards()
    ...
    local tbMail = {Title = "Cầu nguyện bảng xếp hạng ban thưởng", From = "Hệ thống", nLogReazon = Env.LogWay_NewYearChris}
    local nSendNum = 0
...
function tbAct:AddWishText(pPlayer, szText)
	if not szText or szText=="" then
		pPlayer.CenterMsg("Nguyện vọng không thể vì không")
		return false
	end

	if Lib:Utf8Len(szText)>self.nWishTextMaxLen then
		pPlayer.CenterMsg(string.format("Nguyện vọng dài nhất %d Chữ", self.nWishTextMaxLen))
		return false
	end

	local nLeft = self:GetWishesLeft(pPlayer)
	if nLeft<=0 then
		pPlayer.CenterMsg("Còn thừa cầu phúc số lần không đủ")
		return false
	end
