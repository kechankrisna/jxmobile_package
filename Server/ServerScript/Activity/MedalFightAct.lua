...
tbAct.tbTrigger = {
	...
	End={},
    SendWorldNotify = { {"WorldMsg", "Các vị thiếu hiệp, huy hiệu tranh đoạt chiến sắp bắt đầu, mọi người có thể thông qua xem xét tin tức mới nhất hiểu rõ trong hoạt động cho!", 20} },
    SendNotify = {},
    OpenAct = {},
    CloseAct = {},
    NewMatch = {},
...
function tbAct:SendRankReward()
    ...
    local tbMail = {Title = "Huy hiệu tranh đoạt chiến", From = "Hệ thống", nLogReazon = Env.LogWay_MedalFightAct}
    local nSendNum = 0
...
function tbAct:OnReq_Answer(pPlayer, nMatchId, nAnswerId)
    ...
    if tbMatch.nRoundDeadline<nNow then
        return false, "Thời gian đã hao hết"
    end
...
function tbAct:CanJoin(pPlayer)
    ...
    if not Kin:IsInKinMap(pPlayer) then
        return false, "Mời đến bang phái quyền sở hữu bên trong tham gia"
    end
    local nScore = self:GetScore(pPlayer)
    if nScore<=0 then
        return false, "Đại hiệp đã không có huy hiệu!"
    end
    if not self:CheckJoinCount(pPlayer) then
        return false, "Đại hiệp hôm nay tham gia trận đấu số lần đã đạt tới hạn mức cao nhất!"
    end
    return true
end
...
function tbAct:OnReq_Join(pPlayer)
    if not self.bOpen then
        return false, "Tranh tài thời gian chưa tới, mời đến 19:00-19:30 Trong lúc đó lại đến!"
    end
