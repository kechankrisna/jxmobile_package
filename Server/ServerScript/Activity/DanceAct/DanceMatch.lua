...
function DanceMatch:EndAct()
    ...
    local nFrom = 1
    local szTitle = tbSetting.szActName .. "Xếp hạng ban thưởng"
...
function DanceMatch:PlayerSignUp(pPlayer)
    ...
    if not self.nReadyMapId then
        pPlayer.CenterMsg("Hiện tại không có đại hội võ lâm")
        return
    end
    ...
    if pPlayer.nLevel < self.tbSetting.nMinLevel then
        pPlayer.CenterMsg(string.format("Cần %d Cấp mới có thể tham gia", self.tbSetting.nMinLevel))
        return
    end
    ...
    if tbActData and tbActData.nLastPlayDay == Lib:GetLocalDay() and tbActData.nToDayCount >= self.tbSetting.nEveryDayPlayerTimes then
        pPlayer.CenterMsg(string.format("Một ngày chỉ có thể tham dự %d Lần", self.tbSetting.nEveryDayPlayerTimes) )
        return
    end
    if tbActData and tbActData.nTotalPlayerCount and tbActData.nTotalPlayerCount >= self.tbSetting.nTotalPlayTimes then
        pPlayer.CenterMsg(string.format("Nhiều nhất tham dự %d Lần", self.tbSetting.nTotalPlayTimes))
        return
    end
