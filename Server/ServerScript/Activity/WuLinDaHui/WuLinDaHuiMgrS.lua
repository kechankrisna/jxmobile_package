...
function WuLinDaHui:SetPlayersTicketOne( pPlayer, nVal, bRank)
    ...
    if bNotifyMail then
        ...
        Mail:SendSystemMail({
            Title = "Võ lâm đại hội tư cách dự thi";
            Text = szMailContent;
            To = pPlayer.dwID;
        })
    end
...
function WuLinDaHui:IsBaoMingTime()
    ...
    if not bRet then
        return false, "Không tại báo danh thời gian phạm vi bên trong"
    end
	return true;
end
...
function WuLinDaHui:TryCloseMatch(bFinal)
    ...
    if self.tbNotSaveData.bCurIsFinal then
        ...
        if not nWinTeamId then
            ...
            KPlayer.SendWorldNotify(1, 999, string.format("【Võ lâm đại hội】[eebb01]%s[-]Cũng không có sinh ra quán quân[eebb01]", tbGameFormat.szName), 1, 1);
        end
    end
...
function WuLinDaHui:ApplyPlayGame(pPlayer)
    ...
    if not tbNotSaveData.nCurGameType then
    	pPlayer.CenterMsg("Chưa mở ra hoạt động", true);    
    	return
    end
...
function WuLinDaHui:CheckEnterPrepareGame(pPlayer, bNotCheckMap, bOnePlayer, bNotTeamCount)
	...
	if not tbZoneData.nZoneMapId then
		return false, "Tranh tài chưa mở ra!";
	end

	local nGameType = self.tbNotSaveData.nCurGameType
	if not nGameType then
		return false, "Trước mắt chưa mở ra tranh tài"
	end

    local nFightTeamID = self:GetPlayerFightTeam(pPlayer, nGameType)
    local tbFightTeam = self:GetFightTeamByID(nFightTeamID);
    if not tbFightTeam then
        return false, string.format("Trước mắt là %s, ngươi còn không có đối ứng chiến đội", WuLinDaHui.tbGameFormat[nGameType].szName);
    end

    local bRet = tbFightTeam:HavePlayerID(pPlayer.dwID);
    if not bRet then
        return false, "Chiến đội không tồn tại ngươi thông tin, mời cùng phục vụ khách hàng nhân viên liên hệ!";
    end 

    local nJoinCount = tbFightTeam:GetJoinCount();
    if nJoinCount >= tbDef.nPreMatchJoinCount and not bNotTeamCount then
        return false, "Ngài chiến đội đã không có còn thừa tranh tài số lần";
    end

    local tbZoneData = self:GetZoneData()
    local nEnterPreMapID = tbZoneData.nZoneMapId;
    if not nEnterPreMapID then
    	return false, "Chuẩn bị sân bãi đồ còn chưa sáng tạo"
    end

    bRet = Map:CheckEnterOtherMap(pPlayer);
    if not bRet and not bNotCheckMap then
        return false, "Không phải khu vực an toàn không thể tiến vào";
    end   
    ...
    if nTeamId > 0 and not bOnePlayer then
        ...
        for _, nPlayerID in pairs(tbMember) do
            ...
            if not pMember then
                return false, "Có người chơi không tuyến bên trên!"
            end

            local nMemberTeamID = self:GetPlayerFightTeam(pMember, nGameType);
            if nMemberTeamID ~= nFightTeamID then
                return false, string.format("%s Không phải cùng một cái chiến đội", pMember.szName);
            end

            bRet = tbFightTeam:HavePlayerID(pMember.dwID);
            if not bRet then
                return false, string.format("Chiến đội không tồn tại %s Thông tin, mời cùng phục vụ khách hàng nhân viên liên hệ!!", pMember.szName);
            end  

            bRet = Map:CheckEnterOtherMap(pMember);
            if not bRet and not bNotCheckMap then
                return false, string.format("「%s」Không phải khu vực an toàn không thể tiến vào", pMember.szName);
            end    
        end        
    end
...
function WuLinDaHui:EnterPrepareGame(pPlayer, bNotCheckMap, bOnePlayer, bNotTeamCount)
    ...
    if nTeamId > 0 and not bOnePlayer then
        ...
        for _, nPlayerID in pairs(tbMember) do
            ...
            pMember.CenterMsg("Tranh tài sắp bắt đầu, mời các hạ kiên nhẫn chờ chút!", true)
        ...
        pPlayer.CenterMsg("Tranh tài sắp bắt đầu, mời các hạ kiên nhẫn chờ chút!", true)
...
function WuLinDaHui:RequestTopPreFightTeamList(pPlayer, nGameType, nClientSynTime)
    ...
    if not bRet then
        pPlayer.CenterMsg("Võ lâm đại hội còn chưa chính thức khai hỏa")        
        return
    end
    ...
    if not tbData or not nZoneSynTeamDataTime then
        pPlayer.CenterMsg("Trước mắt vô đối ứng xếp hạng tư liệu")
        return
    end
...
function WuLinDaHui:SynCurAllTeamRanks(tbRankTeamIds, bPreEnd, nCurGameType, bTransFinalRank)
    ...
    if bPreEnd and nCurGameType then
        ...
        if tbSaveData.nOpenTimes == WuLinDaHui.tbDef.nOpenPreMatchRound then
            ...
            KPlayer.SendWorldNotify(1, 999, string.format("【 Võ lâm đại hội 】[eebb01]%s Đấu vòng loại [-] Kết thúc, 16 Mạnh chiến đội đã sinh ra, có thể mở ra võ lâm đại hội tranh tài giới mặt thẩm tra!", tbGameFormat.szName), 1, 1);
    ...
    for i,v in ipairs(tbNeedSynRoleInfoIds) do
        ...
        if nServerId == nCurServerId then
            ...
            if pAsync then
                ...
                if tbFightTeam then
                    local tbMail16th = {
                        Title = "Võ lâm đại hội trận chung kết tấn cấp";
                        Text = string.format("  Chúc mừng các hạ chỗ chiến đội [FFFE0D]「%s」[-] Tấn cấp võ lâm đại hội [FFFE0D]"%s" [-] Trận chung kết giai đoạn!", tbFightTeam:GetName(), tbGameFormat.szName) ;
                        To = dwRoleId;
                    }
...
function WuLinDaHui:CheckPlayerEnterFinalsMap(pPlayer)
    ...
    if not nMainID then
        return false, "Địa đồ chưa mở ra";
    end

    local tbFinalsDef = tbDef.tbFinalsGame
    if pPlayer.nLevel < tbFinalsDef.nAudienceMinLevel then
        return false, string.format("Cấp bậc chưa đủ %s Không cách nào tham gia", tbFinalsDef.nAudienceMinLevel);
    end    
    ...
    if tbSaveData["WinnerTeamId" .. nGameType] then
        return false, "Tranh tài đã kết thúc"
    end    
    ...
    if nPlayerCount + nAddPlayerCount > tbFinalsDef.nEnterPlayerCount then
        if not bIsFinal then
            return false, "Tiến vào số người đã đủ";
        end 
    end
        ...
        if nPlayerID then
            ...
            if tbPlayerInfo then
                if Lib:IsEmptyStr(szChangeMsg) then
                    szChangeMsg = string.format("%s Hào vị vì「%s」", nI, tbPlayerInfo.szName);
                else    
                    szChangeMsg = szChangeMsg..string.format(", %s Hào vị vì「%s」", nI, tbPlayerInfo.szName);
                end
            end    
        end    
    end    
    
    szChangeMsg = "Võ lâm đại hội chiến đội số hiệu thay đổi: " ..szChangeMsg;
    pPlayer.CenterMsg("Cải biến đội hình thành công!", true);
