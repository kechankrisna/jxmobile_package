...
tbAct.szGiftMsg        = "[FFFE0D]%s[-] Hơi ôm quyền, xuất ra chuẩn bị đã lâu 99 Đóa hoa hồng, mang theo ý cười đối [FFFE0D]%s[-] Nói: Ngày lễ vui vẻ! Nữ hiệp! Tư thế hiên ngang nữ hiệp thái độ khác thường, một màn kia ngượng ngùng ý cười, có thể khuynh thành!"
tbAct.nImitityMul      = 9
...
function tbAct:SendCompleteAward(pMember, nAct)
    ...
    local szMsg = "    Hiệp sĩ đã hoàn thành tổ đội bí cảnh, lăng tuyệt phong, sơn tặc bí quật, trừng phạt ác nhiệm vụ bốn hạng nhiệm vụ toàn bộ số lần, thành công đạt thành nữ hiệp xông thiên quan hoạt động yêu cầu, mời nhận lấy ban thưởng!"
    local tbMail = {Title = "Nữ hiệp xông thiên quan ban thưởng", tbAttach = self.tbJoinActAward, Text = szMsg, nLogReazon = Env.LogWay_WomensDay, To = pMember.dwID}
