...
function CommerceTask:AcceptTask(pPlayer)
	...
	if not nDegree or nDegree <= 0 then
		pPlayer.CenterMsg("Hôm nay không thể nhận tiếp nhiệm vụ Thương Hội")
		return;
	end
...
function CommerceTask:CommitItem(pPlayer, nIndex)
	...
	if not self:ConsumeBoxNeedItem(pPlayer, tbTask.nTaskId) then
		pPlayer.CenterMsg("Thất bại, đạo cụ không đủ");
		return;
	end
	...
	local szTip = szInfo ~= "" and string.format("Nộp thành công, nhận được thưởng %s", szInfo) or string.format("Nộp thành công")
...
function CommerceTask:FinishTask(pPlayer, bConfirm)
	...
	if nFinishCount < self.COMPLETE_COUNT then
		pPlayer.CenterMsg(string.format("Đã hoàn thành %d", self.COMPLETE_COUNT))
		return;
	end

	if not bConfirm and nFinishCount < self.ALL_COMPLETE_COUNT then
		pPlayer.MsgBox(CommerceTask.szNotAllCompleteMsg, {{"Hoàn thành", CommerceTask.FinishTask, CommerceTask, pPlayer.dwID, true}, {"Hủy"}})
		return
	end
	...
	self:SyncCommerceData(pPlayer);
	pPlayer.CenterMsg("Chúc mừng ngươi đã hoàn thành nhiệm vụ");
...
function CommerceTask:AskHelp(pPlayer, nRequestTaskId)
	if pPlayer.dwKinId == 0 then
		pPlayer.CenterMsg("Ngươi không có Bang Hội, không thể gửi cầu cứu");
		return;
	end
	...
	pPlayer.CenterMsg("Gửi cầu cứu Bang Hội thành công");
	self:SyncCommerceData(pPlayer);

	local szKinMsg = string.format("<Hỗ trợ>「%s」phát cầu cứu nhiệm vụ Thương Hội, đang tìm 【%s】x%d", pPlayer.szName, szItemName, tbTaskSetting.nCount);
...
function CommerceTask:DoHelp(pPlayer, nTarPlayerId, nTaskId, bCoin)
	...
	if not bCoin then
		...
		if nConsumeCount < nCount then
			pPlayer.CenterMsg("Không có vật phẩm để hỗ trợ")
		...
		if not nConsume then
			pPlayer.CenterMsg("Hỗ trợ thất bại, xin thử lại sau")
			return
		end
...
function CommerceTask:OnCostGoldHelpCallback(nPlayerId, bSuccess, nTarPlayerId, nDataPos, nTaskId)
	...
	if not pPlayer then
		return false, "Hỗ trợ trong lúc rời mạng, xin thử lại sau"
	end

	if not bSuccess then
		return false, "Trả tiền thất bại"
	end
...
function CommerceTask:CheckHelpEnable(pPlayer, nTarPlayerId, nTaskId, bCoin)
	if pPlayer.dwID == nTarPlayerId then
		return false, "Không có cách nào cầu cứu Bang Hội giúp đỡ hoàn thành"
	end

	if not Kin:PlayerAtSameKin(pPlayer.dwID, nTarPlayerId) then
		return false, "Không ở cùng Bang Hội, không thể nào hỗ trợ";
	end
	...
	if tbTaskSetting.szType == "Item" then
		if not bCoin then
			local nHas = pPlayer.GetItemCountInAllPos(tbTaskSetting.nTemplateId);
			if nHas < tbTaskSetting.nCount then
				return false, string.format("Ngươi không có đủ %s", tbBaseInfo.szName or "")
			end
		end
	else
		return false, "Hoàn thành nhiệm vụ không có cách nào hỗ trợ"
	end
...
function CommerceTask:OnHelpSuccess(pPlayer, nTarPlayerId, nDataPos, nTaskId)
	...
	local tbBaseInfo = KItem.GetItemBaseProp(tbTaskSetting.nTemplateId);
	local szItemName = tbBaseInfo.szName;
	local szKinMsg = string.format("「%s」giúp「%s」hoàn thành yêu cầu thương hội:【%s】", pPlayer.szName, pRoleStayInfo.szName, szItemName)
	...
	if pPlayer.dwKinId ~= 0 then
		...
		pPlayer.CallClientScript("CommerceTask:OnRespondHelp", string.format("Nhận được %d cống hiến", nAddCount))
	end
...
function CommerceTask:SendNotifyMail(szItemName, nCount, szPlayerName, nTarPlayerId)
	local szMailContent = string.format("Gửi cầu cứu thương hội【%s】x%d, được「%s」giúp hoàn thành.", szItemName, nCount, szPlayerName);
	-- local tbMail =
	-- {
	-- 	To = nTarPlayerId,
	-- 	Title = "商会协助",
