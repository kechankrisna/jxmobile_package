...
function ArenaBattle:ApplyChallenge(pPlayer,nArenaId)
	...
	if not tbArenaInfo then
		pPlayer.CenterMsg("Tìm không thấy số liệu lôi đài")
	...
	if self:CheckIsArenaTeamMan(pPlayer.dwID) then
		pPlayer.CenterMsg("Bạn đã là chủ lôi đài, bạn không thể khiêu chiến người khác.")
		return
	end

	if self:CheckIsChallenger(pPlayer.dwID) then
		pPlayer.CenterMsg("Bạn đang chiến đấu, bạn không thể khiêu chiến người khác.")
		return
	end
	...
	if not tbArenaMan.nArenaManId then
		if pTeamData then 
			if pPlayer.dwID ~= pTeamData.nCaptainID then
				pPlayer.CenterMsg("Đội trường mới có thể xin lên lôi đài")
				return
			end

			bRet,szMsg = self:CheckMemberValidMap(pPlayer.dwTeamID)
			if not bRet then
				szTip = string.format("「%s」không có trong tỷ võ, không thể lên lôi đài",szMsg or "")
	...
	if manTeamData and not pTeamData then
		pPlayer.CenterMsg("Hãy khiêu chiến đội dưới dạng một đội")
		return 
	end
	
	if not manTeamData and pTeamData then
		pPlayer.CenterMsg("Hãy khiêu chiến đội dưới dạng cá nhân")
		return
	end
	...
	if manTeamData then
		...
		if tbArenaMan.tbSingleApply and tbArenaMan.tbSingleApply[nApplyId] then
			pPlayer.CenterMsg("Bạn đã xin lên lôi đài")
			return 
		end
	end
	...
	pPlayer.CenterMsg("Đã gửi khiêu chiến, xin chờ trả lời")
end
...
function ArenaBattle:PickChallenger(pPlayer,tbApplyData,bAuto)
	...
	if not tbArenaMan or not tbArenaMan.nArenaManId then
		pPlayer.CenterMsg("Không tìm được thông tin chủ lôi đài")
	...
	if pPlayer.dwID ~= nArenaManId then
		if manTeamData then
			pPlayer.CenterMsg("Đội trưởng chủ lôi đài mới có thể chọn người khiêu chiến")
		else
			pPlayer.CenterMsg("Đài chủ mới có thể chọn lựa người khiêu chiến ")
	...
	if not tbApplyInfo or not tbApplyInfo.nApplyId or not tbApplyInfo.nTeamId then
		...
		pPlayer.CenterMsg("Thông tin người khiêu chiến đã thay đổi, vui lòng chọn lại")
		return
	end
	...
	if manTeamData then 												-- 组队战
		if not challengerTeamData then
			...
			self:SynChallengerData(pPlayer,nArenaId)
			pPlayer.CenterMsg("Đội khiêu chiến đã giải tán")
			return
		end
		...
		if challengerTeamData then 										-- 单人战申请列表有组队战申请？？
			self:RemoveApply(nArenaId,nIdx)
			self:SynChallengerData(pPlayer,nArenaId)
			pPlayer.CenterMsg("Thông số bất thường")
	...
	if not pChallenger then
		self:RemoveApply(nArenaId,nIdx)
		self:SynChallengerData(pPlayer,nArenaId)
		pPlayer.CenterMsg("Người khiêu chiến đã rời mạng")
		return 
	end
...
function ArenaBattle:AutoPickChallenger(nArenaId)
	...
	if not tbApplyData or not pApply then
		-- 进入阶段一，等待申请
		pMan.CenterMsg("Không tìm thấy người khiêu chiến phù hợp")
		return
	end
	...
	if not bRet then
		-- 进入阶段一，等待申请
		pMan.CenterMsg("Chọn người thách đấu thất bại")
		return 	
	end
...
function ArenaBattle:ApplyArenaFight(nArenaId)
	...
	if not tbArenaFight then
		return false,"Tìm không thấy lôi đài??"
	end

	if tbArenaFight.bStart then
		return false,"Lôi đài đang được sử dụng"
	end
...
function ArenaBattle:CheckChallengerCommond(pPlayer,bIsTeam)
	if not self:CheckValidMap(pPlayer) then
		return false,"Người khiêu chiến không có trong bản đồ"
	end 

	if self:CheckIsChallenger(pPlayer.dwID) then
		return false,"Người khiêu chiến đang trên lôi đài khác"
	end
	...
	if not bIsTeam and pPlayer.dwTeamID > 0 then
		return false,"Người khiêu chiến hiện không thể lên lôi đài"
	end
...
function ArenaBattle:SendBeArenaManTip(pPlayer)
	if pPlayer.dwTeamID > 0 then
		...
		for _,dwID in pairs(tbMember) do
			local pMember = KPlayer.GetPlayerObjById(dwID)
			if pMember then
				Dialog:SendBlackBoardMsg(pMember, "Chúc mừng trở thành đài chủ ");
			end
		end
	else
		Dialog:SendBlackBoardMsg(pPlayer, "Chúc mừng trở thành đài chủ ");
	end
end
		...
		if tbArenaInfo.tbArenaMan and tbArenaInfo.tbArenaMan.nArenaManId then
			...
			if pMan then
				...
				tbArenaData[nArenaId] = {
					...
					szName = pMan.szName or XT("Không"),
					nLevel = pMan.nLevel,
		...
		if nTeamId then
			...
			if pApply and self:CheckChallengerCommond(pApply,bIsTeam) then
				local tbPlayerData = {
					nPlayerID = nApplyId,
					nIdx = nIdx,
					szName = pApply.szName or XT("Không"),
		...
		if pApply then
			...
			if pApply.dwTeamID > 0 then
				...
				for _,dwID in pairs(tbMember) do
					local pMember = KPlayer.GetPlayerObjById(dwID)
					if pMember then
						pMember.CenterMsg(string.format("Lôi đài %s thay đài chủ, hãy khiêu chiến lại",nArenaId),true)
					end
				end
			else
				pApply.CenterMsg(string.format("Lôi đài %s thay đài chủ, hãy khiêu chiến lại",nArenaId),true)
			end
		end
	end
end
...
function ArenaBattle:Leave(pPlayer,bSure)
	...
	if nArenaId then
		local tbArenaFight = self:GetArenaFight(nArenaId)
		if not tbArenaFight then
			pPlayer.CenterMsg("Tìm không thấy lôi đài lối ra ")
			Log("[ArenaBattle] Leave fail",pPlayer.dwID,pPlayer.dwTeamID,pPlayer.szName,pPlayer.nMapTemplateId)
			return
		end

		if tbArenaFight.bStart then
			pPlayer.CenterMsg("Đang thi đấu không thể thoát ra")
			return			
		end
		...
		if pPlayer.dwTeamID > 0 then
			...
			local szMsgTip = bIsCaptain and "Mang toàn đội xuống lôi đài" or "Rời lôi đài thoát tổ đội"
			if not bSure then
				pPlayer.MsgBox(szMsgTip,
				{
					{"Rời đi", fnLeave, pPlayer.dwID},
					{"Hủy"},
				})
			else
				if bIsCaptain then
			...
			if not bSure then
				pPlayer.MsgBox("Nhất đinh rời khỏi lôi đài?",
				{
					{"Rời đi", fnLeave, pPlayer.dwID},
					{"Hủy"},
				})
		...
		pPlayer.CenterMsg("Người đã ở dươi lôi đài")
	end
end
	...
	if me.dwTeamID > 0 then
		...
		if tbTeamData and ArenaBattle:GetApply(me) then
			...
			if pCaptain then
				...
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, XT(string.format("「%s」đã rời lôi đài, hãy khiêu chiến lại",me.szName)), me.dwTeamID);
				local tbMember = TeamMgr:GetMembers(me.dwTeamID);
				for _,dwID in pairs(tbMember) do
					local pMember = KPlayer.GetPlayerObjById(dwID)
					if pMember then
						pMember.CenterMsg(XT(string.format("「%s」đã rời lôi đài, hãy khiêu chiến lại",me.szName)))
