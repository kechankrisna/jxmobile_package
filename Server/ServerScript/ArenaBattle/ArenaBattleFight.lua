...
function ArenaBattleFight:WaiteChallenger()
	...
	self.szStateInfo = "Chờ người khiêu chiến..."
...
function ArenaBattleFight:WaitChooseChalleger()
	...
	self.szStateInfo = "Mời chọn người khiêu chiến："
...
function ArenaBattleFight:Go2Arena(pPlayer,nCamp)
	if not nCamp or (nCamp ~= ArenaBattle.tbForeachType.ARENA_MAN and nCamp ~= ArenaBattle.tbForeachType.CHALLENGER) then
		pPlayer.CenterMsg("Tìm không thấy trận");
	...
	if tbPos then
		if not ArenaBattle:CheckValidMap(pPlayer) then
			if pPlayer.dwTeamID > 0 then
				self:KickOutNotice(nCamp,string.format("「%s」 không ở võ đài, tự động rời khỏi đội",pPlayer.szName),pPlayer.dwTeamID)
		...
		pPlayer.CenterMsg("Tư liệu dị thường, hãy liên lạc chăm sóc khách hàng!");
...
function ArenaBattleFight:SendAutoWinTip(nCamp)
	...
	local function fnAutoWinTip(self,pPlayer)
		pPlayer.CenterMsg("Đối thủ rời lôi đài, tự động chiến thắng.",true)
	end
...
function ArenaBattleFight:SendResultTip(nWinCamp)
	...
	local szTip = string.format("%d lôi đài",tonumber(self.nArenaId))
	local nWinnerId
	...
	if pPlayer then
		if pPlayer.dwTeamID > 0 then
			szTip = szTip ..string.format("「%s」Dẫn đầu đội ngũ chiến thắng!",pPlayer.szName)
		else
			szTip = szTip ..string.format("「%s」Chiến thắng!",pPlayer.szName)
		end
	end
