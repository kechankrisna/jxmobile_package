...
function House:OnMapCreate(nMapId)
	...
	Lib:CallBack({PartnerCard.OnCreateHouseMap, PartnerCard, dwOwnerId, nMapId, tbMapInfo.nMapTemplateId})
end
...
function House:OnMapEnter(nMapId)
	...
	if dwOwnerId == me.dwID and self.tbMapToRemoveInfo and self.tbMapToRemoveInfo[nMapId] and #self.tbMapToRemoveInfo[nMapId] > 0 then
		...
		Mail:SendSystemMail({
			...
			Text = "    Đại hiệp! Bởi vì gia viên xây dựng thêm sau cách cục phát sinh biến hóa, [FFFE0D] Bộ phận gia câu bản cô nương đã giúp ngươi thu hồi đến gia câu nhà kho a![-]",
			From = "「Quản Lý Gia Viên」",
		});
	end

	Activity:OnGlobalEvent("Act_OnHouseMapEnter", me, nMapId)
	Lib:CallBack({PartnerCard.OnEnterHouseMap, PartnerCard, me, dwOwnerId, nMapId})
	Lib:CallBack({self.tbPeach.OnEnterHouseMap, self.tbPeach, me, dwOwnerId, nMapId})

	local tbEnterCall = self.tbHouseMapInfo[dwOwnerId].tbEnterCall or {}
	local fnEnterCall = tbEnterCall[me.dwID]
	if fnEnterCall then
		Lib:CallBack({fnEnterCall, me})
		tbEnterCall[me.dwID] = nil
	end

	local _, nPlayerCount = KPlayer.GetMapPlayer(nMapId)
	if nPlayerCount >= 50 then
		Achievement:SetCount(dwOwnerId, "Mystic_50InHouse", 1)
	end
end
...
function House:OnPlayerTrap(nMapId, szTrapName)
	...
	if szTrapName == "Out_sn" then
		if not House:CheckEnterAccess(me, dwOwnerId) then
			me.CenterMsg("Nhà đối phương đã đóng cửa, không thể vào");
			return;
		end
...
function House:DoPlayerTrap(szTrapName)
	...
	if type(varDst) == "table" then
		...
		if not self:CheckCanRide(me) and me.GetActionMode() ~= Npc.NpcActionModeType.act_mode_none then
			me.CenterMsg("Ngựa chỉ đi được ở sân, nhà phải đi bộ nha");
...
function House:OnMapLogin(nMapId)
	local dwOwnerId, tbMapInfo = self:GetHouseInfoByMapId(nMapId);
	Lib:CallBack({PartnerCard.OnLoginHouseMap, PartnerCard, me, dwOwnerId, nMapId})
	Lib:CallBack({self.tbPeach.OnEnterHouseMap, self.tbPeach, me, dwOwnerId, nMapId})
end
...
function House:OnMapLeave(nMapId)
	...
	local dwOwnerId, tbMapInfo = self:GetHouseInfoByMapId(nMapId);
	Lib:CallBack({PartnerCard.OnLeaveHouseMap, PartnerCard, me, dwOwnerId, nMapId})
end
...
function House:DeleteMap(dwOwnerId, szMsg)
	...
	Lib:CallBack({function () Item:GetClass("NpcLoveToken"):OnDestroyHouseMap(dwOwnerId, nMapId); end });
	Lib:CallBack({PartnerCard.OnDestroyHouseMap, PartnerCard, dwOwnerId, nMapId})
end
