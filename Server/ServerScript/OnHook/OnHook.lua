...
function OnHook:OnUseBaiJuWan(pPlayer,nBJWTime,nType)
	...
	if nAddBJWTime > 0 then
		...
		if nMinute > 0 then
			szTime = szTime ..nMinute .. "phút"
		end
		if nSecond > 0 then
			szTime = szTime ..nSecond .. "giây"
		end
		if szTime ~= "" then
			szItem = nType == OnHook.OnHookType.Pay and "Bạch cầu hoàn" or "Bạch cầu hoàn Đặc hiệu"
			pPlayer.CenterMsg(szItem .."Tăng thời gian" ..szTime);
		end
	end
...
function OnHook:StartOnLineOnHook(pPlayer)
	...
	pPlayer.CenterMsg(string.format("Bắt đầu ủy thác, hiện còn: %s giờ", Lib:TimeDesc5(OnHook:GetOnHookTime(pPlayer))), true)
...
function OnHook:EndOnLineOnHook(pPlayer, nLogType)
	...
	if nLogType and nLogType == OnHook.tbCalcLogType.LogType_Online_Hand and OnHook:IsOnLineOnHookForce(pPlayer) then
		pPlayer.CenterMsg("Kết thúc trạng thái ủy thác")
		return
	end
	...
	pPlayer.CenterMsg(string.format("Kết thúc ủy thác, đã ủy thác %s giờ", szTime), true)
...
function OnHook:StartOnLineOnHookForce(pPlayer)
	...
	if self:OnHookTime(pPlayer) < 60 and not self:IsCrossDay(pPlayer, nNowTime, self:LoginTime(pPlayer)) then
		pPlayer.CenterMsg("Thời gian ủy thác hôm nay đã dùng hết", true)
		return
	end

	if self:OnLineOnHookTime(pPlayer) > 0 then
		pPlayer.CenterMsg("Hiện đang mở ủy thác online", true)
		return
	end
