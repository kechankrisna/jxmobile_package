...
function MapExplore:RequestExplore(pPlayer, nMapTemplateId)
	if not tbMaspIndex[nMapTemplateId] then
		return false, "Không có tấm bản đồ này thăm dò thiết trí"
	end

	if DegreeCtrl:GetDegree(pPlayer, "MapExplore") <= 0 then
		return false, "Số lần đã dùng hết"
	end
	...
	if not AsyncBattle:CanStartAsyncBattle(pPlayer) then
		return false, "Hãy vào khu vực an toàn rồi thử lại"
	end
	...
	if nStep >= MAX_STEP then
		return false, "Trước mắt địa đồ đã thăm dò xong"
	end
	...
	if pPlayer.dwTeamID > 0 then
		...
		pPlayer.MsgBox("Trạng thái tổ đội không thể nào thăm dò, có muốn rời đội không ?",
			{
				{"Rời đội", fnQuitTeamAndRequestExplore},
				{"Hủy"},
			})
		return;
	end
...
function MapExplore:MoveStep(pPlayer, nMapTemplateId, nPassStep)
	...
	if nStep ~= nPassStep then
		pPlayer.CenterMsg("Thăm dò~")
		return
	end
	if nStep >= MAX_STEP then
		pPlayer.CenterMsg("Bản đồ đã được khám phá.")
		return
	end

	if not DegreeCtrl:ReduceDegree(pPlayer, "MapExplore", 1) then
		pPlayer.CenterMsg("Số lần đã dùng hết")
		return
	end
...
function MapExplore.RequestResetCallback(nPlayerId, bSucceed, szBillNo, nMapTemplateId)
	...
	if nToday ~= nLastUpdateDay then
		return false, "Đặt lại lỗi thời gian"
	end
	...
	pPlayer.CenterMsg("Đặt lại thành công!")
