		...
		if tbInfo.nResendTime <= nTimeNow then
			if not self:Resend(nWaitingId, tbInfo) then
				...
				if tbInfo.szType == "Pay" then
					...
					if pPlayer then
						pPlayer.CenterMsg(string.format("Tiêu phí thất bại, do:%s", szMsg or "Lỗi hệ thống thanh toán không xác định"));
					end
				end
			end
		end
...
function AssistClient:UpdateMsdkInfo(pPlayer, tbMsdkInfo, bLogin)
	...
	if tbMsdkInfo.szOpenId ~= pPlayer.szAccount then
		...
		pPlayer.MsgBox("Đăng nhập là bất thường, vui lòng đăng nhập một lần nữa", {{"Xác nhận", fnReturn2Login, pPlayer.dwID}});
		return;
	end
...
function AssistClient:DoPay(pPlayer, nPay, nLogReason1, nLogReason2, fnAfterPay, ...)
	...
	if self.tbNextPlayersPayTime[pPlayer.dwID] and nNow < self.tbNextPlayersPayTime[pPlayer.dwID] then
		...
		pPlayer.CenterMsg("Bạn đang có một giao dịch đang diễn ra, vui lòng đợi");
		return false;
	end
	...
	if not nWaitingId or not szBillNo then
		fnAfterPay(pPlayer.dwID, false, "");
		pPlayer.CenterMsg("Không thể kết nối với máy chủ thanh toán");
		return false;
	end
...
function AssistClient:DoPresent(pPlayer, nGold, nLogReason1, nLogReason2, fnAfterPresent, ...)
	...
	if not nWaitingId then
		fnAfterPresent(pPlayer.dwID, false, "", "Không thể kết nối với máy chủ thanh toán");
		return false;
	end
...
function AssistClient:OnPayRsp(nPlayerId, tbWaitingInfo, tbPayRet)
	...
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
	local szFailReason = "Bạn đang ngoại tuyến trong quá trình mua hàng.";
	if pPlayer then
		...
		elseif tbPayRet.ret == Sdk.eMidasServerRet_LackMoney then
			szFailReason = "Số dư của bạn không đủ";
		...
		if pPlayer then
			AssistClient:DoCancelPay(pPlayer, tbWaitingInfo.nGold, tbPayRet.billno, Env.LogWay_ReturnGold2Player, szRetInfo or szFailReason, function (nPlayerId, bSucc)
				if not bSucc then
					local tbMail = {
						To = nPlayerId;
						Title = "Nguyên bảo trả về";
						From = "Hệ thống";
						Text = string.format("Tiêu phí thất bại hoàn trả nguyên bảo do: %s", szRetInfo or szFailReason);
...
function AssistClient:OnJoinQQGroupRsp(dwPlayerId, szRetData)
	...
	if tbRetInfo.ret ~= Sdk.eQQGroupRet_Suss then
		pPlayer.CenterMsg("加入Q群失敗：" .. (tbRetInfo.message or ""));
...
function AssistClient:OnQueryQQGroupListRsp(nPlayerId, szRetData)
	...
	if tbGroupInfo.ret ~= Sdk.eQQGroupRet_Suss then
		pPlayer.CenterMsg("獲取已有群列表失敗：" .. (tbGroupInfo.message or "未知原因"));
...
function AssistClient:OnBindQQExistGroupRsp(nPlayerId, szRetData)
	...
	if tbRet.ret ~= Sdk.eQQGroupRet_Suss then
		pPlayer.CenterMsg("綁定Q群失敗：" .. (tbRet.message or "未知原因"));
		Log("OnBindQQExistGroupRsp Error", nPlayerId, szRetData);
		return;
	end

	pPlayer.CenterMsg("綁定Q群成功");
...
function AssistClient:OnCreateAndBindQQGroupRsp(nPlayerId, szRetData)
	...
	local tbRet = Lib:DecodeJson(szRetData) or {};
	if tbRet.ret ~= Sdk.eQQGroupRet_Suss then
		pPlayer.CenterMsg("創建並綁定Q群失敗：" .. (tbRet.message or "未知原因"));
		Log("OnCreateAndBindQQGroupRsp Error", nPlayerId, szRetData);
		return;
	end

	pPlayer.CenterMsg("創建並綁定Q群成功");
	local kinData = Kin:GetKinById(pPlayer.dwKinId);
	if kinData then
		AssistClient:UpdateGroupInfo(pPlayer, pPlayer.dwKinId, kinData:GetOrgServerId());
	end
...
function AssistClient:OnQueryFirstRegistInfo(dwPlayerId, szRetData)
	...
	if tbRetInfo.need_pop == "1" then
		pPlayer.MsgBox("Thông tin tên thật của bạn đã tồn tại trong thư viện tên thật của nền tảng Tencent. Bạn có thể đăng nhập vào Tencent Games để trải nghiệm. Vui lòng xác nhận");
	end
end
...
local tbRankSendGiftTips = {
	["not found"] = "Đối tượng không tồn tại";
	["diff player"] = "Thông tin người chơi đưa ra không khớp. Vui lòng thử lại sau.";
	["max send"] = "Hôm nay món quà miễn phí đạt giới hạn";
	["has sent"] = "Hôm nay đã đối nên hảo hữu tiến hành đưa tặng";
}
...
function tbRankServerRspDealer:OnSendGiftRsp(dwPlayerId, szErrMsg, szExtra)
	...
	if szErrMsg ~= "succeed" then
		if pPlayer then
			pPlayer.CenterMsg(tbRankSendGiftTips[szErrMsg] or "Đưa tặng hảo hữu lễ vật lúc phát sinh dị thường");
		end
		return false;
	end
	...
	tbNeighbourCmd.tbMail = {
		To = nTargetPlayerId,
		Title = "Hảo hữu lễ vật",
		Text = string.format("Bạn hữu %s đã tặng bạn một món quà, hãy kiểm tra và nhận.", szFriendName),
		From = "Danh sách bạn bè",
		tbAttach = {{"Coin", 200}},
