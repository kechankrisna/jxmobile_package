...
function TeamBattle:OnSyncFinish(nType)
	...
	NewInformation:AddInfomation("TeamBattle" .. GetTime(), GetTime() + 7 * 3600 * 24, {Type1 = "TeamBattle",Type2 = nType, tbList = tbShowInfo } , {szTitle = "Thông Thiên Tháp系列賽", nReqLevel = 10, szUiName = "StarTower"});
end
...
function TeamBattle:SendQuarterlyTick(nPlayerId, nFloor)
	...
	if nCurInfo ~= nNextOpenSeason then
		...
		local tbQuarterlyMail = {
			To = pPlayer.dwID,
			Title = "Thông báo đủ điều kiện Thông thiên tháp Quý",
			Text = string.format("      Xin chúc mừng về thành tích xuất sắc của bạn trong Thông Thiên Tháp, bạn đã ở tầng %s và hiện bạn đã đủ điều kiện cho vòng đấu quý. Thời gian mở của Thông Thiên Tháp Quý là [FFFE0D]%s [-] và mong là bạn sẽ tham gia! Vinh dự cao hơn đang chờ bạn!", nFloor, szTimeInfo),
			From = "Thông Thiên Tháp",
		...
		local szFriendMsg = string.format("Chúc mừng đại hiệp「%s」trong Thông Thiên Tháp xuất sắc leo lên tầng %s, nhận được vé tham gia Vòng đấu Quý#49", pPlayer.szName, nFloor);
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Friend, szFriendMsg, pPlayer.dwID);
		if pPlayer.dwKinId > 0 then
			local szKinMsg = string.format("Chúc mừng thành viên 「%s」 trong Thông Thiên Tháp xuất sắc leo lên tầng %s, nhận được vé tham gia Vòng đấu Quý#49", pPlayer.szName, nFloor);
...
function TeamBattle:SendYearTick(nPlayerId, nFloor)
	...
	if nCurInfo ~= nNextOpenYear - 1 then
		...
		local tbYearMail = {
			To = pPlayer.dwID,
			Title = "Thông báo đủ điều kiện Thông thiên tháp Năm",
			Text = string.format("      Xin chúc mừng về thành tích xuất sắc của bạn trong Thông Thiên Tháp, bạn đã ở tầng %s và hiện bạn đã đủ điều kiện cho vòng đấu năm. Thời gian mở của Thông Thiên Tháp Năm là [FFFE0D]%s [-] và mong là bạn sẽ tham gia! Vinh dự cao hơn đang chờ bạn!", nFloor, szTimeInfo),
			From = "Thông Thiên Tháp",
		...
		local szFriendMsg = string.format("Chúc mừng đại hiệp「%s」trong Thông Thiên Tháp xuất sắc leo lên tầng %s, nhận được vé tham gia Vòng đấu Năm!#49", pPlayer.szName, nFloor);
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Friend, szFriendMsg, pPlayer.dwID);
		if pPlayer.dwKinId > 0 then
			local szKinMsg = string.format("Chúc mừng thành viên 「%s」 trong Thông Thiên Tháp xuất sắc leo lên tầng %s, nhận được vé tham gia Vòng đấu Năm#49", pPlayer.szName, nFloor);
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szKinMsg, pPlayer.dwKinId);
		end
	end
end
...
function TeamBattle:SendYearResult(nPlayerId, nFloor)
	...
	local szFriendMsg = string.format("Chúc mừng đại hiệp「%s」 trong Thông Thiên Tháp xuất sắc leo lên tầng %s！#49", pPlayer.szName, nFloor);
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Friend, szFriendMsg, pPlayer.dwID);
	-- if pPlayer.dwKinId > 0 then
	-- 	local szKinMsg = string.format("恭喜本家族成员「%s」在季度Thông Thiên Tháp竞技中英勇无敌，一口气登上%s层，现已获得年度Thông Thiên Tháp参赛资格！#49", pPlayer.szName, nFloor);
	-- 	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szKinMsg, pPlayer.dwKinId);
	-- end
end
