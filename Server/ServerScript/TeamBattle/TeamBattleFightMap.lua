...
function tbFightMap:SyncPlayerLeftInfo(pPlayer)
	local tbDmgInfo = {
		szStateInfo = self.szStateInfo or "Chờ đợi để bắt đầu",
...
function tbFightMap:ShowBeyInfo()
	...
	local function fnShowBeyInfo(self, pPlayer)
		pPlayer.SendBlackBoardMsg("Vòng này là trống và trực tiếp vào vòng trong!", true);
...
function tbFightMap:PreStart()
	local function fnPreStart(self, pPlayer)
		...
		if tbBeginFightPos then
			...
			pPlayer.CenterMsg("Dữ liệu bất thường, vui lòng liên hệ với CSKH!");
...
function tbFightMap:GotoRest(bIsMid)
	local function fnGotoRest(self, pPlayer)
		...
		if tbRestPos then
			if bIsMid then
				pPlayer.SendBlackBoardMsg("Hiệp kết thúc, nghỉ giữa trận", true);
			end
