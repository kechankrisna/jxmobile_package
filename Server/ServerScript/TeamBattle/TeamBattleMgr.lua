...
function tbTeamBattleMgr:CostAllPlayerTimes()
	...
	for _, tbMember in pairs(self.tbAllTeamInfo) do
		for _, nPlayerId in ipairs(tbMember) do
			-- 这里nPlayerId不用判断，因为这里的逻辑肯定是本服Thông Thiên Tháp
...
function tbTeamBattleMgr:KickoutSpacePlayer()
	...
	for _, pPlayer in pairs(tbAllMapPlayer) do
		...
		if not self.tbPlayerTeamId[nUniqId] then
			pPlayer.CenterMsg("Thất bại trong việc kết hợp bạn với một đồng đội không thể vào sự kiện này!");
...
function tbTeamBattleMgr:StartFail()
	...
	for _, pPlayer in pairs(tbAllMapPlayer) do
		...
		pPlayer.CenterMsg("Do không đủ số lượng ứng viên, sự kiện này không mở!", true);
		TeamBattle:GoBack(pPlayer);
	end
...
function tbTeamBattleMgr:SendPlayerAward(pPlayer, nFloor)
	...
	local szMsg = "";
	if nAwardItemCount > 0 then
		szMsg = string.format("Chúc mừng đại hiệp lần này lên tầng thứ %s trong Thông Thiên Tháp, nhận được %s điểm vinh dự\nTự động đổi%s cái Thông Thiên tháp bảo rương, còn %s điểm vinh dự\n（Mỗi %s điểm vinh dự，Tự động đổi 1cái Thông Thiên tháp bảo rương）",
								nFloor, nAddHonor, nAwardItemCount, nLastValue, TeamBattle.nAwardItemNeedHonor);
	else
		szMsg = string.format("Chúc mừng đại hiệp lần này lên tầng thứ %s trong Thông Thiên Tháp, nhận được %s điểm vinh dự\nHiện tại còn lại %s điểm vinh dự \n（Mỗi %s điểm vinh dự，Tự động đổi 1cái Thông Thiên tháp bảo rương）",
	...
	if GetTimeFrameState(TeamBattle.szLeagueOpenTimeFrame) == 1 and nFloor >= 7 then
		...
		if nCurInfo ~= nNextOpenMonth - 1 then
			...
			local tbMonthlyMail = {
				To = pPlayer.dwID,
				Title = "Thông báo đủ điều kiện Thông thiên tháp Tháng",
				Text = string.format(" Xin chúc mừng về thành tích xuất sắc của bạn trong Thông Thiên Tháp, bạn đã ở tầng %s và hiện bạn đã đủ điều kiện cho vòng đấu tháng. Thời gian mở của Thông Thiên Tháp Tháng là [FFFE0D]%s [-] và mong là bạn sẽ tham gia! Vinh dự cao hơn đang chờ bạn!", nFloor, szTimeInfo),
			...
			local szFriendMsg = string.format("Chúc mừng đại hiệp「%s」trong Thông Thiên Tháp xuất sắc leo lên tầng %s, nhận được vé tham gia Vòng đấu Tháng!#49", pPlayer.szName, nFloor);
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Friend, szFriendMsg, pPlayer.dwID);
			if pPlayer.dwKinId > 0 then
				local szKinMsg = string.format("Chúc mừng thành viên 「%s」 trong Thông Thiên Tháp xuất sắc leo lên tầng %s, nhận được vé tham gia Vòng đấu Tháng！#49", pPlayer.szName, nFloor);
