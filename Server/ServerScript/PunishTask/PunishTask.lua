...
function PunishTask:StartCreateMapNpc()
    ...
    if not nActivityId then
        TeamMgr:RegisterActivity("PunishTask", "PunishTask", "Nhiệm Vụ Trừng Ác", 
...
function PunishTask:CheckMapLevel(pPlayer, nMapLevel)
    if pPlayer.nLevel < nMapLevel then
        return false, string.format("Cấp chưa đủ %s", nMapLevel);
    end
...
function PunishTask:TeamCheckShow(nMapLevel)
    ...
    if me.nLevel < PunishTask.nMinPlayerLevel then
        return false, string.format("Cấp chưa đủ %s", PunishTask.nMinPlayerLevel);
    end    
...
function PunishTask:PlayerTeamCheckJoin(pPlayer, nMapLevel)
    ...
    if pPlayer.nLevel < PunishTask.nMinPlayerLevel then
        return false, string.format("Cấp chưa đủ %s，không thể Trừng ác", PunishTask.nMinPlayerLevel);
    end

    local bRet = Map:CheckEnterOtherMap(pPlayer);
    if not bRet then
        return false, "Không ở trong khu vực an toàn, không thể vào";
    end  
...
function PunishTask:CheckTeamEnterMap(pPlayer, nMapLevel)
    local bRet = Map:CheckEnterOtherMap(pPlayer);
    if not bRet then
        return false, string.format("「%s」không ở  khu vực an toàn, không thể vào", pPlayer.szName);
    end

    bRet = self:CheckMapLevel(pPlayer, nMapLevel);
    if not bRet then
        return false, string.format("「%s」 cấp chưa đủ", pPlayer.szName);
    end
...
function PunishTask:CheckTeamEnterState(pPlayer, nMapLevel)
    ...
    if nTeamId <= 0 then
        return false, "Không phải đội trưởng";
    end
    ...
    for _, nPlayerID in pairs(tbMember) do
        ...
        if not pMember then
            return false, "Có đội viên không online không thể tiến vào";
        end
    ...
    if pPlayer then
        for _, tbInfo in pairs(tbAward) do
            if tbInfo[1] == "item" and tbInfo[2] == self.nSpecialItem then
                local szTips = string.format("「%s」Thu được 【Manh mối bí cảnh】", pPlayer.szName);
