...
function tbNpc:SendPlayerAward(nPlayerID, tbNpcInfo, tbPunishTaskInfo, nCaptainID)
    ...
    if pPlayer.nLevel < PunishTask.nMinPlayerLevel then
        pPlayer.CenterMsg(string.format("Không đủ %s cấp", PunishTask.nMinPlayerLevel), true);
        return;
    end  
    ...
    pPlayer.Msg(string.format("Thưởng nhiệm vụ Trừng Ác còn %s lần", nShowCount));
    Dialog:SendBlackBoardMsg(pPlayer, string.format("Thưởng nhiệm vụ Trừng Ác còn %s lần", nShowCount));
    ...
    if nCaptainID ~= nPlayerID then
        ...
        pPlayer.CenterMsg("Thưởng đội trưởng thêm 20%");    
    end
