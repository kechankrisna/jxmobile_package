...
function tbCrossOuter:OnInnerNotify()
	...
	local szMsg = string.format("Long trụ dị biến, nương theo lấy đất rung núi chuyển, dường như có một mật đạo dần dần hình thành (%d Giây sau )", nLeftTime)
...
function tbCrossOuter:AddTransferRight(nKinId)
	...
	local szMsg = "Thành công chiếm lĩnh hai cây long trụ, Lâm An vương thành mật đạo đã đối bản bang phái mở ra, mời hoả tốc tiến về [FFFE0D]( Hạn chế tiến vào 40 Người )[-]"
...
function tbCrossOuter:OnInnerCityStart()
	...
	self:UpdateMiniMapInfo("ZY_Longzhu_Z", "Nội thành mật đạo")
	self:UpdateMiniMapInfo("ZY_Longzhu_B", "Nội thành mật đạo")
	self:UpdateMiniMapInfo("ZY_Longzhu_N", "Nội thành mật đạo")

	local szMsg = string.format("Long trụ vị trí xuất hiện thông hướng Lâm An nội thành mật đạo, mời hoả tốc tiến về")
	local function _InnerNotifyMsg(pPlayer)
		pPlayer.SendBlackBoardMsg(szMsg)
...
function tbCrossOuter:OnInnerCityEnd()
	...
	self:UpdateMiniMapInfo("ZY_Longzhu_Z", "Bên trong long trụ")
	self:UpdateMiniMapInfo("ZY_Longzhu_B", "Bắc long trụ")
	self:UpdateMiniMapInfo("ZY_Longzhu_N", "Nam long trụ")

	local function _NotifyMsg(pPlayer)
		pPlayer.SendBlackBoardMsg("Long trụ đã lại xuất hiện, trước mắt đều là không người chiếm lĩnh")
	end
...
function tbCrossOuter.tbTrapFun:ToKing(pPlayer, tbTrapInfo)
	...
	if not self:IsCanTranferToKing(tbPlayerInfo.nKinId) then
		pPlayer.SendBlackBoardMsg("Bản bang phái chưa thu hoạch được Lâm An vương thành tiến vào tư cách")
		return
	end
