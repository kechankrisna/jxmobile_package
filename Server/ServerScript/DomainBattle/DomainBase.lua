		...
		if fChangeFlag <= fP then
			if fP * nMaxLife < nOldHp then
				...
				if dwOldKinId and dwOldKinId ~= -1 then
					ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Bang hội %s——%s%s！", Map:GetMapName(self.nMapTemplateId), pNpc.szName, szDmgInfo), dwOldKinId);	
				end	
			end
			break;
		end
...
function tbBase:StartFight()
	...
	local fnNofiy = function (pPlayer)
		pPlayer.CenterMsg("Chiến đấu bắt đầu!", true)
...
function tbBase:StopFight()
	...
	local szMsg = dwWinId and string.format("Chúc mừng Bang Hội %s đã đồng tâm hiệp lực chiếm lĩnh được %s", szWinName, Map:GetMapName(self.nMapTemplateId)) or "本場無戰果" 
	local fnNofiy = function (pPlayer)
...
function tbBase:OnKillPlayer(pKiller, pDeader)
	...
	if nDomainBattleCombo % 10 == 0 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("「%s」trong Công Thành Chiến, liên trảm %s người!", pKiller.szName, nDomainBattleCombo), pKiller.dwKinId);	
	end
...
function tbBase:CreateCampNpc(nFlagIndex, dwKinId, nDelay)
	...
	local szTitle = string.format("「%s」chiếm lĩnh", tbKin.szName)
...
function tbBase:OnFlagDeath(him, pKiller, nFlagIndex)
	...
	DomainBattle:AddMapKinScore(self.nMapId, dwKillKinId, define.nKillFlagAddScore, string.format("Phá hủy %s nhận được %d điểm!", him.szName, define.nKillFlagAddScore))

	local tbFlagState = self.tbFlagState[nFlagIndex]
	local dwOldKinId = tbFlagState[1]
	if dwOldKinId and dwOldKinId ~= -1 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Bang hội %s-%s tổn thất nghiêm trọng!", Map:GetMapName(self.nMapTemplateId), him.szName ), dwOldKinId);	
	end

	self:CreateCampNpc(nFlagIndex, dwKillKinId, Env.GAME_FPS * 5)
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Bang hội đã chiếm %s-%s!", Map:GetMapName(self.nMapTemplateId), him.szName ), dwKillKinId);
end
...
function tbBase:OnDoorDeath(him, pKiller, nDoorIndex)
	...
	self.tbMapShowInfo[szDoorNpc] = "Kích phá"
	OpenDynamicObstacle(self.nMapId, szObstacle);
	self.tbInvalidTraps[szTrapName] = 1;
	self:BlackMsg(szGateName .. "Đã bị sụp đổ!")
end
	...
	if nKillNpcAddScore then
		if pKiller then
			...
			if pPlayer then
				...
				pPlayer.CenterMsg(string.format("%s bị hủy thu được %d điểm!", him.szName, nKillNpcAddScore))
			end
		end
	end
...
function tbBase:UseSupplys(pPlayer, nItemId)
	...
	if not tbBattleSupply[nItemId] or tbBattleSupply[nItemId] <= 0 then
		pPlayer.CenterMsg("Đạo cụ không có trong tồn kho")
...
function tbBase:UseItemCallDialogNpc(pPlayer, nNpcId, nDir, bIsVehicles, szItemName)
	...
	for i, pNpc in ipairs(tbNpcList) do
		if pNpc.szClass == "DomainBattleChange" then
			return false, "Khu vực gần đã có cùng đạo cụ"
		end
	end
	if bIsVehicles then --如果是攻城车
		if DomainBattle:GetMapLevel(self.nMapTemplateId) == 3 then
			return false, "Công Thành Xa không được sử dụng ngoài khu vực lãnh địa"
		end
		if pPlayer.dwKinId == self.nOwnerKin then
			return false, "Không thể dùng Công Thành Xa"
		end
	end

	if pPlayer.nFightMode ~= 0 then
		return false, "Chỉ sử dụng được ở doanh trại"
	end
	...
	if pNpc then
		...
		return true,  string.format("Sử dụng thành công, xuất hiện một「%s」, hãy kích vào để biến thân", szItemName)
	end
end
...
function tbBase:UseItemCallAttackNpc(pPlayer, nNpcId, nDir, szNpcName, nKillNpcAddScore)
	...
	if pPlayer.nFightMode ~= 1 then
		return false, "Chỉ sử dụng ở khu vực chiến đấu"
	end
	...
	if pNpc then
		...
		return true, string.format("Sử dụng thành công, xuất hiện một「%s」!", szNpcName)
	end
end
...
function tbBase:OnSelectNpcDialog(pPlayer, nNpcId, nChangeSkillId, szText, nDuraSeconds)
	if pPlayer.GetNpc().nShapeShiftNpcTID ~= 0 then
		pPlayer.CenterMsg("Ngươi hiện nay đã biến thân")
		return
	end
	...
	if pPlayer.nFightMode ~= 0 then
		pPlayer.CenterMsg("Chỉ sử dụng được ở doanh trại")
		return
	end
	...
	if nChangeSkillId then
		...
		pPlayer.CenterMsg(string.format("Biến thân thành công thành「%s」", szText))
