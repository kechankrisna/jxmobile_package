...
function tbCross:LeaveRequest(pPlayer)
	...
	if pPlayer.nFightMode  == 1 then
		GeneralProcess:StartProcess(pPlayer, 5 * Env.GAME_FPS, "Truyền tống bên trong...", _Leave, pPlayer.dwID);
		return
	end
...
function tbCross:UseSupplysReq(pPlayer, nItemId)
	...
	if nLimitNum and tbKinInfo.tbUsedSupply[nItemId] >= nLimitNum then
		pPlayer.CenterMsg(string.format("Nên đạo cụ sử dụng số lần đã đạt bổn tràng hạn mức cao nhất %d Lần", nLimitNum), true)
		return
	end
