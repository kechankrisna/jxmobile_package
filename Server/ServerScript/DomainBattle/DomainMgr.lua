...
function DomainBattle:StartDeclareWar()
	...
	KPlayer.SendWorldNotify(self.define.nMinLevel, 999, "Công Thành Chiến bước vào giai đoạn tuyên chiến! Các đại hiệp có thể tuyên chiến, hãy chọn một mục tiêu để tuyên chiến.", 1, 1)
...
function DomainBattle:NotifyKinDomainWar()
	...
	local fnNotifyKin = function (kinData, timeVal)
		...
		if not self:GetKinWarDeclare()[nKinId] then
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Công Thành Chiến sẽ bắt đầu sau %d phút, bang hội vẫn chưa tuyên chiến, Bang Chủ hoặc Bang Phó mau tuyên chiến!", math.ceil(timeVal / 60) ), nKinId);
		end
	end
...
function DomainBattle:NotifyEndDomainWar()
	KPlayer.SendWorldNotify(self.define.nMinLevel, 999, "Công Thành Chiến sẽ bắt đầu sau 10 phút, yêu cầu các Bang Hội mau đi tuyên chiến, hãy chọn một mục tiêu để tuyên chiến.", 1, 1)
end
	...
	for dwKinId, tbInfo in pairs(self.tbKinBattleReport) do
		...
		for i, v in ipairs(tbRankAward) do
			...
			for nRank = nLastRankEnd + 1, v.nRankEnd do
				...
				{
					To 	  		= dwRoleId,
					Title 		= "Thưởng Công Thành Chiến",
					Text        = string.format("ông Thành Chiến kết thúc, điểm bang hội đạt hạng %d, hãy nhận thưởng đính kèm", nRank),
					tbAttach 	= v.Award,
...
function DomainBattle:SendCityMasterAward()
	...
	if nLeaderId and nLeaderId ~= 0 then
		...
		{
			To 	  		= nLeaderId,
			Title 		= "ương Dương Thành Chủ",
			Text        = "Ngươi thật anh minh thần vũ lãnh đạo, bang hội của ngươi đã chống chọi quyết liệt, đẫm máu chiến đấu, cuối cùng đã chiếm được thành Tương Dương, nhận được vinh dự thắng lợi, ngươi cũng đã trở thành Thành Chủ Tương Dương mới, đặc biệt nhận được phần thưởng đặc sắc nhất, hãy bày tỏ tâm ý.",
...
function DomainBattle:StopActivity(nMapTemplateId)
	...
	local szNews = "Công Thành Chiến đã kết thúc";
	local tbMapOwner = self:GetDomainMapOwner()
	...
	if tbKinInfo then
		szNews = szNews .. string.format(", chúc mừng bang hội「%s」đã chiếm được thành Tương Dương!", tbKinInfo.szName) 
	end
...
function DomainBattle:KinSignUp(pPlayer, nMapTemplateId)
	if not self.bCanDeclarerWar then
		pPlayer.CenterMsg("Hiện nay không thể tuyên chiến")
		return
	end
	local dwKinId = pPlayer.dwKinId
	if dwKinId == 0 then
	...
	if not define.tbCanDeclareCareer[nCareer] then
		pPlayer.CenterMsg("Ngươi không phải là Bang Chủ hoặc Bang Phó, không thể nào tuyên chiến")
		return
	end
	...
	if nHasOwnerMapId then
		...
		if nAutoFightMap then
			pPlayer.CenterMsg("Bang hội đã tự động tuyên chiến" .. Map:GetMapName(nAutoFightMap))
			return
		else
			pPlayer.CenterMsg("Bang hội bây giờ chỉ cần phòng thủ")
			return
		end
	end
	...
	if self:GetKinWarDeclare()[dwKinId] == nMapTemplateId then
		pPlayer.CenterMsg("Đã tuyên chiến khu vực này rồi")
		return
	end

	if nMapTemplateId == self.tbMapSetting.nMapTemplateId then
		pPlayer.CenterMsg("Bang hội phải chiếm cứ điểm Thôn Trấn mới có thể tuyên chiến thành Tương Dương", true)
		return
	end

	self:SetData(dwKinId, self:GetKinWarDeclare(), dwKinId, nMapTemplateId)
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Bang hội đã thay đổi tuyên chiến「%s」, hãy chiếm lấy mục tiêu.",Map:GetMapName(nMapTemplateId)), dwKinId);
...
function DomainBattle:PlayerSignUp(pPlayer)
	if not Env:CheckSystemSwitch(pPlayer, Env.SW_SwitchMap) then
		pPlayer.CenterMsg("Trạng thái hiện tại không thể tham gia bản đồ")
		return
	end
	local dwKinId = pPlayer.dwKinId
	if dwKinId == 0 then
	...
	if not tbFifhtMapId or not self.nActivyStartTime then
		pPlayer.CenterMsg("Hiện tại không thể tham chiến")
		return
	end

	if pPlayer.nLevel < self.define.nMinLevel then
		pPlayer.CenterMsg(string.format("Cấp %d mới có thể tham gia", self.define.nMinLevel))
		return
	end
	...
	if tbMember:GetJoinTime() > self.nActivyStartTime then
		pPlayer.CenterMsg("Ngươi mới tham gia vào Bang Hội, hãy đợi trận sau tham chiến")
		return
	end

	local nGotoWarMapId = self:GetDomainOwnerMap()[dwKinId] or self:GetKinWarDeclare()[dwKinId]
	if not nGotoWarMapId then
		pPlayer.CenterMsg("Bang hội không có mục tiêu tuyên chiến")
		return
	end
	local nMapId = tbFifhtMapId[nGotoWarMapId]
	if not nMapId then
		pPlayer.CenterMsg("Hãy thử lại sau")
		return
	end

	if not AsyncBattle:CanStartAsyncBattle(pPlayer)  then
		pPlayer.CenterMsg("Hãy trở về khu an toàn tham gia hoạt động")
		return
	end
...
function DomainBattle:RequestLeave(pPlayer)
	...
	if nSchedulePos ~= 3 then
		if pPlayer.nFightMode  == 1 then
			GeneralProcess:StartProcess(pPlayer, 5 * Env.GAME_FPS, "Đang truyền tống...", self.SwitchMapDirectly, self);
			return
		end
...
function DomainBattle:GotoAttackMap(pPlayer)
	...
	if self:GetDomainMapOwner()[nMapTemplateId] ~= pPlayer.dwKinId then
		pPlayer.CenterMsg( string.format("Chiếm lĩnh %s mới được truyền tống", Map:GetMapName(nMapTemplateId)) )
		return
	end
	if not self.tbFifhtMapId then
		return
...
function DomainBattle:GotoDefendMap(pPlayer, nCampIndex)
	...
	if self:GetDomainMapOwner()[nGotoMapTemplateId] ~= pPlayer.dwKinId then
		pPlayer.CenterMsg(string.format("Chiếm lĩnh %s mới được truyền tống", Map:GetMapName(nGotoMapTemplateId)) )
		return
	end
	if not self.tbFifhtMapId then
		return
...
function DomainBattle:UseSupplys(pPlayer, nItemId)
	...
	local tbInst = self.tbMapInst[nMapId]
	if not tbInst then
		pPlayer.CenterMsg("Chỉ sử dụng ở Công Thành Chiến")
		return
	end
	...
	if not self.tbKinUseSpplyTimes then
		pPlayer.CenterMsg("Hiện tại không thể sử dụng đạo cụ Công Thành")
		return
	end
	...
	if nLimitNum and tbLimit[nItemId] >= nLimitNum then
		pPlayer.CenterMsg(string.format("Đạo cụ đã sử dụng %d lần quá mức được cho phép", nLimitNum), true)
		return
	end
...
function DomainBattle:SelectCamp(pPlayer, nIndex)
	...
	if not nDeclareMapId then
		pPlayer.CenterMsg("Không thể đổi doanh địa chinh chiến")
		return
	end
	...
	if not self.define.tbCanUseItemCareer[nCareer] then
		pPlayer.CenterMsg("Ngươi không đủ quyền hạn")
		return
	end

	local tbInfo = DomainBattle:GetMapSetting(nDeclareMapId)
	if not tbInfo.tbAtackPos[nIndex] then
		pPlayer.CenterMsg("Không có doanh địa chinh chiến")
		return
	end
	...
	if nIndex == tbInst.tbKinAttackCampIndex[dwKinId] then
		pPlayer.CenterMsg("Doanh địa chinh chiến giống nhau!")
		return
	end
	...
	pPlayer.CenterMsg(string.format("Bang hội đổi mục tiêu tiến công doanh địa chinh chiến thành「%s」!", tbInfo.tbAtackPos[nIndex][4]), true)
end
...
function DomainBattle:GetActionAwardFromParam(tbFlagAwardSetting, nSocre, dwKinId)
	...
	local fnGetRandomAward = function (nItemId, nCount, tbItems)
		for i=1,nCount do
			...
			local tbAllAward;
			if szClass == "RandomItemByTimeFrame" then
				local nRet, szMsg, tbTempAllAward = Item:GetClass(szClass):GetAwardListByTimeFrame(nParamId, "Lãnh địa hành thương")
				tbAllAward = tbTempAllAward;
			elseif szClass == "RandomItemByLevel" then
				local nRet, szMsg, tbTempAllAward = Item:GetClass(szClass):GetAwardListByLevel(GetMaxLevel(), nParamId, "Lãnh địa hành thương")
				tbAllAward = tbTempAllAward;
			else
...
function DomainBattle:NotifyBattleOpen()
	KPlayer.SendWorldNotify(self.define.nMinLevel, 999, "Công Thành Chiến đã bắt đầu, các thành viên hãy tham gia tiến nhập tham chiến, chiếm lấy lãnh địa!", 1, 1)
