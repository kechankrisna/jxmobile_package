...
function tbCrossInner:OnInnerNotify()
	...
	local szMsg = string.format("Lâm An nội thành đem với %d Giây sau đổ sụp", nLeftTime)
	...
	if nLeftTime <= 10 then
		local function _NotifyMsg(pPlayer)
			pPlayer.SendBlackBoardMsg("Ngoại thành long trụ đã lại xuất hiện, trước mắt đều là không người chiếm lĩnh")
		end
