...
function tbCrossWithThrone:OccupyThroneReq(pPlayer)
	if self.nOccupyThronePlayerId or not self:IsCanOccupyThrone() then
		pPlayer.SendBlackBoardMsg("Lâm An trong vương thành long trụ đều bị chiếm cứ sau, mở ra vương tọa tranh đoạt")
		return
	end
	...
	if not pPlayerNpc or pPlayerNpc.nShapeShiftNpcTID ~= 0 then
		pPlayer.CenterMsg("Ở vào biến thân trạng thái lúc không thể thao tác")
		return
	end
...
function tbCrossWithThrone:CheckCanLeave(pPlayer)
	if self.nOccupyThronePlayerId == pPlayer.dwID then
		pPlayer.SendBlackBoardMsg("Trước mắt trạng thái không thể rời đi")
		return false
	end
	return true;
end
...
function tbCrossWithThrone.tbOnSpawnNpcFun:cross_domain_throne_fight(tbNpcInfo, pNpc, nKinId, nPlayerId)
	...
	pNpc.SetName(string.format("「%s」Chiếm lĩnh %s", pPlayer.szName, szNpcName))
	...
	if nKinId > 0 then
		tbCross:CallZoneClientScriptByKinId(nKinId, "ChatMgr:SendSystemMsg", ChatMgr.SystemMsgType.Kin,
						string.format("Bang phái thành viên「%s」Thành công chiếm cứ %s——%s!", pPlayer.szName, self.szName, szNpcName), tbKinInfo.nOrgKinId);
	end
end
