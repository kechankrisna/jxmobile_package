...
NewPackageGift.MAIL_CONTENT    = "    %d năm %d tháng %d hào trước sáng tạo tạm chờ cấp tại %d Cấp trở lên nhân vật, như tiến đến [c8ff00] [url=openNewPackageUrl:Tải phiên bản mới, %s, %s][-]，Liền đem thu hoạch được [FFFE0D]%s[-] Ban thưởng. Thiên hạ võ công, duy khoái bất phá. Giờ phút này không đi download, chờ đến khi nào!"
...
function NewPackageGift:TryGainGift(nVersion)
    ...
    if not bCanGain then
        me.CenterMsg("Sự kiện đã hết hạn hoặc đã được nhận")
        return
    end

    local tbData = ScriptData:GetValue(self.MAIN_KEY)
    if tbData.nVersion ~= nVersion then
        me.CenterMsg("Phiên bản này không thể tham gia vào sự kiện")
        return
    end
