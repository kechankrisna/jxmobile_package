...
function FriendRecall:AddRenownAward(nPlayerId, szType)
	...
	if nSaveKey then
		...
		if nAwardCount >= self.Def.MAX_RENOWN_COUNT_WEEKLY then
			pPlayer.CenterMsg(XT("Tuần này thông qua này hoạt động thu hoạch được người chơi triệu hồi ngoài định mức danh vọng đã đạt hạn mức cao nhất "));
			return
		end
	...
	if nAddValue < nValue then
		pPlayer.CenterMsg(string.format(XT("Tuần này thông qua người chơi triệu hồi thu hoạch được ngoài định mức danh vọng đã đạt %d hạn mức cao nhất "), self.Def.MAX_RENOWN_WEEKLY));
	end
end
...
function FriendRecall:SendRecalledMail(pPlayer, tbList)
	...
	local szContent = string.format(XT([[Hiệp sĩ %s Đã một lần nữa trở lại đặc sắc xuất hiện giang hồ, các hạ cùng hắn quen biết đã lâu, ăn ý còn tại, chỉ cần hai người tổ đội tham gia đặc biệt hoạt động, đem thu hoạch được [FFFE0D] Độ thân mật thu hoạch gia tăng 100%%\n Dã ngoại đánh quái kinh nghiệm tăng thêm +5%%, lực công kích +70, kháng tất cả +70, sinh mệnh +1500[-] Tăng thêm trạng thái, [FFFE0D] Tổ đội bí cảnh, lăng tuyệt phong, sơn tặc bí quật, trừng phạt ác nhiệm vụ, Thông Thiên tháp, tâm ma huyễn cảnh [-] Các hạ còn đem ngoài định mức thu hoạch được danh vọng ban thưởng, thời hạn có hiệu lực [FFFE0D]30[-] Trời ]]), pPlayer.szName)

	local tbMail = {Title = "Trở Lại Giang Hồ", From = "Độc Cô Kiếm ", Text = szContent}
