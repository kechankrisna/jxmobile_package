...
function Pray:CanPray(pPlayer)
	if DegreeCtrl:GetDegree(pPlayer, "Pray") < 1 then
		return false, "Còn thừa số lần không đủ";
	end

	if self:IsEndWuxing(pPlayer) then
		return false, "Nhận phần thưởng trước khi bạn có thể tiếp tục cầu nguyện";
	end

	if not self:IsLevelEnough(pPlayer) then
		return false, string.format("Cấp %d mới có thể cầu nguyện", self.PRAY_OPEN_LEVEL);
	end
...
function Pray:GainReward(pPlayer)
	...
	pPlayer.CenterMsg("Lĩnh thưởng thành công");
	self:Sync(pPlayer);
end
