...
function RankActivity:SendGuideMail(nNewLevel)
	...
	if me.nLevel == self.nBindingPhoneGuideLevel then
		local tbMail = {
			To = me.dwID;
			Title = "Khóa lại điện thoại nhắc nhở";
			From = "系統";
...
function LevelRankActivity:OnPlayerLevelup(nNewLevel)
	...
	if me.nLevel == RankActivity.LEVEL_RANK_REWARD_LEVEL then
		...
		local szTitle = "Phần thương hạng cấp"
		local szText  = string.format("Chúc mừng hiệp sĩ ở hoạt động đua hạng cấp là người đầu tiên đạt tới %d cấp! Đây là phần thưởng khích lệ tặng cho hiệp sĩ tham dự lần này!",RankActivity.LEVEL_RANK_REWARD_LEVEL)
			...
			if tbRankList then
				for nRank,tbPlayerInfo in ipairs(tbRankList) do
					...
					if tbReward and next(tbReward) then
						local szTitle = "Đua Hạng Chiến Lực"
						local szText  = "Chúc mừng hiệp sĩ ở hoạt động đua hạng chiến lực đã là người dẫn đầu bảng! Đây là phần thưởng khích lệ tặng cho hiệp sĩ tham dự lần này!"
						...
						if pPlayerStay then
							...
							tbPlayerInfo.nPlayerID = nPlayerID
							tbPlayerInfo.szName = pPlayerStay.szName or XT("Không")
							tbPlayerInfo.nFaction = pPlayerStay.nFaction
							tbPlayerInfo.szKinName = pKinData.szName or "-"
