...
function WuLinDaShi:OnStartUp()
    self.nActivityId = TeamMgr:RegisterActivity("WuLinDaShiFuben", "WuLinDaShiFuben", "Long Đàm Hổ Huyệt ",
...
function WuLinDaShi:CheckEnter(pPlayer)
    if not self:GetMapCurInfo() then
        return false, "Không có trong thời gian hoạt động"
    end

    local nHour = Lib:GetLocalDayHour()
    if nHour >= self.DAYEND and nHour < self.DAYBEGIN then
        return false, "Quân Kim tạm hoãn công thành, thiếu hiệp trước tạm thời nghỉ ngơi một chút đi!"
    end

    if not self:GetCycleTask(pPlayer) then
        return false, "Thiếu hiệp trên thân không có nhiệm vụ!"
    end
...
function WuLinDaShi:CheckPlayerCanEnterFuben(pPlayer)
    if pPlayer.nLevel < self.FUBEN_MIN_LEVEL then
        return false, string.format("Cấp bậc chưa đủ %d, không cách nào tham gia ", self.FUBEN_MIN_LEVEL)
    end

    if not Env:CheckSystemSwitch(pPlayer, Env.SW_SwitchMap) then
        return false, "Tình trạng hiện tại không được phép tham gia"
    end
    ...
    if not Fuben.tbSafeMap[pPlayer.nMapTemplateId] and Map:GetClassDesc(pPlayer.nMapTemplateId) ~= "fight" then
        return false, "Bản đồ hiện tại không thể vào phó bản"
    end

    if Map:GetClassDesc(pPlayer.nMapTemplateId) == "fight" and pPlayer.nFightMode ~= 0 then
        return false, "Không ở trong khu vực an toàn, không được phép vào"
    end
...
function WuLinDaShi:CheckCanCreateFuben(pPlayer)
    ...
    if teamData and teamData.nCaptainID ~= pPlayer.dwID then
        return false, "Chỉ có đội trưởng có thể mở phó bản!"
    end

    if #tbMember < self.MIN_PLAYER_COUNT then
        return false, string.format("Số lượng thành viên không đủ %d người, không thể vào phó bản!", self.MIN_PLAYER_COUNT), tbMember
    end

    if #tbMember > self.MAX_PLAYER_COUNT then
        return false, string.format("Số lượng thành viên quá %d người, không thể vào phó bản!", self.MAX_PLAYER_COUNT), tbMember
    end

    for _, nPlayerId in pairs(tbMember) do
        local pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
        if not pPlayer then
            return false, "Thành viên nhóm không xác định, không thể mở phó bản!", tbMember
        end
...
function WuLinDaShi:CreateFuben(nPlayerId)
    ...
    if not self:CheckFubenTask(pPlayer) then
        pPlayer.CenterMsg("Không có nhiệm vụ tương ứng")
        return
    end
...
function WuLinDaShi:TrySetTarget()
    if not self.nActivityId then
        me.CenterMsg("Không tìm được hoạt động")
        return
    end
    ...
    if not teamData then
        ...
        if teamData:IsCaptain(me.dwID) then
            ...
            me.CenterMsg("Không phải đội trưởng, không thể tham gia hoạt động")
        end
    end
end
...
tbMap.tbTrapMsg =
{
    tips1 = "Cuộc chiến này có liên quan đến sự sống còn của Đại Tống, cổng thành không được phép mở, chàng trai trẻ có thể vào thành phố bằng con đường bí mật!",
    tips2 = "Cuộc chiến này có liên quan đến sự sống còn của Đại Tống, cổng thành không được phép mở, chàng trai trẻ có thể ra khỏi thành phố bằng con đường bí mật!",
}
