        ...
        for i,v in ipairs(self.tbRankAward) do
            for nPos = nFrom, v.nRankEnd do
                ...
                if tbInfo then
                    Mail:SendSystemMail({
                        To = tbInfo.dwUnitID,
                        Title = "Phần thưởng xếp hạng",
                        Text = string.format("Đại hiệp, lượt này chúc phúc hoạt động ngươi thu được đến từ hảo hữu %s Chúc phúc giá trị, xếp hạng %d, nhân khí coi như không tệ a! Đây là ngài ban thưởng, mời kiểm tra và nhận.", tbInfo.szValue, nPos),
                        tbAttach = { v.tbAward },
                    });
...
function SendBless:SendBless(pPlayer, dwRoleId2, bCostGold, szWord)
    ...
    if not FriendShip:IsFriend(dwRoleId1, dwRoleId2) then
        pPlayer.CenterMsg("Không phải hảo hữu ")
        return
    end
...
function SendBless.SendBlessCallBack(dwRoleId1, bSucceed, szBillNo, dwRoleId2, bUseGold, szWord)
    ...
    if not pPlayer2 then
        pPlayer.CenterMsg("Đối phương offline và không thể gửi lời chúc phúc!")
        return false
    end

    if pPlayer2.nLevel < SendBless.nMinLevel then
        pPlayer.CenterMsg("Đối phương cấp bậc chưa đủ" .. SendBless.nMinLevel)
        return false
    end
