...
function Battle:AssignTeam(tbPlayers, nMapId, nReadyMapIndex)
    ...
    for _, pPlayer in ipairs(tbPlayers) do
    	...
    	pPlayer.CenterMsg(string.format("Ngươi gia nhập %s", tbTeamNames[nTeamIndex]))
    end
...
function Battle:CheckPlayerCanInvite(dwRoleId)
	...
	if not self.LegalMap[pPlayer.nMapTemplateId] then
		pPlayer.CenterMsg("Ngài trước mắt không cách nào bị truyền vào chiến trường địa đồ, đã rời khỏi chờ đợi hàng")
		return
	end
...
function Battle:SignUp(pPlayer, bSkipZone)
	...
	if not tbBattleSetting then
		pPlayer.CenterMsg("Hiện tại không có chiến trường")
		return
	end
	...
	if pPlayer.nLevel < tbCurBattleSetting.nMinLevel then
		pPlayer.CenterMsg(string.format("Cần %d Cấp mới có thể tham gia", tbCurBattleSetting.nMinLevel))
		return
	end
...
function Battle:OnAllReadyMapCreateEvent()
	local tbSetting = self.tbCurBattleSetting
    assert(tbSetting)
   	local szBattleName = "Tống Kim đại chiến";
   	if tbSetting.bShowName then
   		szBattleName = tbSetting.szName
   	end
    local szMsg = string.format("%s Hết sức căng thẳng, chúng hiệp sĩ mời hoả tốc báo danh gia nhập!", szBattleName) 
        ...
        for nSettingIndex, _ in pairs(self.tbNowOpenZoneSetting) do
            ...
            if tbBattleSetting.bZone then
                if tbBattleSetting.nShowLevel > nLastLevel and ( not tbBattleSetting.nRealNeedLevel or  GetMaxLevel() >= tbBattleSetting.nRealNeedLevel)  then
                    szMsg = string.format("%s Sắp mở ra, chúng hiệp sĩ mời hoả tốc báo danh tham chiến!", tbBattleSetting.szName) 
                end
            end
				...
				if tbBattleInfo.nRank <= tbBattleSetting.nQualifyMinRank then
					...
					if tbQUalifySetting and GetTimeFrameState(tbQUalifySetting.OpenTimeFrame) == 1 and tbQUalifySetting.ChechConditionFunc then
						...
						if not bNowCanInNext then
							...
							local szNotifyMsg = string.format("Chúc mừng hiệp sĩ「%s」Tham gia %s Tiến vào trước %d Tên, thành công thu hoạch được võ lâm tranh đấu thi đấu %s Tư cách.", pPlayer.szName, tbBattleSetting.szName, tbBattleSetting.nQualifyMinRank, tbQUalifySetting.szName)
...
function Battle:OnGetBattleFirstEnven(dwRoleId, szLogicClass, nReadyMapIndex)
	...
	if pRole and tbBattleSetting then
		...
		if dwKinId ~= 0 then
			...
			if tbKin then
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("Chúc mừng bản bang phái「%s」Thu được %s Hạng nhất!", pRole.szName, tbBattleSetting.szName), dwKinId);
		...
		if tbBattleSetting.bZone then
			...
			if tbZoneLevelSetting then
				if tbZoneLevelSetting.nLevelEnd >= GetMaxLevel() then
					KPlayer.SendWorldNotify(1, 999,  string.format("Chúc mừng「%s」Dũng đoạt %s Hạng nhất!", pRole.szName, tbBattleSetting.szName), 1, 1)
				end
			end
		end
