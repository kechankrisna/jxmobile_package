...
function tbBase:StartFight()
	...
	local fnNofiy = function (pPlayer)
		pPlayer.CenterMsg("Chiến đấu bắt đầu! Mời các vị tiến về tiền tuyến")
...
function tbBase:OnPlayerTrap(szTrapName)
	if self.nBattleOpen ~= 1 then
		me.CenterMsg("Chiến đấu còn không có khai hỏa a, xin chờ chốc lát")
		return
	end
...
function tbBase:OnKillPlayer(pPlayerNpc, pDeader, nComboLevel)
	...
	self.nFirstBloodPlayerId = true
		self:BattleFieldTips(string.format("%s Đánh bại %s, cầm tới First Blood", pPlayerNpc.szName, pDeader.szName))
	end
...
function tbBase:OnBossNpcDeath(him, pKiller)
	...
	self:BattleFieldTips(string.format("%s Thành công bắt được %s, sắp gia nhập chiến trường", self.tbBattleSetting.tbTeamNames[nKillSide], him.szName))
end
...
function tbBase:OnBuildNpcDeath(him, pKiller)
	...
	if not next(self.tbAllBuildNpc[nTeam]) then
		self:BattleFieldTips(string.format("%s Đại doanh bị phá hủy", self.tbBattleSetting.tbTeamNames[nTeam]) );	
	...
	self:BattleFieldTips(string.format("%s Một tòa tháp phòng ngự bị phá hủy", self.tbBattleSetting.tbTeamNames[nTeam]) );	
...
function tbBase:AddBossNpc(nX, nY, nDir)
	...
	self:BlackMsg(string.format("%s Hiện thân chiến trường, đánh giết một phương có thể đạt được hiệp trợ", pNpc.szName))
end
