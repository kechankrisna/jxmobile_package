...
function ImperialTomb:Open()
	...
	if nOpenServerDay == nToday then
		local tbMail =
		{
			Title = 'Tặng Dạ Minh Châu',
			Text = [[    Tần Thủy Hoàng lăng nặng hiện hậu thế ，cổ tần sĩ tốt nổi lên bốn phía thương nhân ，các vị sinh hoàng lăng xuất hiện thì chờ cấp dĩ đạt 60cấp ，đặc biệt tặng [FFFE0D]「Dạ Minh Châu 」[-]nhất khỏa ，vật ấy có tị độc công hiệu ，thủy hoàng đến trái đất thì ，chỉ có huề vật ấy lại vừa đi trước hoàng lăng 。hoàn ngắm chư vị có thể trừ thử họa hại ，hoàn giang hồ bình tĩnh. nếu nhưng không hề mổ ở chỗ ，nhưng thông qua phía dưới liên kết đi trước kiểm tra thuyết minh.
                                     Kiểm tra hoạt động: [47f005][url=openwnd:Mật Thất Tần Lăng, CalendarPanel, 1, 37][-]
                                     Kiểm tra hoạt động: [47f005][url=openwnd:Tần Thủy Hoàng Giáng Thế, CalendarPanel, 1, 38][-] ]],
			From = 'Độc Cô Kiếm',
...
function ImperialTomb:EnterTomb()
	...
	if not Env:CheckSystemSwitch(me, Env.SW_SwitchMap) then
		me.CenterMsg("Trạng thái hiện tại không thể vào bản đồ")
...
function ImperialTomb:CheckEnterEmperorRoom(pPlayer, nIndex)
	if GetTimeFrameState(self.OPEN_TIME_FRAME) ~= 1 then
		return false, XT("Hoạt động chưa mở");
	end
	...
	if pPlayer.nMapTemplateId ~= self.MAP_TEMPLATE_ID[self.MAP_TYPE.THIRD_FLOOR] and pPlayer.nMapTemplateId ~= self.MAP_TEMPLATE_ID[self.MAP_TYPE.FEMALE_EMPEROR_FLOOR] then
		return false, "Bản đồ hiện tại không thể vào";
	end
...
function ImperialTomb:CheckStayTime(pPlayer)
	...
	if nLeftTime and nLeftTime <= 0 then
		pPlayer.Msg(XT("Hoàng lăng cực độc, ngươi không thể nào chống chọi được nữa, ngươi tạm rời khỏi tần lăng trị thương"))
		pPlayer.Revive(1);
		pPlayer.GotoEntryPoint();
	end
end
...
function ImperialTomb:SwitchMapDirectly(nMapId, nMapTemplateId)
	...
	return false, "Không thể di chuyển bản đồ này"
end
...
function ImperialTomb:OnPlayerDeath(pKiller)
	if self:IsEmperorMirrorMapByTemplate(me.nMapTemplateId) or self:IsFemaleEmperorMirrorMapByTemplate(me.nMapTemplateId) then
		...
		me.Msg(string.format(XT("%d giây sau hồi sinh", nTime)))
	end
end
