function ImperialTomb:SendSecretInvite(nMapType, pPlayer, szTitle)
	...
	if pPlayer and pPlayer.dwTeamID > 0 then
		...
		if tbTeam and tbTeam:GetCaptainId() == pPlayer.dwID then
			ChatMgr:SendTeamOrSysMsg(pPlayer, string.format("Tại [FFFE0D]%s[-] trên thân phát hiện một phần thông hướng < Tần Thủy Hoàng lăng mật thất > Bản đồ, bản vẽ ngay tại dần dần biến mất, [FFFE0D]1 Phút [-] Sau đem không cách nào tiến vào, phải chăng lập tức khởi hành tiến về?", szTitle), 
...
function ImperialTomb:SecretEnterRequest(pPlayer, nType)
	...
	if not tbPlayerList or not tbPlayerList[nPlayerId] then
		return false, "Bản đồ không thể nào truyền tống"
	end

	if math.abs(GetTime() - tbPlayerList[nPlayerId]) > self.SECRET_ROOM_INVITE_TIME_OUT then
		pPlayer.CenterMsg(XT("Thời gian gia nhập mật thất đã hết, xem ra ngươi chậm một bước, lần sau hãy thử lại!"))
		tbPlayerList[nPlayerId] = nil
		return false, "Bản đồ không thể nào truyền tống"
	end

	if self:GetSecretPlayerCount(nType) >= self.SECRET_ROOM_MAX_PLAYER then
		pPlayer.CenterMsg(XT("Mật thất thông lộ đã phong tỏa, chỉ sợ là tiến về nhân số quá nhiều, dẫn đến con đường phong bế"))
		return false, "Bản đồ không thể nào truyền tống"
	end
