...
function ImperialTomb:OnBossDeath(pNpc, pKiller)
	...
	if szKinName and szPlayerName then
		szMsg = string.format(XT("Bang hội「%s」-「%s」đã tiêu diệt %s", szKinName, szPlayerName, szNpcName))
	elseif szPlayerName then
		szMsg = string.format(XT("Người chơi「%s」đã tiêu diệt %s", szPlayerName, szNpcName))
	end
	...
	for nMapType,tbMapInfo in pairs(self.tbNormalMapList) do
		...
		if tbTrapList then
			for _,tbPos in pairs(tbTrapList) do
				self:_AddBossTrapNpc(tbMapInfo.nMapTemplateId, tbMapInfo.nMapId, tbPos[1], tbPos[2], nTrapId, nTrapIndex, XT("Đến Hộ Chính Điện"))
				nTrapId = nTrapId + 1;
...
function ImperialTomb:CallFemaleEmperorBossTrapNpc()
	...
	for _,tbTrapList in ipairs(self.FEMALE_EMPEROR_BOSS_ROOM_TRAP_POS) do
		...
		self:_AddBossTrapNpc(tbMapInfo.nMapTemplateId, tbMapInfo.nMapId, tbPos1[1], tbPos1[2], tbPos1[3], nTrapIndex, XT("Đến Vĩnh Sinh Đài"))
		nTrapIndex = nTrapIndex + 1;

		--有2个对应的区域是2个trap点 其他1个
		if tbSelectZone1 == tbTrapList or tbSelectZone2 == tbTrapList then
			self:_AddBossTrapNpc(tbMapInfo.nMapTemplateId, tbMapInfo.nMapId, tbPos2[1], tbPos2[2], tbPos2[3], nTrapIndex, XT("Đến Vĩnh Sinh Đài"))
			nTrapIndex = nTrapIndex + 1;
		end
	end
end
