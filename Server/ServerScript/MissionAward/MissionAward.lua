...
function MissionAward:GetAwardListList(pPlayer)
	...
	for nType, tbData in pairs(tbAllData) do
		...
		for nId, tbRecord in ipairs(tbData) do
			local szName = tbRecord.szName or self.tbMissionName[nType] or "Chưa lĩnh thưởng";
...
function MissionAward:AddAwardIdx(pPlayer, nType, nRecordId)
	...
	if tbRecord.nCurIdx > nMaxCount then
		pPlayer.CenterMsg("Hiện tại đã hết số lần rút thưởng, không thể thêm được nữa");
		return;
	end
	...
	if not bRet then
		pPlayer.CenterMsg("Trừ KNB không thành công!");
		return;
	end
end
...
function MissionAward:CostGold_AddAwardIdx(nPlayerId, bSuccess, nType, nRecordId, nAwardIdx)
	...
	if not pPlayer then
		return false, "Mua số lần nửa đường, ngài offline";
	end
	...
	nMaxCount = math.min(nMaxCount, #tbRecord.tbAwardPos);
	if tbRecord.nCurIdx > nMaxCount then
		return false, "Đã đạt hết giới hạn số lần rút thưởng";
	end
...
function MissionAward:GetAwardInfo(pPlayer, nType, nRecordId)
	...
	if not tbRecord then
		pPlayer.Msg("Không tìm thấy phần thưởng!");
		Lib:LogTB(tbData);
...
function MissionAward:GetAward(pPlayer, nType, nRecordId)
	...
	local tbRecord = tbData[nRecordId];
	if not tbRecord then
		pPlayer.Msg("Tìm kiếm phần thưởng thất bại!");
		Lib:LogTB(tbData);
	...
	if tbAward.bSendWorld and bItemAward then
		local tbItemInfo = KItem.GetItemBaseProp(tbAward.nItemId) or {szName = "Đạo cụ không rõ"};
		KPlayer.SendWorldNotify(0, 1000, string.format("%s tại phó bản rút thưởng bên trong may mắn nhận đc %s", pPlayer.szName, tbItemInfo.szName), 1, 1);
	end
