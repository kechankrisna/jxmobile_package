...
function SwornFriends:OnRequest(szFunc, ...)
	...
	if self.bClosed then
		me.CenterMsg("Hệ thống kết bái đang được bảo trì")
		return
	end
...
function SwornFriends:_GetRelativeName(pMe, nPid)
	...
	if nPid~=pMe.dwID then
		...
		szName = pStay and pStay.szName or "Đồng đội"
	end
	return szName
end
...
function SwornFriends:CheckBeforeConnect(pPlayer)
	...
	if not tbTeam then
		return false, "Không có đội ngũ"
	end

	if tbTeam:GetCaptainId()~=pPlayer.dwID then
		return false, "Ngươi không phải đội trưởng"
	end
	...
	if nMemberCount<self.Def.nMinPlayer or nMemberCount>self.Def.nMaxPlayer then
		return false, string.format("Kết bái cần %d~%d người", self.Def.nMinPlayer, self.Def.nMaxPlayer)
	end

	for _, nPid in ipairs(tbMembers) do
		if self:_GetConnectInfo(nPid) then
			return false, string.format("%s đã kết bái, không thể kết bái nữa", self:_GetRelativeName(pPlayer, nPid))
		end
	end
	...
	if nItemCount<=0 then
		local pStay = KPlayer.GetRoleStayInfo(nPid)
		return false, string.format("%s hiện không có Kim Lan Phổ", self:_GetRelativeName(pPlayer, nPid))
	end
	
	local nMinLevel, nPid = self:_GetPlayersMinLevel(tbMembers)
	if nMinLevel<self.Def.nMinLevel then
		return false, string.format("%s chưa đủ cấp %d", self:_GetRelativeName(pPlayer, nPid), self.Def.nMinLevel)
	end
	...
	if nMinImitity<self.Def.nMinImitity then
		...
		return false, (pStay1 and pStay2) and 
			string.format("%s cùng %s độ thân mật không đủ cấp %d", pStay1.szName, pStay2.szName, self.Def.nMinImitity) or
			string.format("Độ thân mật không đủ cấp %d", self.Def.nMinImitity)
	end
...
function SwornFriends:_DoConnect(tbPids, szMainTitle)
	for _, nPid in ipairs(tbPids) do
		...
		if not pPlayer or pPlayer.ConsumeItemInBag(self.Def.nConnectItemId, 1, Env.LogWay_SwornFriends)~=1 then
			...
			return false, "Tiêu hao kết bái đạo cụ thất bại"
		end
	end
...
function SwornFriends:_GenMainTitle(szHead, szTail, nMemberCount)
	...
	if nHeadLen<=0 or nTailLen<=0 then
		return false, "Vui lòng điền danh hiệu kết bái"
	end
	if nHeadLen>self.Def.nTitleHeadMax or nHeadLen<self.Def.nTitleHeadMin or nTailLen>self.Def.nTitleTailMax or nTailLen<self.Def.nTitleTailMin then
		return false, string.format("Chữ đầu %d-%d ký tự,chữ sau %d~%d ", self.Def.nTitleHeadMin, self.Def.nTitleHeadMax, self.Def.nTitleTailMin, self.Def.nTitleTailMax)
	end

	if not CheckNameAvailable(szHead) or not CheckNameAvailable(szTail) then
		return false, "Từ nhạy cảm trong danh hiệu"
	end
	...
	if self:_IsMainTitleUsed(szTitle) then
		return false, "Danh hiệu kết bái đã được người khác sử dụng"
	end
	return true, szTitle
end
...
function SwornFriends:CheckBeforeDisconnect(pPlayer)
	...
	if not tbInfo then
		return false, "Ngươi không có kết bái"
	end
	return true
end
...
function SwornFriends:DisconnectReq()
	...
	me.Msg("Bạn đã kết thúc quan hệ kết bái")
	for nPid in pairs(tbInfo[2] or {}) do
		local pPlayer = KPlayer.GetPlayerObjById(nPid)
		if pPlayer then
			self:_PushConnectIds(pPlayer)
			pPlayer.Msg(string.format("%s Đã cùng ngươi kết thúc quan hệ kết bái", me.szName))
		end
...
function SwornFriends:ChangePersonalTitle(szTitle, bConsumeItem)
	...
	if nLen<=0 then
		return false, "Mời điền kết bái danh hiệu"
	end
	if nLen>self.Def.nPersonalTitleMax or nLen<self.Def.nPersonalTitleMin then
		return false, string.format("Danh hiệu %d-%d ký tự", self.Def.nPersonalTitleMin, self.Def.nPersonalTitleMax)
	end
	...
	for nPid, sz in pairs(tbInfo[2]) do
		if sz==szTitle then
			return false, nPid==nMyId and "Bạn đã có danh hiệu này." or "Danh hiệu kết bái đã được người khác sử dụng"
		end
	end
...
function SwornFriends:_SetPlayerTitle(pPlayer, szMainTitle, szPersonalTitle)
	...
	local nTitleId = self.Def.nTitleId
	local szTitle = string.format("%s-%s", szMainTitle, szPersonalTitle)
...
function SwornFriends:_FinishConnectionAction(tbIds)
	...
	local szNames = table.concat(tbNames, "、")
	local szMsg = string.format("%s Cùng chung chí hướng, tại Tương Dương thành vọng nguyệt đài bái thiên địa, kết nghĩa với「%s」", szNames, szTitle)
