...
function QYHCrossFight:ClcResult()
	...
	self.szStateInfo = "Tranh tài kết thúc";
	self.bStart = false
...
function QYHCrossFight:TryKeepTeam(pPlayer)
	...
	if not self.tbDismissTeam[pPlayer.dwID] then
		pPlayer.CenterMsg("Lời mời của đội đã được hoàn thành", true)
		return
	end
	local tbTeam = TeamMgr:GetTeamById(pPlayer.dwTeamID);
	if not tbTeam then
		pPlayer.CenterMsg("Vui lòng thành lập một nhóm hai người đầu tiên.", true)
		return
	end
	local bMeRequest = self:CheckRequestingKeepTeam(pPlayer)
	if bMeRequest then
		pPlayer.CenterMsg("Vui lòng đợi quá trình xử lý hoàn tất", true)
		return
	end
	...
	if nMemberCount ~= QunYingHuiCross.nFightPlayerNum then
		pPlayer.CenterMsg(string.format("Mời tổ đội %d người", QunYingHuiCross.nFightPlayerNum), true)
		return
	end
	local pAssist 
	...
	if not pAssist then
		pPlayer.CenterMsg("Tìm không thấy ngươi đồng đội", true)
		return
	end
	...
	if bAssistRequest then
		...
		pPlayer.CenterMsg("Đồng ý thành công", true)
		pAssist.CenterMsg(string.format("「%s」Đồng ý với lời mời của đội bạn, trận đấu tiếp theo sẽ được tổ chức", pPlayer.szName), true)
		...
		local fnRefuse = function (self, nRefuseId, nRequestpPlayerId)
			...
			if pRequester then
				pRequester.CenterMsg(string.format("Thật có lỗi, 「%s」từ chối lời mời", pRefuser and pRefuser.szName or ""), true)
		...
		local fnOverTime = function()
			if me then
				...
				me.CenterMsg("Đã tự động cự tuyệt", true)
				me.CallClientScript("Ui:CloseWindow", "MessageBox")
				local pRequester = KPlayer.GetPlayerObjById(nRequestId or 0)
				if pRequester then
					pRequester.CenterMsg(string.format("Thật có lỗi, 「%s」từ chối lời mời", me.szName), true)
				end 
			end
		end
		pAssist.MsgBox(string.format("[FFFE0D]「%s」[-]Mời ngươi tiếp tục tổ đội cộng đồng chiến đấu, có đồng ý hay không?", pPlayer.szName)  .."\n（%d giây sau tự động cự tuyệt）", {{"Đồng ý", fnAgree, self, nResponseId}, {"Hủy bỏ", fnRefuse, self, nResponseId, nRequestId}}, nil, QunYingHuiCross.nKeepTeamWaitTime, fnOverTime)
		self:RequestKeepTeam(pPlayer)
		pPlayer.CenterMsg(string.format("Đã gửi lời mời tổ đội đến 「%s」", pAssist.szName), true)
	end
end
