...
function JingMai:TryDoJingMaiLevelUp(pPlayer, nJingMaiId)
	...
	if nRequestTime ~= 0 and nNowTime >= nRequestTime + self.nJingMaiLevelUpTime then
		...
		if tbLevelAttrib[nJingMaiId] and tbLevelAttrib[nJingMaiId][nNextLevelIndex] then
			...
			pPlayer.CenterMsg(string.format("%sThành công thăng cấp đến thứ %s", JingMai:GetJingMaiLevelName(nJingMaiId), nNextLevelIndex), true)
		...
		pPlayer.CenterMsg("Thời gian chưa tới", true)
	end
end
...
function JingMai:RequestJingMaiLevelUp(pPlayer, nJingMaiId)
	...
	if tbCost then
		for _, tbInfo in ipairs(tbCost) do
			...
			elseif nType == Player.award_type_money then
				...
				if not bResult then
					pPlayer.CenterMsg("Khấu trừ tiền tệ thất bại!", true);
	...
	pPlayer.CenterMsg(string.format("%s sẽ hoàn thành sau %s nữa", szLevelName or "經脈", Lib:TimeDesc9(nNowTime + self.nJingMaiLevelUpTime)), true)
