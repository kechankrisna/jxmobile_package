...
function RankBattle:SyncRankEnemy(pPlayer, nVersion, bRefresh)
	...
	if bRefresh and self.tbEnemy[nPlayerId] and self.tbEnemy[nPlayerId].nFrashEnemyCD >= nCurTime then
		pPlayer.CenterMsg("Tạo mới đối thủ quá nhanh!")
		return
	end
...
function RankBattle:StartBattle(pPlayer, nFightType, nRankNo, nId)
	...
	if not self.tbEnemy[nPlayerId] then
		pPlayer.CenterMsg("Thứ hạng của bạn đã thay đổi, cập nhật lại danh sách thử thách của bạn");
		self:SyncRankEnemy(pPlayer, 0)
		return;
	end

	if pPlayer.nLevel < 12 then
		pPlayer.CenterMsg("Võ Thần Điện cấp 12 mới mở");
		return;
	end

	if not AsyncBattle:CanStartAsyncBattle(pPlayer) then
		pPlayer.CenterMsg("Bản đồ hiện tại không thể khiêu chiến, hãy về[FFFE0D]「Tương Dương」[-]thử lại")
		return;
	end

	if pPlayer.dwTeamID > 0 then
		pPlayer.MsgBox("Trạng thái tổ đội không được khiêu chiến, thoát khỏi đội ngũ hiện tại?",
			{
				{"Rời đội", TeamMgr.QuiteTeam, TeamMgr, pPlayer.dwTeamID, pPlayer.dwID},
				{"Hủy"},
			})
		return;
	end
	...
	if DegreeCtrl:GetDegree(pPlayer, "RankBattle") <= 0 then
		pPlayer.CenterMsg("Số lần khiêu chiến không đủ")
		return;
	end
    ...
    if nResult == 0 then
    	pPlayer.CenterMsg("Trạng thái hiện tại không thể tham gia");
        return;
    end
	...
	if tbEnemy then
		if tbEnemy.nFightType ~= nFightType or (nFightType == self.FIGHT_TYPE_NPC and self.tbRank[nRankNo].nPlayerId) or
			...
			-- 该名次已经易主
			pPlayer.CenterMsg("Thứ hạng người chơi bạn muốn khiêu chiến đã thay đổi.")
		...
		if self.tbLockNo[nRankNo] then
			if self.tbLockNo[nRankNo].nTimeOut > nCurTime then	-- 木有超时
				pPlayer.CenterMsg("Đối phương đang khiêu chiến, xin thử lại sau")
				return;
			else
		...
		if tbMyRankInfo and self.tbLockNo[tbMyRankInfo.nRankNo] then
			if self.tbLockNo[tbMyRankInfo.nRankNo].nTimeOut > nCurTime then	-- 木有超时
				pPlayer.CenterMsg("Ngươi đang bị người khác khiêu chiến, xin thử lại sau")
				return;
			else
		...
		pPlayer.CenterMsg("Thứ hạng của bạn đã thay đổi");
		self:SyncRankEnemy(pPlayer, nil, 1);
	end
...
function RankBattle:OnRankBattleResult(pPlayer, nResult, tbBattleObj, nEnemyId, nRankNo, nBeginTime)
	if nResult == 1 then
		if Forbid:IsBanning(pPlayer,Forbid.BanType.WuShenRank) then
	        ...
	        pPlayer.MsgBox(string.format("Ngài bởi vì %s Bị cấm chỉ lên bảng, giải cấm thời gian %s",Forbid:BanTips(pPlayer,Forbid.BanType.WuShenRank), szTime or ""), {{"Xác nhận"}, {"Hủy"}})
		end
		local nPlayerId = pPlayer.dwID;
		...
		pPlayer.CenterMsg(string.format("Nhận [FFFE0D] %d [-] điểm kinh nghiệm", pPlayer.TrueChangeExp(RankBattle.WIN_AWARD * pPlayer.GetBaseAwardExp())))
	else
		self:UnLockRankNo(nRankNo);

		pPlayer.CenterMsg(string.format("Nhận [FFFE0D] %d [-] điểm kinh nghiệm", pPlayer.TrueChangeExp(RankBattle.LOST_AWARD* pPlayer.GetBaseAwardExp())))
	end
...
function RankBattle:FetchRankBattleAward(pPlayer)
	...
	if pAsyncData then		-- 异步数据存在就加，不在就不加了
		...
		if nSlvBoxNum + nGoldBoxNum > 0 then
			...
			pPlayer.CenterMsg("Bạn không có phần thưởng để nhận");
		end
