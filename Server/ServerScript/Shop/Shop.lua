...
function Shop:BuyAfterCost(pPlayer, tbWare, nCount, nPrice, bRand, szMoneyType, nRequestIndex)
	...
	pPlayer.CallClientScript("Shop:OnBuyResponse", true, "Mua thành công", nRequestIndex + 1, tbWare.nGoodsId);
...
function Shop:Buy(pPlayer, szShopType, nTemplateId, nCount, nClientPrice, nRequestIndex)
	if pPlayer.nShopRequestIndex and nRequestIndex and pPlayer.nShopRequestIndex == nRequestIndex then
		pPlayer.CallClientScript("Shop:OnBuyResponse", false, "Giao dịch đã thành công", pPlayer.nShopRequestIndex + 1);
		return
	end
	...
	if bRand then
	    ...
	    if self:IsNeedRefresh(nLastDay) then
	    	...
	    	pPlayer.CenterMsg("Thương phẩm đã xuất hiện vật phẩm mới, hãy tới xem thử");
	    	return;
	    end
	end
...
function Shop:BuyGoods(pPlayer, szShopType, nGoodsId, nCount, nClientPrice, nRequestIndex)
	if pPlayer.nShopRequestIndex and nRequestIndex and pPlayer.nShopRequestIndex == nRequestIndex then
		pPlayer.CallClientScript("Shop:OnBuyResponse", false, "Giao dịch thành công", pPlayer.nShopRequestIndex + 1);
		return
	end
...
function Shop:DoBuyWare(pPlayer, tbWare, nCount, nClientPrice, nRequestIndex)
	...
	if nClientPrice ~= nPrice then
		pPlayer.CenterMsg("Giá thương phẩm vô hiệu")
		return
	end
...
function Shop:BuyWarWare(pPlayer, nTemplateId, nCount, nRequestIndex)
	...
	if not tbKin:CostFound(nPrice * nCount) then
		pPlayer.CenterMsg("Tiêu hao điểm kiến thiết bang hội lỗi")
		return
	end
	...
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("%s「%s」mua %d <%s>", Kin.Def.Career_Name[nCareer], pPlayer.szName, nCount, szItemName), pPlayer.dwKinId, 
	...
	pPlayer.CallClientScript("Shop:OnBuyResponse", true, string.format("Ngươi mua %d「%s」, đã tự động lưu trữ vào kho đạo cụ", nCount, szItemName) , (nRequestIndex or 0) + 1);
end
...
function Shop:Sell(pPlayer, tbGoods, nRequestIndex)
	if pPlayer.nShopRequestIndex and nRequestIndex and pPlayer.nShopRequestIndex == nRequestIndex then
		pPlayer.CallClientScript("Shop:OnSellResponse", false, "Giao dịch thành công", pPlayer.nShopRequestIndex + 1);
		return
	end
	local tbAfterCheck = {};
	...
	for i,v in ipairs(tbGoods) do
		...
		if not nSumPrice or not szMoneyType then
			pPlayer.CallClientScript("Shop:OnSellResponse", false, "Không có cấu hình giá bán");
			return
		end
	...
	for i,v in ipairs(tbAfterCheck) do
		...
		pPlayer.CenterMsg(string.format("Nhận được %s:%s", szMoneyEmotion, v.nSumPrice))
...
function Shop:MakeEquip(pPlayer, nId)
	...
	if nIdx==3 and nKinId>0 then
		...
		if tbKinData then
			...
			local szSysMsg = string.format("「%s」trong %s chế tạo thành công <%s>, thật may mắn!", pPlayer.szName, szBuildingName, szEquipName)
...
function Shop:RenownShopBuy(pPlayer, nId, nCount, nClientPrice)
	...
	if not pPlayer.CostMoney("Renown", nCost, Env.LogWay_RenownShop) then
		pPlayer.CenterMsg("Tiêu hao danh vọng thất bại")
		return
	end
