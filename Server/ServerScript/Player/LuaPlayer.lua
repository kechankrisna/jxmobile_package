...
function _LuaPlayer.AddTimeTitle(nTitleID, nEndTime, bActive, bShowInfo)
    ...
    if nTimeout <= 0 and nEndTime ~= 0 then
    	...
    	if tbTitleTemp then
    		self.CenterMsg(string.format("Danh hiệu %s đã quá hạn!", tbTitleTemp.Name));
    	end
	...
	if szType == "Gold" and Sdk:IsMsdk() then
		...
		AssistClient:DoPresent(self, nPoint, nLogReazon, nLogReazon2, function (nPlayerId, bSucceed)
			if not bSucceed then
				local tbMail = {
					To = nPlayerId;
					Title = "Nguyên bảo tặng";
					From = "Hệ thống thanh toán";
					Text = "Rất xin lỗi, tại thu hoạch hệ thống đưa tặng Nguyên bảo lúc, hệ thống phát sinh dị thường. Cho nên đem Nguyên bảo lấy thư tín phụ kiện hình thức gửi đi cho ngài, mời dùng tay nhận lấy.";
