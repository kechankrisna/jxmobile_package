...
Player.tbFieldComboKillFun =
{
	["FieldBoss"] = function (pPlayer, nFieldComboSkill)
		...
		local szShowMsg = string.format("「%s」tại khiêu chiến danh tướng, liên trảm %s người!", pPlayer.szName, nFieldComboSkill);
	...
	["ImperialTomb"] = function (pPlayer, nFieldComboSkill)
		...
		local szShowMsg = string.format("「%s」tại Tần Thủy Hoàng thi triển thân thủ, liên trảm %s người!", pPlayer.szName, nFieldComboSkill);
	...
	["FemaleImperialTomb"] = function (pPlayer, nFieldComboSkill)
		...
		local szShowMsg = string.format("「%s」tại Nữ Đế nghi chủng thi triển thân thủ, liên trảm %s người!", pPlayer.szName, nFieldComboSkill);
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szShowMsg, pPlayer.dwKinId);
	end,
}
...
function Player:CheckFocusSelfAllPet(pPlayer)
	...
	if not bRet then
		return false, "Bản đồ không thể thao tác";
	end
...
function Player:CheckExcercise(pNpc1, pNpc2)
	...
	if nPkMode1 == Player.MODE_EXCERCISE or nPkMode2 == Player.MODE_EXCERCISE then
		return false, string.format("%s đang tiến hành thao tác, không thể thao tác với nhiều người cùng lúc", nPkMode1 == Player.MODE_EXCERCISE and pNpc1.szName or pNpc2.szName)
	end
	if nPkMode1 ~= Player.MODE_PEACE or nPkMode2 ~= Player.MODE_PEACE then
		return false, "Dạng PK 2 bên là Hòa Bình";
	end
	if pNpc1.nFightMode ~= 1 or pNpc2.nFightMode ~= 1 then
		return false, "Tỷ thí không ở khu an toàn";
	end
...
function Player:RequestExcercise(pRequestor, nPlayerId)
	...
	pRequestor.CenterMsg(string.format("Yêu cầu tỉ thí với 「%s」", pPlayer.szName));
...
function Player:OnExcerciseRespond(pPlayerAnswer, nRequestPlayerId, bResult)
	if not self.tbExcerciseRequest[nRequestPlayerId] then
		pPlayerAnswer.CenterMsg("Yêu cầu đã hết hạn");
		return;
	end
	...
	if not bResult then
		pRequestor.CenterMsg(string.format("「%s」từ chối yêu cầu của bạn", pPlayerAnswer.szName))
		return
	end
...
function Player:ChangePKMode(pPlayer, nMode)
	...
	if pPlayer.bForbidChangePk == 1 then
		...
		if nForce > 0 then
			pPlayer.CenterMsg("Không thể tự chuyển đổi chế độ trong bản đồ chiến đấu", true);
		end
	...
	if nMode == Player.MODE_KILLER and PKValue:CheckMaxValue(pPlayer) then
		pPlayer.CenterMsg("PK giá trị quá cao, không cách nào mở Đồ Sát!");
		pPlayer.CallClientScript("Player:ChangePkResult", 0, 0)
		return;
	end
	...
	if nCurMode == Player.MODE_KILLER and
		...
		pPlayer.CenterMsg("Trong trận chiến, tạm thời không thể chuyển đổi chế độ");
		pPlayer.CallClientScript("Player:ChangePkResult", 0, 0)
		return;
	end
...
{
	["NpcW::SetPkMode"] = "「%s」đổi trạng thái PK, tỉ thí kết thúc",
	["SetPkMode"] = "「%s」đổi trạng thái PK, tỉ thí kết thúc",
	["XPlayerServer::DelayLogout"] = "「%s」ngắt kết nối, tỷ thí kết thúc",
	["DelayLogout"] = "「%s」ngắt kết nối, tỷ thí kết thúc",
	["NpcW::ChangeSubWorld"] = "「%s」rời bản đồ, tỷ thí kết thúc",
	["ChangeSubWorld"] = "「%s」rời bản đồ, tỷ thí kết thúc",
	["NpcW::SetFightMode"] = "「%s」trở lại khu vực an toàn, tỷ thí kết thúc",
	["SetFightMode"] = "「%s」trở lại khu vực an toàn, tỷ thí kết thúc",
	["Player::SetAlone"] = "「%s」rời bản đồ, tỷ thí kết thúc",
	["SetAlone"] = "「%s」rời bản đồ, tỷ thí kết thúc",
}
...
function Player:OnFetchRechargeResult(szAccount, nRoleId, nGold, nResult)
	...
	if nRoleId == 0 then
		...
		if nGold > 0 and nResult == 1 then
			...
			{
				To = nRoleId,
				Title = "Closed Beta trữ giá trị trả về",
				Text = "Cảm tạ ngài tại 《 Kiếm hiệp tình duyên 》 Closed Beta trong lúc đó bên trong tiến hành trữ giá trị, phụ kiện vì đối ứng trữ giá trị Nguyên bảo trả về, xin chú ý kiểm tra và nhận",
				tbAttach 	= {{"Gold", nGold}},
		...
		if pPlayer then
			pPlayer.CenterMsg(nResult == 1 and "Nhận lấy thành công, mời xem xét bưu kiện" or "Nhận lấy thất bại")
...
function Player:OnRechargeBackNewsInfoCallback(bConfirm)
	...
	if not bConfirm then
		me.MsgBox("Chỉ có thể từ một vai nhận lấy trữ giá trị trả về, phải chăng nhận lấy?", {{"Xác nhận", self.OnRechargeBackNewsInfoCallback, self, true}, {"Hủy"}})
		return
	end
...
function Player:SyncRechargeBackNewsInfo(pPlayer)
	...
	if nGold > 0 then
		local tbNewsData =
		{
			szTitle = "Closed Beta trữ giá trị trả về";
			szContent = string.format("     Closed Beta trong lúc đó tham dự trữ giá trị người chơi có thể thu hoạch được đối ứng trữ giá trị trả về, thấp với với 1000 Nguyên bộ phận theo 150%% Trả về, cao hơn 1000 Nguyên bộ phận theo 100%% Trả về, trả về không đưa vào VIP Kinh nghiệm.\n\n    [FFFE0D] Chú ý: Mỗi cái số tài khoản trả về chỉ có thể từ một vai nhận lấy một lần, mời cẩn thận lựa chọn![-]\n\n    Căn cứ ngài trữ giá trị ghi chép, [FFFE0D] Ngài có thể thu hoạch được %d Nguyên bảo [-]", nGold);
			szBtnText = "Nhận lấy trả về";
			szBtnType = "ServerCmd";
...
function Player:PeekPlayer(pPeeker, dwRoleId)
	local pPlayer = KPlayer.GetPlayerObjById(dwRoleId)
	if not pPlayer then
		pPeeker.CenterMsg("Đối phương chưa online");
		return
	end
	...
	if not CanPeekMap[szMapClass] then
		pPeeker.CenterMsg("Bản đồ hiện tại không cho phép quan chiến");
		return;
	end
	if Map:IsForbidPeeking(pPlayer.nMapTemplateId) then
		pPeeker.CenterMsg("Bản đồ đối phương cấm quan chiến");
		return
	end
	if Wedding:IsPlayerTouring(dwRoleId) then
		pPeeker.CenterMsg("Đang trong quá trình rước kiệu không thể quan chiến");
		return
	end
	if not Env:CheckSystemSwitch(pPeeker, Env.SW_ChuangGong) then
		pPeeker.CenterMsg("Trạng thái hiện tại của bạn không thể được xem từ xa");
		return
	end
	if pPeeker.PeekPlayer(dwRoleId) == 0 then
		pPeeker.CenterMsg("Đối phương trước mắt trạng thái không cách nào tiến hành ở xa quan chiến");
		return
	end
end
    ...
    ["Coin"] =
    {
        ...
        fnAddP = function (pPlayer, nValue, nLogType)
            ...
            if nPoint ~= nValue then
            	pPlayer.CenterMsg(string.format("Nhận được * %s", nPoint - nValue), true);
            end
