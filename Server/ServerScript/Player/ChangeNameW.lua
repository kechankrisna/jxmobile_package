...
function ChangeName:OnDBResponse( dwRoleId, nCode )
    ...
    if nCode ~= 0  then
        ...
        Mail:SendSystemMail({
            To = dwRoleId,
            Text = "Đại hiệp, đổi tên thất bại xin kiểm tra lại từ ngữ không hợp lệ",
            tbAttach = tbAttach,
        ...
        if pPlayer then
            ...
            local szMsg = string.format("Đại hiệp[f9ffa3]「%s」[-]sửa tên thành [f9ffa3]「%s」[-]", szOLdName, szNewName)
