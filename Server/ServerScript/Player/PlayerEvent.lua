...
function PlayerEvent:OnReConnect()
	...
	Lib:CallBack({Toy.OnLogin, Toy, me, true})
end
...
function PlayerEvent:OnDeath(pKiller)
	...
	if pPlayer then
		pPlayer.CenterMsg(string.format("Đã đánh 「%s」 bị thương!", (me.szNickName or me.szName) ), true);
	...
	if pPlayer then
		...
		szKillerName = pKiller and pKiller.szName or "Người ngoài hành tinh :D"
	end
	me.Msg(string.format("Bị「%s」đánh trọng thương!",  szKillerName));
	...
	if pPlayer then
		if self.tbPKMap[me.nMapTemplateId] then
			...
			if nExp > 0 then
				...
				me.Msg(string.format("Kinh nghiệm giảm %s điểm", nExp));
			end
	...
	local nMeID = me.dwID;
	me.MsgBox(string.format("Bị「[FFC300FF]%s[-]」đánh trọng thương\n", szKillerName) .. "%s giây sau về thành", {{"Về thành", fnRevive, me.dwID}}, nil, 10, function () fnRevive(nMeID) end);
	...
	for i,v in ipairs(TimeFrame.tbOpenNewMaxLevelTimeFrame) do
		...
		if v.nTimeFrameOpenTime > nTimeNow then
			...
			Mail:SendSystemMail({
				To        = pPlayer.dwID,
				Title = "Hướng dẫn",
				Text      = string.format("  Đại hiệp đã đạt cấp tối đa, mở cấp tối đa %d còn %d ngày(4:00 %d %d)\n[FFFE0D]  Chú ý: Sau khi đạt cấp tối đa và nhận 100% %, EXP nhận tiếp sẽ giảm một nữa[-]", v.nMaxLevel, nOpenDay - nToday, tbTime.month, tbTime.day),
				});
...
function PlayerEvent:OnLevelUp(nNewLevel)
	...
	if nNewLevel == 10 then
		Mail:SendSystemMail({
			To = me.dwID,
			Title = "Quà Tặng Chân Nhi",
			Text = "     Ngươi đã rời đảo một thời gian rồi, tuy nội thương đã gần khỏi, nhưng hành tẩu giang hồ hành sự cẩn trọng, yêu bản thân. Hừm, lại thêm lần nữa ta sẽ không cứu ngươi đâu! Ta bảo người đưa thư đưa đến ít lộ phí, nếu rảnh hãy về đảo xem thử",
			From = "Chân Nhi",
			tbAttach = {
				{"Gold", 100},
			},
...
function PlayerEvent:OnPlayerLoginVipAward()
	...
	for i = 11, 13, 2 do
		...
		if nBuydeBit == 1 and nBuydeBitEx == 0 then
			Mail:SendSystemMail({
				To = me.dwID,
				Title = "Đặc quyền gói quà đền bù",
				Text = string.format("     hiệp sĩ hảo !bởi vì kiếm hiệp %d đặc biệt quyền lễ túi gia tăng rồi 【danh tướng lệnh 】tưởng lệ ，phụ kiện vi ngài đích 【danh tướng lệnh 】bồi thường 。", i-1) ,
				From = "Hệ thống",
				tbAttach = {
					{"item", 1394, 1},
				},
