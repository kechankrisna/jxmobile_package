...
function FightPower:Setup()
	...
	self.tbFightPowerTypeSeriesStonePos = {
		[Item.EQUIPPOS_WEAPON_SERIES] 	= 1;
		[Item.EQUIPPOS_ARMOR_SERIES] 	= 1;
		[Item.EQUIPPOS_RING_SERIES] 	= 1;
		[Item.EQUIPPOS_NECKLACE_SERIES] = 1;
		[Item.EQUIPPOS_AMULET_SERIES] 	= 1;
		[Item.EQUIPPOS_BOOTS_SERIES] 	= 1;
		[Item.EQUIPPOS_BELT_SERIES] 	= 1;
		[Item.EQUIPPOS_HELM_SERIES] 	= 1;
		[Item.EQUIPPOS_CUFF_SERIES] 	= 1;
		[Item.EQUIPPOS_PENDANT_SERIES] 	= 1;
	};
end
...
function FightPower:GetFightPowerTypeByEquipPos(nEquipPos)
	...
	elseif self.tbFightPowerTypeHorsePos[nEquipPos] then
		return "Horse"
	elseif self.tbFightPowerTypeSeriesStonePos[nEquipPos] then
		return "SeriesStone"
	elseif nEquipPos >= Item.EQUIPPOS_SKILL_BOOK and nEquipPos <= Item.EQUIPPOS_SKILL_BOOK_End then
		return "SkillBook"
	elseif nEquipPos >= Item.EQUIPPOS_JUEXUE_BEGIN and nEquipPos <= Item.EQUIPPOS_JUEXUE_END then
		return "JueXue"
	else
		return "Equip"
	end
...
function FightPower:OnLogin(pPlayer)
	...
	if not tbFightPowerData.nPartnerCardFightPower then
		tbFightPowerData.nPartnerCardFightPower = self:CalcPartnerCardFightPower(pPlayer);
		tbFightPowerData.nPartnerCardFightPower = 0;
		self:ChangeFightPower("PartnerCard", pPlayer, true);
	end
	...
	if not tbFightPowerData.nSeriesStoneFightPower then
		tbFightPowerData.nSeriesStoneFightPower = 0
		self:ChangeFightPower("SeriesStone", pPlayer, true)
	end

	--技能点，秘籍，绝学排行榜在同一个版本更新出去
	if not tbFightPowerData.nSkillBookFightPower then
		tbFightPowerData.nSkillFightPower     = 0
		tbFightPowerData.nSkillBookFightPower = 0
		tbFightPowerData.nJueXueFightPower    = 0
		self:ChangeFightPower("Skill", pPlayer, true);
		self:ChangeFightPower("SkillBook", pPlayer, true);
		self:ChangeFightPower("JueXue", pPlayer, true);
		self:ChangeFightPower("Equip", pPlayer, true);
	end
...
function FightPower:GetTotalFightPower(pPlayer)
	...
	nTotal = nTotal + (tbFightPowerData.nSeriesStoneFightPower or 0);
	nTotal = nTotal + (tbFightPowerData.nPartnerCardFightPower or 0);
	nTotal = nTotal + (tbFightPowerData.nSkillBookFightPower or 0);
	nTotal = nTotal + (tbFightPowerData.nJueXueFightPower or 0);
	return nTotal;
end
...
function FightPower:CalcPartnerCardFightPower(pPlayer)
	return PartnerCard:CalcFightPower(pPlayer)
end
...
function FightPower:CalcSeriesStoneFightPower(pPlayer)
	local nEquipFightPower = 0;
	for nPos,_ in pairs(self.tbFightPowerTypeSeriesStonePos) do
		local pEquip = pPlayer.GetEquipByPos(nPos);
		if pEquip then
			nEquipFightPower = nEquipFightPower + pEquip.nFightPower;
		end
...
function FightPower:CalcSkillBookFightPower(pPlayer)
	local nFightPower = 0
	for nPos = Item.EQUIPPOS_SKILL_BOOK, Item.EQUIPPOS_SKILL_BOOK_End do
		local pEquip = pPlayer.GetEquipByPos(nPos)
		if pEquip then
			nFightPower = nFightPower + pEquip.nFightPower
		end
	end
	return nFightPower
end

function FightPower:CalcJueXueFightPower(pPlayer)
	return JueXue:GetAllFightPower(pPlayer)
end