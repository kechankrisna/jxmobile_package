...
function MagicBowl:GetData(nPlayerId)
	if MODULE_ZONESERVER then
		...
		return false, "Không có Tụ Bảo Bồn"
	end

	local tbHouse = House:GetHouse(nPlayerId)
	if not tbHouse then
		return false, "Không có gia viên"
	end
...
function MagicBowl:InscriptionStartStage(pPlayer, tbMaterials)
	...
	if szState~="rest" then
		return false, "Trạng thái hiện tại không thể mở rèn"
	end
...
function MagicBowl:ConsumeMaterials(pPlayer, tbMaterials, nCachedValue, nNeedValue)
	...
	for nItemId, nCount in pairs(tbMaterials) do
		...
		if not pItem then
			...
			return false, "Chỉ định tiêu hao vật liệu không tồn tại"
		end
		...
		if nConsumeCount~=nCount then
			...
			return false, "Tiêu hao vật phẩm thất bại"
		end
...
function MagicBowl:InscriptionHarvest(pPlayer)
	...
	if szState~="finished" then
		return false, "Trạng thái hiện tại không thể thu hoạch"
	end
...
function MagicBowl:Pray(pPlayer)
	...
	if #tbMagicBowl.tbAttrs<=0 then
		return false, "Mời sử dụng minh văn vì Tụ Bảo Bồn tẩy luyện thuộc tính sau lại tiến hành cầu phúc đi"
	end
	...
	if nNextCost>0 then
		...
		if not pPlayer.CostMoney(self.Def.szPrayCostType, nNextCost, Env.LogWay_MagicBowl) then
			return false, string.format("Khấu trừ %s thất bại", szMoneyName)
		end

		local szMsg = string.format("Tiêu hao %s: %d( Gia viên Tụ Bảo Bồn cầu phúc )", szMoneyName, nNextCost)
		pPlayer.Msg(szMsg)
...
function MagicBowl:Refinement(pPlayer, nItemId, nSrcPos, nTargetPos)
	...
	local tbData = self:GetData(nPlayerId)
	if not tbData then
		pPlayer.CallClientScript("House:MagicBowlOnRefinementResult", false, "Không có Tụ Bảo Bồn")
		return false, "Không có Tụ Bảo Bồn"
	end
	
	local pItem = pPlayer.GetItemInBag(nItemId)
	if not pItem then
		pPlayer.CallClientScript("House:MagicBowlOnRefinementResult", false, "Minh văn không tồn tại")
		return false, "Minh văn không tồn tại"
	end
	...
	if not tbItemAttr then
		...
		pPlayer.CallClientScript("House:MagicBowlOnRefinementResult", false, "Tẩy luyện thất bại")
		return false
	end
	...
	for i, nSaveData in ipairs(tbData.tbAttrs) do
		...
		if nAttribId==tbItemAttr.nAttribId and i~=nTargetPos then
			return false, "Thuộc tính tương tự đã tồn tại"
		end
	end
