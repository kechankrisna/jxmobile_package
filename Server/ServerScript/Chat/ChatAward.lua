...
ChatAward.tbAllAward =
{
	...
	[6] ={
		nType = ChatAward.TYPE_GOLD;
		szTips = "Lâm thiếu hiệp hồng bao";
	...
	if nIdx then
		for szKey, tbInfo in pairs(self.tbChatAwardInfo) do
			if nIdx == tbInfo.nIdx then
				...
				local szMsg = string.format("Phần thưởng mật khẩu này đã kết thúc! Mật khẩu chính xác:“%s”", szKey);
				if nCount > 0 then
					szMsg = szMsg .. string.format(", chúc mừng %s đại hiệp trúng thưởng", nCount);
				end
...
function ChatAward:SendChatAward_Gold(szKey, pPlayer, tbChatAward)
	...
	local tbRedBag = {
		szId = "chat_award";
		szTips = tbAwardInfo.szTips or "Mật khẩu hội nghị hồng bao";
		szSprite = tbAwardInfo.szSprite;
		szContent = string.format("Mật khẩu:“%s”", szKey);
	...
	pPlayer.CenterMsg(string.format("Chúc mừng hiệp sĩ! Lần này hồng bao khẩu lệnh chính xác, thu hoạch được %s Nguyên bảo", nAward), true);
	if bNotify then
		KPlayer.SendWorldNotify(0, 999, string.format("Chúc mừng 「%s」 tại khẩu lệnh hồng bao, thu hoạch được %s Nguyên bảo!", pPlayer.szName, nAward), 1, 1);
	end
...
function ChatAward:SendChatAward_Item(szKey, pPlayer, tbChatAward)
	...
	if bNotify then
		...
		KPlayer.SendWorldNotify(0, 999, string.format("Chúc mừng 「%s」 tại khẩu lệnh hồng bao, thu hoạch được%s！", pPlayer.szName, szName), 1, 1);
	end
