...
function ChatMgr:CheckSendMsg(playerId, channelId)
	...
	elseif channelId == ChatMgr.ChannelType.Kin then
		...
		if nLeftTime and nLeftTime > 0 then
			player.CenterMsg(string.format("Ngươi đang bị cấm chát, còn %s", Lib:TimeDesc2(nLeftTime)));
			return false;
		end
		...
		if forbidEndTime == 0 or  forbidEndTime > nCurTime then
			...
			if player then
				if forbidEndTime == 0 then
					player.CenterMsg("Ngươi đang bị cấm chat");
				else
					player.CenterMsg(string.format("Ngươi bị cấm chat, còn %s", Lib:TimeDesc2(forbidEndTime - nCurTime)));
				end
			end
...
function ChatMgr:CheckFactionChannelMsg(pPlayer, nChannelId)
	...
	if nLastSend and (nLastSend + 10) > nNow then
		pPlayer.CenterMsg(string.format("Còn %d giây sau mới được nói tiếp", (nLastSend + 10) - nNow));
		return false;
	end
	...
	if self.tbDynamicChatMaxCountInfo[playerId] >= nLimit then
		pPlayer.CenterMsg(string.format("Ngươi hôm nay đã nói %d lần, đã đạt giới hạn không thể nói thêm được nữa", nLimit));
		return false;
	end
...
function ChatMgr:CheckPlayerChatActionBQ(pPlayer, nChatID, nNpcType, bAuto)
	local pNpc = pPlayer.GetNpc();
    if not pNpc then
        return false, "Không thể sử dụng động tác";
    end

    if ActionInteract:IsInteract(pPlayer) then
        return false, "Không thể được sử dụng dưới sự tương tác hành động"
    end
	...
	if not Toy:IsFree(pPlayer) then
    	...
    	return false, "Không thể sử dụng động tác biểu cảm"
    end

    if pNpc.nShapeShiftNpcTID > 0 and not Npc.tbActionBQNpcID[pNpc.nShapeShiftNpcTID] then
        return false, "Hiện đang biến thân không thể nào sử dụng động tác";
    end
    local tbChatInfo
    ...
    if pPlayer.nFightMode ~= Npc.FIGHT_MODE.emFightMode_None then
    	return false, "Trạng thái chiến đấu không thể nào sử dụng động tác";
    end

    local nResult = pNpc.CanChangeDoing(Npc.Doing.skill);
    local nActRes = pNpc.GetActionResType();
    if nResult == 0 and nActRes == 0 then
        return false, "Trạng thái hiện tại không thể sử dụng động tác";
    end
...
function ChatMgr:PlayerChatActionBQ(pPlayer, nChatID, nNpcType, bAuto)
	local bRet, szMsg, tbChatInfo = self:CheckPlayerChatActionBQ(pPlayer, nChatID, nNpcType, bAuto);
	if not bRet then
	...
	if nNpcType == ChatEquipBQ.nNpcType then
		if not ChatEquipBQ:CheckEquipBQ(pPlayer, nChatID) then
			pPlayer.CenterMsg("Vui lòng trang bị thiết bị trước", true);
			return
		end
	end
...
function ChatMgr:SetCrossHostAuth(pPlayer, nAuth)
	if nAuth ~= ChatMgr.ChatCrossAuthType.emHost
		...
		then
		ChatMgr:LeaveCrossChannel(pPlayer);
		pPlayer.CenterMsg("Ngươi đã bị hủy bỏ quyền dẫn chương trình");
	end
...
function ChatMgr:SendCrossHostNotify(tbHostInfo, bNew)
	if not bNew then
		local szMsg = string.format("Dẫn chương trình 「%s」 đã hạ mic, chúng ta hãy hẹn gặp lại!!", tbHostInfo.szName or "null");
	...
	local szMsg = string.format("Đến từ %s 「%s」Đã đăng nhập dẫn chương trình kênh, mau tới cùng khanh đàm tiếu giang hồ, đem cười làm rượu, uống cạn ưu sầu! Tiến về nói chuyện phiếm cột 【Trực tiếp】 Kênh điểm kích nghe đài lập tức gặp gỡ bất ngờ.", szServerName, tbHostInfo.szName);
...
local tbJoinHostErrMsg = {
	[1] = "Trước mắt dẫn chương trình đã đủ viên, ngài không cách nào gia nhập trực tiếp ";
	[2] = "Ngài trước mắt ngay tại trực tiếp, không cách nào lại lần gia nhập ";
};
...
function ChatMgr:SetNamePrefixById(pPlayer, nPrefixId, nExpireTime)
	...
	if nExpireTime > GetTime() then
		pPlayer.CenterMsg(string.format("Nhận được tiền tố trò chuyện 「%s」", ChatMgr:GetNamePrefixInfo(nPrefixId).Desc or "某"));
