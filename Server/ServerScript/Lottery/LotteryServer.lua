...
function Lottery:SendAward(dwPlayerId, nRank)
	...
	{
        To = dwPlayerId;
        Title = "Quà Tặng Minh Chủ";
        From = "Minh Chủ Võ Lâm";
        Text = "Thiếu hiệp đã có cống hiến kiệt xuất cho võ lâm, nay biếu tặng quà nhỏ để hỗ trợ cho thiếu hiệp. Mong thiếu hiệp hãy tiếp tục nỗ lực.";
        tbAttach = tbAward;
...
function Lottery:IsAvailable()
	if not Lottery:IsOpen() then
		return false, "Hoạt động chưa mở, xin vui lòng chờ!";
	end

	if Lottery.IsForbid then
		return false, "Hệ thống đang bận, xin vui lòng thử lại sau ít phút";
	end
...
function Lottery:ExchangeTicket(pPlayer)
	...
	pPlayer.CenterMsg(string.format("Dùng thành công, tuần này đã dùng %d Quà Tặng Minh chủ", nCount, 1));
...
function Lottery:Notify()
	...
	local tbSetting = { szTitle = "Quà Tặng Minh Chủ", szUiName = "LotteryNotify" }; 
...
function Lottery:SendWorldMsg(nMinute)
	...
	local szMsg = string.format("Hoạt động Quà Tặng Minh Chủ sẽ tổng kết sau %d phút nữa, quý đồng đạo sử dụng thẻ[FF0066]「Quà Tặng Minh Chủ」[-]sẽ may mắn nhận được những phần quà may mắn quý hiếm, xin vui lòng chờ!", nMinute);
