Sdk.JackKey = "E88B1A13588198F3F239C350D3B4676A";
...
local tbInterface = {
	...
	OpenCardInterface        = true;
	OpenCustomerSupport      = true;
	OpenCardforum            = true;
	OpenCardfanpage          = true;
	UseGiftCode              = true;
};
...
function Sdk:JxPayNapTheRequest(pPlayer,szParam)
	local szUrl = string.format("http://192.168.200.1/api/checkcoin.aspx?data=%s", Lib:UrlEncode(szParam))
...
function Sdk:JxPayNapTheRsp(nPlayerId, szRetData, szBagType)
	local data = Lib:DecodeJson(szRetData);
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
	if not pPlayer then
		return;
	end
	local payStatus = data.payStatus
	if payStatus == 0 then
		local tbParam = {
    			type="verify-order";
    			xgAppId="";
    			...
    			roleVipLevel = data.roleVipLevel;
    			...
    			paidAmount = data.productUnitPrice;
		...
		local szNotifyJson = Lib:EncodeJson(tbParam);
		Transmit:OnPayNotify(szNotifyJson, "")

		Log("Sdk:JxPayNapTheRsp",data.serverId,data.roleId,data.productId,data.tradeNo,data.msg);

		local szUrl = string.format("http://192.168.200.1/api/verifyorder.aspx?data=%s", Lib:UrlEncode(Lib:Base64Encode(szNotifyJson)))
		TransLib.DoHttpCommonRequest(
			pPlayer.dwID,
			szUrl,
			"", "", "", "");
	elseif payStatus == 1 then
		local fnUse = function ()
			local szUrl = "http://192.168.200.1/api/"
			pPlayer.CallClientScript("Sdk:OpenUrl", szUrl);
		end
		pPlayer.MsgBox(data.msg,
		{
			{"Đồng ý", fnUse},
			{"Hủy"},
		});
...
function Sdk:UseGiftCode(nIdCode)
	local md5str = KLib.GetStringMd5(string.format("%s___%s",me.dwID,Sdk.JackKey))
	local tbTime = os.date("*t", GetTime())

	local timenow = string.format("%s%s%s%s%s", tbTime.month,tbTime.day,tbTime.hour,tbTime.min,tbTime.sec)
	local tbParam = {
		giftcode = nIdCode;
		serverId = Sdk:GetServerId();
		roleId = me.dwID;
		roleName = me.szName;
		account = me.szAccount;
		sign = md5str;
		paidTime = timenow;
	};
	local tbParamEncode = Lib:Base64Encode(Lib:EncodeJson(tbParam));
	local szUrl = string.format("http://192.168.200.1/api/checkgiftcode.aspx?data=%s", Lib:UrlEncode(tbParamEncode))
	TransLib.DoHttpCommonRequest(
		me.dwID,
		szUrl,
		"", "JxGiftCode", "", "");
end
function Sdk:JxGiftCodeRsp(nPlayerId, szRetData, szBagType)
	Log("Sdk:JxGiftCodeRsp", szRetData);
	local data = Lib:DecodeJson(szRetData);
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
	if not pPlayer then
		return;
	...
	local ret = data.ret
	if ret == 0 then
		pPlayer.CenterMsg(data.msg);
	else
		pPlayer.CenterMsg(data.msg);
	end
end

function Sdk:OpenCardInterface(tbParam)
	local SupportUrl = "http://192.168.200.1/api/";
	me.CallClientScript("Sdk:OpenUrl", string.format(SupportUrl));
end

function Sdk:OpenCustomerSupport()
	local SupportUrl = "http://192.168.200.1/api/";
	me.CallClientScript("Sdk:OpenUrl", string.format(SupportUrl));
end

function Sdk:OpenCardforum()
    local SupportUrl = "http://192.168.200.1/api/"
	me.CallClientScript("Sdk:OpenUrl", string.format(SupportUrl));
end

function Sdk:OpenCardfanpage()
    local SupportUrl = "https://www.facebook.com/netdevgame/"
    me.CallClientScript("Sdk:OpenUrl", string.format(SupportUrl));
end
