...
function WeddingTourBase:SendCandy(pPlayer)
	...
	pPlayer.CostGold(Wedding.nCandyTourCost, Env.LogWay_WeddingTour, string.format("%s,%s", self.nBoyPlayerId or "", self.nGirlPlayerId or ""),
			function (nPlayerId, bSuccess, szBilloNo)
				...
				if not pPlayer then
					szFailMsg = "Giữa chừng mua hàng, bạn đang offline"
					return false, szFailMsg;
				end
				if not bSuccess then
					szFailMsg = "Thanh toán không thành công, vui lòng thử lại sau"
...
function WeddingTourBase:CheckSendCandy(pPlayer)
	if not self.tbRole[pPlayer.dwID] then
		return false, "Bạn không có quyền hoạt động"
	end
	if not self.bCandy then
		return false, "Thời gian phát kẹo đã hết"
	end
	self.tbCandyTimes = self.tbCandyTimes or {}
	self.tbCandyTimes[pPlayer.dwID] = self.tbCandyTimes[pPlayer.dwID] or {}
	...
	return false, "Kẹo mừng đã phát xong "
end
...
function WeddingTourBase:SendMsg()
	...
	local function fnMsg(self, pPlayer)
		 local tbNofityData = {
   		...
   		tbNofityData.szContent = string.format("Hảo hữu 「%s」tại %s Cử hành rước kiệu hoa diễu hành thành, muốn về xem không?", pPlayer.szName, szMapName)
  	 	...
  	 	if pPlayer.dwKinId > 0 then
		    ...
		    tbNofityData.szContent = string.format("Thành viên bang 「%s」tại %s Cử hành rước kiệu hoa diễu hành thành, muốn về xem không?", pPlayer.szName, szMapName)
	...
	for _, v in ipairs(Wedding.tbAllNpc or {}) do
		...
		if pNpc then
			...
			if nNpcTID == Wedding.nBoyNpcTID then
				self:SetNpcInfo(pNpc, self.nBoyPlayerId, "Tân Lang")
			elseif nNpcTID == Wedding.nGirlNpcTID then
				self:SetNpcInfo(pNpc, self.nGirlPlayerId, "Tân Nương")
			end
...
function WeddingTourBase:SetNpcInfo(pNpc, nPlayerId, szDefaultName)
	...
	if szKinName then
		szKinName = string.format("[FF69B4]［Bang Hội］%s[-]", szKinName or "-")
