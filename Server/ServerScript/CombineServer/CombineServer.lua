...
function CombineServer:SendChangeKinNameAward(tbKinData)
	...
	local tbMail = {
		...
		Title = "Hợp phục bang phái trùng tên đền bù";
		Text = "      Tôn kính hiệp sĩ, bởi vì ngài bang phái tên đã lặp lại, hiện đưa tặng một viên bang phái thay tên khiến làm đền bù, còn xin hiệp sĩ phí chút tâm tư, một lần nữa thiết kế một bang phái tên, ngày sau tên này định sẽ vang triệt võ lâm!";
...
function CombineServer:SendChangeNameAward(tbPlayerInfo)
	local tbMail = {
		To = 0;
		Title = "Hợp phục nhân vật trùng tên đền bù";
		Text = "      Tôn kính hiệp sĩ, bởi vì tên họ của ngài đã lặp lại, hiện đưa tặng một viên thay tên khiến làm đền bù, còn xin hiệp sĩ khác lấy một, ngày sau các hạ tên này định sẽ vang triệt võ lâm!";
		From = "Lưu Vân";
...
function CombineServer:SendRankBattleAward(nPlayerId)
	local tbMail = {
		To = nPlayerId;
		Title = "Hợp phục Võ Thần chiến bài đền bù";
		Text = "      Tôn kính hiệp sĩ, bởi vì võ lâm rung chuyển, bây giờ Võ Thần điện đã tiến hành một lần nữa xếp hạng, các hạ chỗ thứ tự có lẽ thụ ảnh hưởng, cho nên nơi này đền bù Võ Thần chiến bài một viên, mong rằng các hạ bằng trận chiến này bài, sớm ngày lại lên cao vị, rộng lòng tha thứ.";
		From = "Dương Thiết Tâm";
...
function CombineServer:OnStartup()
    ...
    Tôn kính hiệp sĩ, hôm nay thiên hạ đại loạn, mà trăm nhà đua tiếng, hợp tung liên hoành, không thể tránh được, hiện nay lưỡng địa hiệp khách chung sống một thành, mong rằng chư vị ngày sau nhiều hơn luận bàn, tương hỗ ấn giám!
    Vì thế võ lâm minh chủ đặc phê, sẽ cử hành 3 Trời khánh điển hoạt động cùng 3 Trời Tây Vực hành thương đấu giá! Từ rày về sau võ lâm, chắc hẳn càng thêm đặc sắc!( Như trước mắt đang tiến hành khánh điển hoạt động hoặc Tây Vực hành thương, thì sẽ tại vốn có cơ sở bên trên ngoài định mức lại kéo dài 3 Trời )
    Chú: Chiếm hữu lãnh địa bang phái sẽ thu hoạch được một lần [FFFE0D] Công thành chiến đấu giá [FFFE0D] Đền bù, đồng thời lần tiếp theo công thành chiến trước đó sẽ tiếp tục tiến hành [FFFE0D] Lãnh địa hành thương đấu giá [FFFE0D] A!
]]}, {szTitle = "Võ lâm thông cáo"} )
			...
			while true do
				...
				if SwornFriends:_IsMainTitleUsed(szTitle) then
					...
					for nPid in pairs(tbMembers) do
						local tbMail = {
							To = nPid,
							Title = "Hợp phục kết bái trùng tên đền bù",
							Text = "      Tôn kính hiệp sĩ, bởi vì ngài kết bái xưng hào đã lặp lại, hiện đưa tặng một bản kim lan phổ làm đền bù, còn xin hiệp sĩ phí chút tâm tư, một lần nữa thiết kế một cái xưng hào, ngày sau tên này định sẽ vang triệt võ lâm!";
							From = "Hệ thống",
					...
					if tbMapSetting and tbMapSetting.bBook then
						...
						-- 退款
							...
							for dwID, szCost in pairs(tbCost) do
								...
								local tbMail = {
									To = dwID;
									Title = "Hợp phục hôn lễ dự định lui khoản";
									From = "Nguyệt lão";
									Text = string.format("      Tôn kính hiệp sĩ, bởi vì server tư liệu tiến hành liên hệ. Ngài dự định [FFFE0D]「%s」[-]Hôn lễ, phí tổn đã toàn bộ trả lại, còn xin hiệp sĩ khác chọn ngày tốt một lần nữa dự định hôn lễ!", tbMapSetting.szWeddingName);
									tbAttach = tbAward;
								};
								Mail:SendSystemMail(tbMail);
