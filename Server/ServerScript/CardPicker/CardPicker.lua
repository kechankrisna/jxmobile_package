CardPicker.tbPartnerCardPick = CardPicker.tbPartnerCardPick or {}
function CardPicker:LoadSettingServer()
	local tbSetting = LoadTabFile("Setting/CardPicker/PartnerCardPicker.tab", "sddd", nil, {"szItemType", "nItemId", "nProb", "nCount"})
	CardPicker.tbPartnerCardPick.nTotalProb = 0
	CardPicker.tbPartnerCardPick.tbItems = {}
	for _, v in ipairs(tbSetting) do
		if v.nProb > 0 and v.nCount > 0 then
			CardPicker.tbPartnerCardPick.nTotalProb = CardPicker.tbPartnerCardPick.nTotalProb + v.nProb
			table.insert(CardPicker.tbPartnerCardPick.tbItems, v)
		end
	end
end

if not next(CardPicker.tbPartnerCardPick) then
	CardPicker:LoadSettingServer()
end
...
function CardPicker:SelectRandomItems(tbItems, szPickerType)
	...
	assert(false, szPickerType .. "Số lượng:" .. #tbItems .. "Tỷ lệ:" .. nMyProb .. "/" .. nAllProbVip);
end
...
function CardPicker:CoinPick()
	if not self:CheckIsOpen() then
		return false, "Chiêu mộ đồng hành!";
	end
	...
	if me.GetMoney("Coin") < CardPicker.Def.nCoinCost then
		return false, "Bạc không đủ để tiến hành chiêu mộ!";
	end
	...
	if not me.CostMoney("Coin", CardPicker.Def.nCoinCost, Env.LogWay_CoinPick, tbItem.nItemId) then
		return false, "Khấu trừ bạc xảy ra lỗi!";
	end
...
function CardPicker:CoinFreePick()
	...
	if nNow < tbCardPickData.nNextCoinFreePickTime then
		return false, "Thời gian miễn phí chưa tới";
	end
...
function CardPicker:GoldPick()
	...
	if me.GetMoney("Gold") < CardPicker.Def.nGoldCost then
		return false, "Nguyên Bảo không đủ để tiến hành chiêu mộ!";
	end
	...
	me.CostGold(CardPicker.Def.nGoldCost, Env.LogWay_GoldPick, tbItem.nItemId, function (nPlayerId, bSuccess)
		...
		if not pPlayer then
			return false, "Nguyên Bảo chiêu mộ xảy ra lỗi, thử lại sau.";
		end

		if not bSuccess then
			return false, "Chiêu mộ Nguyên Bảo thất bại";
		end
		...
		local tbAwrd = tbItem.szItemType == "PartnerCard" and {tbItem.szItemType, tbItem.nItemId} or {tbItem.szItemType, tbItem.nItemId, tbItem.nCount}

		pPlayer.SendAward({tbAwrd}, false, false, Env.LogWay_GoldPick);
...
function CardPicker:GoldTenPick(nPrice)
	...
	if not nPrice then
		return false, "Phiên bản quá cũ, vui lòng thử khởi động lại client và thử lại.";
	end

	local nRealCost = CardPicker:GetGoldTenCostInfo(me);
	if nPrice ~= nRealCost then
		CardPicker:UpdateCardPickInfo();
		return false, "Giá trị thẻ thay đổi, vui lòng thử lại";
	end

	if me.GetMoney("Gold") < nRealCost then
		return false, "Nguyên Bảo không đủ để tiến hành chiêu mộ 10 lần";
	end
	...
	me.CostGold(nRealCost, Env.LogWay_GoldTenPick, nil, function (nPlayerId, bSuccess, szBillNo)
		...
		if nPrice ~= nRealCost then
			return false, "Giá trị khấu trừ thay đổi, vui lòng thử lại";
		end
		...
		local tbPartnerCardItems = CardPicker:GetGoldTenPickPartnerCard(pPlayer)
		tbItems = tbPartnerCardItems or tbItems
		local tbSendAward = {}
		local strGetItemIds = ""
		for _, tbItem in ipairs(tbItems) do
			if tbItem.szItemType == "PartnerCard" then
				table.insert(tbSendAward, {tbItem.szItemType, tbItem.nItemId})
			else
				table.insert(tbSendAward, {tbItem.szItemType, tbItem.nItemId, tbItem.nCount})
			end
			strGetItemIds = strGetItemIds .. tbItem.nItemId .. ","
		end
		-- 如果是门客的话将重复的门客转换成道具显示，要在奖励给之前进行转换（因为转换为判断是否拥有门客，如果给了门客奖励会影响判断）
		if tbPartnerCardItems then
			CardPicker:FormatPickPartnerCardItem(pPlayer, tbItems)
		end
		Task:OnTaskExtInfo(pPlayer, Task.ExtInfo_YuanBaoZhaoMu);
		...
		CardPicker:AddGoldTenPickPartnerCard(pPlayer)
...
function CardPicker:GoldFreePick()
	...
	tbItem = unpack(tbItems);
	local tbAward = tbItem.szItemType == "PartnerCard" and {{tbItem.szItemType, tbItem.nItemId}} or {{tbItem.szItemType, tbItem.nItemId, tbItem.nCount}}
	Task:OnTaskExtInfo(me, Task.ExtInfo_YuanBaoZhaoMu);
	me.SendAward(tbAward, false, false, Env.LogWay_GoldFreePick);
...
function CardPicker:SendSpecialPartnerNewInformation()
	local szTitle = "Tuyệt Thế Danh Hiệp";
	local tbContent = {" Giờ đây võ lâm sóng gió, một số danh hiệp xuất hiện trong giang hồ. Theo tình hình báo thì họ sẽ đến Ẩn Hương Lâu theo thời gian bân dưới, mọi người có thể nhân thời cơ hãy [00FF00][url=openwnd:Chiêu Mộ-Nguyên Bảo, Partner, \"CardPickingPanel\"][-] để thu nạp họ.\n  [FFFE0D]Chú ý: Tiến hành [url=openwnd:Chiêu Mộ-Nguyên Bảo, Partner, \"CardPickingPanel\"] sẽ có tỉ lệ nhận được Đồng Hành SS[-]"};
	local szPartnerFormate = "\n    [FFFE0D]%s[-]  Đồng Hành SS  [FFFE0D][url=openwnd:%s, PartnerDetail, nil, nil, nil, %d][-] "

	if version_tx then
		tbContent = {" Giờ đây võ lâm sóng gió, một số danh hiệp xuất hiện trong giang hồ. Theo tình hình báo thì họ sẽ đến Ẩn Hương Lâu theo thời gian bân dưới, mọi người có thể nhân thời cơ hãy [00FF00][url=openwnd:Chiêu Mộ-Nguyên Bảo, Partner, \"CardPickingPanel\"][-] để thu nạp họ.\n  [FFFE0D]Chú ý: Tiến hành [url=openwnd:Chiêu Mộ-Nguyên Bảo, Partner, \"CardPickingPanel\"] sẽ có tỉ lệ nhận được Đồng Hành SS[-]"};
		szPartnerFormate = "\n    [FFFE0D]%s[-]  Đồng Hành SS  [FFFE0D][url=openwnd:%s, PartnerDetail, nil, nil, nil, %d][-] "	
	end
...
function CardPicker:ClientRequest(szRequestType, ...)
	if not CardPicker:CheckCardPickCount(me.dwID, szRequestType) then
		me.CallClientScript("CardPicker:OnCardPickFail", "Hôm nay ngươi đã chiêu mộ đạt mức cao nhất, ngày mai hãy quay lại thử lại sau!");
		return;
	end
...
function CardPicker:RegisterPartnerCardPickAct(fnGet, tbInst)
	self.fnGetPickTimes = fnGet
	self.tbPartnerCardPickActInst = tbInst
end

function CardPicker:CheckPartnerCardPick()
	if not Activity:__IsActInProcessByType("PartnerCardPickAct") then
		return false
	end
	return self.tbPartnerCardPickActInst
end

function CardPicker:GetTodayGoldTenPickCount(pPlayer)
	local tbInst = CardPicker:CheckPartnerCardPick()
	if not tbInst then
		return 
	end
	return self.fnGetPickTimes(tbInst, pPlayer)
end

function CardPicker:GetGoldTenPickPartnerCard(pPlayer)
	if not PartnerCard:IsOpen() then
		return
	end
	local tbItems
	if not CardPicker:CheckPartnerCardPick() then
		return
	end
	local nPickCount = self:GetTodayGoldTenPickCount(pPlayer)
	if nPickCount and nPickCount < PartnerCard.nMaxPickCard then
		tbItems = {}
		for i=1,10 do
			local tbItem = CardPicker:RandomPartnerCardItems()
			table.insert(tbItems, tbItem)
		end
	end
	return tbItems
...
-- 将重复的门客转换道具
function CardPicker:FormatPickPartnerCardItem(pPlayer, tbItems)
	local tbAllCard = {}
	for _, v in ipairs(tbItems) do
		if v.szItemType == "PartnerCard" then
			local nCardId = v.nItemId
			local tbCardInfo = PartnerCard:GetCardInfo(nCardId)
			if tbCardInfo then
				local tbRepeatAward = tbCardInfo.tbRepeat and tbCardInfo.tbRepeat[1]
				if tbRepeatAward then
					local bHadCard = PartnerCard:IsHaveCard(pPlayer, nCardId)
					if bHadCard or tbAllCard[nCardId] then
						v.szItemType = tbRepeatAward[1]
						v.nItemId = tbRepeatAward[2]
						v.nCount = tbRepeatAward[3]
					end
					tbAllCard[nCardId] = true
				end
			end
		end
	end
...
function CardPicker:AddGoldTenPickPartnerCard(pPlayer)
	Activity:OnPlayerEvent(pPlayer, "Act_AddGoldTenPickTimes")
end

function CardPicker:RandomPartnerCardItems()
	local nMyProb = MathRandom(1, CardPicker.tbPartnerCardPick.nTotalProb);
	for nIdx, tbItem in ipairs(CardPicker.tbPartnerCardPick.tbItems) do
		local nItemPro = tbItem.nProb;
		if nItemPro >= nMyProb then
			return Lib:CopyTB(tbItem), nIdx;
		else
			nMyProb = nMyProb - nItemPro;
		end
	end

	assert(false, "Số lượng:" .. #CardPicker.tbPartnerCardPick.tbItems .. "Tỷ lệ:" .. nMyProb .. "/" .. CardPicker.tbPartnerCardPick.nTotalProb);
end