...
function Boss:StartBossFight(nRound)
	...
	assert(tbCurBossData, "Võ lâm minh chủ Tìm không thấy thiết lập");
	...
	KPlayer.SendWorldNotify(Boss.Def.nPlayerEnterLevel - 1, 1000,
		"Khiêu chiến[FFFF00]「Minh Chủ Võ Lâm」[-]đã mở! Đến [FF0066]「Hoạt động」[-]để khiêu chiến, để thể hiện bản thân nào",
...
function Boss:FinishBoss(pPlayer)
	...
	if tbKinTop1 then
		local szMsgFormat = "Khiêu chiến Minh Chủ Võ Lâm, bang hội[FFFF0E]「%s」[-]đạt [FFFF0E]%d[-] điểm, xếp hạng [FFFF0E]1[-] tổng điểm bang hội!";
	...
	if tbPlayerTop1 then
		local szMsgFormat = "[FFFF0E]「%s」[-]võ công cái thế, giao đấu với Minh Chủ Võ Lâm không hề kém thế, đoạt được [FFFF0E]%d[-] điểm, đứng đầu bảng!";
...
function Boss:SendPlayerReward(nPlayerId, nScore, nRank)
	...
	local szMsgFormat = "Đại hiệp thân mến,\n\n  Khiêu chiến Minh chủ Võ Lâm lần này đạt [FFFE0D]hạng %d[-], lần chiến Bang Hội đạt hạng [FFFE0D]%s[-], này tổng cộng nhận [FFFE0D]%d[-] cống hiến %s.\n\nNhắc nhở: Số người Bang Hội tham gia càng nhiều phần thưởng càng nhiều!";
	local szKinInfo = "";
	if bossKinData and bossKinData.nRank then
		szKinInfo = string.format("[FFFE0D]%d[-]", bossKinData.nRank);
	end
...
function Boss:SendKinReward(tbKinData, nRank)
	...
	local szMsg = string.format("Bang hội khiêu chiến Minh Chủ Võ Lâm đạt hạng [FFFF0E]%d[-], nhận [FFFF0E]%d[-] uy danh", nRank, nPrestige);
...
function Boss:KinJoin(nKinId)
	if not self._tbKinData[nKinId] and nKinId ~= 0 then
		...
		local tbKinData = {
			...
			szMasterName = leader and leader:GetName() or "Rỗng";
			nScore       = 0;
...
function Boss:FightBoss()
	if not Boss:CheckJoinLevel(me) then
		return false, string.format("Cấp %d mới có thể tham gia hoạt động", Boss.Def.nPlayerEnterLevel);
	end

	if not Boss:IsOpen() then
		return false, "Khiêu chiến kết thúc";
	end

    if not Env:CheckSystemSwitch(me, Env.SW_SwitchMap) then
        return false, "Trạng thái hiện tại không thể vào khiêu chiến";
    end

	if not Boss:CanJoinBoss(me) then
		return false, "Trạng thái hiện tại không thể khiêu chiến, hãy đến[FFFE0D] Khu an toàn[-] thử lại";
	end
	...
	if nResult == 0 then
		return false, "Tình trạng hiện tại không thể tham gia";
	end
	...
	if nCurTime < tbPlayerData.nNextFightTime then
		return false, "Thời gian làm mới chưa đến."
	end
...
function Boss:OnBossFightBattleResult(pPlayer, nResult, tbBattleObj, nFightBeginTime)
	if not self.bStart or not tbBattleObj.nDamage then
		pPlayer.CenterMsg("BOSS đã bị giết chết, khiêu chiến vô hiệu");
		return;
	end
	...
	if Forbid:IsBanning(pPlayer,Forbid.BanType.WuLinMengZhu) then                                       -- 功能冻结
		...
		pPlayer.MsgBox(string.format("Ngươi %s bị cấm khiêu chiến ，bỏ lệnh cấm thời gian %s",Forbid:BanTips(pPlayer,Forbid.BanType.WuLinMengZhu), szTime or ""), {{"Xác nhận"}, {"Hủy"}})
	end
	...
	pPlayer.CallClientScript("Boss:OnSyncMyData", tbPlayerData);
	pPlayer.CallClientScript("Boss:OnMyMsg", string.format("Khiêu chiến nhận được [FFFF0E]%d[-] điểm", math.floor(nFightScore)));

	if pPlayer.dwKinId ~= 0 then
		local szBroadcastMsg = string.format("[FFFF0E]「%s」[-]khiêu chiến Minh Chủ nhận [FFFF0E]%d[-] điểm", pPlayer.szName, math.floor(nFightScore));
...
function Boss:RobPlayer(nTargetPlayerId)
	...
	if not Boss:IsOpen() then
		return false, "Khiêu chiến Minh Chủ Võ Lâm kết thúc";
	end
	...
	if FriendShip:IsFriend(me.dwID, nTargetPlayerId) then
		return false, "Đối phương là hảo hữu không thể cướp";
	end
	...
	if me.dwKinId == tbTargetPlayerData.nKinId and me.dwKinId ~= 0 then
		return false, "Đối phương ở trong bang không thể cướp điểm";
	end

	-- 头衔高于自身2级的玩家不可抢分
	if tbTargetPlayerData.nHonorLevel > me.nHonorLevel + 2 then
		return false, "Quân hàm người này quá cao không thể cướp";
	end

	local nCurTime = GetTime();
	if nCurTime < tbTargetPlayerData.nProtectRobTime then
		return false, "Đối phương đang khiêu chiến thử lại sau";
	end
	...
	if not me.SyncOtherPlayerAsyncData(nTargetPlayerId) then
		return false, "Đồng bộ dữ liệu lỗi";
	end
	...
	if nCurTime < tbPlayerData.nNextRobTime then
		return false, "Thời gian được cướp chưa đến";
	end
...
function Boss:OnRobBattleResult(pPlayer, nResult, tbBattleObj, nTargetPlayerId, nFightBeginTime)
	...
	if not Boss:IsOpen() then
		pPlayer.CenterMsg("Khiêu chiến Võ lâm minh chủ đã kết thúc, cướp lần này vô hiệu");
		return;
	end
	...
	pPlayer.CallClientScript("Boss:OnSyncMyData", tbPlayerData);
	local szMyMsg = string.format("Cướp được [FFFF0E]%d[-] điểm", nRobScore);
	if nRobScore == 0 then
		szMyMsg = "Đoạt điểm thất bại, đáng tiếc không được điểm nào..";
	end
	...
	if pTargetPlayer then
		...
		if nRobScore == 0 then
			pTargetPlayer.CallClientScript("Boss:OnMyMsg", string.format("[FFFF0E]「%s」[-]thử đoạt điểm của ngươi, đáng tiếc không thu được gì", pPlayer.szName));
		else
			pTargetPlayer.CallClientScript("Boss:OnMyMsg", string.format("Bị[FFFF0E]「%s」[-]đoạt [FFFF0E]%d[-] điểm", pPlayer.szName, nRobScore));
		end
	end


	local szBroadcastMsg = string.format("[FFFF0E]「%s」[-]đoạt[FFFF0E]「%s」[-][FF0066]%d[-] điểm", pPlayer.szName, tbTargetPlayerData.szName, nRobScore);
	if nRobScore == 0 then
		szBroadcastMsg = string.format("[FFFF0E]「%s」[-]thử đoạt[FFFF0E]「%s」[-],tiếc là không được gì", pPlayer.szName, tbTargetPlayerData.szName);
	end
	if pPlayer.dwKinId ~= 0 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Boss, szBroadcastMsg, pPlayer.dwKinId);
	end
...
function Boss:SyncRobList()
	if not Boss:IsOpen() then
		return false, "Khiêu chiến chưa mở";
	end
...
function Boss:ZCSendKinNotifyMail()
	...
	local szMailFormat = [[    Chúc mừng đại hiệp, Bang hội[ffff00]%s[-]trong Minh chủ võ lâm có biểu hiện xuất sắc！ Nhận được tư các tham dự Minh chủ liên server. Thời gian diễn ra 2 trận[ffff00]%s Trưa:12:30/Tối: 19:30(hai trận)[-]hãy sắp xếp thời gian tham gia cùng bang hội để nhận được phần thương phong phú hơn nhé!]];
	...
	for nKinId, _ in pairs(tbKins) do
		...
		if tbKinData then
			...
			local szMsg = "Chúc mừng bang hội tại khiêu chiến Võ Lâm Minh Chủ biểu hiện siêu quần, nhận được tư cách tham gia Võ Lâm Minh Chủ liên server, thật đáng mừng!#49";
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, nKinId);
		end
	end

	if next(tbKinNames) then
		local szMsg = "Chúc mừng bang %s tại khiêu chiến Võ Lâm Minh Chủ biểu hiện siêu quần, nhận được tư cách tham gia Võ Lâm Minh Chủ liên server, thật đáng mừng!";
...
function Boss:ZCKinJoin(nKinId)
	...
	local tbKinData = {
		nKinId       = nKinId;
		szName       = kinData.szName;
		szMasterName = leader and leader:GetName() or "Trống";
		nScore       = 0;
		nJoinMember  = 0;
...
function Boss:ZCSendPlayerReward(tbPlayerData)
	...
	if tbPlayerData.nRank == 1 then
		local szMsgFormat = "Chúc mừng bang hội [FFFF0E]「%s」[-] thành viên [FFFF0E]%s[-] trong khiêu chiến Võ Lâm Minh Chủ đoạt vị trí [FFFF0E]Quán Quân[-] khẳng định vị thế Duy ngã độc tôn！";
		...
		szMsgFormat = "Chúc mừng thành viên bang hội [FFFF0E]%s[-] trong khiêu chiến Võ Lâm Minh Chủ đoạt vị trí [FFFF0E]Quán Quân[-] khẳng định vị thế Duy ngã độc tôn！#49";
	...
	local szMsgFormat = "Kính gửi người chơi:\n\n    Lần này khiêu chiến võ lâm minh chủ, ngài thu được [FFFE0D]hạng %d[-]，%s lần này khiêu chiến nhận được [FFFE0D]%d[-] Cống hiến %s,hãy kiểm tra và nhận \n\nMẹo：Bang hội càng nhiều người tham gia càng có nhiều phần thưởng phong phú";
	local szKinInfo = string.format("[FFFE0D]%d[-]", tbKinData.nRank);
	...
	local tbGiftBoxMail = {
		...
		Title = "Võ lâm Minh Chủ liên server";
...
function Boss:ZCSendKinReward(tbKinData)
	...
	if nRank == 1 then
		local szMsgFormat = "Chúc mừng bang hội [FFFF0E]「%s」[-] khiêu chiến Minh Chủ liên server đạt được vị trí [FFFF0E]Quán Quân[-]! Thực lực ban hội khiến người khác nể phục!";
		KPlayer.SendWorldNotify(1, 1000,string.format(szMsgFormat, tbKinData.szName),ChatMgr.ChannelType.Public, 1);

		local szMsg = "Chúc mừng bang hội khiêu chiến Minh Chủ liên server đạt được vị trí [FFFF0E]Quán Quân[-]Thực lực ban hội khiến người khác nể phục!#49";
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, tbKinData.nKinId);
	end
	...
	local szMsg = string.format("Bang hội trong Minh Chủ Liên Server, đạt được hạng[FFFF0E]%d[-], nhận được [FFFF0E]%d[-] uy danh", nRank, nPrestige);
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, nKinId);
end
...
function Boss:ZCCheckFightState(pPlayer)
	if not Boss:CheckJoinLevel(pPlayer) then
		return false, string.format("Cấp %d mới có thể tham gia", Boss.Def.nPlayerEnterLevel);
	end

	if not Boss:ZCIsCrossOpen() then
		return false, "Lượt này khiêu chiến đã kết thúc ";
	end
	...
	if nCurTime < tbBossData.nStartTime then
		return false, "Hoạt động chưa mở hãy thử lại sau";
	end
...
function Boss:ZCOnFightBattleResult(pPlayer, nResult, tbBattleObj, nFightBeginTime)
	if not self.bZoneBossOpen or not tbBattleObj.nDamage then
		pPlayer.CenterMsg("BOSS đã bị giết, khiêu chiến vô hiệu");
		return;
	end
	...
	if Forbid:IsBanning(pPlayer,Forbid.BanType.WuLinMengZhu) then                                       -- 功能冻结
		...
		local nEndTime = Forbid:BanEndTime(pPlayer,Forbid.BanType.WuLinMengZhu)
		local szTime = Lib:GetTimeStr3(nEndTime)
		pPlayer.MsgBox(string.format("Ngươi %s bị cấm tham gia hoạt động, thời gian hết cấm: %s",Forbid:BanTips(pPlayer,Forbid.BanType.WuLinMengZhu), szTime or ""), {{"Xác nhận"}, {"Hủy"}})
	end
...
function Boss:ZCOnRobResult(pPlayer, nResult, tbBattleObj, nTargetPlayerId, nFightBeginTime)
	...
	if not Boss:ZCIsCrossOpen() then
		pPlayer.CenterMsg("Khiêu chiến kết thúc, đoạt lần này vô hiệu");
		return;
	end
...
function Boss:ZCRobPlayer(nTargetUid, nTargetPlayerId, nTargetServerId)
	...
	local nCurTime = GetTime();
	if nCurTime < tbPlayerData.nNextRobTime then
		return false, "Chưa đến thời gian cướp điểm";
	end
