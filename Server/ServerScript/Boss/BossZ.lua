...
function Boss:ZSReportRobScore(nPlayerId, nTargetUid, nRobScore)
	...
	local szMyMsg = string.format("Thành công cướp đoạt đến [FFFF0E]%d[-] Điểm tích lũy", nRobScore);
	local szTargetMsg = string.format("Bị [FFFF0E]%s「%s」[-]Cướp đi %d Điểm tích lũy", szMyServer, tbPlayerData.szName, nRobScore);
	if nRobScore == 0 then
		szMyMsg = "Ngươi nếm thử cướp đoạt điểm tích lũy, đáng tiếc cũng không có thu hoạch..";
		szTargetMsg = string.format("[FFFF0E]%s「%s」[-]Nếm thử cướp đoạt ngài điểm tích lũy, đáng tiếc hắn tài nghệ không bằng người, cướp đoạt thất bại", szMyServer, tbPlayerData.szName);
	end
	...
	local szBroadcastMsg = string.format("[FFFF0E]%s「%s」[-]Thành công cướp đi [FFFF0E]%s「%s」[-]%d Điểm tích lũy", szMyServer, tbPlayerData.szName, szTargetServer, tbTargetData.szName, nRobScore);
	if nRobScore == 0 then
		szBroadcastMsg = string.format("[FFFF0E]%s「%s」[-]Nếm thử đối [FFFF0E]%s「%s」[-]Tiến hành cướp đoạt, đáng tiếc tài nghệ không bằng người", szMyServer, tbPlayerData.szName, szTargetServer, tbTargetData.szName);
	end
...
function Boss:ZSCheckRobState(tbPlayerData, tbTargetData)
	if not tbTargetData then
		return false, "Cướp đoạt đối tượng không tồn tại";
	end

	local nCurTime = GetTime();
	if nCurTime < tbPlayerData.nNextRobTime then
		return false, "Cướp đoạt thời gian cooldown chưa tới.";
	end

	if tbTargetData.nHonorLevel > tbPlayerData.nHonorLevel + Boss.ZDef.nRobMaxHigherHonorLevel then
		return false, "Người này danh hiệu quá cao, ẩn ẩn truyền đến một cỗ uy áp, mời tăng lên danh hiệu sau lại nếm thử!";
	end

	if nCurTime < tbTargetData.nProtectRobTime then
		return false, "Nên hiệp sĩ đang cùng cái khác hiệp sĩ giao thủ, xin sau lại nếm thử khiêu chiến";
	end
	...
	if not tbTargetData.szAsyncData then
		return false, "Nên hiệp sĩ số liệu dị thường, xin sau thử lại";
	end
