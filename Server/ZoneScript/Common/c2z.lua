...
function c2z:PlayerLeaveMap(szMsg)
	...
	local szMsg = szMsg or "Xác nhận rời hoạt động?"
...
function c2z:LeaveFuben(bIsPersonalFuben, bShowStronger, bIsWin)
    ...
    if bIsPersonalFuben then
        ...
        elseif me.nCanLeaveMapId and me.nCanLeaveMapId == me.nMapId then
            me.ZoneLogout();
        elseif me.nMapTemplateId == Fuben.KeyQuestFuben.DEFINE.FIGHT_MAP_ID[#Fuben.KeyQuestFuben.DEFINE.FIGHT_MAP_ID] then
            Fuben.KeyQuestFuben:RequestLeaveKeyQuestFuben(me)
        else
...
function c2z:ApplyChangeMode(nMode)
    if nMode == Player.MODE_CAMP and Kin.Def.bForbidCamp then
        me.CenterMsg("Không thể thao tác");
        return;
    end
...
function c2z:KinEncounterReq(szType, ...)
